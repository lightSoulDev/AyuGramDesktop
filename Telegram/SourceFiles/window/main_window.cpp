...
[[nodiscard]] QPoint ChildSkip() {
	const auto skipx =
		st::defaultDialogRow.padding.left() + st::defaultDialogRow.photoSize + st::defaultDialogRow.padding.left();
...
[[nodiscard]] QString TitleFromSeparateId(const Core::WindowTitleContent &settings, const SeparateId &id) {
	if (id.sharedMedia == SeparateSharedMediaType::None || !id.sharedMediaPeer()) {
		return QString();
	}
	const auto result = (id.sharedMedia == SeparateSharedMediaType::Photos) ? tr::lng_media_type_photos(tr::now)
		: (id.sharedMedia == SeparateSharedMediaType::Videos)				? tr::lng_media_type_videos(tr::now)
		: (id.sharedMedia == SeparateSharedMediaType::Files)				? tr::lng_media_type_files(tr::now)
		: (id.sharedMedia == SeparateSharedMediaType::Audio)				? tr::lng_media_type_songs(tr::now)
		: (id.sharedMedia == SeparateSharedMediaType::Links)				? tr::lng_media_type_links(tr::now)
		: (id.sharedMedia == SeparateSharedMediaType::GIF)					? tr::lng_media_type_gifs(tr::now)
		: (id.sharedMedia == SeparateSharedMediaType::Voices)				? tr::lng_media_type_audios(tr::now)
																			: QString();
	...
	const auto name = topic ? topic->title() : peer->isSelf() ? tr::lng_saved_messages(tr::now) : peer->name();
...
QImage Logo() { return AyuAssets::currentAppLogo(); }

QImage LogoNoMargin() { return AyuAssets::currentAppLogoNoMargin(); }
...
void ConvertIconToBlack(QImage &image) {
	...
	auto shifter = [](double value) { return uint32(value * shift); };
	...
	for (auto j = 0; j != height; ++j) {
		for (auto i = 0; i != width; ++i) {
			...
			const auto gray =
				(((value >> 16) & 0xFF) * ired + ((value >> 8) & 0xFF) * igreen + (value & 0xFF) * iblue) >> 24;
			...
			*pixel++ = (value & 0xFF000000U) | (component << 16) | (component << 8) | component;
		}
		pixel += intsPerLineAdded;
	}
}

void OverrideApplicationIcon(QImage image) { OverridenIcon() = std::move(image); }

QIcon CreateOfficialIcon(Main::Session *session) { return QIcon(Ui::PixmapFromImage(AyuAssets::currentAppLogo())); }
...
QIcon CreateIcon(Main::Session *session, bool returnNullIfDefault) {
	...
	const auto iconFromTheme = QIcon::fromTheme(base::IconName(), result);
	...
	for (const auto iconSize : iconSizes) {
		...
		const auto iconPixmapSize = iconPixmap.size() / iconPixmap.devicePixelRatio();
...
QImage GenerateCounterLayer(CounterLayerArgs &&args) {
	...
	const auto text = (count < 1000) ? QString::number(count) : u"..%1"_q.arg(count % 100, 2, 10, QChar('0'));
	...
	auto result = QImage(QSize(d.size, d.size) * args.devicePixelRatio, QImage::Format_ARGB32);
	...
	p.setPen(Qt::NoPen);
	p.drawRoundedRect(
		QRect(d.size - w - d.delta * 2, d.size - f->height, w + d.delta * 2, f->height), d.radius, d.radius);
...
QImage WithSmallCounter(QImage image, CounterLayerArgs &&args) {
	...
	const auto count = args.count.value();
	const auto text = (count < 100) ? QString::number(count) : QString("..%1").arg(count % 10, 1, 10, QChar('0'));
	const auto textSize = text.size();
...
MainWindow::MainWindow(not_null<Controller *> controller)
	: _controller(controller), _positionUpdatedTimer([=] { savePosition(); }),
	  _outdated(Ui::CreateOutdatedBar(body(), cWorkingDir())), _body(body()) {
	...
	Core::App().settings().workModeChanges() |
		rpl::start_with_next([=](Core::Settings::WorkMode mode) { workmodeUpdated(mode); }, lifetime());

	if (isPrimary()) {
		Ui::Toast::SetDefaultParent(_body.data());
	}

	windowActiveValue() | rpl::skip(1) |
		rpl::start_with_next([=](bool active) { InvokeQueued(this, [=] { handleActiveChanged(active); }); },
							 lifetime());

	shownValue() | rpl::skip(1) |
		rpl::start_with_next([=](bool visible) { InvokeQueued(this, [=] { handleVisibleChanged(visible); }); },
							 lifetime());

	body()->sizeValue() | rpl::start_with_next([=](QSize size) { updateControlsGeometry(); }, lifetime());
...
Main::Account &MainWindow::account() const { return _controller->account(); }

Window::SeparateId MainWindow::id() const { return _controller->id(); }

bool MainWindow::isPrimary() const { return _controller->isPrimary(); }

Window::SessionController *MainWindow::sessionController() const { return _controller->sessionController(); }
...
bool MainWindow::hideNoQuit() {
	...
	const auto behavior = Platform::IsMac() ? Behavior::RunInBackground : Core::App().settings().closeBehavior();
...
void MainWindow::handleStateChanged(Qt::WindowState state) {
	...
	if (state == Qt::WindowMinimized && (Core::App().settings().workMode() == WorkMode::TrayOnly)) {
		minimizeToTray();
	}
	savePosition(state);
}
...
void MainWindow::showFromTray() {
	InvokeQueued(this, [=] { updateGlobalMenu(); });
	activate();
	unreadCounterChangedHook();
	Core::App().tray().updateIconCounters();
}

void MainWindow::quitFromTray() { Core::Quit(); }
...
void MainWindow::updateMinimumSize() { setMinimumSize(QSize(computeMinWidth(), computeMinHeight())); }
...
WindowPosition MainWindow::initialPosition() const {
	...
	return (!active || active == &controller())
		? Core::AdjustToScale(Core::App().settings().windowPosition(), u"Window"_q)
...
WindowPosition MainWindow::nextInitialChildPosition(SeparateId childId) {
	...
	const auto adjust = [&](int value) { return (value * 3 / 4); };
	const auto width = OptionNewWindowsSizeAsFirst.value() ? Core::App().settings().windowPosition().w
		: childId.primary()								   ? st::windowDefaultWidth
		: childId.hasChatsList() ? (st::columnMinimalWidthLeft + adjust(st::windowDefaultWidth))
								 : adjust(st::windowDefaultWidth);
	const auto height = OptionNewWindowsSizeAsFirst.value() ? Core::App().settings().windowPosition().h
		: childId.primary()									? st::windowDefaultHeight
															: adjust(st::windowDefaultHeight);
	const auto skip = ChildSkip();
	const auto delta = _lastChildIndex ? (_lastMyChildCreatePosition - position) : skip;
...
QRect MainWindow::countInitialGeometry(WindowPosition position) {
	...
	const auto primaryAvailable = primaryScreen ? primaryScreen->availableGeometry()
												: QRect(0, 0, st::windowDefaultWidth, st::windowDefaultHeight);
	const auto initialWidth =
		Core::Settings::ThirdColumnByDefault() ? st::windowBigDefaultWidth : st::windowDefaultWidth;
	const auto initialHeight =
		Core::Settings::ThirdColumnByDefault() ? st::windowBigDefaultHeight : st::windowDefaultHeight;
	const auto initial = WindowPosition{
		.x = (primaryAvailable.x() + std::max((primaryAvailable.width() - initialWidth) / 2, 0)),
		.y = (primaryAvailable.y() + std::max((primaryAvailable.height() - initialHeight) / 2, 0)),
		.w = initialWidth,
		.h = initialHeight,
	};
	return countInitialGeometry(position, initial, {st::windowMinWidth, st::windowMinHeight});
}

QRect MainWindow::countInitialGeometry(WindowPosition position, WindowPosition initial, QSize minSize) const {
...
void MainWindow::positionUpdated() { _positionUpdatedTimer.callOnce(kSaveWindowPositionTimeout); }

void MainWindow::setPositionInited() { _positionInited = true; }

void MainWindow::imeCompositionStartReceived() { _imeCompositionStartReceived.fire({}); }

rpl::producer<> MainWindow::leaveEvents() const { return _leaveEvents.events(); }

rpl::producer<> MainWindow::imeCompositionStarts() const { return _imeCompositionStartReceived.events(); }

void MainWindow::leaveEventHook(QEvent *e) { _leaveEvents.fire({}); }
...
void MainWindow::updateControlsGeometry() {
	...
	if (_rightColumn) {
		...
		_rightColumn->setGeometry(
			bodyWidth, bodyTop, inner.width() - bodyWidth, inner.height() - (bodyTop - inner.y()));
	}
...
void MainWindow::updateTitle() {
	...
	const auto counter = settings.hideTotalUnread ? 0 : Core::App().unreadBadge();
	const auto added = (counter > 0) ? u" (%1)"_q.arg(counter) : QString();
	const auto session = locked ? nullptr : _controller->sessionController();
	const auto user = (session && !settings.hideAccountName && Core::App().domain().accountsAuthedCount() > 1)
		? st::wrap_rtl(session->authedName())
		: QString();
	const auto separateIdTitle = session ? TitleFromSeparateId(settings, session->windowId()) : QString();
	if (!separateIdTitle.isEmpty()) {
		setTitle(separateIdTitle);
		return;
	}
	const auto key = (session && !settings.hideChatName) ? session->activeChatCurrent() : Dialogs::Key();
	const auto thread = key ? key.thread() : nullptr;
	if (!thread) {
		setTitle((user.isEmpty() ? u"ViGram"_q : user) + added);
		return;
	}
	const auto history = thread->owningHistory();
	const auto topic = thread->asTopic();
	const auto name = topic		  ? topic->title()
		: history->peer->isSelf() ? tr::lng_saved_messages(tr::now)
								  : history->peer->name();
	const auto wrapped = st::wrap_rtl(name);
	const auto threadCounter = thread->chatListBadgesState().unreadCounter;
	const auto primary = (threadCounter > 0) ? u"(%1) %2"_q.arg(threadCounter).arg(wrapped) : wrapped;
	const auto middle = !user.isEmpty() ? (u" @ "_q + user) : !added.isEmpty() ? u" \u2013"_q : QString();
	setTitle(primary + middle + added);
}

QRect MainWindow::computeDesktopRect() const { return screen()->availableGeometry(); }
...
void MainWindow::savePosition(Qt::WindowState state) {
	...
	if (state == Qt::WindowMinimized || !isVisible() || !Core::App().savingPositionFor(&controller()) ||
		!positionInited()) {
		return;
	}
	...
	if (state == Qt::WindowMaximized) {
		...
		DEBUG_LOG(("Window Pos: Saving non-maximized position: %1, %2, %3, %4")
					  .arg(realPosition.x)
					  .arg(realPosition.y)
					  .arg(realPosition.w)
					  .arg(realPosition.h));
	...
	if (realPosition.w >= st::windowMinWidth && realPosition.h >= st::windowMinHeight) {
		if (realPosition.x != savedPosition.x || realPosition.y != savedPosition.y ||
			...
			realPosition.scale != savedPosition.scale || realPosition.moncrc != savedPosition.moncrc ||
...
WindowPosition MainWindow::withScreenInPosition(WindowPosition position) const {
	return PositionWithScreen(position, this, {st::windowMinWidth, st::windowMinHeight});
}
...
int MainWindow::tryToExtendWidthBy(int addToWidth) {
	auto desktop = screen()->availableGeometry();
	auto inner = body()->mapToGlobal(body()->rect());
	accumulate_min(addToWidth, std::max(desktop.width() - inner.width(), 0));
	auto newWidth = inner.width() + addToWidth;
	auto newLeft = std::min(inner.x(), desktop.x() + desktop.width() - newWidth);
...
void MainWindow::launchDrag(std::unique_ptr<QMimeData> data, Fn<void()> &&callback) {
...
WindowPosition PositionWithScreen(WindowPosition position, const QScreen *chosen, QSize minimal) {
	if (!chosen) {
		return position;
	}
	const auto available = chosen->availableGeometry();
	if (available.width() < minimal.width() || available.height() < minimal.height()) {
		return position;
	}
...
WindowPosition PositionWithScreen(WindowPosition position, not_null<const QWidget *> widget, QSize minimal) {
	const auto screen = widget->screen();
	return PositionWithScreen(position, screen ? screen : QGuiApplication::primaryScreen(), minimal);
}
