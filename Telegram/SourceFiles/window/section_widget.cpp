...
[[nodiscard]] rpl::producer<QString> PeerThemeEmojiValue(not_null<PeerData *> peer) {
	return peer->session().changes().peerFlagsValue(peer, Data::PeerUpdate::Flag::ChatThemeEmoji) |
		rpl::map([=] { return peer->themeEmoji(); });
}
...
[[nodiscard]] rpl::producer<const Data::WallPaper *> PeerWallPaperMapped(not_null<PeerData *> peer) {
	return peer->session().changes().peerFlagsValue(peer, Data::PeerUpdate::Flag::ChatWallPaper) |
		rpl::map([=]() -> rpl::producer<const Data::WallPaper *>
				 { return WallPaperResolved(&peer->owner(), peer->wallPaper()); }) |
		rpl::flatten_latest();
}

[[nodiscard]] rpl::producer<std::optional<ResolvedPaper>> PeerWallPaperValue(not_null<PeerData *> peer) {
	return PeerWallPaperMapped(peer) |
		rpl::map(
			   [=](const Data::WallPaper *paper) -> rpl::producer<std::optional<ResolvedPaper>>
			   {
				   const auto single = [](std::optional<ResolvedPaper> value) { return rpl::single(std::move(value)); };
				   if (!paper) {
					   return single({});
				   }
				   ...
				   return single(value) |
					   rpl::then(document->session().downloaderTaskFinished() |
								 rpl::filter([=] { return value.media->loaded(true); }) | rpl::take(1) |
								 rpl::map_to(std::optional<ResolvedPaper>(value)));
			   }) |
		rpl::flatten_latest();
}

[[nodiscard]] auto MaybeChatThemeDataValueFromPeer(not_null<PeerData *> peer)
	-> rpl::producer<std::optional<Data::CloudTheme>> {
	return PeerThemeEmojiValue(peer) |
		rpl::map([=](const QString &emoji) -> rpl::producer<std::optional<Data::CloudTheme>>
...
[[nodiscard]] auto MaybeCloudThemeValueFromPeer(not_null<PeerData *> peer) -> rpl::producer<ResolvedTheme> {
	return rpl::combine(MaybeChatThemeDataValueFromPeer(peer),
...
rpl::producer<const Data::WallPaper *> WallPaperResolved(not_null<Data::Session *> owner,
	...
	auto fromThemes = [=](bool force) -> rpl::producer<const Data::WallPaper *>
	{
		...
		return Window::Theme::IsNightModeValue() |
			rpl::map(
				   [=](bool dark) -> const Data::WallPaper *
				   {
					   const auto &list = themes->chatThemes();
					   const auto i = ranges::find(list, id, &Data::CloudTheme::emoticon);
					   if (i != end(list)) {
	...
	return rpl::single<const Data::WallPaper *>(nullptr) |
		rpl::then(themes->chatThemesUpdated() | rpl::take(1) | rpl::map([=] { return fromThemes(true); }) |
				  rpl::flatten_latest());
}

AbstractSectionWidget::AbstractSectionWidget(QWidget *parent,
											 not_null<SessionController *> controller,
											 rpl::producer<PeerData *> peerForBackground)
	: RpWidget(parent), _controller(controller) {
	std::move(peerForBackground) |
		rpl::map(
			[=](PeerData *peer) -> rpl::producer<>
			{
				if (!peer) {
					return rpl::single(rpl::empty) |
						rpl::then(controller->defaultChatTheme()->repaintBackgroundRequests());
				}
				return ChatThemeValueFromPeer(controller, peer) |
					rpl::map([](const std::shared_ptr<Ui::ChatTheme> &theme)
							 { return rpl::single(rpl::empty) | rpl::then(theme->repaintBackgroundRequests()); }) |
					rpl::flatten_latest();
			}) |
		rpl::flatten_latest() | rpl::start_with_next([=] { update(); }, lifetime());
}

Main::Session &AbstractSectionWidget::session() const { return _controller->session(); }
...
SectionWidget::SectionWidget(QWidget *parent,
	...
	: AbstractSectionWidget(parent, controller, rpl::single(peerForBackground.get())) {}
...
std::shared_ptr<SectionMemento> SectionWidget::createMemento() { return nullptr; }
...
QPixmap SectionWidget::grabForShowAnimation(const SectionSlideParams &params) { return Ui::GrabWidget(this); }

void SectionWidget::PaintBackground(not_null<Window::SessionController *> controller,
									not_null<Ui::ChatTheme *> theme,
									not_null<QWidget *> widget,
									QRect clip) {
	PaintBackground(theme, widget, controller->content()->height(), controller->content()->backgroundFromY(), clip);
}

void SectionWidget::PaintBackground(
	not_null<Ui::ChatTheme *> theme, not_null<QWidget *> widget, int fillHeight, int fromy, QRect clip) {
...
void SectionWidget::PaintBackground(QPainter &p, not_null<Ui::ChatTheme *> theme, QSize fill, QRect clip) {
	...
	{
		const auto to = QRect(QPoint(cache.x, cache.y), cache.pixmap.size() / style::DevicePixelRatio());
		if (cache.waitingForNegativePattern) {
			...
			{ return (value >= 0.) ? int(std::ceil(value)) : int(std::floor(value)); };
	...
	if (prepared.isNull()) {
		...
		for (auto i = 0; i != cols; ++i) {
			p.drawImage(QRect(xshift + i * w, 0, w, fill.height()), prepared, QRect(QPoint(), prepared.size()));
		}
		...
		const auto rects = Ui::ComputeChatBackgroundRects(fill, prepared.size());
...
bool SectionWidget::animatingShow() const { return (_showAnimation != nullptr); }
...
rpl::producer<int> SectionWidget::desiredHeight() const { return rpl::single(height()); }

SectionWidget::~SectionWidget() = default;

auto ChatThemeValueFromPeer(not_null<SessionController *> controller, not_null<PeerData *> peer)
	-> rpl::producer<std::shared_ptr<Ui::ChatTheme>> {
	auto cloud = MaybeCloudThemeValueFromPeer(peer) |
		rpl::map(
					 [=](ResolvedTheme resolved) -> rpl::producer<std::shared_ptr<Ui::ChatTheme>>
					 {
						 ...
						 const auto paper = resolved.paper ? resolved.paper->paper : Data::WallPaper(0);
						 const auto type = resolved.dark ? Data::CloudThemeType::Dark : Data::CloudThemeType::Light;
						 if (paper.document() && resolved.paper->media && !resolved.paper->media->loaded() &&
	...
	return rpl::combine(std::move(cloud), controller->peerThemeOverrideValue()) |
		rpl::map(
			   [=](std::shared_ptr<Ui::ChatTheme> &&cloud, PeerThemeOverride &&overriden)
			   {
				   return (overriden.peer == peer.get() && Ui::Emoji::Find(peer->themeEmoji()) != overriden.emoji)
					   ? std::move(overriden.theme)
					   : std::move(cloud);
			   });
}

bool ShowSendPremiumError(not_null<SessionController *> controller, not_null<DocumentData *> document) {
	return ShowSendPremiumError(controller->uiShow(), document);
}

bool ShowSendPremiumError(std::shared_ptr<ChatHelpers::Show> show, not_null<DocumentData *> document) {
	if (!document->isPremiumSticker() || document->session().premium()) {
		return false;
	}
...
bool ShowReactPremiumError(not_null<SessionController *> controller,
	...
	if (item->reactionsAreTags()) {
		...
		return true;
	} else if (controller->session().premium() || ranges::contains(item->chosenReactions(), id) ||
