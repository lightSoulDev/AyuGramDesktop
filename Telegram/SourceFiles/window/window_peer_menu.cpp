...
void SetActionText(not_null<QAction *> action, rpl::producer<QString> &&text) {
	...
	std::move(text) | rpl::start_with_next([=](const QString &actionText) { action->setText(actionText); }, *lifetime);
}
...
void TogglePinnedThread(not_null<Window::SessionController *> controller, not_null<Dialogs::Entry *> entry) {
	...
	if (const auto history = entry->asHistory()) {
		...
		owner->session()
			...
			.done([=] { owner->notifyPinnedDialogsOrderUpdated(); })
			.send();
		if (isPinned) {
		...
		owner->session()
			.api()
			.request(MTPchannels_UpdatePinnedForumTopic(
				topic->channel()->inputChannel, MTP_int(topic->rootId()), MTP_bool(isPinned)))
			.done([=](const MTPUpdates &result) { owner->session().api().applyUpdates(result); })
			.send();
		...
		owner->session()
			.api()
			.request(MTPmessages_ToggleSavedDialogPin(MTP_flags(flags), MTP_inputDialogPeer(sublist->peer()->input)))
			.done([=] { owner->notifyPinnedDialogsOrderUpdated(); })
			.send();
		// if (isPinned) {
		//	controller->content()->dialogsToUp();
...
void Filler::addToggleTopicClosed() {
	...
	_addAction(
		closed ? tr::lng_forum_topic_reopen(tr::now) : tr::lng_forum_topic_close(tr::now),
		[=]
		{
...
void Filler::addInfo() {
	if (_peer && (_peer->isSelf() || _peer->isRepliesChat() || _peer->isVerifyCodes())) {
		return;
	} else if (!_thread) {
		return;
	...
	const auto text = _thread->asTopic()			? tr::lng_context_view_topic(tr::now)
		: (_peer->isChat() || _peer->isMegagroup()) ? tr::lng_context_view_group(tr::now)
		: _peer->isUser()							? tr::lng_context_view_profile(tr::now)
...
void Filler::addStoryArchive() {
	...
	_addAction(
		...
		{
			if (const auto strong = weak.get()) {
				controller->showSection(Info::Stories::Make(channel, Info::Stories::Tab::Archive));
			}
		},
		...
		_addAction(
			...
			[=]
			{
				...
				if (const auto sublist = weak.get()) {
					...
					controller->showInNewWindow(SeparateId(SeparateType::SavedSublist, peer->owner().history(peer)));
				}
			},
			&st::menuIconNewWindow);
		AddSeparatorAndShiftUp(_addAction);
...
void Filler::addToggleArchive() {
	...
	const auto isArchived = [=] { return (history->folder() != nullptr); };
...
void Filler::addDeleteChat() {
	...
	_addAction({
		.text =
			(_peer->isUser() ? tr::lng_profile_delete_conversation(tr::now) : tr::lng_profile_clear_and_exit(tr::now)),
...
void Filler::addExportChat() {
	...
	const auto peer = _peer;
	_addAction(tr::lng_profile_export_chat(tr::now), [=] { PeerMenuExportChat(peer); }, &st::menuIconExport);
}
...
void Filler::addTopicLink() {
	...
	_addAction(
		...
		{
			const auto base =
				channel->hasUsername() ? channel->username() : "c/" + QString::number(peerToChannel(channel->id).bare);
...
void Filler::addCreatePoll() {
	...
	auto callback = [=] { PeerMenuCreatePoll(controller, peer, replyTo, flag, flag, source, {sendMenuType}); };
...
void Filler::addViewAsMessages() {
	...
	_addAction({.text = tr::lng_forum_view_as_messages(tr::now),
				.handler = open,
...
void Filler::addViewAsTopics() {
	if (!_peer || !_peer->isForum() || !_controller->adaptive().isOneColumn()) {
		return;
	}
	const auto peer = _peer;
	const auto controller = _controller;
...
void Filler::fillScheduledActions() { addCreatePoll(); }
...
void PeerMenuDeleteTopic(not_null<Window::SessionNavigation *> navigation,
	...
	api->request(MTPchannels_DeleteTopicHistory(channel->inputChannel, MTP_int(rootId)))
		.done(
...
void PeerMenuShareContactBox(not_null<Window::SessionNavigation *> navigation, not_null<UserData *> user) {
	...
	auto callback = [=](not_null<Data::Thread *> thread)
	{
		...
		state->share = [=](Api::SendOptions options)
		{
			...
			const auto checked =
				state->sendPayment.check(navigation, peer, 1, options.starsApproved, withPaymentApproved);
			if (!checked) {
				return;
			}
...
void PeerMenuCreatePoll(not_null<Window::SessionController *> controller,
	...
	auto box = Box<CreatePollBox>(controller, chosen, disabled, std::move(starsRequired), sendType, sendMenuDetails);
	struct State
	{
	...
	state->create = [=](const CreatePollBox::Result &result)
	{
		...
		const auto checked =
			state->sendPayment.check(controller, peer, 1, result.options.starsApproved, withPaymentApproved);
	...
	box->submitRequests() | rpl::start_with_next(state->create, box->lifetime());
	controller->show(std::move(box), Ui::LayerOption::CloseOther);
}
...
void PeerMenuBlockUserBox(not_null<Ui::GenericBox *> box,
	...
	if (user) {
		box->addRow(object_ptr<Ui::FlatLabel>(
			box,
			tr::lng_blocked_list_confirm_text(lt_name, rpl::single(Ui::Text::Bold(name)), Ui::Text::WithEntities),
	...
	const auto allFromUser = v::is<ClearReply>(suggestClear)
		? box->addRow(object_ptr<Ui::Checkbox>(
			  box,
			  tr::lng_delete_all_from_user(tr::now, lt_user, Ui::Text::Bold(peer->name()), Ui::Text::WithEntities),
			  true,
							...
							if (!waiting.empty()) {
								...
								if (!session->credits().loaded()) {
									session->credits().loadedValue() | rpl::filter(rpl::mappers::_1) | rpl::take(1) |
										rpl::start_with_next([=] { withPaymentApproved(alreadyApproved); },
															 state->submitLifetime);
								}
								return;
...
QPointer<Ui::BoxContent> ShowChooseRecipientBox(not_null<Window::SessionNavigation *> navigation,
												FnMut<bool(not_null<Data::Thread *>)> &&chosen,
												rpl::producer<QString> titleOverride,
												FnMut<void()> &&successCallback,
												InlineBots::PeerTypes typesRestriction) {
...
QPointer<Ui::BoxContent> ShowForwardMessagesBox(std::shared_ptr<ChatHelpers::Show> show,
	...
	const auto state = [&]
	{
		...
		auto init = [=](not_null<ListBox *> box)
		{
			...
			rpl::combine(chatsFilters->heightValue(),
				...
				rpl::start_with_next([box](int h, bool hasQuery) { box->setAddedTopScrollSkip(hasQuery ? 0 : h); },
									 box->lifetime());
			box->multiSelectHeightValue() |
				rpl::start_with_next([=](int h) { chatsFilters->moveToLeft(0, h); }, chatsFilters->lifetime());
		};
	...
	{ // Chosen a single.
		auto chosen = [show, draft = std::move(draft)](not_null<Data::Thread *> thread) mutable
		{
			...
			const auto id = SeparateId((peer->isForum() ? SeparateType::Forum : SeparateType::Chat), thread);
		...
		state->controller->singleChosen() | rpl::start_with_next(std::move(callback), state->box->lifetime());
	}
	...
	state->submit = [=](Api::SendOptions options)
	{
		...
		const auto checkPaid = [=]
		{
			...
			if (!waiting.empty()) {
				...
				if (!session->credits().loaded()) {
					session->credits().loadedValue() | rpl::filter(rpl::mappers::_1) | rpl::take(1) |
						rpl::start_with_next([=] { withPaymentApproved(alreadyApproved); }, state->submitLifetime);
				}
				return false;
			} else if (totalStars > alreadyApproved) {
				ShowSendPaidConfirm(show,
			...
			auto createView = [&](rpl::producer<QString> &&text, bool checked)
			{
				auto item =
					base::make_unique_q<Menu::ItemWithCheck>(state->menu->menu(),
															 ...
															 nullptr);
				std::move(text) |
					rpl::start_with_next([action = item->action()](QString text) { action->setText(text); },
										 item->lifetime());
				item->init(checked);
	...
	field->changes() | rpl::start_with_next([=] { state->refreshStarsToSend(); }, field->lifetime());
...
QPointer<Ui::BoxContent> ShowForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
												Data::ForwardDraft &&draft,
												Fn<void()> &&successCallback) {
...
QPointer<Ui::BoxContent> ShowForwardMessagesBox(not_null<Window::SessionNavigation *> navigation,
												...
												Fn<void()> &&successCallback) {
	return ShowForwardMessagesBox(navigation, Data::ForwardDraft{.ids = std::move(items)}, std::move(successCallback));
}
...
QPointer<Ui::BoxContent> ShowDropMediaBox(not_null<Window::SessionNavigation *> navigation,
	...
	auto initBox = [=](not_null<PeerListBox *> box)
	{
		...
		forum->destroyed() | rpl::start_with_next([=] { box->closeBox(); }, box->lifetime());
	};
	*weak = navigation->parentController()->show(
...
QPointer<Ui::BoxContent> ShowSendNowMessagesBox(not_null<Window::SessionNavigation *> navigation,
												not_null<History *> history,
												MessageIdsList &&items,
												Fn<void()> &&successCallback) {
	...
	const auto callback =
		crl::guard(navigation,
				   [=](Fn<void()> &&close)
				   {
					   ...
					   const auto top = Data::ResolveTopPinnedId(peer, topicRootId, migrated);
					   const auto universal = !top					? MsgId(0)
						   : (migrated && !peerIsChannel(top.peer)) ? (top.msg - ServerMaxMsgId)
																	: top.msg;
					   if (universal) {
						   session.settings().setHiddenPinnedMessageId(peer->id, topicRootId, universal);
		...
		show->show(Box(
					   [=](not_null<Ui::GenericBox *> box)
					   {
						   ...
						   auto text = rpl::combine(tr::lng_context_mark_read_all_sure(),
													tr::lng_context_mark_read_all_sure_2(Ui::Text::RichLangValue)) |
							   rpl::map(
...
Fn<void()> ClearHistoryHandler(not_null<Window::SessionController *> controller, not_null<PeerData *> peer) {
	return [=] { controller->show(Box<DeleteMessagesBox>(peer, true)); };
}
