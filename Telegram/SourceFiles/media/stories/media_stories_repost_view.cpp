...
RepostView::RepostView(not_null<Controller *> controller, not_null<Data::Story *> story)
	: _controller(controller), _story(story),
	  _sourcePeer(_story->repost() ? _story->repostSourcePeer()
								   : _story->owner().peer(_story->channelPosts().front().itemId.peer).get()) {
	Expects(_story->repost() || !_story->channelPosts().empty());

	if (!_story->repost()) {
		_link = MakeChannelPostHandler(&_story->session(), _story->channelPosts().front().itemId);
	}

	_story->session().colorIndicesValue() |
		rpl::start_with_next(
			[=](Ui::ColorIndicesCompressed &&indices)
			{
...
int RepostView::height() const {
	return st::historyReplyPadding.top() + st::semiboldFont->height + st::normalFont->height +
...
void RepostView::draw(Painter &p, int x, int y, int availableWidth) {
	...
	const auto backgroundEmojiId = (!simple && _sourcePeer) ? _sourcePeer->backgroundEmojiId() : DocumentId();
	const auto cache = &_quoteCache;
	const auto &quoteSt = simple ? st::storiesRepostSimpleStyle : st::messageQuoteStyle;
	const auto backgroundEmoji = backgroundEmojiId ? &_backgroundEmojiData : nullptr;
	const auto backgroundEmojiCache = backgroundEmoji ? &backgroundEmoji->caches[0] : nullptr;
	...
	if (!settings->simpleQuotesAndReplies && backgroundEmoji) {
		...
		if (backgroundEmoji->firstFrameMask.isNull() && !backgroundEmoji->emoji) {
			backgroundEmoji->emoji = CreateBackgroundEmojiInstance(
				&_story->owner(), backgroundEmojiId, crl::guard(this, [=] { _controller->repaint(); }));
		}
		ValidateBackgroundEmoji(backgroundEmojiId, backgroundEmoji, backgroundEmojiCache, cache);
		if (!backgroundEmojiCache->frames[0].isNull()) {
			const auto hasQuoteIcon = false;
			FillBackgroundEmoji(p, rect, hasQuoteIcon, *backgroundEmojiCache);
		}
	}
	...
	if (w > st::historyReplyPadding.left()) {
		...
		if (namew > 0) {
			...
			_name.drawLeftElided(
				p, x + st::historyReplyPadding.left(), y + st::historyReplyPadding.top(), namew, w + 2 * x);
			if (!simple) {
				const auto textLeft = x + st::historyReplyPadding.left();
				const auto textTop = y + st::historyReplyPadding.top() + st::semiboldFont->height;
				_text.draw(p,
						   {
			...
			crl::guard(this,
					   [=]
					   {
						   ...
						   if (const auto id = peer ? _story->repostSourceId() : 0) {
							   ...
							   _controller->uiShow()->showToast(tr::lng_forwarded_story_expired(tr::now));
						   }
					   }));
	}
...
PeerData *RepostView::fromPeer() const { return _sourcePeer; }

QString RepostView::fromName() const { return _sourcePeer ? _sourcePeer->name() : _story->repostSourceName(); }

void RepostView::recountDimensions() {
	const auto name = _sourcePeer ? _sourcePeer->name() : _story->repostSourceName();
	const auto owner = &_story->owner();
	const auto repostId = _story->repost() ? _story->repostSourceId() : 0;

	const auto colorIndexPlusOne = _sourcePeer ? (_sourcePeer->colorIndex() + 1) : 1;
	const auto dark = true;
	const auto colorPattern = colorIndexPlusOne ? Ui::ColorPatternIndex(_colorIndices, colorIndexPlusOne - 1, dark) : 0;
	Assert(colorPattern < Ui::Text::kMaxQuoteOutlines);
	const auto values = Ui::SimpleColorIndexValues(QColor(255, 255, 255), colorPattern);
	...
	_name.setMarkedText(st::semiboldTextStyle, nameFull, Ui::NameTextOptions(), context);
	context.repaint = crl::guard(this, [=] { _controller->repaint(); });
	_text.setMarkedText(st::defaultTextStyle, text, Ui::DialogTextOptions(), context);

	const auto nameMaxWidth = _name.maxWidth();
	const auto optimalTextWidth = _text.isEmpty() ? 0 : std::min(_text.maxWidth(), st::maxSignatureSize);
...
void RepostView::clickHandlerPressedChanged(const ClickHandlerPtr &action, bool pressed) {
	if (action == _link) {
		if (pressed) {
			const auto simple = _text.isEmpty();
			const auto skip = simple ? st::normalFont->height : 0;
			if (!_ripple) {
				...
				_ripple = std::make_unique<Ui::RippleAnimation>(
					...
					Ui::RippleAnimation::RoundRectMask(
						QSize(_lastWidth, h), (simple ? st::storiesRepostSimpleStyle : st::messageQuoteStyle).radius),
					[=] { _controller->repaint(); });
			}
