...
public:
	...
	bool hasTopBarShadow() const override { return true; }
...
public:
	...
	SectionMemento(not_null<PeerData *> peer, HistoryItem *item, ID topicId)
		: _peer(peer), _item(item), _topicId(topicId) {}
	...
	not_null<PeerData *> getPeer() const { return _peer; }

	void setScrollTop(int scrollTop) { _scrollTop = scrollTop; }

	int getScrollTop() const { return _scrollTop; }

	void setItems(std::vector<OwnedItem> &&items, std::set<uint64> &&eventIds, bool upLoaded, bool downLoaded) {
	...
	std::vector<OwnedItem> takeItems() { return std::move(_items); }

	std::set<uint64> takeMessageIds() { return std::move(_messageIds); }

	bool upLoaded() const { return _upLoaded; }

	bool downLoaded() const { return _downLoaded; }
