...
{
public:
	Rows(QWidget *parent, const std::vector<Font> &data, const QString &chosen);
...
private:
	...
	struct RowSelection
	{
		...
		inline bool operator==(const RowSelection &other) const { return (index == other.index); }
	};
	...
	std::unique_ptr<Ui::RippleAnimation> &rippleBySelection(Selection selected);

	[[maybe_unused]] const std::unique_ptr<Ui::RippleAnimation> &rippleBySelection(Selection selected) const;

	std::unique_ptr<Ui::RippleAnimation> &rippleBySelection(not_null<Row *> row, Selection selected);

	[[maybe_unused]] const std::unique_ptr<Ui::RippleAnimation> &rippleBySelection(not_null<const Row *> row,
...
{
public:
	Content(QWidget *parent, const std::vector<Font> &fonts);
...
private:
	void setupContent(const std::vector<Font> &fonts);
...
std::vector<Font> PrepareFonts() {
	...
	for (const auto &font : base.families()) {
		Font fontItem = {.FontName = font, .id = font};
...
Rows::Rows(QWidget *parent, const std::vector<Font> &data, const QString &chosen) : RpWidget(parent), _chosen(chosen) {
	const auto descriptionOptions = TextParseOptions{TextParseMultiline, 0, 0, Qt::LayoutDirectionAuto};
	_rows.reserve(data.size());
	for (const auto &item : data) {
		_rows.push_back(Row{item});
		auto &row = _rows.back();
		row.check = std::make_unique<Ui::RadioView>(
			st::langsRadio, (row.data.id == _chosen), [=, row = &row] { repaint(*row); });
		row.title.setText(st::defaultTextStyle, item.FontName, Ui::NameTextOptions());
		row.keywords = TextUtilities::PrepareSearchWords(item.FontName + ' ' + item.id);
	}
	resizeToWidth(width());
...
void Rows::mousePressEvent(QMouseEvent *e) {
	...
	if (!v::is_null(_pressed) && !rowBySelection(_pressed).menuToggleForceRippled) {
...
void Rows::ensureRippleBySelection(Selection selected) { ensureRippleBySelection(&rowBySelection(selected), selected); }
...
void Rows::ensureRippleBySelection(not_null<Row *> row, Selection selected) {
	...
	ripple = std::make_unique<Ui::RippleAnimation>(
		st::defaultRippleAnimation, std::move(mask), [=] { repaintChecked(row); });
}
...
void Rows::mouseReleaseEvent(QMouseEvent *e) {
	...
	if (pressed == _selected) {
		v::match(pressed, [&](RowSelection data) { activateByIndex(data.index); }, [](v::null_t) {});
	}
}

void Rows::restore(not_null<Row *> row) { row->removed = false; }

void Rows::setForceRippled(not_null<Row *> row, bool rippled) { repaint(*row); }

void Rows::activateByIndex(int index) { _activations.fire_copy(rowByIndex(index).data); }

void Rows::leaveEventHook(QEvent *e) { updateSelected({}); }
...
void Rows::filter(const QString &query) {
	...
	const auto skip = [](const QStringList &haystack, const QStringList &needles)
	{
		const auto find = [](const QStringList &haystack, const QString &needle)
		{
...
int Rows::count() const { return _query.isEmpty() ? _rows.size() : _filtered.size(); }

int Rows::indexFromSelection(Selection selected) const {
	return v::match(selected, [&](RowSelection data) { return data.index; }, [](v::null_t) { return -1; });
}

int Rows::selected() const { return indexFromSelection(_selected); }
...
rpl::producer<Font> Rows::activations() const { return _activations.events(); }
...
rpl::producer<bool> Rows::hasSelection() const { return _hasSelection.events(); }

rpl::producer<bool> Rows::isEmpty() const {
	return _isEmpty.events_starting_with(count() == 0) | rpl::distinct_until_changed();
}

void Rows::repaint(Selection selected) {
	v::match(selected, [](v::null_t) {}, [&](const auto &data) { repaint(data.index); });
}
...
void Rows::repaint(const Row &row) { update(0, row.top, width(), row.height); }
...
Rows::Row &Rows::rowBySelection(Selection selected) { return rowByIndex(indexFromSelection(selected)); }

const Rows::Row &Rows::rowBySelection(Selection selected) const { return rowByIndex(indexFromSelection(selected)); }

std::unique_ptr<Ui::RippleAnimation> &Rows::rippleBySelection(Selection selected) {
	return rippleBySelection(&rowBySelection(selected), selected);
}

const std::unique_ptr<Ui::RippleAnimation> &Rows::rippleBySelection(Selection selected) const {
	return rippleBySelection(&rowBySelection(selected), selected);
}

std::unique_ptr<Ui::RippleAnimation> &Rows::rippleBySelection(not_null<Row *> row, Selection selected) {
	return row->ripple;
}

const std::unique_ptr<Ui::RippleAnimation> &Rows::rippleBySelection(not_null<const Row *> row,
																	Selection selected) const {
	return const_cast<Rows *>(this)->rippleBySelection(const_cast<Row *>(row.get()), selected);
}
...
int Rows::DefaultRowHeight() {
	return st::passportRowPadding.top() + st::semiboldFont->height + st::passportRowSkip + st::normalFont->height +
...
int Rows::resizeGetHeight(int newWidth) {
	...
	for (auto i = 0, till = count(); i != till; ++i) {
		...
		row.height = st::passportRowPadding.top() + row.titleHeight + st::passportRowSkip + row.descriptionHeight +
			st::passportRowPadding.bottom();
		result += row.height;
	}
	return result;
...
int Rows::countAvailableWidth(int newWidth) const {
	const auto right = 0;
	return newWidth - st::passportRowPadding.left() - st::langsRadio.diameter - st::passportRowPadding.left() - right -
		st::passportRowIconSkip;
}

int Rows::countAvailableWidth() const { return countAvailableWidth(width()); }
...
void Rows::paintEvent(QPaintEvent *e) {
	...
	const auto left = checkLeft + st::langsRadio.diameter + st::passportRowPadding.left();
...
Content::Content(QWidget *parent, const std::vector<Font> &fonts) : RpWidget(parent) { setupContent(fonts); }

void Content::setupContent(const std::vector<Font> &fonts) {
	...
	const auto add = [&](const std::vector<Font> &list)
	{
		...
		const auto wrap = content->add(
			object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(content, object_ptr<Ui::VerticalLayout>(content)));
		const auto inner = wrap->entity();
		inner->add(object_ptr<Ui::FixedHeightWidget>(inner, st::defaultBox.margin.top()));
		const auto rows = inner->add(object_ptr<Rows>(inner, list, current));
		inner->add(object_ptr<Ui::FixedHeightWidget>(inner, st::defaultBox.margin.top()));

		rows->isEmpty() |
			rpl::start_with_next([=](bool empty) { wrap->toggle(!empty, anim::type::instant); }, rows->lifetime());

		return rows;
	};
	const auto main = add(fonts);
	const auto divider = content->add(
		object_ptr<Ui::SlideWrap<Ui::BoxContentDivider>>(content, object_ptr<Ui::BoxContentDivider>(content)));
	const auto empty = content->add(object_ptr<Ui::SlideWrap<Ui::FixedHeightWidget>>(
		content, object_ptr<Ui::FixedHeightWidget>(content, st::membersAbout.style.font->height * 9)));
	const auto label =
		Ui::CreateChild<Ui::FlatLabel>(empty->entity(), rpl::single(qs("No fonts found.")), st::membersAbout);
	...
	empty->toggleOn(main ? main->isEmpty() : rpl::single(true), anim::type::instant);
	...
	const auto count = [](Rows *widget) { return widget ? widget->count() : 0; };
	const auto selected = [](Rows *widget) { return widget ? widget->selected() : -1; };
	const auto rowsCount = [=] { return count(main); };
	...
	const auto selectedCoords = [=]
	{
		const auto coords = [=](Rows *rows, int index)
		{
			...
			const auto shift = rows->mapToGlobal({0, 0}).y() - mapToGlobal({0, 0}).y();
			return Ui::ScrollToRequest(result.ymin + shift, result.ymax + shift);
		};
		if (const auto index = selected(main); index >= 0) {
	...
	_filter = [=](const QString &query) { filter(main, query); };
	_activations = [=]
	{
		if (!main) {
			return rpl::never<Font>() | rpl::type_erased();
		}
		return rpl::merge(main->activations()) | rpl::type_erased();
	};
...
void Content::filter(const QString &query) { _filter(query); }

rpl::producer<Font> Content::activations() const { return _activations(); }

void Content::changeChosen(const QString &chosen) { _changeChosen(chosen); }

void Content::activateBySubmit() { _activateBySubmit(); }

Ui::ScrollToRequest Content::jump(int rows) { return _jump(rows); }
...
void AyuUi::FontSelectorBox::prepare() {
	...
	const auto select = topContainer->add(
		object_ptr<Ui::MultiSelect>(topContainer, st::defaultMultiSelect, tr::lng_participant_filter()));
	topContainer->resizeToWidth(st::boxWidth);

	using namespace rpl::mappers;

	const auto fonts = AyuUi::PrepareFonts();
	const auto inner = setInnerWidget(object_ptr<AyuUi::Content>(this, fonts), st::boxScroll, topContainer->height());
	inner->resizeToWidth(st::boxWidth);

	const auto max = lifetime().make_state<int>(0);
	rpl::combine(inner->heightValue(), topContainer->heightValue(), _1 + _2) |
		rpl::start_with_next(
			[=](int height)
			{
			...
			inner->lifetime());
	topContainer->heightValue() | rpl::start_with_next([=](int height) { setInnerTopSkip(height); }, inner->lifetime());

	select->setSubmittedCallback([=](Qt::KeyboardModifiers) { inner->activateBySubmit(); });
	select->setQueryChangedCallback([=](const QString &query) { inner->filter(query); });
	select->setCancelledCallback([=] { select->clearQuery(); });
	...
	_setInnerFocus = [=] { select->setInnerFocus(); };
	_jump = [=](int rows) { return inner->jump(rows); };
}
...
int AyuUi::FontSelectorBox::rowsInPage() const { return std::max(height() / AyuUi::Rows::DefaultRowHeight(), 1); }

void AyuUi::FontSelectorBox::setInnerFocus() { _setInnerFocus(); }

base::binary_guard AyuUi::FontSelectorBox::Show(Window::SessionController *controller,
												const Fn<void(QString font)> hook) {
