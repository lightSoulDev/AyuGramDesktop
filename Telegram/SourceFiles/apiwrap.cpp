...
[[nodiscard]] std::shared_ptr<ChatHelpers::Show> ShowForPeer(not_null<PeerData *> peer) {
...
void ShowChannelsLimitBox(not_null<PeerData *> peer) {
	if (const auto window = Core::App().windowFor(peer)) {
		window->invokeForSessionController(&peer->session().account(),
										   peer,
...
[[nodiscard]] FileLoadTo FileLoadTaskOptions(const Api::SendAction &action) {
	...
	return FileLoadTo(peer->id, action.options, action.replyTo, action.replaceMediaOf);
}
...
ApiWrap::ApiWrap(not_null<Main::Session *> session)
	: MTP::Sender(&session->account().mtp()), _session(session),
	  _messageDataResolveDelayed([=] { resolveMessageDatas(); }), _webPagesTimer([=] { resolveWebPages(); }),
	  _draftsSaveTimer([=] { saveDraftsToCloud(); }), _featuredSetsReadTimer([=] { readFeaturedSets(); }),
	  _dialogsLoadState(std::make_unique<DialogsLoadState>()),
	  _fileLoader(std::make_unique<TaskQueue>(kFileLoaderQueueStopTimeout)),
	  _topPromotionTimer([=] { refreshTopPromotion(); }), _updateNotifyTimer([=] { sendNotifySettingsUpdates(); }),
	  ...
	  _userPrivacy(std::make_unique<Api::UserPrivacy>(this)), _inviteLinks(std::make_unique<Api::InviteLinks>(this)),
	  _chatLinks(std::make_unique<Api::ChatLinks>(this)), _views(std::make_unique<Api::ViewsManager>(this)),
	  _confirmPhone(std::make_unique<Api::ConfirmPhone>(this)), _peerPhoto(std::make_unique<Api::PeerPhoto>(this)),
	  _polls(std::make_unique<Api::Polls>(this)), _chatParticipants(std::make_unique<Api::ChatParticipants>(this)),
	  _unreadThings(std::make_unique<Api::UnreadThings>(this)), _ringtones(std::make_unique<Api::Ringtones>(this)),
	  _transcribes(std::make_unique<Api::Transcribes>(this)), _premium(std::make_unique<Api::Premium>(this)),
	  _usernames(std::make_unique<Api::Usernames>(this)), _websites(std::make_unique<Api::Websites>(this)),
	...
	crl::on_main(session,
				 [=]
				 {
					 ...
					 _session->data().chatsFilters().changed() |
						 rpl::filter([=] { return _session->data().chatsFilters().archiveNeeded(); }) |
						 rpl::start_with_next([=] { requestMoreDialogsIfNeeded(); }, _session->lifetime());

					 setupSupportMode();

					 Core::App().settings().proxy().connectionTypeValue() |
						 rpl::start_with_next([=] { refreshTopPromotion(); }, _session->lifetime());
				 });
}
...
void ApiWrap::requestChangelog(const QString &sinceVersion, Fn<void(const MTPUpdates &result)> callback) {
...
void ApiWrap::refreshTopPromotion() {
	...
	const auto next = (_topPromotionNextRequestTime != 0) ? _topPromotionNextRequestTime : now;
	...
	_topPromotionRequestId = request(MTPhelp_GetPromoData())
								 .done(
									 [=](const MTPhelp_PromoData &result)
									 {
								 ...
								 .fail(
									 [=]
									 {
										 _topPromotionRequestId = 0;
										 const auto now = base::unixtime::now();
										 const auto next = _topPromotionNextRequestTime = now + kTopPromotionInterval;
...
void ApiWrap::getTopPromotionDelayed(TimeId now, TimeId next) {
	_topPromotionTimer.callOnce(std::min(std::max(next - now, kTopPromotionMinDelay), kTopPromotionInterval) *
								crl::time(1000));
};

void ApiWrap::topPromotionDone(const MTPhelp_PromoData &proxy) {
	_topPromotionNextRequestTime = proxy.match([&](const auto &data) { return data.vexpires().v; });
	getTopPromotionDelayed(base::unixtime::now(), _topPromotionNextRequestTime);
	...
	proxy.match([&](const MTPDhelp_promoDataEmpty &data)
				...
				[&](const MTPDhelp_promoData &data)
				{
					...
					_session->data().setTopPromoted(
						history, data.vpsa_type().value_or_empty(), data.vpsa_message().value_or_empty());
				});
}

void ApiWrap::requestDeepLinkInfo(const QString &path,
	...
	_deepLinkInfoRequestId =
		request(MTPhelp_GetDeepLinkInfo(MTP_string(path)))
			.done(
				[=](const MTPhelp_DeepLinkInfo &result)
				{
...
void ApiWrap::requestTermsUpdate() {
	...
	if (_termsUpdateSendAt && now < _termsUpdateSendAt) {
		base::call_delayed(_termsUpdateSendAt - now, _session, [=] { requestTermsUpdate(); });
		return;
	}
	...
	_termsUpdateRequestId =
		request(MTPhelp_GetTermsOfServiceUpdate())
			.done(
				[=](const MTPhelp_TermsOfServiceUpdate &result)
				{
					...
					const auto requestNext = [&](auto &&data)
					{
						...
						_termsUpdateSendAt = crl::now() +
							std::clamp(timeout * crl::time(1000), kTermsUpdateTimeoutMin, kTermsUpdateTimeoutMax);
						requestTermsUpdate();
					};
					switch (result.type()) {
						...
						case mtpc_help_termsOfServiceUpdate: {
							...
							session().lockByTerms(Window::TermsLock::FromMTP(_session, fields));
							requestNext(data);
						} break;
...
void ApiWrap::acceptTerms(bytes::const_span id) {
	request(MTPhelp_AcceptTermsOfService(MTP_dataJSON(MTP_bytes(id)))).done([=] { requestTermsUpdate(); }).send();
}
...
void ApiWrap::checkChatInvite(const QString &hash,
	...
	_checkInviteRequestId =
		request(MTPmessages_CheckChatInvite(MTP_string(hash))).done(std::move(done)).fail(std::move(fail)).send();
}

void ApiWrap::checkFilterInvite(const QString &slug,
	...
	_checkFilterInviteRequestId =
		request(MTPchatlists_CheckChatlistInvite(MTP_string(slug))).done(std::move(done)).fail(std::move(fail)).send();
}
...
void ApiWrap::savePinnedOrder(Data::Folder *folder) {
	...
	request(MTPmessages_ReorderPinnedDialogs(MTP_flags(MTPmessages_ReorderPinnedDialogs::Flag::f_force),
...
void ApiWrap::savePinnedOrder(not_null<Data::Forum *> forum) {
	...
	request(MTPchannels_ReorderPinnedForumTopics(MTP_flags(MTPchannels_ReorderPinnedForumTopics::Flag::f_force),
...
void ApiWrap::savePinnedOrder(not_null<Data::SavedMessages *> saved) {
	...
	peers.reserve(order.size());
	ranges::transform(order, ranges::back_inserter(peers), input);
	request(MTPmessages_ReorderPinnedSavedDialogs(MTP_flags(MTPmessages_ReorderPinnedSavedDialogs::Flag::f_force),
												  MTP_vector(peers)))
		.send();
}

void ApiWrap::toggleHistoryArchived(not_null<History *> history, bool archived, Fn<void()> callback) {
	...
	const auto requestId =
		request(MTPfolders_EditPeerFolders(MTP_vector<MTPInputFolderPeer>(
					1, MTP_inputFolderPeer(history->peer->input, MTP_int(archived ? archiveId : 0)))))
			.done(
...
void ApiWrap::sendMessageFail(const MTP::Error &error, not_null<PeerData *> peer, uint64 randomId, FullMsgId itemId) {
	sendMessageFail(error.type(), peer, randomId, itemId);
}

void ApiWrap::sendMessageFail(const QString &error, not_null<PeerData *> peer, uint64 randomId, FullMsgId itemId) {
	const auto show = ShowForPeer(peer);
	const auto paidStarsPrefix = u"ALLOW_PAYMENT_REQUIRED_"_q;
	if (show && error == u"PEER_FLOOD"_q) {
		show->showBox(Ui::MakeInformBox(PeerFloodErrorText(&session(), PeerFloodType::Send)),
					  Ui::LayerOption::CloseOther);
	} else if (show && error == u"USER_BANNED_IN_CHANNEL"_q) {
		const auto link =
			Ui::Text::Link(tr::lng_cant_more_info(tr::now), session().createInternalLinkFull(u"spambot"_q));
		show->showBox(
			Ui::MakeInformBox(tr::lng_error_public_groups_denied(tr::now, lt_more_info, link, Ui::Text::WithEntities)),
			Ui::LayerOption::CloseOther);
		...
		if (const auto channel = peer->asChannel()) {
			...
			if (seconds >= left) {
				channel->growSlowmodeLastMessage(base::unixtime::now() - (left - seconds));
			} else {
		...
		if (const auto item = scheduled.lookupItem(peer->id, itemId.msg)) {
			...
			if (show) {
				show->showBox(Ui::MakeInformBox(tr::lng_cant_do_this()), Ui::LayerOption::CloseOther);
			}
		}
	} else if (show && error == u"CHAT_FORWARDS_RESTRICTED"_q) {
		show->showToast(peer->isBroadcast() ? tr::lng_error_noforwards_channel(tr::now)
...
void ApiWrap::requestMessageData(PeerData *peer, MsgId msgId, Fn<void()> done) {
	auto &requests = (peer && peer->isChannel()) ? _channelMessageDataRequests[peer->asChannel()][msgId]
...
auto ApiWrap::messageDataRequests(ChannelData *channel, bool onlyExisting) -> MessageDataRequests * {
	if (!channel) {
	...
	return &_channelMessageDataRequests.emplace(channel, MessageDataRequests()).first->second;
}
...
void ApiWrap::resolveMessageDatas() {
	...
	if (!ids.isEmpty()) {
		const auto requestId = request(MTPmessages_GetMessages(MTP_vector<MTPInputMessage>(ids)))
								   .done(
									   [=](const MTPmessages_Messages &result, mtpRequestId requestId)
									   {
	...
	for (auto j = _channelMessageDataRequests.begin(); j != _channelMessageDataRequests.cend();) {
		...
		if (!ids.isEmpty()) {
			...
			const auto requestId =
				request(MTPchannels_GetMessages(channel->inputChannel, MTP_vector<MTPInputMessage>(ids)))
					.done(
						[=](const MTPmessages_Messages &result, mtpRequestId requestId)
						{
...
void ApiWrap::finalizeMessageDataRequest(ChannelData *channel, mtpRequestId requestId) {
	...
	for (auto i = requests->begin(); i != requests->cend();) {
		if (i->second.requestId == requestId) {
			...
			if (callbacks.empty()) {
				...
				callbacks.insert(
					end(callbacks), std::make_move_iterator(begin(list)), std::make_move_iterator(end(list)));
			}
			i = requests->erase(i);
		} else {
			++i;
...
QString ApiWrap::exportDirectMessageLink(not_null<HistoryItem *> item,
	...
	const auto fallback = [&]
	{
		...
		if (inRepliesContext) {
			...
			const auto rootId = linkThreadIsTopic ? item->topicRootId() : item->replyToTop();
			if (rootId) {
		...
		const auto query = base + '/' +
			(linkCommentId								? (post + "?comment=" + QString::number(linkCommentId.bare))
				 : (linkThreadId && !linkThreadIsTopic) ? (post + "?thread=" + QString::number(linkThreadId.bare))
	...
	const auto current = (i != end(_unlikelyMessageLinks)) ? i->second : fallback();
	request(MTPchannels_ExportMessageLink(MTP_flags(inRepliesContext ? MTPchannels_ExportMessageLink::Flag::f_thread
																	 : MTPchannels_ExportMessageLink::Flag(0)),
										  channel->inputChannel,
										  MTP_int(item->id)))
		.done(
			[=](const MTPExportedMessageLink &result)
			{
		...
		.send();
	const auto addTimestamp = channel->hasUsername() && !inRepliesContext && videoTimestamp.has_value();
	const auto addedSeparator = (current.indexOf('?') >= 0) ? '&' : '?';
	const auto addedTimestamp =
		addTimestamp ? (addedSeparator + u"t="_q + FormatVideoTimestamp(*videoTimestamp)) : QString();
...
QString ApiWrap::exportDirectStoryLink(not_null<Data::Story *> story) {
	...
	request(MTPstories_ExportStoryLink(peer->input, MTP_int(story->id())))
		.done(
			[=](const MTPExportedStoryLink &result)
			{
				const auto link = qs(result.data().vlink());
				if (current != link) {
...
void ApiWrap::requestContacts() {
	...
	_contactsRequestId = request(MTPcontacts_GetContacts(MTP_long(0) // hash
														 ))
							 .done(
...
void ApiWrap::requestMoreDialogs(Data::Folder *folder) {
	...
	const auto flags = MTPmessages_GetDialogs::Flag::f_exclude_pinned | MTPmessages_GetDialogs::Flag::f_folder_id;
	const auto hash = uint64(0);
	state->requestId =
		request(MTPmessages_GetDialogs(MTP_flags(flags),
									   MTP_int(folder ? folder->id() : 0),
									   MTP_int(state->offsetDate),
									   MTP_int(state->offsetId),
									   (state->offsetPeer ? state->offsetPeer->input : MTP_inputPeerEmpty()),
									   MTP_int(loadCount),
									   MTP_long(hash)))
			.done(
				[=](const MTPmessages_Dialogs &result)
				{
					...
					const auto count = result.match(
						...
						{
							updateDialogsOffset(folder, data.vdialogs().v, data.vmessages().v);
					...
					result.match([](const MTPDmessages_dialogsNotModified &data)
								 ...
								 [&](const auto &data)
								 {
									 ...
									 _session->data().applyDialogs(
										 folder, data.vmessages().v, data.vdialogs().v, count);
								 });

					if (!folder && (!_dialogsLoadState || !_dialogsLoadState->listReceived)) {
						refreshDialogsLoadBlocked();
					}
...
void ApiWrap::refreshDialogsLoadBlocked() {
	_dialogsLoadMayBlockByDate = _dialogsLoadState && !_dialogsLoadState->listReceived && (_dialogsLoadTill > 0);
	_dialogsLoadBlockedByDate = _dialogsLoadState && !_dialogsLoadState->listReceived &&
		!_dialogsLoadState->requestId && (_dialogsLoadTill > 0) && (_dialogsLoadState->offsetDate > 0) &&
		(_dialogsLoadState->offsetDate <= _dialogsLoadTill);
}

void ApiWrap::requestMoreDialogsIfNeeded() {
	const auto dialogsReady = !_dialogsLoadState || _dialogsLoadState->listReceived;
	...
	} else if (const auto folder = _session->data().folderLoaded(Data::Folder::kId)) {
		...
		dialog.match(
			[&](const auto &dialog)
			{
				...
				for (const auto &message : ranges::views::reverse(messages)) {
					if (IdFromMessage(message) == messageId && PeerFromMessage(message) == peer) {
...
void ApiWrap::dialogsLoadFinish(Data::Folder *folder) {
	const auto notify = [&]
	{ Core::App().postponeCall(crl::guard(_session, [=] { _session->data().chatsListDone(folder); })); };
	const auto state = dialogsLoadState(folder);
		...
		request(MTPmessages_GetPinnedDialogs(MTP_int(folder ? folder->id() : 0)))
			.done(
				[=](const MTPmessages_PeerDialogs &result)
				{
					finalize();
					result.match(
						[&](const MTPDmessages_peerDialogs &data)
						{
							_session->data().processUsers(data.vusers());
							_session->data().processChats(data.vchats());
							_session->data().clearPinnedChats(folder);
							_session->data().applyDialogs(folder, data.vmessages().v, data.vdialogs().v);
							_session->data().chatsListChanged(folder);
							_session->data().notifyPinnedDialogsOrderUpdated();
						});
				})
...
void ApiWrap::requestMoreBlockedByDateDialogs() {
	...
	_dialogsLoadTill =
		_dialogsLoadState->offsetDate ? (_dialogsLoadState->offsetDate - max) : (base::unixtime::now() - max);
	refreshDialogsLoadBlocked();
	requestDialogs();
}

rpl::producer<bool> ApiWrap::dialogsLoadMayBlockByDate() const { return _dialogsLoadMayBlockByDate.value(); }

rpl::producer<bool> ApiWrap::dialogsLoadBlockedByDate() const { return _dialogsLoadBlockedByDate.value(); }

void ApiWrap::requestWallPaper(const QString &slug, Fn<void(const Data::WallPaper &)> done, Fn<void()> fail) {
	...
	_wallPaperRequestId = request(MTPaccount_GetWallPaper(MTP_inputWallPaperSlug(MTP_string(slug))))
							  .done(
								  [=](const MTPWallPaper &result)
								  {
	...
	const auto requestId = [&]
	{
		...
		if (const auto user = peer->asUser()) {
			...
			return request(MTPmessages_GetFullChat(chat->inputChat))
				.done([=](const MTPmessages_ChatFull &result) { gotChatFull(peer, result); })
				.fail(failHandler)
				.send();
		} else if (const auto channel = peer->asChannel()) {
...
void ApiWrap::processFullPeer(not_null<PeerData *> peer, const MTPmessages_ChatFull &result) {
	gotChatFull(peer, result);
}

void ApiWrap::gotChatFull(not_null<PeerData *> peer, const MTPmessages_ChatFull &result) {
...
void ApiWrap::gotUserFull(not_null<UserData *> user, const MTPusers_UserFull &result) {
	result.match(
		[&](const MTPDusers_userFull &data)
		{
			data.vfull_user().match(
				[&](const MTPDuserFull &fields)
				{
					if (user == _session->user() && !_session->validateSelf(fields.vid().v)) {
						constexpr auto kRequestUserAgainTimeout = crl::time(10000);
						base::call_delayed(kRequestUserAgainTimeout, _session, [=] { requestFullPeer(user); });
						return;
					}
...
void ApiWrap::requestPeerSettings(not_null<PeerData *> peer) {
	...
	request(MTPmessages_GetPeerSettings(peer->input))
		.done(
			[=](const MTPmessages_PeerSettings &result)
			{
				result.match(
					[&](const MTPDmessages_peerSettings &data)
					{
						_session->data().processUsers(data.vusers());
						_session->data().processChats(data.vchats());
		...
		.fail([=] { _requestedPeerSettings.erase(peer); })
		.send();
}
...
void ApiWrap::migrateChat(not_null<ChatData *> chat,
	...
	const auto callback = [&] { return MigrateCallbacks{std::move(done), std::move(fail)}; };
	...
	if (const auto channel = chat->migrateTo()) {
		...
		crl::on_main([=]
					 { migrateFail(chat, MTP::Error::Local("BAD_MIGRATION", "Chat is already deactivated").type()); });
		return;
		...
		crl::on_main(
			...
			{
				migrateFail(chat,
							MTP::Error::Local("BAD_MIGRATION", "Current user is not the creator of that chat").type());
			});
		return;
	}
		...
		.done(
			[=](const MTPUpdates &result)
			{
				...
				if (const auto channel = chat->migrateTo()) {
					...
					migrateFail(chat, MTP::Error::Local("MIGRATION_FAIL", "No channel").type());
				}
			})
...
void ApiWrap::migrateDone(not_null<PeerData *> peer, not_null<ChannelData *> channel) {
	session().changes().sendNotifications();
...
void ApiWrap::markContentsRead(const base::flat_set<not_null<HistoryItem *>> &items) {
	const auto settings = &AyuSettings::getInstance();

	auto markedIds = QVector<MTPint>();
	auto channelMarkedIds = base::flat_map<not_null<ChannelData *>, QVector<MTPint>>();
	...
	if (!markedIds.isEmpty()) {
		request(MTPmessages_ReadMessageContents(MTP_vector<MTPint>(markedIds)))
			.done([=](const MTPmessages_AffectedMessages &result) { applyAffectedMessages(result); })
			.send();
	}
	for (const auto &channelIds : channelMarkedIds) {
		request(MTPchannels_ReadMessageContents(channelIds.first->inputChannel, MTP_vector<MTPint>(channelIds.second)))
			.send();
	}
}
...
void ApiWrap::markContentsRead(not_null<HistoryItem *> item) {
	...
	if (const auto channel = item->history()->peer->asChannel()) {
		request(MTPchannels_ReadMessageContents(channel->inputChannel, ids)).send();
	} else {
...
void ApiWrap::deleteAllFromParticipant(not_null<ChannelData *> channel, not_null<PeerData *> from) {
	const auto history = _session->data().historyLoaded(channel);
	const auto ids = history ? history->collectMessagesFromParticipantToDelete(from) : std::vector<MsgId>();
...
void ApiWrap::deleteAllFromParticipantSend(not_null<ChannelData *> channel, not_null<PeerData *> from) {
	request(MTPchannels_DeleteParticipantHistory(channel->inputChannel, from->input))
		.done(
...
void ApiWrap::requestStickerSets() {
	for (auto &[id, info] : _stickerSetRequests) {
		...
		info.id = request(MTPmessages_GetStickerSet(MTP_inputStickerSetID(MTP_long(id), MTP_long(info.accessHash)),
													MTP_int(0) // hash
													))
					  .done([=, setId = id](const MTPmessages_StickerSet &result) { gotStickerSet(setId, result); })
...
void ApiWrap::saveStickerSets(const Data::StickersSetsOrder &localOrder,
	...
	auto &setDisenableRequests = (type == Data::StickersType::Emoji) ? _customEmojiSetDisenableRequests
	...
	const auto stickersSaveOrder = [=]
	{
		...
		const auto flags = (type == Data::StickersType::Emoji) ? Flag::f_emojis
			: (type == Data::StickersType::Masks)			   ? Flag::f_masks
															   : Flag(0);
		reorderRequestId() = request(MTPmessages_ReorderStickerSets(MTP_flags(flags), MTP_vector<MTPlong>(mtpOrder)))
	...
	auto writeInstalled = true, writeRecent = false, writeCloudRecent = false, writeCloudRecentAttached = false,
		 writeFaved = false, writeArchived = false;
	auto &recent = _session->data().stickers().getRecentPack();
	auto &sets = _session->data().stickers().setsRef();

	auto &order = (type == Data::StickersType::Emoji) ? _session->data().stickers().emojiSetsOrder()
		: (type == Data::StickersType::Masks)		  ? _session->data().stickers().maskSetsOrder()
													  : _session->data().stickers().setsOrder();
	auto &orderRef = (type == Data::StickersType::Emoji) ? _session->data().stickers().emojiSetsOrderRef()
		: (type == Data::StickersType::Masks)			 ? _session->data().stickers().maskSetsOrderRef()
	...
	for (const auto removedSetId : localRemoved) {
		if ((removedSetId == Data::Stickers::CloudRecentSetId) ||
			...
			const auto isAttached = (removedSetId == Data::Stickers::CloudRecentAttachedSetId);
			const auto flags = isAttached ? MTPmessages_ClearRecentStickers::Flag::f_attached
										  : MTPmessages_ClearRecentStickers::Flags(0);
			auto &requestId = isAttached ? _stickersClearRecentAttachedRequestId : _stickersClearRecentRequestId;
			const auto finish = [=]
			{ (isAttached ? _stickersClearRecentAttachedRequestId : _stickersClearRecentRequestId) = 0; };
			requestId = request(MTPmessages_ClearRecentStickers(MTP_flags(flags))).done(finish).fail(finish).send();
			continue;
		}
			...
			if (!archived) {
				...
				auto requestId =
					request(MTPmessages_UninstallStickerSet(setId))
						.done([=](const MTPBool &result, mtpRequestId requestId) { stickerSetDisenabled(requestId); })
						.fail([=](const MTP::Error &error, mtpRequestId requestId) { stickerSetDisenabled(requestId); })
						.afterDelay(kSmallDelayMs)
						.send();
	...
	for (auto &[id, set] : sets) {
		const auto archived = !!(set->flags & Flag::Archived);
		const auto thatType = !!(set->flags & Flag::Emoji) ? Data::StickersType::Emoji
		...
		if (archived && !localRemoved.contains(set->id)) {
			...
			const auto requestId =
				request(MTPmessages_InstallStickerSet(mtpSetId, MTP_boolFalse()))
					.done([=](const MTPmessages_StickerSetInstallResult &result, mtpRequestId requestId)
	...
	for (auto it = sets.begin(); it != sets.cend();) {
		const auto set = it->second.get();
		if ((set->flags & Flag::Featured) || (set->flags & Flag::Installed) || (set->flags & Flag::Archived) ||
			(set->flags & Flag::Special) || (set->flags & Flag::Emoji) || (set->locked > 0)) {
			++it;
		} else {
			it = sets.erase(it);
		}
...
void ApiWrap::joinChannel(not_null<ChannelData *> channel) {
	if (channel->amIn()) {
		session().changes().peerUpdated(channel, Data::PeerUpdate::Flag::ChannelAmIn);
			...
			request(MTPchannels_JoinChannel(channel->inputChannel))
				...
				.fail(
					[=](const MTP::Error &error)
					{
						...
						if (type == u"CHANNEL_PRIVATE"_q && channel->invitePeekExpires()) {
							...
							const auto text = [&]
							{
								if (type == u"INVITE_REQUEST_SENT"_q) {
									return channel->isMegagroup() ? tr::lng_group_request_sent(tr::now)
																  : tr::lng_group_request_sent_channel(tr::now);
								} else if (type == u"CHANNEL_PRIVATE"_q || type == u"CHANNEL_PUBLIC_GROUP_NA"_q ||
										   type == u"USER_BANNED_IN_CHANNEL"_q) {
									return channel->isMegagroup() ? tr::lng_group_not_accessible(tr::now)
...
void ApiWrap::requestNotifySettings(const MTPInputNotifyPeer &peer) {
	const auto bad = peer.match([](const MTPDinputNotifyUsers &) { return false; },
	...
	{
		return inputPeer.match([&](const MTPDinputPeerSelf &) { return _session->userPeerId(); },
							   [](const MTPDinputPeerEmpty &) { return PeerId(0); },
							   [](const MTPDinputPeerChannel &data) { return peerFromChannel(data.vchannel_id()); },
							   [](const MTPDinputPeerChat &data) { return peerFromChat(data.vchat_id()); },
							   [](const MTPDinputPeerUser &data) { return peerFromUser(data.vuser_id()); },
							   [](const auto &) -> PeerId
							   { Unexpected("Type in ApiRequest::requestNotifySettings peer."); });
	};
	const auto key =
		peer.match([](const MTPDinputNotifyUsers &) { return NotifySettingsKey{peerFromUser(1)}; },
				   [](const MTPDinputNotifyChats &) { return NotifySettingsKey{peerFromChat(1)}; },
				   [](const MTPDinputNotifyBroadcasts &) { return NotifySettingsKey{peerFromChannel(1)}; },
				   [&](const MTPDinputNotifyPeer &data) { return NotifySettingsKey{peerFromInput(data.vpeer())}; },
				   [&](const MTPDinputNotifyForumTopic &data)
				   {
	...
	const auto requestId =
		request(MTPaccount_GetNotifySettings(peer))
			.done(
				[=](const MTPPeerNotifySettings &result)
				{
			...
			.fail(
				[=]
				{
					_session->data().notifySettings().apply(peer,
															MTP_peerNotifySettings(MTP_flags(0),
																				   MTPBool(),
																				   MTPBool(),
																				   MTPint(),
...
void ApiWrap::updateNotifySettingsDelayed(not_null<const Data::Thread *> thread) {
	...
	if (_updateNotifyTopics.emplace(topic).second) {
		topic->destroyed() |
			rpl::start_with_next([=] { _updateNotifyTopics.remove(topic); }, _updateNotifyQueueLifetime);
...
void ApiWrap::sendNotifySettingsUpdates() {
	...
	for (const auto topic : base::take(_updateNotifyTopics)) {
		request(MTPaccount_UpdateNotifySettings(
					MTP_inputNotifyForumTopic(topic->channel()->input, MTP_int(topic->rootId())),
					topic->notify().serialize()))
			.afterDelay(kSmallDelayMs)
			.send();
	}
	for (const auto peer : base::take(_updateNotifyPeers)) {
		request(MTPaccount_UpdateNotifySettings(MTP_inputNotifyPeer(peer->input), peer->notify().serialize()))
			.afterDelay(kSmallDelayMs)
			.send();
	}
	...
	for (const auto type : base::take(_updateNotifyDefaults)) {
		request(
			MTPaccount_UpdateNotifySettings(Data::DefaultNotifyToMTP(type), settings.defaultSettings(type).serialize()))
			.afterDelay(kSmallDelayMs)
			.send();
	}
...
void ApiWrap::updatePrivacyLastSeens() {
	...
	if (!_session->premium()) {
		_session->data().enumerateUsers(
			[&](not_null<UserData *> user)
			{
				...
				user->updateLastseen((till + 3 * 86400 >= now)		  ? Data::LastseenStatus::Recently(true)
	...
	_contactsStatusesRequestId =
		request(MTPcontacts_GetStatuses())
			.done(
				[=](const MTPVector<MTPContactStatus> &result)
				{
					...
					for (const auto &status : result.v) {
						...
						if (const auto user = _session->data().userLoaded(userId)) {
							const auto status = LastseenFromMTP(data.vstatus(), user->lastseen());
...
void ApiWrap::clearHistory(not_null<PeerData *> peer, bool revoke) { deleteHistory(peer, true, revoke); }

void ApiWrap::deleteConversation(not_null<PeerData *> peer, bool revoke) {
	if (const auto chat = peer->asChat()) {
		request(MTPmessages_DeleteChatUser(MTP_flags(0), chat->inputChat, _session->user()->inputUser))
			.done(
				[=](const MTPUpdates &result)
				{
					applyUpdates(result);
	...
	if (const auto channel = peer->asChannel()) {
		if (!justClear && !revoke) {
			...
			if (deleteTillId || (!justClear && revoke)) {
				history->owner().histories().deleteAllMessages(history, deleteTillId, justClear, revoke);
			}
		}
	} else {
		history->owner().histories().deleteAllMessages(history, deleteTillId, justClear, revoke);
...
void ApiWrap::applyUpdates(const MTPUpdates &updates, uint64 sentMessageRandomId) const {
	this->updates().applyUpdates(updates, sentMessageRandomId);
}

int ApiWrap::applyAffectedHistory(PeerData *peer, const MTPmessages_AffectedHistory &result) const {
...
void ApiWrap::applyAffectedMessages(not_null<PeerData *> peer, const MTPmessages_AffectedMessages &result) {
...
void ApiWrap::applyAffectedMessages(const MTPmessages_AffectedMessages &result) const {
	const auto &data = result.c_messages_affectedMessages();
	updates().updateAndApply(data.vpts().v, data.vpts_count().v);
}
...
void ApiWrap::saveCurrentDraftToCloud() {
	...
	for (const auto &controller : _session->windows()) {
		if (const auto thread = controller->activeChatCurrent().thread()) {
			...
			if (!Data::DraftsAreEqual(localDraft, cloudDraft) && !_session->supportMode()) {
...
void ApiWrap::saveDraftsToCloud() {
	for (auto i = begin(_draftsSaveRequestIds); i != end(_draftsSaveRequestIds);) {
		...
		auto entities = Api::EntitiesToMTP(
			_session, TextUtilities::ConvertTextTagsToEntities(textWithTags.tags), Api::ConvertOption::SkipLocal);

		history->startSavingCloudDraft(topicRootId);
		cloudDraft->saveRequestId =
			request(MTPmessages_SaveDraft(MTP_flags(flags),
										  ReplyToForMTP(history, cloudDraft->reply),
										  history->peer->input,
										  MTP_string(textWithTags.text),
										  entities,
										  Data::WebPageForMTP(cloudDraft->webpage, textWithTags.text.isEmpty()),
										  MTP_long(0) // effect
										  ))
				.done(
					[=](const MTPBool &result, const MTP::Response &response)
					{
...
void ApiWrap::registerModifyRequest(const QString &key, mtpRequestId requestId) {
...
void ApiWrap::clearModifyRequest(const QString &key) { _modifyRequests.remove(key); }

void ApiWrap::gotStickerSet(uint64 setId, const MTPmessages_StickerSet &result) {
	_stickerSetRequests.remove(setId);
	result.match([&](const MTPDmessages_stickerSet &data) { _session->data().stickers().feedSetFull(data); },
...
void ApiWrap::resolveWebPages() {
			...
			if (const auto item = _session->data().findWebPageItem(page)) {
				if (const auto channel = item->history()->peer->asChannel()) {
					...
					if (channelMap == idsByChannel.cend()) {
						channelMap = idsByChannel
										 .emplace(channel,
												  IndexAndMessageIds(idsByChannel.size(),
																	 QVector<MTPInputMessage>(
																		 1, MTP_inputMessageID(MTP_int(item->id)))))
										 .first;
					} else {
						channelMap->second.second.push_back(MTP_inputMessageID(MTP_int(item->id)));
					}
	...
	if (!ids.isEmpty()) {
		requestId = request(MTPmessages_GetMessages(MTP_vector<MTPInputMessage>(ids)))
	...
	for (auto i = idsByChannel.cbegin(), e = idsByChannel.cend(); i != e; ++i) {
		reqsByIndex[i->second.first] =
			request(MTPchannels_GetMessages(i->first->inputChannel, MTP_vector<MTPInputMessage>(i->second.second)))
				.done([=, channel = i->first](const MTPmessages_Messages &result, mtpRequestId requestId)
...
void ApiWrap::requestFileReference(Data::FileOrigin origin, FileReferencesHandler &&handler, Request &&data) {
		...
		.done(
			[=](const auto &result)
			{
				...
				for (const auto &p : parsed.data) {
					...
					if (documentId) {
						_session->data().document(documentId->id)->refreshFileReference(reference);
					}
					const auto photoId = std::get_if<PhotoFileLocationId>(&origin);
					if (photoId) {
						_session->data().photo(photoId->id)->refreshFileReference(reference);
					}
				}
				const auto i = _fileReferenceHandlers.find(origin);
...
void ApiWrap::refreshFileReference(Data::FileOrigin origin,
	...
	return refreshFileReference(origin,
								crl::guard(task,
										   [=](const UpdatedFileReferences &data)
										   { task->refreshFileReferenceFrom(data, requestId, current); }));
}

void ApiWrap::refreshFileReference(Data::FileOrigin origin, FileReferencesHandler &&handler) {
	...
	v::match(
		...
		[&](Data::FileOriginMessage data)
		{
			if (const auto item = _session->data().message(data)) {
				...
				if (storyId) {
					request(MTPstories_GetStoriesByID(_session->data().peer(storyId.peer)->input,
													  MTP_vector<MTPint>(1, MTP_int(storyId.story))));
				} else if (item->isScheduled()) {
					const auto realId = _session->scheduledMessages().lookupId(item);
					request(MTPmessages_GetScheduledMessages(item->history()->peer->input,
					...
					request(MTPchannels_GetMessages(
						channel->inputChannel, MTP_vector<MTPInputMessage>(1, MTP_inputMessageID(MTP_int(item->id)))));
				} else {
					request(
						MTPmessages_GetMessages(MTP_vector<MTPInputMessage>(1, MTP_inputMessageID(MTP_int(item->id)))));
				}
			} else {
				fail();
			}
		...
		[&](Data::FileOriginUserPhoto data)
		{
			if (const auto user = _session->data().user(data.userId)) {
				request(MTPphotos_GetUserPhotos(user->inputUser, MTP_int(-1), MTP_long(data.photoId), MTP_int(1)));
			} else {
				fail();
			}
		},
		...
		[&](Data::FileOriginPeerPhoto data) { fail(); },
		[&](Data::FileOriginStickerSet data)
		{
			const auto isRecentAttached = (data.setId == Data::Stickers::CloudRecentAttachedSetId);
			if (data.setId == Data::Stickers::CloudRecentSetId || data.setId == Data::Stickers::RecentSetId ||
				isRecentAttached) {
				auto done = [=]
				{
				...
				request(MTPmessages_GetRecentStickers(MTP_flags(isRecentAttached
				...
				request(
					MTPmessages_GetStickerSet(MTP_inputStickerSetID(MTP_long(data.setId), MTP_long(data.accessHash)),
											  MTP_int(0)), // hash
					[=]
					{
						crl::on_main(_session,
		...
		{
			const auto useSlug = data.ownerId && (data.ownerId != session().userId()) && !data.slug.isEmpty();
			request(MTPaccount_GetWallPaper(
				useSlug ? MTP_inputWallPaperSlug(MTP_string(data.slug))
						: MTP_inputWallPaper(MTP_long(data.paperId), MTP_long(data.accessHash))));
		},
		[&](Data::FileOriginTheme data)
		{
			request(MTPaccount_GetTheme(MTP_string(Data::CloudThemes::Format()),
										MTP_inputTheme(MTP_long(data.themeId), MTP_long(data.accessHash))));
		},
		[&](Data::FileOriginRingtones data) { request(MTPaccount_GetSavedRingtones(MTP_long(0))); },
		[&](Data::FileOriginPremiumPreviews data) { request(MTPhelp_GetPremiumPromo()); },
		[&](Data::FileOriginWebPage data) { request(MTPmessages_GetWebPage(MTP_string(data.url), MTP_int(0))); },
		[&](Data::FileOriginStory data)
		{
			request(MTPstories_GetStoriesByID(_session->data().peer(data.peer)->input,
...
void ApiWrap::requestSpecialStickersForce(bool faved, bool recent, bool attached) {
	if (faved) {
...
void ApiWrap::setGroupStickerSet(not_null<ChannelData *> megagroup, const StickerSetIdentifier &set) {
	Expects(megagroup->mgInfo != nullptr);

	megagroup->mgInfo->stickerSet = set;
	request(MTPchannels_SetStickers(megagroup->inputChannel, Data::InputStickerSet(set))).send();
	_session->data().stickers().notifyUpdated(Data::StickersType::Stickers);
}

void ApiWrap::setGroupEmojiSet(not_null<ChannelData *> megagroup, const StickerSetIdentifier &set) {
	Expects(megagroup->mgInfo != nullptr);

	megagroup->mgInfo->emojiSet = set;
	request(MTPchannels_SetEmojiStickers(megagroup->inputChannel, Data::InputStickerSet(set))).send();
...
std::vector<not_null<DocumentData *>> *ApiWrap::stickersByEmoji(const QString &key) {
	...
	const auto sendRequest = [&]
	{
		...
		const auto now = crl::now();
		return (received > 0) && (received + kStickersByEmojiInvalidateTimeout) <= now;
	}();
	if (sendRequest) {
		const auto hash = (it != _stickersByEmoji.end()) ? it->second.hash : uint64(0);
		request(MTPmessages_GetStickers(MTP_string(key), MTP_long(hash)))
			.done(
				[=](const MTPmessages_Stickers &result)
				{
...
void ApiWrap::requestStickers(TimeId now) {
	if (!_session->data().stickers().updateNeeded(now) || _stickersUpdateRequest) {
		return;
	}
	const auto done = [=](const MTPmessages_AllStickers &result)
	{
		...
		result.match([&](const MTPDmessages_allStickersNotModified &) {},
					 ...
					 { _session->data().stickers().setsReceived(data.vsets().v, data.vhash().v); });
	};
	_stickersUpdateRequest = request(MTPmessages_GetAllStickers(MTP_long(Api::CountStickersHash(_session, true))))
								 .done(done)
								 .fail(
									 [=]
									 {
...
void ApiWrap::requestMasks(TimeId now) {
	if (!_session->data().stickers().masksUpdateNeeded(now) || _masksUpdateRequest) {
		return;
	}
	const auto done = [=](const MTPmessages_AllStickers &result)
	{
		...
		result.match([&](const MTPDmessages_allStickersNotModified &) {},
					 [&](const MTPDmessages_allStickers &data)
					 { _session->data().stickers().masksReceived(data.vsets().v, data.vhash().v); });
	};
	_masksUpdateRequest = request(MTPmessages_GetMaskStickers(MTP_long(Api::CountMasksHash(_session, true))))
							  .done(done)
							  .fail(
								  [=]
								  {
...
void ApiWrap::requestCustomEmoji(TimeId now) {
	if (!_session->data().stickers().emojiUpdateNeeded(now) || _customEmojiUpdateRequest) {
		return;
	}
	const auto done = [=](const MTPmessages_AllStickers &result)
	{
		...
		result.match([&](const MTPDmessages_allStickersNotModified &) {},
					 [&](const MTPDmessages_allStickers &data)
					 { _session->data().stickers().emojiReceived(data.vsets().v, data.vhash().v); });
	};
	_customEmojiUpdateRequest =
		request(MTPmessages_GetEmojiStickers(MTP_long(Api::CountCustomEmojiHash(_session, true))))
			.done(done)
			.fail(
				[=]
				{
...
void ApiWrap::requestRecentStickers(std::optional<TimeId> now, bool attached) {
	...
	{ return attached ? _recentAttachedStickersUpdateRequest : _recentStickersUpdateRequest; };
	if (requestId()) {
		return;
	}
	const auto finish = [=]
	...
	const auto flags =
		attached ? MTPmessages_getRecentStickers::Flag::f_attached : MTPmessages_getRecentStickers::Flags(0);
	requestId() =
		request(MTPmessages_GetRecentStickers(MTP_flags(flags),
											  MTP_long(now ? Api::CountRecentStickersHash(_session, attached) : 0)))
			.done(
				[=](const MTPmessages_RecentStickers &result)
				{
					finish();
					...
					switch (result.type()) {
						...
						case mtpc_messages_recentStickers: {
							...
							_session->data().stickers().specialSetReceived(
								attached ? Data::Stickers::CloudRecentAttachedSetId : Data::Stickers::CloudRecentSetId,
...
void ApiWrap::requestFavedStickers(std::optional<TimeId> now) {
	if (now) {
		if (!_session->data().stickers().favedUpdateNeeded(*now) || _favedStickersUpdateRequest) {
			return;
		}
	}
	_favedStickersUpdateRequest =
		request(MTPmessages_GetFavedStickers(MTP_long(now ? Api::CountFavedStickersHash(_session) : 0)))
			.done(
				[=](const MTPmessages_FavedStickers &result)
				{
					...
					switch (result.type()) {
						...
						case mtpc_messages_favedStickers: {
							...
							_session->data().stickers().specialSetReceived(Data::Stickers::FavedSetId,
...
void ApiWrap::requestFeaturedStickers(TimeId now) {
	if (!_session->data().stickers().featuredUpdateNeeded(now) || _featuredStickersUpdateRequest) {
		return;
	}
	_featuredStickersUpdateRequest =
		request(MTPmessages_GetFeaturedStickers(MTP_long(Api::CountFeaturedStickersHash(_session))))
			.done(
...
void ApiWrap::requestFeaturedEmoji(TimeId now) {
	if (!_session->data().stickers().featuredEmojiUpdateNeeded(now) || _featuredEmojiUpdateRequest) {
		return;
	}
	_featuredEmojiUpdateRequest =
		request(MTPmessages_GetFeaturedEmojiStickers(MTP_long(Api::CountFeaturedStickersHash(_session))))
			.done(
				[=](const MTPmessages_FeaturedStickers &result)
				{
...
void ApiWrap::requestSavedGifs(TimeId now) {
	if (!_session->data().stickers().savedGifsUpdateNeeded(now) || _savedGifsUpdateRequest) {
		return;
	}
	_savedGifsUpdateRequest = request(MTPmessages_GetSavedGifs(MTP_long(Api::CountSavedGifsHash(_session))))
								  .done(
									  [=](const MTPmessages_SavedGifs &result)
									  {
										  ...
										  switch (result.type()) {
											  ...
											  case mtpc_messages_savedGifs: {
												  ...
												  _session->data().stickers().gifsReceived(d.vgifs().v, d.vhash().v);
											  }
												  return;
...
void ApiWrap::readFeaturedSets() {
	...
	if (!wrappedIds.empty()) {
		auto requestData = MTPmessages_ReadFeaturedStickers(MTP_vector<MTPlong>(wrappedIds));
...
void ApiWrap::resolveJumpToDate(Dialogs::Key chat, const QDate &date, Fn<void(not_null<PeerData *>, MsgId)> callback) {
...
void ApiWrap::requestMessageAfterDate(not_null<PeerData *> peer,
	...
	auto send = [&](auto &&serialized)
	{
		request(std::move(serialized))
			.done(
				[=, callback = std::forward<Callback>(callback)](const MTPmessages_Messages &result)
				{
					...
					const auto list =
						result.match([&](const MTPDmessages_messages &data) { return handleMessages(data); },
									 [&](const MTPDmessages_messagesSlice &data) { return handleMessages(data); },
	...
	if (topicRootId) {
		send(MTPmessages_GetReplies(peer->input,
									MTP_int(topicRootId),
									MTP_int(offsetId),
									MTP_int(offsetDate),
									MTP_int(addOffset),
		...
		send(MTPmessages_GetHistory(peer->input,
									MTP_int(offsetId),
									MTP_int(offsetDate),
									MTP_int(addOffset),
									MTP_int(limit),
...
void ApiWrap::resolveJumpToHistoryDate(not_null<PeerData *> peer,
									   MsgId topicRootId,
									   const QDate &date,
									   Fn<void(not_null<PeerData *>, MsgId)> callback) {
	if (const auto channel = peer->migrateTo()) {
		return resolveJumpToHistoryDate(channel, topicRootId, date, std::move(callback));
	}
	const auto jumpToDateInPeer = [=]
	{ requestMessageAfterDate(peer, topicRootId, date, [=](MsgId itemId) { callback(peer, itemId); }); };
	if (const auto chat = topicRootId ? nullptr : peer->migrateFrom()) {
		requestMessageAfterDate(chat,
								0,
								date,
								[=](MsgId itemId)
								{
...
void ApiWrap::requestHistory(not_null<History *> history, MsgId messageId, SliceType slice) {
...
void ApiWrap::requestSharedMedia(
	not_null<PeerData *> peer, MsgId topicRootId, SharedMediaType type, MsgId messageId, SliceType slice) {
	...
	const auto prepared = Api::PrepareSearchRequest(peer, topicRootId, type, QString(), messageId, slice);
	if (!prepared) {
		return;
	}
	...
	histories.sendRequest(history,
						  ...
						  [=](Fn<void()> finish)
						  {
							  return request(std::move(*prepared))
								  .done(
									  [=](const Api::SearchRequestResult &result)
									  {
										  _sharedMediaRequests.remove(key);
										  auto parsed = Api::ParseSearchResult(peer, type, messageId, slice, result);
...
void ApiWrap::sharedMediaDone(not_null<PeerData *> peer,
							  MsgId topicRootId,
	...
	_session->storage().add(Storage::SharedMediaAddSlice(
		peer->id, topicRootId, type, std::move(parsed.messageIds), parsed.noSkipRange, parsed.fullCount));
...
mtpRequestId ApiWrap::requestGlobalMedia(Storage::SharedMediaType type,
										 const QString &query,
										 int32 offsetRate,
										 Data::MessagePosition offsetPosition,
										 Fn<void(Api::GlobalMediaResult)> done) {
	auto prepared = Api::PrepareGlobalMediaRequest(_session, offsetRate, offsetPosition, type, query);
	if (!prepared) {
		done({});
		return 0;
	}
	return request(std::move(*prepared))
		.done([=](const Api::SearchRequestResult &result) { done(Api::ParseGlobalMediaResult(_session, result)); })
		.fail([=] { done({}); })
		.send();
}

void ApiWrap::sendAction(const SendAction &action) {
	if (!action.options.scheduled && !action.options.shortcutId && !action.replaceMediaOf) {
		const auto topicRootId = action.replyTo.topicRootId;
		const auto topic = topicRootId ? action.history->peer->forumTopicFor(topicRootId) : nullptr;
		if (topic) {
			topic->readTillEnd();
		} else {
...
void ApiWrap::finishForwarding(const SendAction &action) {
	...
	if (!toForward.items.empty()) {
		const auto error = GetErrorForSending(history->peer,
											  {
...
void ApiWrap::forwardMessages(Data::ResolvedForwardDraft &&draft, SendAction action, FnMut<void()> &&successCallback) {
	...
	const auto shared = successCallback ? std::make_shared<SharedCallback>() : std::shared_ptr<SharedCallback>();
	if (successCallback) {
	...
	const auto genClientSideMessage =
		action.generateLocal && (count < 2) && (draft.options == Data::ForwardOptions::PreserveInfo);
	const auto history = action.history;
	const auto peer = history->peer;
	...
	const auto sendAccumulated = [&]
	{
		...
		const auto starsPaid = std::min(action.options.starsApproved, int(ids.size() * peer->starsPerMessageChecked()));
		...
		histories.sendRequest(
			...
			[=](Fn<void()> finish)
			{
				history->sendRequestId =
					request(MTPmessages_ForwardMessages(MTP_flags(oneFlags),
														forwardFrom->input,
...
void ApiWrap::shareContact(const QString &phone,
						   const QString &firstName,
						   const QString &lastName,
						   const SendAction &action,
						   Fn<void(bool)> done) {
	const auto userId = UserId(0);
	sendSharedContact(phone, firstName, lastName, userId, action, std::move(done));
}

void ApiWrap::shareContact(not_null<UserData *> user, const SendAction &action, Fn<void(bool)> done) {
	...
	return sendSharedContact(phone, user->firstName, user->lastName, userId, action, std::move(done));
}

void ApiWrap::sendSharedContact(const QString &phone,
								const QString &firstName,
								const QString &lastName,
								UserId userId,
								const SendAction &action,
	...
	const auto item = history->addNewLocalMessage(
		...
		TextWithEntities(),
		MTP_messageMediaContact(MTP_string(phone),
								MTP_string(firstName),
								MTP_string(lastName),
								MTP_string(), // vcard
								MTP_long(userId.bare)));

	const auto media = MTP_inputMediaContact(MTP_string(phone),
											 MTP_string(firstName),
											 MTP_string(lastName),
											 MTP_string()); // vcard
	...
	_session->changes().historyUpdated(
		history,
		(action.options.scheduled ? Data::HistoryUpdate::Flag::ScheduledSent : Data::HistoryUpdate::Flag::MessageSent));
}

void ApiWrap::sendVoiceMessage(
	QByteArray result, VoiceWaveform waveform, crl::time duration, bool video, const SendAction &action) {
	const auto caption = TextWithTags();
	const auto to = FileLoadTaskOptions(action);
	_fileLoader->addTask(std::make_unique<FileLoadTask>(&session(), result, duration, waveform, video, to, caption));
}

void ApiWrap::editMedia(Ui::PreparedList &&list, SendMediaType type, TextWithTags &&caption, const SendAction &action) {
	...
	_fileLoader->addTask(std::make_unique<FileLoadTask>(
		...
		(file.videoCover ? std::make_unique<FileLoadTask>(&session(),
...
void ApiWrap::sendFiles(Ui::PreparedList &&list,
						SendMediaType type,
						TextWithTags &&caption,
						std::shared_ptr<SendingAlbum> album,
						const SendAction &action) {
	const auto haveCaption = !caption.text.isEmpty();
	if (haveCaption && !list.canAddCaption(album != nullptr, type == SendMediaType::Photo)) {
	...
	for (auto &file : list.files) {
		const auto uploadWithType = !album												  ? type
			: (file.type == Ui::PreparedFile::Type::Photo && type != SendMediaType::File) ? SendMediaType::Photo
																						  : SendMediaType::File;
...
void ApiWrap::sendFile(const QByteArray &fileContent, SendMediaType type, const SendAction &action) {
	const auto to = FileLoadTaskOptions(action);
	...
	_fileLoader->addTask(std::make_unique<FileLoadTask>(
		&session(), QString(), fileContent, information, videoCover, type, to, caption, spoiler));
}

void ApiWrap::sendUploadedPhoto(FullMsgId localId, Api::RemoteFileInfo info, Api::SendOptions options) {
...
void ApiWrap::sendUploadedDocument(FullMsgId localId, Api::RemoteFileInfo info, Api::SendOptions options) {
	if (const auto item = _session->data().message(localId)) {
...
void ApiWrap::sendShortcutMessages(not_null<PeerData *> peer, BusinessShortcutId id) {
	auto ids = QVector<MTPint>();
	auto randomIds = QVector<MTPlong>();
	request(MTPmessages_SendQuickReplyMessages(
				peer->input, MTP_int(id), MTP_vector<MTPint>(ids), MTP_vector<MTPlong>(randomIds)))
		.done([=](const MTPUpdates &result) { applyUpdates(result); })
...
void ApiWrap::sendMessage(MessageToSend &&message) {
	...
	const auto replyTo = action.replyTo.messageId ? peer->owner().message(action.replyTo.messageId) : nullptr;
	const auto topicRootId = draftTopicRootId ? draftTopicRootId
		: replyTo							  ? replyTo->topicRootId()
											  : Data::ForumTopic::kGeneralId;
	const auto topic = peer->forumTopicFor(topicRootId);
	if (!(topic ? Data::CanSendTexts(topic) : Data::CanSendTexts(peer)) || Api::SendDice(message)) {
		return;
	}
	...
	auto isFirst = true;
	while (TextUtilities::CutPart(sending, left, MaxMessageSize) || (isFirst && exactWebPage)) {
		TextUtilities::Trim(left);
		const auto isLast = left.empty();

		auto newId = FullMsgId(peer->id, _session->data().nextLocalMessageId());
		...
		const auto ignoreWebPage = message.webPage.removed || (exactWebPage && !isLast);
		const auto manualWebPage = exactWebPage && !ignoreWebPage && (message.webPage.manual || (isLast && !isFirst));
		...
		if (ignoreWebPage) {
			...
			media = MTP_messageMediaWebPage(
				MTP_flags(PageFlag() | (manualWebPage ? PageFlag::f_manual : PageFlag()) |
						  (fields.forceLargeMedia ? PageFlag::f_force_large_media : PageFlag()) |
						  (fields.forceSmallMedia ? PageFlag::f_force_small_media : PageFlag())),
				MTP_webPagePending(MTP_flags(PendingFlag::f_url),
								   MTP_long(fields.id),
		...
		const auto silentPost = ShouldSendSilent(peer, action.options);
		FillMessagePostFlags(action, peer, flags);
		if ((exactWebPage && !ignoreWebPage && message.webPage.invert) || action.options.invertCaption) {
		...
		const auto sentEntities = Api::EntitiesToMTP(_session, sending.entities, Api::ConvertOption::SkipLocal);
		...
		lastMessage = history->addNewLocalMessage(
			...
			sending,
			media);
		const auto done = [=](const MTPUpdates &result, const MTP::Response &response)
		{
			if (clearCloudDraft) {
				history->finishSavingCloudDraft(draftTopicRootId, UnixtimeFromMsgId(response.outerMsgId));
			}

			AyuWorker::markAsOnline(_session);
		};
		const auto fail = [=](const MTP::Error &error, const MTP::Response &response)
		{
		...
		const auto mtpShortcut = Data::ShortcutIdToMTP(_session, action.options.shortcutId);
		if (exactWebPage && !ignoreWebPage && (manualWebPage || sending.empty())) {
			histories.sendPreparedMessage(
				history,
				action.replyTo,
				randomId,
				Data::Histories::PrepareMessage<MTPmessages_SendMedia>(MTP_flags(mediaFlags),
																	   peer->input,
			...
			histories.sendPreparedMessage(history,
										  action.replyTo,
										  randomId,
...
void ApiWrap::sendBotStart(std::shared_ptr<Ui::Show> show,
	...
	if (token.isEmpty()) {
		auto message = MessageToSend(Api::SendAction(_session->data().history(chat ? chat : bot.get())));
	...
	request(MTPmessages_StartBot(
				bot->inputUser, chat ? chat->input : MTP_inputPeerEmpty(), MTP_long(randomId), MTP_string(token)))
		.done(
			[=](const MTPUpdates &result)
			{
				applyUpdates(result);
...
void ApiWrap::sendInlineResult(not_null<UserData *> bot,
	...
	const auto history = action.history;
	const auto peer = history->peer;
	const auto newId = FullMsgId(peer->id, localMessageId ? (*localMessageId) : _session->data().nextLocalMessageId());
	const auto randomId = base::RandomValue<uint64>();
	...
	data->addToHistory(history,
					   {
						   ...
						   .shortcutId = action.options.shortcutId,
						   .starsPaid = starsPaid,
						   .viaBotId = ((bot && !action.options.hideViaBot) ? peerToUser(bot->id) : UserId()),
						   .postAuthor = NewMessagePostAuthor(action),
					   });
			...
			[=](const MTPMessageMedia &result)
			{
				...
				switch (result.type()) {
					case mtpc_messageMediaPhoto: {
						...
						const auto flags = Flag() | (data.vttl_seconds() ? Flag::f_ttl_seconds : Flag()) |
							(spoiler ? Flag::f_spoiler : Flag());
						const auto media = MTP_inputMediaPhoto(
							MTP_flags(flags),
							MTP_inputPhoto(fields.vid(), fields.vaccess_hash(), fields.vfile_reference()),
					...
					case mtpc_messageMediaDocument: {
						...
						const auto cover = (mtpCover && mtpCover->type() == mtpc_photo) ? &(mtpCover->c_photo())
						...
						const auto media = MTP_inputMediaDocument(
							MTP_flags(flags),
							MTP_inputDocument(fields.vid(), fields.vaccess_hash(), fields.vfile_reference()),
							(cover ? MTP_inputPhoto(cover->vid(), cover->vaccess_hash(), cover->vfile_reference())
								   : MTPInputPhoto()),
...
void ApiWrap::sendMediaWithRandomId(not_null<HistoryItem *> item,
	...
	auto sentEntities = Api::EntitiesToMTP(_session, caption.entities, Api::ConvertOption::SkipLocal);
	...
	const auto flags = Flag(0) | (replyTo ? Flag::f_reply_to : Flag(0)) |
		(ShouldSendSilent(history->peer, options) ? Flag::f_silent : Flag(0)) |
		(!sentEntities.v.isEmpty() ? Flag::f_entities : Flag(0)) |
		(options.scheduled ? Flag::f_schedule_date : Flag(0)) | (options.sendAs ? Flag::f_send_as : Flag(0)) |
		(options.shortcutId ? Flag::f_quick_reply_shortcut : Flag(0)) | (options.effectId ? Flag::f_effect : Flag(0)) |
		(options.invertCaption ? Flag::f_invert_media : Flag(0)) | (starsPaid ? Flag::f_allow_paid_stars : Flag(0));
		...
		Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
			...
			(options.price
				 ? MTPInputMedia(MTP_inputMediaPaidMedia(
					   MTP_flags(0), MTP_long(options.price), MTP_vector<MTPInputMedia>(1, media), MTPstring()))
				 : media),
...
void ApiWrap::sendMultiPaidMedia(not_null<HistoryItem *> item, not_null<SendingAlbum *> album, Fn<void(bool)> done) {
	...
	histories.sendPreparedMessage(
		...
		Data::Histories::PrepareMessage<MTPmessages_SendMedia>(
			...
			MTP_inputMediaPaidMedia(
				MTP_flags(0), MTP_long(options.price), MTP_vector<MTPInputMedia>(std::move(medias)), MTPstring()),
			MTP_string(caption.text),
			MTP_long(randomId),
			MTPReplyMarkup(),
			sentEntities,
...
void ApiWrap::sendAlbumWithCancelled(not_null<HistoryItem *> item, const MessageGroupId &groupId) {
	const auto albumIt = _sendingAlbums.find(groupId.raw());
...
void ApiWrap::sendAlbumIfReady(not_null<SendingAlbum *> album) {
	...
	if (!sample) {
		...
		album->sent = true;
		sendMediaWithRandomId(sample, single.vmedia(), album->options, single.vrandom_id().v);
		_sendingAlbums.remove(groupId);
		return;
	}
	...
	const auto starsPaid =
		std::min(history->peer->starsPerMessageChecked() * int(medias.size()), album->options.starsApproved);
	if (starsPaid) {
		album->options.starsApproved -= starsPaid;
	}
	using Flag = MTPmessages_SendMultiMedia::Flag;
	const auto flags = Flag(0) | (replyTo ? Flag::f_reply_to : Flag(0)) |
		(ShouldSendSilent(history->peer, album->options) ? Flag::f_silent : Flag(0)) |
		(album->options.scheduled ? Flag::f_schedule_date : Flag(0)) | (sendAs ? Flag::f_send_as : Flag(0)) |
...
rpl::producer<bool> ApiWrap::contactSignupSilent() const {
	return _contactSignupSilent ? _contactSignupSilentChanges.events_starting_with_copy(*_contactSignupSilent)
								: (_contactSignupSilentChanges.events() | rpl::type_erased());
}

std::optional<bool> ApiWrap::contactSignupSilentCurrent() const { return _contactSignupSilent; }

void ApiWrap::saveContactSignupSilent(bool silent) {
	request(base::take(_contactSignupSilentRequestId)).cancel();

	const auto requestId = request(MTPaccount_SetContactSignUpNotification(MTP_bool(silent)))
							   .done(
								   [=]
								   {
									   _contactSignupSilentRequestId = 0;
...
auto ApiWrap::botCommonGroups(not_null<UserData *> bot) const -> std::optional<std::vector<not_null<PeerData *>>> {
	const auto i = _botCommonGroups.find(bot);
	return (i != end(_botCommonGroups)) ? i->second : std::optional<std::vector<not_null<PeerData *>>>();
}

void ApiWrap::requestBotCommonGroups(not_null<UserData *> bot, Fn<void()> done) {
	...
	request(MTPmessages_GetCommonChats(bot->inputUser,
		...
		.done(
			...
			{
				const auto chats = result.match([](const auto &data) { return &data.vchats().v; });
...
void ApiWrap::saveSelfBio(const QString &text) {
	...
	_bio.requestId =
		request(MTPaccount_UpdateProfile(
					MTP_flags(MTPaccount_UpdateProfile::Flag::f_about), MTPstring(), MTPstring(), MTP_string(text)))
			.done(
...
void ApiWrap::registerStatsRequest(MTP::DcId dcId, mtpRequestId id) { _statsRequests[dcId].emplace(id); }
...
Api::AttachedStickers &ApiWrap::attachedStickers() { return *_attachedStickers; }
...
Api::SensitiveContent &ApiWrap::sensitiveContent() { return *_sensitiveContent; }
...
Api::ChatParticipants &ApiWrap::chatParticipants() { return *_chatParticipants; }
