...
public:
	...
	bool elementIsChatWide() override { return _widget ? _widget->elementIsChatWide() : false; }
...
HistoryInner::HistoryInner(not_null<HistoryWidget *> historyWidget,
	...
	session().data().peerDecorationsUpdated() | rpl::start_with_next([=] { update(); }, lifetime());
	session().data().itemRemoved() | rpl::start_with_next([this](auto item) { itemRemoved(item); }, lifetime());
	session().data().viewRemoved() | rpl::start_with_next([this](auto view) { viewRemoved(view); }, lifetime());
	rpl::merge(session().data().historyUnloaded(), session().data().historyCleared()) |
		...
		rpl::start_with_next([this] { mouseActionCancel(); }, lifetime());
	session().data().viewRepaintRequest() |
		rpl::start_with_next([this](not_null<const Element *> view) { repaintItem(view); }, lifetime());
	session().data().viewLayoutChanged() |
		rpl::filter([=](not_null<const Element *> view)
					{ return (view == viewByItem(view->data())) && view->isUnderCursor(); }) |
		rpl::start_with_next([=](not_null<const Element *> view) { mouseActionUpdate(); }, lifetime());
	...
	_selectScroll.scrolls() |
		rpl::start_with_next([=](int d) { _scroll->scrollToY(_scroll->scrollTop() + d); }, _scroll->lifetime());
...
void HistoryInner::setupSwipeReply() {
	...
	HistoryView::SetupSwipeHandler(
		this,
		_scroll,
		[=, history = _history](HistoryView::ChatPaintGestureHorizontalData data)
		{
...
void HistoryInner::paintEvent(QPaintEvent *e) {
	if (_controller->contentOverlapped(this, e) || hasPendingResizedItems()) {
		return;
...
void HistoryInner::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
	...
	if (link && !link->property(kSendReactionEmojiProperty).value<Data::ReactionId>().empty() &&
	...
	if (!clickedReaction.empty() && leaderOrSelf && Api::WhoReactedExists(leaderOrSelf, Api::WhoReactedList::One)) {
	...
	if (lnkPhoto || lnkDocument) {
		...
		if (sponsored) {
			Menu::FillSponsored(
				this, Ui::Menu::CreateAddActionCallback(_menu), controller->uiShow(), sponsored->fullId(), false);
		}
		if (isUponSelected > 0) {
			addReplyAction(item);
			const auto selectedText = getSelectedText();
			...
			if (item && !isUponSelected) {
				...
				if (const auto media = item->media()) {
					if (const auto poll = media->poll()) {
						HistoryView::AddPollActions(_menu, poll, item, HistoryView::Context::History, _controller);
		...
		if (sponsored) {
			...
			if (!hasAbout) {
				...
				auto item =
					base::make_unique_q<Ui::Menu::MultilineAction>(_menu,
																   st::menuWithIcons,
																   ...
																   &st::menuIconInfo);
				item->clicks() |
					rpl::start_with_next([=] { controller->show(Box(Ui::AboutSponsoredBox)); }, item->lifetime());
...
void HistoryInner::resizeEvent(QResizeEvent *e) { mouseActionUpdate(); }
	...
	const auto wrapItem = [&](not_null<HistoryItem *> item, TextForMimeData &&unwrapped)
	{
		const auto i =
			texts
				...
				.first;
		fullSize += i->second.name.size() + i->second.time.size() + i->second.unwrapped.expanded.size();
	};
...
void HistoryInner::recountHistoryGeometry(bool initial) {
	...
	auto oldHistoryPaddingTop = qMax(visibleHeight - historyHeight() - st::historyPaddingBottom, 0);
	if (_aboutView) {
	...
	if (const auto view = _aboutView ? _aboutView->view() : nullptr) {
		...
		_aboutView->top =
			qMin(_historyPaddingTop - _aboutView->height, qMax(0, (_scroll->height() - _aboutView->height) / 2));
	} else if (_aboutView) {
		_aboutView->top = _aboutView->height = 0;
	}

	auto newHistoryPaddingTop = qMax(visibleHeight - historyHeight() - st::historyPaddingBottom, 0);
	if (_aboutView) {
...
void HistoryInner::updateSize() {
	...
	if (_aboutView && _aboutView->height > 0) {
		_aboutView->top =
			qMin(newHistoryPaddingTop - _aboutView->height, qMax(0, (_scroll->height() - _aboutView->height) / 2));
	}
...
bool HistoryInner::elementIsChatWide() { return _isChatWide; }
...
int HistoryInner::migratedTop() const { return (_migrated && !_migrated->isEmpty()) ? _historyPaddingTop : -1; }

int HistoryInner::historyTop() const {
	int mig = migratedTop();
	return !_history->isEmpty() ? (mig >= 0 ? (mig + _migrated->height() - _historySkipHeight) : _historyPaddingTop)
								: -1;
}
...
void HistoryInner::refreshAboutView(bool force) {
	const auto refresh = [&]
	{
		...
		if (!_aboutView) {
			...
			_aboutView->refreshRequests() | rpl::start_with_next([=] { updateBotInfo(); }, _aboutView->lifetime());
...
void HistoryInner::forwardItem(FullMsgId itemId) { Window::ShowForwardMessagesBox(_controller, {1, itemId}); }
...
bool CanSendReply(not_null<const HistoryItem *> item) {
	...
	return topic ? Data::CanSendAnything(topic)
				 : (Data::CanSendAnything(peer) && (!peer->isChannel() || peer->asChannel()->amIn()));
}
