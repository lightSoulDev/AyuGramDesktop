...
constexpr auto kMaxSamples = ::Media::Player::kDefaultFrequency * kAudioVoiceMaxLength;
constexpr auto kMinSamples = ::Media::Player::kDefaultFrequency / 5; // 0.2 seconds
...
{
public:
	SoundedPreview(not_null<DocumentData *> document, rpl::producer<> repaints);
...
SoundedPreview::SoundedPreview(not_null<DocumentData *> document, rpl::producer<> repaints) : _document(document) {
	std::move(repaints) |
		rpl::start_with_next(
			[=]
			{
...
std::shared_ptr<Ui::DynamicImage> SoundedPreview::clone() { Unexpected("ListenWrap::videoPreview::clone."); }
...
void SoundedPreview::subscribeToUpdates(Fn<void()> callback) { _repaint = std::move(callback); }

[[nodiscard]] auto InactiveColor(const QColor &c) {
	return QColor(c.red(), c.green(), c.blue(), kInactiveWaveformBarAlpha);
}

[[nodiscard]] auto Progress(int low, int high) { return std::clamp(float64(low) / high, 0., 1.); }

[[nodiscard]] auto FormatVoiceDuration(int samples) {
	const int duration = kPrecision * (float64(samples) / ::Media::Player::kDefaultFrequency);
...
[[nodiscard]] std::unique_ptr<VoiceData> ProcessCaptureResult(const VoiceWaveform &waveform) {
	auto voiceData = std::make_unique<VoiceData>();
	voiceData->waveform = waveform;
	voiceData->wavemax = voiceData->waveform.empty() ? uchar(0) : *ranges::max_element(voiceData->waveform);
	return voiceData;
}

[[nodiscard]] not_null<DocumentData *> DummyDocument(not_null<Data::Session *> owner) {
...
void PaintWaveform(QPainter &p,
	...
	const auto samplesCount = wf ? wf->size() : ::Media::Player::kWaveformSamplesCount;
	...
	const auto paintBar = [&](const auto &barValue)
	{
		...
		const auto rect = [&](const auto &l, const auto &w) { return QRectF(l, barTop, w, barHeight); };
...
void FillWithMinithumbs(QPainter &p, not_null<const Ui::RoundVideoResult *> data, QRect rect, float64 progress) {
	...
	for (auto i = 0; i != thumbs - 1; ++i) {
		...
		p.drawImage(QRect(rect.x() + i * size, rect.y(), size, size),
					data->minithumbs,
					QRect((index % perrow) * single, (index / perrow) * single, single, single));
	}
	const auto last = rect.width() - (thumbs - 1) * size;
	const auto index = ((thumbs - 1) * data->minithumbsCount) / thumbs;
	p.drawImage(QRect(rect.x() + (thumbs - 1) * size, rect.y(), last, size),
				data->minithumbs,
				QRect((index % perrow) * single, (index / perrow) * single, (last * single) / size, single));
}

[[nodiscard]] QRect
DrawLockCircle(QPainter &p, const QRect &widgetRect, const style::RecordBarLock &st, float64 progress) {
	...
	const auto bottomMargin =
		anim::interpolate(0, widgetRect.height() - shadowTop.height() - shadowBottom.height(), progress);

	const auto topMargin = anim::interpolate(widgetRect.height() / 4, 0, progress);

	const auto full = widgetRect - QMargins(0, topMargin, 0, bottomMargin);
	const auto inner = full - shadowMargins;
	const auto content = inner - style::margins(0, originTop.height(), 0, originBottom.height());
	const auto contentShadow = full - style::margins(0, shadowTop.height(), 0, shadowBottom.height());
	...
	{
		const auto shadowPos = QPoint(full.x(), contentShadow.y() + contentShadow.height());
		const auto originPos = QPoint(inner.x(), content.y() + content.height());
	...
	if (progress < 1.) {
		...
		const auto arrowRect =
			QRect(inner.x(), content.y() + content.height() - arrow.height() / 2, inner.width(), arrow.height());
...
{
public:
	TTLButton(not_null<Ui::RpWidget *> parent, const style::RecordBar &st, bool recordingVideo);
...
TTLButton::TTLButton(not_null<Ui::RpWidget *> parent, const style::RecordBar &st, bool recordingVideo)
	: RippleButton(parent, st.lock.ripple), _st(st),
	  _rippleRect(Rect(Size(st::historyRecordLockTopShadow.width())) - (st::historyRecordLockRippleMargin)) {
	...
	Ui::AbstractButton::setClickedCallback(
		[=]
		{
			...
			_activeAnimation.start([=] { update(); }, isActive ? 0. : 1., isActive ? 1. : 0., st::universalDuration);
		});

	Ui::RpWidget::shownValue() |
		rpl::start_with_next(
			[=](bool shown)
			{
				if (!shown) {
				...
				auto text =
					rpl::conditional(Core::App().settings().ttlVoiceClickTooltipHiddenValue(),
									 (recordingVideo ? tr::lng_record_once_active_video
													 : tr::lng_record_once_active_tooltip)(Ui::Text::RichLangValue),
									 tr::lng_record_once_first_tooltip(Ui::Text::RichLangValue));
				_tooltip.reset(Ui::CreateChild<Ui::ImportantTooltip>(
					parent.get(),
					object_ptr<Ui::PaddingWrap<Ui::FlatLabel>>(
						parent.get(),
						Ui::MakeNiceTooltipLabel(parent,
												 std::move(text),
					...
					rpl::start_with_next(
						[=](const QRect &r)
						{
							...
							_tooltip->pointAt(r,
											  ...
											  [=](QSize size)
											  {
												  return QPoint(r.left() - size.width() -
																	st::defaultImportantTooltip.padding.left(),
																r.top() + r.height() - size.height() +
				...
				if (!Core::App().settings().ttlVoiceClickTooltipHidden()) {
					clicks() | rpl::take(1) |
						rpl::start_with_next([=] { Core::App().settings().setTtlVoiceClickTooltipHidden(true); },
											 _tooltip->lifetime());
				...
				Ui::RpWidget::geometryValue() |
					rpl::map([=](const QRect &r) { return (r.left() + r.width() > parentWidget()->width()); }) |
					rpl::distinct_until_changed() |
					rpl::start_with_next(
						[=](bool toHide)
						{
							const auto isFirstTooltip = !Core::App().settings().ttlVoiceClickTooltipHidden();
	...
	paintRequest() |
		rpl::start_with_next(
			[=](const QRect &clip)
			{
				...
				const auto activeProgress = _activeAnimation.value(!Ui::AbstractButton::isDisabled() ? 1 : 0);
...
QImage TTLButton::prepareRippleMask() const { return Ui::RippleAnimation::EllipseMask(_rippleRect.size()); }

QPoint TTLButton::prepareRippleStartPosition() const { return mapFromGlobal(QCursor::pos()) - _rippleRect.topLeft(); }
...
{
public:
	ListenWrap(not_null<Ui::RpWidget *> parent,
...
private:
	...
	[[nodiscard]] bool isInPlayer(const ::Media::Player::TrackState &state) const;
...
ListenWrap::ListenWrap(not_null<Ui::RpWidget *> parent,
					   const style::RecordBar &st,
					   std::shared_ptr<Ui::SendButton> send,
					   not_null<Main::Session *> session,
					   not_null<Ui::RoundVideoResult *> data,
					   const style::font &font)
	: _parent(parent), _st(st), _send(send), _session(session), _document(DummyDocument(&session->data())),
	  _voiceData(ProcessCaptureResult(data->waveform)), _mediaView(_document->createMediaView()), _data(data),
	  _delete(base::make_unique_q<Ui::IconButton>(parent, _st.remove)), _durationFont(font),
	  _duration(Ui::FormatDurationText(_data->duration / 1000)), _durationWidth(_durationFont->width(_duration)),
	  _playPauseSt(st::mediaPlayerButton), _playPauseButton(base::make_unique_q<Ui::AbstractButton>(parent)),
...
void ListenWrap::init() {
	auto deleteShow =
		_showProgress.value() | rpl::map([](auto value) { return value == 1.; }) | rpl::distinct_until_changed();
	_delete->showOn(std::move(deleteShow));

	rpl::combine(_parent->sizeValue(), _send->widthValue()) |
		rpl::start_with_next(
			[=](QSize size, int send)
			{
				_waveformBgRect = QRect({0, 0}, size).marginsRemoved(st::historyRecordWaveformBgMargins);
				{
					const auto skip = _waveformBgRect.height() / 2;
					const auto left = _st.remove.width + skip;
					const auto right = send + skip;
					_waveformBgFinalCenterRect = _waveformBgRect.marginsRemoved(style::margins(left, 0, right, 0));
				}
				{
					const auto &play = _playPauseSt.playOuter;
					const auto &final = _waveformBgFinalCenterRect;
					_playPauseButton->moveToLeft(final.x() - (final.height() - play.width()) / 2, final.y());
				}
	...
	_parent->paintRequest() |
		rpl::start_with_next(
			[=](const QRect &clip)
			{
				...
				{
					const auto hideOffset =
						_isShowAnimation ? 0 : anim::interpolate(kHideWaveformBgOffset, 0, progress);
					const auto deleteIconLeft = remove.iconPosition.x();
					const auto bgRectRight =
						anim::interpolate(deleteIconLeft, _send->width(), _isShowAnimation ? progress : 1.);
					const auto bgRectLeft =
						anim::interpolate(_parent->width() - deleteIconLeft - _waveformBgRect.height(),
										  remove.width,
										  _isShowAnimation ? progress : 1.);
					const auto bgRectMargins = style::margins(bgRectLeft - hideOffset, 0, bgRectRight + hideOffset, 0);
					const auto bgRect = _waveformBgRect.marginsRemoved(bgRectMargins);

					const auto horizontalMargin = bgRect.width() - bgRect.height();
					const auto bgLeftCircleRect = bgRect.marginsRemoved(style::margins(0, 0, horizontalMargin, 0));
					const auto bgRightCircleRect = bgRect.marginsRemoved(style::margins(horizontalMargin, 0, 0, 0));

					const auto halfHeight = bgRect.height() / 2;
					const auto bgCenterRect = bgRect.marginsRemoved(style::margins(halfHeight, 0, halfHeight, 0));
					...
					// Duration paint.
					{
						...
						const auto rect = bgCenterRect.marginsRemoved(
							style::margins(bgCenterRect.width() - _durationWidth, top, 0, top));
						p.drawText(rect, style::al_left, _duration);
					}

					// Waveform paint.
					const auto waveformRect = (progress == 1.) ? _waveformFgRect : computeWaveformRect(bgCenterRect);
					if (!waveformRect.isEmpty()) {
						...
						if (_data->minithumbs.isNull()) {
							...
							FillWithMinithumbs(p, _data, waveformRect, playProgress);
						}
					}
				}
			},
...
void ListenWrap::initPlayButton() {
	...
	_document->type = _data->minithumbs.isNull() ? VoiceDocument : RoundVideoDocument;
	...
	_playPauseButton->setClickedCallback([=] { instance()->playPause({_document, FullMsgId()}); });

	const auto showPause = _lifetime.make_state<rpl::variable<bool>>(false);
	showPause->changes() |
		rpl::start_with_next(
			[=](bool pause)
			{ _playPause.setState(pause ? PlayButtonLayout::State::Pause : PlayButtonLayout::State::Play); },
			_lifetime);
	...
	instance()->stops(AudioMsgId::Type::Voice) | rpl::start_with_next([=] { *showPause = false; }, _lifetime);
...
void ListenWrap::initPlayProgress() {
	...
	const auto updateCursor = [=](const QPoint &p) { *isPointer = isInPlayer() ? _waveformFgRect.contains(p) : false; };

	rpl::merge(instance()->startsPlay(voice) | rpl::map_to(true), instance()->stops(voice) | rpl::map_to(false)) |
		rpl::start_with_next(
			[=](bool play)
			{
	...
	instance()->updatedNotifier() |
		rpl::start_with_next(
			[=](const State &state)
			{
				...
				const auto progress = state.length ? Progress(state.position, state.length) : 0.;
	...
	auto animationCallback = [=](crl::time now)
	{
		...
		const auto dt = (now - animation->started()) / float64(kAudioVoiceUpdateView);
		if (dt >= 1.) {
			animation->stop();
	...
	isPointer->changes() |
		rpl::start_with_next(
			[=](bool pointer) { _parent->setCursor(pointer ? style::cur_pointer : style::cur_default); }, _lifetime);

	_parent->events() |
		rpl::filter(
			[=](not_null<QEvent *> e)
			{
				return (e->type() == QEvent::MouseMove || e->type() == QEvent::MouseButtonPress ||
...
void ListenWrap::playPause() { _playPauseButton->clicked(Qt::NoModifier, Qt::LeftButton); }

QRect ListenWrap::computeWaveformRect(const QRect &centerRect) const {
	const auto top = computeTopMargin(st::msgWaveformMax);
	const auto left = (_playPauseSt.playOuter.width() + centerRect.height()) / 2;
...
int ListenWrap::computeTopMargin(int height) const { return (_waveformBgRect.height() - height) / 2; }
...
rpl::producer<> ListenWrap::stopRequests() const { return _delete->clicks() | rpl::to_empty; }

std::shared_ptr<Ui::DynamicImage> ListenWrap::videoPreview() {
	return std::make_shared<SoundedPreview>(_document, _videoRepaints.events());
}

rpl::lifetime &ListenWrap::lifetime() { return _lifetime; }

class RecordLock final : public Ui::RippleButton
{
public:
	RecordLock(not_null<Ui::RpWidget *> parent, const style::RecordBarLock &st);
...
RecordLock::RecordLock(not_null<Ui::RpWidget *> parent, const style::RecordBarLock &st)
	: RippleButton(parent, st.ripple), _st(st),
	  _rippleRect(Rect(Size(st::historyRecordLockTopShadow.width())) - (st::historyRecordLockRippleMargin)),
	  _arcPen(QColor(Qt::white), st::historyRecordLockIconLineWidth, Qt::SolidLine, Qt::SquareCap, Qt::RoundJoin) {
	init();
}

void RecordLock::setVisibleTopPart(int part) { _visibleTopPart = part; }

void RecordLock::setRecordingVideo(bool value) { _recordingVideo = value; }
...
void RecordLock::init() {
	shownValue() |
		rpl::start_with_next(
			[=](bool shown)
			{
				resize(st::historyRecordLockTopShadow.width(), st::historyRecordLockSize.height());
				if (!shown) {
			...
			[=](const QRect &clip)
			{
				...
				if (isLocked()) {
					const auto top = anim::interpolate(
						0, height() - st::historyRecordLockTopShadow.height() * 2, _lockToStopProgress);
					p.translate(0, top);
					drawProgress(p);
					return;
				}
...
void RecordLock::drawProgress(QPainter &p) {
	...
	}
	{
		...
		const auto blockRectWidth =
			anim::interpolateToF(size.width(), st::historyRecordStopIconWidth, _lockToStopProgress);
		const auto blockRectHeight =
			anim::interpolateToF(blockHeight, st::historyRecordStopIconWidth, _lockToStopProgress);
		const auto blockRectTop = anim::interpolateToF(
			size.height() - blockHeight, base::SafeRound((size.height() - blockRectHeight) / 2.), _lockToStopProgress);

		const auto blockRect =
			QRectF((size.width() - blockRectWidth) / 2, blockRectTop, blockRectWidth, blockRectHeight);
		const auto &lineHeight = st::historyRecordLockIconLineHeight;

		const auto lockTranslation =
			QPoint((inner.width() - size.width()) / 2, (_st.originTop.height() * 2 - size.height()) / 2);
		const auto xRadius = anim::interpolateF(2, 3, _lockToStopProgress);

		const auto pauseLineOffset = blockRectWidth / 2 + st::historyRecordLockIconLineWidth;
		if (_lockToStopProgress == 1.) {
			...
			if (_pauseToInputProgress > 0.) {
				...
				const auto &icon = _recordingVideo ? st::historyRecordLockRound : st::historyRecordLockInput;
				icon.paintInCenter(p, blockRect.toRect());
				p.setOpacity(1. - _pauseToInputProgress);
			}
			p.drawRoundedRect(blockRect - QMargins(0, 0, pauseLineOffset, 0), xRadius, 3);
			p.drawRoundedRect(blockRect - QMargins(pauseLineOffset, 0, 0, 0), xRadius, 3);
		} else {
			// Paint an animation frame.
			auto frame = QImage(inner.size() * style::DevicePixelRatio(), QImage::Format_ARGB32_Premultiplied);
			...
			{
				const auto offset = anim::interpolateF(0, pauseLineOffset, _lockToStopProgress);
				q.drawRoundedRect(blockRect - QMarginsF(0, 0, offset, 0), xRadius, 3);
				q.drawRoundedRect(blockRect - QMarginsF(offset, 0, 0, 0), xRadius, 3);
			}

			const auto offsetTranslate = _lockToStopProgress * (lineHeight + arcHeight + _arcPen.width() * 2);
			q.translate(size.width() - arcOffset, blockRect.y() + offsetTranslate);
			...
			if (progress == 1. && lockProgress < 1.) {
				q.drawLine(-arcWidth, rLine.y2(), -arcWidth, rLine.dy() * lockProgress);
			}
			q.end();

			p.drawImage(inner.topLeft(), style::colorizeImage(frame, _st.fg));
		}
	}
}

void RecordLock::startLockingAnimation(float64 to) {
	_lockEnderAnimation.start([=](float64 value) { setProgress(value); }, 0., to, st::universalDuration);
}

void RecordLock::requestPaintProgress(float64 progress) {
	if (isHidden() || isLocked() || _lockEnderAnimation.animating() || (_progress.current() == progress)) {
		return;
	}
...
void RecordLock::requestPaintLockToStopProgress(float64 progress) {
	...
	if (isStopState()) {
		...
		resize(st::historyRecordLockTopShadow.width(), st::historyRecordLockTopShadow.width());
	}
	update();
}
...
float64 RecordLock::lockToStopProgress() const { return _lockToStopProgress; }
...
bool RecordLock::isLocked() const { return _progress.current() == 1.; }

bool RecordLock::isStopState() const { return isLocked() && (_lockToStopProgress == 1.); }

rpl::producer<> RecordLock::locks() const {
	return _progress.changes() | rpl::filter([=] { return isLocked(); }) | rpl::to_empty;
}

QImage RecordLock::prepareRippleMask() const { return Ui::RippleAnimation::EllipseMask(_rippleRect.size()); }

QPoint RecordLock::prepareRippleStartPosition() const { return mapFromGlobal(QCursor::pos()) - _rippleRect.topLeft(); }

class CancelButton final : public Ui::RippleButton
{
public:
	CancelButton(not_null<Ui::RpWidget *> parent, const style::RecordBar &st, int height);
...
CancelButton::CancelButton(not_null<Ui::RpWidget *> parent, const style::RecordBar &st, int height)
	: Ui::RippleButton(parent, st.cancelRipple), _st(st), _width(st::historyRecordCancelButtonWidth),
...
void CancelButton::init() {
	_showProgress.value() | rpl::map(rpl::mappers::_1 > 0.) | rpl::distinct_until_changed() |
		rpl::start_with_next([=](bool hasProgress) { setVisible(hasProgress); }, lifetime());
...
QImage CancelButton::prepareRippleMask() const { return Ui::RippleAnimation::EllipseMask(_rippleRect.size()); }
...
VoiceRecordBar::VoiceRecordBar(not_null<Ui::RpWidget *> parent, VoiceRecordBarDescriptor &&descriptor)
	: RpWidget(parent), _st(descriptor.stOverride ? *descriptor.stOverride : st::defaultRecordBar),
	  _outerContainer(descriptor.outerContainer), _show(std::move(descriptor.show)), _send(std::move(descriptor.send)),
	  ...
	  _message(st::historyRecordTextStyle,
			   (!descriptor.customCancelText.isEmpty() ? descriptor.customCancelText : tr::lng_record_cancel(tr::now)),
			   TextParseOptions{TextParseMultiline, 0, 0, Qt::LayoutDirectionAuto}),
	  _lockFromBottom(descriptor.lockFromBottom), _cancelFont(st::historyRecordFont) {
	resize(QSize(parent->width(), descriptor.recorderHeight));
	init();
	hideFast();
}

VoiceRecordBar::VoiceRecordBar(not_null<Ui::RpWidget *> parent,
...
void VoiceRecordBar::updateMessageGeometry() {
	...
	const auto right = width() - _send->width() - st::historyRecordTextRight;
	const auto textWidth = _message.maxWidth();
	const auto width = ((right - left) < textWidth) ? st::historyRecordTextWidthForWrap : textWidth;
	const auto countLines = std::ceil((float) textWidth / width);
	const auto textHeight = _message.minHeight() * countLines;
	_messageRect = QRect(left + (right - left - width) / 2, (height() - textHeight) / 2, width, textHeight);
}

void VoiceRecordBar::updateLockGeometry() {
	const auto parent = parentWidget();
	const auto me = Ui::MapFrom(_outerContainer, parent, geometry());
	const auto finalTop = me.y() - st::historyRecordLockPosition.y() - _lock->height();
	const auto finalRight = _outerContainer->width() - rect::right(me) + st::historyRecordLockPosition.x();
	const auto progress = _showLockAnimation.value(_lockShowing.current() ? 1. : 0.);
	if (_lockFromBottom) {
...
void VoiceRecordBar::updateTTLGeometry(TTLAnimationType type, float64 progress) {
	if (!_ttlButton) {
		return;
	}
	const auto parent = parentWidget();
	...
	const auto lockHiddenProgress = (_lockShowing.current() || !_fullRecord) ? 0. : (1. - _showLockAnimation.value(0.));
	const auto ttlFrom = anyTop - _ttlButton->height() - (_ttlButton->height() * (1. - lockHiddenProgress));
...
void VoiceRecordBar::init() {
	...
	rpl::single() |
		rpl::then(_send->events() |
				  rpl::filter([](not_null<QEvent *> e) { return e->type() == QEvent::ZOrderChange; }) | rpl::to_empty) |
		rpl::start_with_next([=] { orderControls(); }, lifetime());
	...
	sizeValue() |
		rpl::start_with_next(
			[=](QSize size)
			{
				...
				{
					const auto durationLeft =
						_redCircleRect.x() + _redCircleRect.width() + st::historyRecordDurationSkip;
					const auto &ascent = _cancelFont->ascent;
					_durationRect = QRect(durationLeft,
	...
	paintRequest() |
		rpl::start_with_next(
			[=](const QRect &clip)
			{
				...
				_cancel->requestPaintProgress(_lock->isStopState() ? (opacity * _lock->lockToStopProgress()) : 0.);
	...
	_inField.changes() | rpl::start_with_next([=](bool value) { activeAnimate(value); }, lifetime());
	...
	const auto setLevelAsSend = [=]
	{
		...
		_level->clicks() | rpl::start_with_next([=] { stop(true); }, _recordingLifetime);

		rpl::single(false) | rpl::then(_level->actives()) |
			rpl::start_with_next([=](bool enter) { _inField = enter; }, _recordingLifetime);
	};
	...
	_paused.value() | rpl::distinct_until_changed() |
		rpl::start_with_next(
			[=](bool paused)
			{
				...
				_showListenAnimation.stop();
				_showListenAnimation.start(std::move(callback), 0., to, st::universalDuration);
			},
			lifetime());
		...
		rpl::start_with_next(
			[=]
			{
				if (_hasTTLFilter && _hasTTLFilter()) {
					if (!_ttlButton) {
						_ttlButton = std::make_unique<TTLButton>(_outerContainer, _st, _recordingVideo);
					}
					_ttlButton->show();
				}
				...
				_lockToStopAnimation.start(std::move(callback), 0., 1., st::universalDuration);
			},
			lifetime());
	...
	_send->events() |
		rpl::filter(
			[=](not_null<QEvent *> e)
			{
				return isTypeRecord() && !isRecording() && !_showAnimation.animating() && !_lock->isLocked() &&
	...
	_listenChanges.events() | rpl::filter([=] { return _listen != nullptr; }) |
		rpl::start_with_next(
			[=]
			{
				_listen->stopRequests() | rpl::take(1) |
					rpl::start_with_next([=] { hideAnimated(); }, _listen->lifetime());
	...
	_cancel->setClickedCallback([=] { hideAnimated(); });
...
void VoiceRecordBar::activeAnimate(bool active) {
	...
	if (_activeAnimation.animating()) {
		...
		_activeAnimation.start(std::move(callback), active ? 0. : 1., to, st::universalDuration);
	}
}
...
void VoiceRecordBar::visibilityAnimate(bool show, Fn<void()> &&callback) {
	...
	_showAnimation.start(std::move(animationCallback), from, to, st::universalDuration);
}

void VoiceRecordBar::setStartRecordingFilter(FilterCallback &&callback) { _startRecordingFilter = std::move(callback); }

void VoiceRecordBar::setTTLFilter(FilterCallback &&callback) { _hasTTLFilter = std::move(callback); }

void VoiceRecordBar::setPauseInsteadSend(bool pauseInsteadSend) { _pauseInsteadSend = pauseInsteadSend; }

void VoiceRecordBar::initLockGeometry() {
	const auto parent = static_cast<Ui::RpWidget *>(parentWidget());
	rpl::merge(_lock->heightValue() | rpl::to_empty,
			   geometryValue() | rpl::to_empty,
			   parent->geometryValue() | rpl::to_empty) |
		rpl::start_with_next([=] { updateLockGeometry(); }, lifetime());
	parent->geometryValue() |
		rpl::start_with_next([=] { updateTTLGeometry(TTLAnimationType::RightLeft, 1.); }, lifetime());
}

void VoiceRecordBar::initLevelGeometry() {
	rpl::combine(
		_send->geometryValue(), geometryValue(), static_cast<Ui::RpWidget *>(parentWidget())->geometryValue()) |
		rpl::start_with_next(
			[=](QRect send, auto, auto)
			{
				const auto mapped = Ui::MapFrom(_outerContainer, _send->parentWidget(), send);
...
void VoiceRecordBar::startRecording() {
	...
	auto appearanceCallback = [=]
	{
		...
		if (_paused.current()) {
			...
			instance()->start(_videoRecorder ? _videoRecorder->audioChunkProcessor() : nullptr);
		}
		instance()->updated() |
			rpl::start_with_next_error([=](const Update &update) { recordUpdated(update.level, update.samples); },
									   [=] { stop(false); },
									   _recordingLifetime);
		if (_videoRecorder) {
		...
		_recordingLifetime.add([=] { _recording = false; });
	};
	...
	const auto stateOwned =
		_recordingLifetime.make_state<std::unique_ptr<FloatingState>>(std::make_unique<FloatingState>());
	const auto state = stateOwned->get();

	_lock->locks() | rpl::start_with_next([=] { stateOwned->reset(); }, state->lifetime);

	constexpr auto kAnimationThreshold = 0.35;
	const auto calcStateRatio = [=](float64 counter) { return (1 - std::cos(std::fmod(counter, 2 * M_PI))) * 0.5; };
	state->animation.init(
		[=](crl::time now)
		{
			...
			const auto progress =
				anim::interpolateF(state->cursorProgress, kAnimationThreshold, calcStateRatio(state->frameCounter));
	...
	_send->events() |
		rpl::filter(
			[=](not_null<QEvent *> e)
			{
				return (e->type() == QEvent::MouseMove || e->type() == QEvent::MouseButtonRelease) && isTypeRecord() &&
					!_lock->isLocked();
			}) |
		rpl::start_with_next(
			[=](not_null<QEvent *> e)
			{
				...
				if (type == QEvent::MouseMove) {
					...
					_inField = inField ? inField : _level->inCircle(_level->mapFromGlobal(globalPos));
...
void VoiceRecordBar::recordUpdated(quint16 level, int samples) {
	...
	const auto type = _recordingVideo ? Api::SendProgressType::RecordRound : Api::SendProgressType::RecordVoice;
...
void VoiceRecordBar::stopRecording(StopType type, bool ttlBeforeHide) {
	...
	if (type == StopType::Cancel) {
		...
		instance()->stop(crl::guard(this, [=](Result &&data) { _cancelRequests.fire({}); }));
					...
					crl::on_main(weak,
								 [=, data = std::move(data)]() mutable
								 {
									 ...
									 _listen = std::make_unique<ListenWrap>(
										 this, _st, _send, &_show->session(), &_data, _cancelFont);
									 _listenChanges.fire({});

									 using SilentPreview = ::Media::Streaming::RoundPreview;
									 recorder->showPreview(
										 std::make_shared<SilentPreview>(_data.content, recorder->previewSize()),
						...
						[=, data = std::move(data)]() mutable
						{
							if (weak) {
								...
								const auto options = Api::SendOptions{
									.ttlSeconds = (ttlBeforeHide ? std::numeric_limits<int>::max() : 0),
								};
		...
		instance()->stop(crl::guard(this,
									[=](Result &&data)
									{
										...
										auto sendVoiceCallback = crl::guard(this,
																			[=, this](Fn<void()> &&close)
																			{
																				_sendVoiceRequests.fire({
																					.bytes = _data.content,
										...
										if (settings->voiceConfirmation) {
											_show->showBox(
												Ui::MakeConfirmBox({.text = tr::ayu_ConfirmationVoice(),
...
void VoiceRecordBar::startRedCircleAnimation() {
	...
	const auto animation = _recordingLifetime.make_state<Ui::Animations::Basic>();
	animation->init(
		[=](crl::time now)
		{
...
void VoiceRecordBar::requestToSendWithOptions(Api::SendOptions options) {
	if (isListenState()) {
		...
		if (settings->voiceConfirmation) {
			_show->showBox(Ui::MakeConfirmBox({.text = tr::ayu_ConfirmationVoice(),
											   .confirmed = std::move(sendVoiceCallback),
											   .confirmText = tr::lng_send_button()}));
		} else {
			sendVoiceCallback([] {});
...
rpl::producer<SendActionUpdate> VoiceRecordBar::sendActionUpdates() const { return _sendActionUpdates.events(); }

rpl::producer<VoiceToSend> VoiceRecordBar::sendVoiceRequests() const { return _sendVoiceRequests.events(); }

rpl::producer<> VoiceRecordBar::cancelRequests() const { return _cancelRequests.events(); }

bool VoiceRecordBar::isRecording() const { return _recording.current() && !_paused.current(); }

bool VoiceRecordBar::isRecordingLocked() const { return isRecording() && _lock->isLocked(); }

bool VoiceRecordBar::isActive() const { return isRecording() || isListenState(); }
...
void VoiceRecordBar::finishAnimating() { _showAnimation.stop(); }

rpl::producer<bool> VoiceRecordBar::recordingStateChanges() const { return _recording.changes(); }

rpl::producer<bool> VoiceRecordBar::lockShowStarts() const { return _lockShowing.changes(); }

rpl::producer<not_null<QEvent *>> VoiceRecordBar::lockViewportEvents() const {
	return _lock->events() | rpl::filter([=](not_null<QEvent *> e) { return e->type() == QEvent::Wheel; });
}

rpl::producer<> VoiceRecordBar::updateSendButtonTypeRequests() const {
	return rpl::merge(::Media::Capture::instance()->startedChanges() |
						  rpl::filter(
							  [=]
							  {
...
rpl::producer<> VoiceRecordBar::recordingTipRequests() const { return _recordingTipRequests.events(); }

auto VoiceRecordBar::errors() const -> rpl::producer<Error> { return _errors.events(); }

bool VoiceRecordBar::isLockPresent() const { return _lockShowing.current(); }

bool VoiceRecordBar::isListenState() const { return _listen != nullptr; }

bool VoiceRecordBar::isTypeRecord() const {
	return (_send->type() == Ui::SendButton::Type::Record) || (_send->type() == Ui::SendButton::Type::Round);
}
...
bool VoiceRecordBar::isTTLButtonShown() const { return _ttlButton && !_ttlButton->isHidden(); }

bool VoiceRecordBar::hasDuration() const { return _recordingSamples > 0; }

float64 VoiceRecordBar::activeAnimationRatio() const { return _activeAnimation.value(_inField.current() ? 1. : 0.); }
...
bool VoiceRecordBar::peekTTLState() const { return _ttlButton && !_ttlButton->isDisabled(); }
...
void VoiceRecordBar::installListenStateFilter() {
	auto keyFilterCallback = [=](not_null<QEvent *> e)
	{
		...
		if (!(_send->type() == Ui::SendButton::Type::Send || _send->type() == Ui::SendButton::Type::Schedule)) {
		...
		switch (e->type()) {
			case QEvent::KeyPress: {
				...
				const auto isEnter = (key == Qt::Key_Enter || key == Qt::Key_Return);
	...
	auto keyFilter = base::install_event_filter(QCoreApplication::instance(), std::move(keyFilterCallback));

	_listen->lifetime().make_state<base::unique_qptr<QObject>>(std::move(keyFilter));
}

void VoiceRecordBar::showDiscardBox(Fn<void()> &&callback, anim::type animated) {
	...
	_show->showBox(Ui::MakeConfirmBox({
		.text = (isListenState()
					 ? (_recordingVideo ? tr::lng_record_listen_cancel_sure_round : tr::lng_record_listen_cancel_sure)
					 : (_recordingVideo ? tr::lng_record_lock_cancel_sure_round : tr::lng_record_lock_cancel_sure))(),
...
bool VoiceRecordBar::createVideoRecorder() {
	...
	const auto hidden = [=](not_null<Ui::RoundVideoRecorder *> which)
	{
		...
		_videoHiding.erase(ranges::remove(_videoHiding, which.get(), &std::unique_ptr<Ui::RoundVideoRecorder>::get),
						   end(_videoHiding));
	};
	auto capturer = Core::App().calls().getVideoCapture();
	auto track = std::make_shared<Webrtc::VideoTrack>(Webrtc::VideoState::Active);
	capturer->setOutput(track->sink());
	capturer->setPreferredAspectRatio(1.);
	_videoCapturerLifetime =
		track->stateValue() |
		rpl::start_with_next(
			[=](Webrtc::VideoState state)
			{
				capturer->setState((state == Webrtc::VideoState::Active) ? tgcalls::VideoState::Active
																		 : tgcalls::VideoState::Inactive);
			});
	_videoRecorder = std::make_unique<Ui::RoundVideoRecorder>(Ui::RoundVideoRecorderDescriptor{
	...
	_videoRecorder->placeholderUpdates() |
		rpl::start_with_next([=](QImage &&placeholder) { _show->session().local().writeRoundPlaceholder(placeholder); },
							 _videoCapturerLifetime);
