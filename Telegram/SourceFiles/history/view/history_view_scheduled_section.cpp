...
ScheduledMemento::ScheduledMemento(not_null<History *> history, MsgId sentToScheduledId)
	: _history(history), _forumTopic(nullptr), _sentToScheduledId(sentToScheduledId) {
...
ScheduledMemento::ScheduledMemento(not_null<Data::ForumTopic *> forumTopic)
	: _history(forumTopic->owningHistory()), _forumTopic(forumTopic) {
	const auto list = _history->session().scheduledMessages().list(_forumTopic);
	if (!list.ids.empty()) {
		_list.setScrollTopState({.item = {.fullId = list.ids.front()}});
	}
}

object_ptr<Window::SectionWidget> ScheduledMemento::createWidget(QWidget *parent,
	...
	auto result = object_ptr<ScheduledWidget>(parent, controller, _history, _forumTopic);
	result->setInternalState(geometry, this);
	return result;
}

ScheduledWidget::ScheduledWidget(QWidget *parent,
								 not_null<Window::SessionController *> controller,
								 not_null<History *> history,
								 const Data::ForumTopic *forumTopic)
	: Window::SectionWidget(parent, controller, history->peer), WindowListDelegate(controller),
	  _show(controller->uiShow()), _history(history), _forumTopic(forumTopic),
	  _scroll(this, controller->chatStyle()->value(lifetime(), st::historyScroll), false), _topBar(this, controller),
	  _topBarShadow(this),
	...
	_topBar->sendNowSelectionRequest() | rpl::start_with_next([=] { confirmSendNowSelected(); }, _topBar->lifetime());
	...
	{
		auto emptyInfo =
			base::make_unique_q<EmptyListBubbleWidget>(_inner, controller->chatStyle(), st::msgServicePadding);
		const auto emptyText = Ui::Text::Semibold(tr::lng_scheduled_messages_empty(tr::now));
...
void ScheduledWidget::setupComposeControls() {
	...
	_composeControls->sendRequests() | rpl::start_with_next([=] { send(); }, lifetime());

	_composeControls->sendVoiceRequests() |
		rpl::start_with_next([=](ComposeControls::VoiceToSend &&data) { sendVoice(std::move(data)); }, lifetime());

	_composeControls->sendCommandRequests() |
		rpl::start_with_next([=](const QString &command) { listSendBotCommand(command, FullMsgId()); }, lifetime());
	...
	_composeControls->fileChosen() |
		rpl::start_with_next(
			[=](ChatHelpers::FileChosen data)
			{
				...
				controller()->show(PrepareScheduleBox(this, _show, sendMenuDetails(), callback));
			},
			lifetime());

	_composeControls->photoChosen() |
		rpl::start_with_next([=](ChatHelpers::PhotoChosen chosen) { sendExistingPhoto(chosen.photo); }, lifetime());

	_composeControls->inlineResultChosen() |
		rpl::start_with_next([=](ChatHelpers::InlineChosen chosen) { sendInlineResult(chosen.result, chosen.bot); },
							 lifetime());
				   ...
				   [=](FileDialog::OpenResult &&result)
				   {
					   ...
					   if (!result.remoteContent.isEmpty()) {
						   ...
						   if (!read.image.isNull() && !read.animated) {
							   confirmSendingFiles(std::move(read.image), std::move(result.remoteContent));
						   } else {
...
bool ScheduledWidget::confirmSendingFiles(not_null<const QMimeData *> data,
...
bool ScheduledWidget::confirmSendingFiles(Ui::PreparedList &&list, const QString &insertTextOnCancel) {
	...
	auto box = Box<SendFilesBox>(
		...
		_history->peer,
		(CanScheduleUntilOnline(_history->peer) ? Api::SendType::ScheduledToUser : Api::SendType::Scheduled),
		SendMenu::Details());
...
void ScheduledWidget::sendingFilesConfirmed(Ui::PreparedList &&list,
											Ui::SendFilesWay way,
											TextWithTags &&caption,
											Api::SendOptions options,
											bool ctrlShiftEnter) {
	...
	const auto type = way.sendImagesAsPhotos() ? SendMediaType::Photo : SendMediaType::File;
	auto action = prepareSendAction(options);
	action.clearDraft = false;
	if ((groups.size() != 1 || !groups.front().sentWithCaption()) && !caption.text.isEmpty()) {
	...
	for (auto &group : groups) {
		...
		session().api().sendFiles(std::move(group.list), type, base::take(caption), album, action);
	}
}

bool ScheduledWidget::confirmSendingFiles(QImage &&image,
										  QByteArray &&content,
										  std::optional<bool> overrideSendImagesAsPhotos,
										  const QString &insertTextOnCancel) {
	if (image.isNull()) {
...
void ScheduledWidget::uploadFile(const QByteArray &fileContent, SendMediaType type) {
	const auto callback = [=](Api::SendOptions options)
	{ session().api().sendFile(fileContent, type, prepareSendAction(options)); };
	controller()->show(PrepareScheduleBox(this, _show, sendMenuDetails(), callback));
}

bool ScheduledWidget::showSendingFilesError(const Ui::PreparedList &list) const {
	return showSendingFilesError(list, std::nullopt);
}

bool ScheduledWidget::showSendingFilesError(const Ui::PreparedList &list, std::optional<bool> compress) const {
...
Api::SendAction ScheduledWidget::prepareSendAction(Api::SendOptions options) const {
	auto result = Api::SendAction(_history, options);
	result.options.sendAs = _composeControls->sendAsPeer();
	if (_forumTopic) {
		result.replyTo.topicRootId = _forumTopic->topicRootId();
		result.replyTo.messageId = FullMsgId(history()->peer->id, _forumTopic->topicRootId());
	}
	return result;
}
...
void ScheduledWidget::send() {
	...
	const auto error = GetErrorForSending(_history->peer,
										  {
											  .topicRootId = _forumTopic ? _forumTopic->topicRootId()
												  : history()->isForum() ? MsgId(1)
																		 : MsgId(),
											  .forward = nullptr,
											  .text = &textWithTags,
...
void ScheduledWidget::sendVoice(const Controls::VoiceToSend &data) {
	const auto callback = [=](Api::SendOptions options) { sendVoice(base::duplicate(data), options); };
	controller()->show(PrepareScheduleBox(this, _show, sendMenuDetails(), callback));
}

void ScheduledWidget::sendVoice(const Controls::VoiceToSend &data, Api::SendOptions options) {
	session().api().sendVoiceMessage(data.bytes, data.waveform, data.duration, data.video, prepareSendAction(options));
	_composeControls->clearListenState();
}

void ScheduledWidget::edit(not_null<HistoryItem *> item,
						   Api::SendOptions options,
...
bool ScheduledWidget::sendExistingDocument(not_null<DocumentData *> document, Api::MessageToSend messageToSend) {
...
void ScheduledWidget::sendExistingPhoto(not_null<PhotoData *> photo) {
	const auto callback = [=](Api::SendOptions options) { sendExistingPhoto(photo, options); };
	controller()->show(PrepareScheduleBox(this, _show, sendMenuDetails(), callback));
}

bool ScheduledWidget::sendExistingPhoto(not_null<PhotoData *> photo, Api::SendOptions options) {
...
void ScheduledWidget::sendInlineResult(std::shared_ptr<InlineBots::Result> result, not_null<UserData *> bot) {
	if (const auto error = result->getErrorOnSend(_history)) {
		Data::ShowSendErrorToast(controller(), _history->peer, error);
		return;
	}
	const auto callback = [=](Api::SendOptions options) { sendInlineResult(result, bot, options); };
	controller()->show(PrepareScheduleBox(this, _show, sendMenuDetails(), callback));
}

void ScheduledWidget::sendInlineResult(std::shared_ptr<InlineBots::Result> result,
									   not_null<UserData *> bot,
									   Api::SendOptions options) {
	auto action = prepareSendAction(options);
	action.generateLocal = true;
	session().api().sendInlineResult(bot, result.get(), action, std::nullopt);
...
SendMenu::Details ScheduledWidget::sendMenuDetails() const {
	const auto type = _history->peer->isSelf()				  ? SendMenu::Type::Reminder
		: HistoryView::CanScheduleUntilOnline(_history->peer) ? SendMenu::Type::ScheduledToUser
															  : SendMenu::Type::Scheduled;
	const auto effectAllowed = _history->peer->isUser();
	return {.type = type, .effectAllowed = effectAllowed};
}

void ScheduledWidget::cornerButtonsShowAtPosition(Data::MessagePosition position) { showAtPosition(position); }

Data::Thread *ScheduledWidget::cornerButtonsThread() { return _history; }

FullMsgId ScheduledWidget::cornerButtonsCurrentId() { return {}; }

bool ScheduledWidget::cornerButtonsIgnoreVisibility() { return animatingShow(); }
...
bool ScheduledWidget::cornerButtonsUnreadMayBeShown() {
	return _inner->loadedAtBottomKnown() && !_composeControls->isLockPresent() && !_composeControls->isTTLButtonShown();
}

bool ScheduledWidget::cornerButtonsHas(CornerButtonType type) { return (type == CornerButtonType::Down); }

void ScheduledWidget::showAtPosition(Data::MessagePosition position, FullMsgId originId) {
...
not_null<History *> ScheduledWidget::history() const { return _history; }
...
void ScheduledWidget::checkActivation() { _inner->checkActivation(); }

void ScheduledWidget::doSetInnerFocus() { _composeControls->focus(); }

bool ScheduledWidget::showInternal(not_null<Window::SectionMemento *> memento, const Window::SectionShow &params) {
...
void ScheduledWidget::setInternalState(const QRect &geometry, not_null<ScheduledMemento *> memento) {
	setGeometry(geometry);
	Ui::SendPendingMoveResizeEvents(this);
	restoreState(memento);
}

bool ScheduledWidget::pushTabbedSelectorToThirdSection(not_null<Data::Thread *> thread,
													   const Window::SectionShow &params) {
	return _composeControls->pushTabbedSelectorToThirdSection(thread, params);
}

bool ScheduledWidget::returnTabbedSelector() { return _composeControls->returnTabbedSelector(); }
...
void ScheduledWidget::saveState(not_null<ScheduledMemento *> memento) { _inner->saveState(memento->list()); }
...
void ScheduledWidget::initProcessingVideoView(not_null<Element *> view) {
			...
			[=](const Data::SentFromScheduled &value)
			{
				if (value.item->position() == _processingVideoPosition) {
					controller()->showPeerHistory(
						value.item->history(), Window::SectionShow::Way::Backward, value.sentId);
				}
			},
	...
	controller()->session().data().viewResizeRequest() |
		rpl::start_with_next(
			[this](not_null<const Element *> view)
			{
...
void ScheduledWidget::checkProcessingVideoTooltip(int visibleTop, int visibleBottom) {
	if (_processingVideoTooltip || _processingVideoTooltipShown || !_processingVideoCanShow) {
		return;
	}
	...
	if (rect.top() > visibleTop && rect.top() + rect.height() <= visibleBottom) {
...
void ScheduledWidget::updateProcessingVideoTooltipPosition() {
	...
	const auto countPosition = [=](QSize size)
	{
		...
		return origin - QPoint(size.width() / 2, size.height() + st::processingVideoTipShift);
	};
...
void ScheduledWidget::showProcessingVideoTooltip() {
	...
	_processingVideoTooltip = std::make_unique<Ui::ImportantTooltip>(
		_inner.data(),
		Ui::MakeNiceTooltipLabel(_inner.data(),
	...
	const auto destroy = [=] { delete weak.data(); };
	...
	tooltip->setHiddenCallback(
		[=]
		{
			...
			crl::on_main(tip, [=] { delete tip; });
		});
	updateProcessingVideoTooltipPosition();
	tooltip->toggleAnimated(true);
	_processingVideoTipTimer.setCallback(crl::guard(tooltip, [=] { tooltip->toggleAnimated(false); }));
	_processingVideoTipTimer.callOnce(kVideoProcessingInfoDuration);
}

void ScheduledWidget::showAnimatedHook(const Window::SectionSlideParams &params) {
...
bool ScheduledWidget::floatPlayerHandleWheelEvent(QEvent *e) { return _scroll->viewportEvent(e); }

QRect ScheduledWidget::floatPlayerAvailableRect() { return mapToGlobal(_scroll->geometry()); }

Context ScheduledWidget::listContext() { return _forumTopic ? Context::ScheduledTopic : Context::History; }
...
void ScheduledWidget::listDeleteRequest() { confirmDeleteSelected(); }

void ScheduledWidget::listTryProcessKeyInput(not_null<QKeyEvent *> e) { _composeControls->tryProcessKeyInput(e); }

rpl::producer<Data::MessagesSlice>
ScheduledWidget::listSource(Data::MessagePosition aroundId, int limitBefore, int limitAfter) {
	const auto session = &controller()->session();
	return rpl::single(rpl::empty) | rpl::then(session->scheduledMessages().updates(_history)) |
		rpl::map(
			   [=]
			   {
				   return _forumTopic ? session->scheduledMessages().list(_forumTopic)
									  : session->scheduledMessages().list(_history);
			   }) |
		rpl::after_next([=](const Data::MessagesSlice &slice) { highlightSingleNewMessage(slice); });
}

void ScheduledWidget::highlightSingleNewMessage(const Data::MessagesSlice &slice) {
	const auto guard = gsl::finally([&] { _lastSlice = slice; });
	if (_lastSlice.ids.empty() || (slice.ids.size() != _lastSlice.ids.size() + 1)) {
		return;
	}
...
bool ScheduledWidget::listAllowsMultiSelect() { return true; }

bool ScheduledWidget::listIsItemGoodForSelection(not_null<HistoryItem *> item) {
	return !item->isSending() && !item->hasFailed();
}

bool ScheduledWidget::listIsLessInOrder(not_null<HistoryItem *> first, not_null<HistoryItem *> second) {
...
void ScheduledWidget::listMarkContentsRead(const base::flat_set<not_null<HistoryItem *>> &items) {}

MessagesBarData ScheduledWidget::listMessagesBar(const std::vector<not_null<Element *>> &elements) { return {}; }

void ScheduledWidget::listContentRefreshed() {}

void ScheduledWidget::listUpdateDateLink(ClickHandlerPtr &link, not_null<Element *> view) {}
...
bool ScheduledWidget::listElementShownUnread(not_null<const Element *> view) { return true; }

bool ScheduledWidget::listIsGoodForAroundPosition(not_null<const Element *> view) { return true; }

bool ScheduledWidget::showMessage(PeerId peerId, const Window::SectionShow &params, MsgId messageId) {
	...
	const auto originItem = [&]() -> HistoryItem *
	{
		...
		if (const auto origin = std::get_if<OriginMessage>(&params.origin)) {
			if (const auto returnTo = session().data().message(origin->id)) {
				if (_inner->viewByPosition(returnTo->position()) && _cornerButtons.replyReturn() != returnTo) {
					return returnTo;
				}
			}
		}
	...
	showAtPosition(message->position(), originItem ? originItem->fullId() : FullMsgId());
	return true;
}

Window::SectionActionResult ScheduledWidget::sendBotCommand(Bot::SendCommandRequest request) {
...
void ScheduledWidget::listSendBotCommand(const QString &command, const FullMsgId &context) {
	const auto callback = [=](Api::SendOptions options)
	{
...
void ScheduledWidget::listSearch(const QString &query, const FullMsgId &context) {
...
not_null<Ui::ChatTheme *> ScheduledWidget::listChatTheme() { return _theme.get(); }

CopyRestrictionType ScheduledWidget::listCopyRestrictionType(HistoryItem *item) { return CopyRestrictionType::None; }

CopyRestrictionType ScheduledWidget::listCopyMediaRestrictionType(not_null<HistoryItem *> item) {
	if (const auto media = item->media()) {
...
CopyRestrictionType ScheduledWidget::listSelectRestrictionType() { return CopyRestrictionType::None; }

auto ScheduledWidget::listAllowedReactionsValue() -> rpl::producer<Data::AllowedReactions> {
	return rpl::single(Data::AllowedReactions());
}

void ScheduledWidget::listShowPremiumToast(not_null<DocumentData *> document) {
	if (!_stickerToast) {
		_stickerToast =
...
void ScheduledWidget::listOpenPhoto(not_null<PhotoData *> photo, FullMsgId context) {
	controller()->openPhoto(photo, {context});
}

void ScheduledWidget::listOpenDocument(not_null<DocumentData *> document, FullMsgId context, bool showInMediaView) {
	controller()->openDocument(document, showInMediaView, {context});
}

void ScheduledWidget::listPaintEmpty(Painter &p, const Ui::ChatPaintContext &context) {}

QString ScheduledWidget::listElementAuthorRank(not_null<const Element *> view) { return {}; }

bool ScheduledWidget::listElementHideTopicButton(not_null<const Element *> view) { return true; }

History *ScheduledWidget::listTranslateHistory() { return nullptr; }

void ScheduledWidget::listAddTranslatedItems(not_null<TranslateTracker *> tracker) {}

void ScheduledWidget::confirmSendNowSelected() { ConfirmSendNowSelectedItems(_inner); }

void ScheduledWidget::confirmDeleteSelected() { ConfirmDeleteSelectedItems(_inner); }

void ScheduledWidget::clearSelected() { _inner->cancelSelection(); }
...
bool ShowScheduledVideoPublished(not_null<Window::SessionController *> controller,
	...
	const auto text = tr::lng_scheduled_video_published(tr::now, Ui::Text::Bold);
	...
	const auto additional = QMargins(
		skip + size, 0, (st::processingVideoView.style.font->width(view) - (st::processingVideoView.width / 2)), 0);
	...
	const auto clickableBackground = Ui::CreateChild<Ui::AbstractButton>(widget.get());
	clickableBackground->setPointerCursor(false);
	clickableBackground->setAcceptBoth();
	clickableBackground->show();
	clickableBackground->addClickHandler(
	...
	const auto button = Ui::CreateChild<Ui::RoundButton>(widget.get(), rpl::single(view), st::processingVideoView);
	button->setTextTransform(Ui::RoundButton::TextTransform::NoTransform);
	button->show();
	rpl::combine(widget->sizeValue(), button->sizeValue()) |
		rpl::start_with_next(
			[=](QSize outer, QSize inner)
			{
	...
	const auto thumbnail = Ui::MakeDocumentThumbnail(document, FullMsgId(history->peer->id, itemId));
	thumbnail->subscribeToUpdates([=] { preview->update(); });
	preview->paintRequest() |
		rpl::start_with_next(
			[=]
			{
				auto p = QPainter(preview);
				const auto image = Images::Round(thumbnail->image(size), ImageRoundRadius::Small);
	...
	button->setClickedCallback(
		[=]
		{
			controller->showPeerHistory(history, Window::SectionShow::Way::Forward, itemId);
			hideToast();
		});
