...
[[nodiscard]] QImage CacheDiceImage(const QString &emoji, int index, const QImage &image) {
...
Sticker::Sticker(not_null<Element *> parent,
				 not_null<DocumentData *> data,
				 bool skipPremiumEffect,
				 Element *replacing,
				 const Lottie::ColorReplacements *replacements)
	: _parent(parent), _data(data), _replacements(replacements),
	  _cachingTag(ChatHelpers::StickerLottieSize::MessageHistory), _skipPremiumEffect(skipPremiumEffect) {
	...
	if (const auto media = replacing ? replacing->media() : nullptr) {
		...
		if (_player) {
			if (hasPremiumEffect() && !_premiumEffectPlayed) {
				...
				if (On(PowerSaving::kStickersChat) && !_premiumEffectSkipped) {
...
bool Sticker::hasPremiumEffect() const { return !_skipPremiumEffect && _data->isPremiumSticker(); }

bool Sticker::customEmojiPart() const { return _customEmojiPart; }

bool Sticker::emojiSticker() const { return _emojiSticker; }

bool Sticker::webpagePart() const { return _webpagePart; }

void Sticker::initSize(int customSize) {
	if (customSize > 0) {
		const auto original = Size(_data);
		const auto proposed = QSize{customSize, customSize};
		_size = original.isEmpty() ? proposed : DownscaledSize(original, proposed);
...
bool Sticker::readyToDrawAnimationFrame() {
	...
	const auto waitingForPremium = hasPremiumEffect() && _dataMedia->videoThumbnailContent().isEmpty();
...
QSize Sticker::Size(not_null<DocumentData *> document) { return DownscaledSize(document->dimensions, Size()); }

QSize Sticker::PremiumEffectSize(not_null<DocumentData *> document) { return Size(document) * kPremiumMultiplier; }

QSize Sticker::UsualPremiumEffectSize() {
	return DownscaledSize({kMaxSizeFixed, kMaxSizeFixed}, Size()) * kPremiumMultiplier;
}

QSize Sticker::EmojiEffectSize() { return EmojiSize() * kEmojiMultiplier; }

QSize Sticker::MessageEffectSize() { return EmojiSize() * kMessageEffectMultiplier; }
...
void Sticker::draw(Painter &p, const PaintContext &context, const QRect &r) {
	if (!customEmojiPart()) {
	...
	} else if (!_data->sticker() || (_data->sticker()->isLottie() && _replacements) || !paintPixmap(p, context, r)) {
		paintPath(p, context, r);
	}
}

ClickHandlerPtr Sticker::link() { return _link; }

bool Sticker::ready() const { return _player && _player->ready(); }

DocumentData *Sticker::document() { return _data; }
...
void Sticker::paintAnimationFrame(Painter &p, const PaintContext &context, const QRect &r) {
	const auto colored = (customEmojiPart() && _data->emojiUsesTextColor()) ? ComputeEmojiTextColor(context)
		: (context.selected() && !_nextLastDiceFrame)						? context.st->msgStickerOverlay()->c
																			: QColor(0, 0, 0, 0);
	const auto powerSavingFlag =
		(emojiSticker() || _diceIndex >= 0) ? PowerSaving::kEmojiChat : PowerSaving::kStickersChat;
	const auto paused = context.paused || On(powerSavingFlag);
	const auto frame =
		_player ? _player->frame(_size, colored, mirrorHorizontal(), context.now, paused) : StickerPlayer::FrameInfo();
	if (_nextLastDiceFrame) {
		_nextLastDiceFrame = false;
		_lastDiceFrame = CacheDiceImage(_diceEmoji, _diceIndex, frame.image);
	}
	const auto &image = _lastDiceFrame.isNull() ? frame.image : _lastDiceFrame;
	...
	auto prepared = (!_lastDiceFrame.isNull() && context.selected())
		? Images::Colored(base::duplicate(image), context.st->msgStickerOverlay()->c)
		: image;
	prepared = Images::Round(std::move(prepared), MediaRoundingMask(rounding));
	const auto size = prepared.size() / style::DevicePixelRatio();
	p.drawImage(QRect(QPoint(r.x() + (r.width() - size.width()) / 2, r.y() + (r.height() - size.height()) / 2), size),
				prepared);
	if (!_lastDiceFrame.isNull()) {
		return;
	}
	...
	_framesCount = count;
	_nextLastDiceFrame = !paused && (_diceIndex > 0) && (_frameIndex + 2 == count);
	const auto playOnce = (_playingOnce || _diceIndex > 0) ? true
		: (_diceIndex == 0)
		? false
		: ((!customEmojiPart() && emojiSticker()) || !Core::App().settings().loopAnimatedStickers());
	const auto lastDiceFrame = (_diceIndex > 0) && atTheEnd();
	const auto switchToNext = !playOnce || (!lastDiceFrame && (_frameIndex != 0 || !_oncePlayed));
	if (!paused && switchToNext && _player->markFrameShown() && playOnce && !_oncePlayed) {
		_oncePlayed = true;
...
bool Sticker::paintPixmap(Painter &p, const PaintContext &context, const QRect &r) {
	const auto pixmap = paintedPixmap(context);
	if (pixmap.isNull()) {
		return false;
	}
	const auto size = pixmap.size() / pixmap.devicePixelRatio();
	const auto position = QPoint(r.x() + (r.width() - size.width()) / 2, r.y() + (r.height() - size.height()) / 2);
	const auto mirror = mirrorHorizontal();
	if (mirror) {
		p.save();
		const auto middle = QPointF(position.x() + size.width() / 2., position.y() + size.height() / 2.);
		p.translate(middle);
		p.scale(-1., 1.);
		p.translate(-middle);
	}
...
void Sticker::paintPath(Painter &p, const PaintContext &context, const QRect &r) {
	...
	if (customEmojiPart() && _data->emojiUsesTextColor()) {
		helper.emplace(Ui::CustomEmoji::PreviewColorFromTextColor(ComputeEmojiTextColor(context)));
		pathGradient->overrideColors(helper->color(), helper->color());
	} else if (webpagePart()) {
		pathGradient->overrideColors(st::shadowFg, st::shadowFg);
	} else if (context.selected()) {
		pathGradient->overrideColors(context.st->msgServiceBgSelected(), context.st->msgServiceBg());
	} else {
		pathGradient->clearOverridenColors();
	}
	p.setBrush(context.imageStyle()->msgServiceBg);
	ChatHelpers::PaintStickerThumbnailPath(p, _dataMedia.get(), r, pathGradient, mirrorHorizontal());
	if (helper) {
		pathGradient->clearOverridenColors();
	}
}
...
QPixmap Sticker::paintedPixmap(const PaintContext &context) const {
	...
	const auto adjust = [&](int side) { return (((side * ratio) / 8) * 8) / ratio; };
	const auto useSize =
		(sticker && sticker->type == StickerType::Tgs) ? QSize(adjust(_size.width()), adjust(_size.height())) : _size;
	const auto colored = (customEmojiPart() && _data->emojiUsesTextColor())
		? &helper.emplace(ComputeEmojiTextColor(context)).color()
		: context.selected() ? &context.st->msgStickerOverlay()
							 : nullptr;
	...
	if (const auto image = _dataMedia->getStickerLarge()) {
		...
		return thumbnail->pix(useSize, {.colored = colored, .options = Images::Option::Blur | roundOptions});
	}
	return QPixmap();
}
	...
	if (emojiSticker()) {
		...
		if (hasPremiumEffect()) {
			...
			_link = ShowSetHandler(_data);
			   ...
			   (_data->dimensions.width() > kStickerSideSize || _data->dimensions.height() > kStickerSideSize) &&
		...
		_link = std::make_shared<DocumentOpenClickHandler>(
			_data,
			crl::guard(this, [=](FullMsgId id) { _parent->delegate()->elementOpenDocument(_data, id); }),
			_parent->data()->fullId());
	}
}
...
void Sticker::setPlayingOnce(bool once) { _playingOnce = once; }

void Sticker::setCustomCachingTag(ChatHelpers::StickerLottieSize tag) { _cachingTag = tag; }

void Sticker::setCustomEmojiPart() { _customEmojiPart = true; }

void Sticker::setEmojiSticker() { _emojiSticker = true; }

void Sticker::setWebpagePart() { _webpagePart = true; }
...
void Sticker::setupPlayer() {
	...
	if (_data->sticker()->isLottie()) {
		_player = std::make_unique<LottiePlayer>(
			ChatHelpers::LottiePlayerFromDocument(_dataMedia.get(),
												  _replacements,
												  _cachingTag,
		...
		_player =
			std::make_unique<WebmPlayer>(_dataMedia->owner()->location(), _dataMedia->bytes(), countOptimalSize());
	}
...
bool Sticker::hasHeavyPart() const { return _player || _dataMedia; }
...
std::unique_ptr<StickerPlayer> Sticker::stickerTakePlayer(not_null<DocumentData *> data,
														  const Lottie::ColorReplacements *replacements) {
	return (data == _data && replacements == _replacements) ? std::move(_player) : nullptr;
}
