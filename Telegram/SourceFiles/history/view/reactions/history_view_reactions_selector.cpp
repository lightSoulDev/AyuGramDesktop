...
{
public:
	StripEmoji(std::unique_ptr<Ui::Text::CustomEmoji> wrapped, not_null<Strip *> strip, QPoint shift, int index);
...
StripEmoji::StripEmoji(std::unique_ptr<Ui::Text::CustomEmoji> wrapped, not_null<Strip *> strip, QPoint shift, int index)
	: _wrapped(std::move(wrapped)), _strip(strip), _shift(shift), _index(index) {}

int StripEmoji::width() { return _wrapped->width(); }

QString StripEmoji::entityData() { return _wrapped->entityData(); }

void StripEmoji::paint(QPainter &p, const Context &context) {
	if (_switched) {
		_wrapped->paint(p, context);
	} else if (_wrapped->readyInDefaultState() && _strip->inDefaultState(_index)) {
		_switched = true;
		_wrapped->paint(p, context);
	} else {
...
bool StripEmoji::ready() { return _wrapped->ready(); }

bool StripEmoji::readyInDefaultState() { return _wrapped->readyInDefaultState(); }

} // namespace

UnifiedFactoryOwner::UnifiedFactoryOwner(not_null<Main::Session *> session,
										 const std::vector<Data::Reaction> &reactions,
										 Strip *strip)
	: _session(session), _strip(strip) {
	auto index = 0;
	...
	_stripPaintOneShift = [&]
	{
		...
		const auto areaPosition = QPoint((size - area.width()) / 2, (size - area.height()) / 2);
		const auto esize = Ui::Emoji::GetSizeLarge() / style::DevicePixelRatio();
		const auto innerPosition = QPoint((area.width() - esize) / 2, (area.height() - esize) / 2);
	...
	_defaultReactionShift =
		QPoint((st::reactStripSize - st::reactStripImage) / 2, (st::reactStripSize - st::reactStripImage) / 2) -
		_stripPaintOneShift;
}

Data::ReactionId UnifiedFactoryOwner::lookupReactionId(DocumentId unifiedId) const {
	const auto i = _defaultReactionIds.find(unifiedId);
	return (i != end(_defaultReactionIds)) ? i->second : Data::ReactionId{unifiedId};
}

UnifiedFactoryOwner::RecentFactory UnifiedFactoryOwner::factory() {
	return [=](DocumentId id, Fn<void()> repaint) -> std::unique_ptr<Ui::Text::CustomEmoji>
	{
		const auto tag = Data::CustomEmojiManager::SizeTag::Large;
		const auto sizeOverride = st::reactStripImage;
		const auto i = _defaultReactionIds.find(id);
		const auto isDefaultReaction = (i != end(_defaultReactionIds)) && !i->second.custom();
		const auto manager = &_session->data().customEmojiManager();
		auto result = isDefaultReaction
			? std::make_unique<Ui::Text::ShiftedEmoji>(manager->create(id, std::move(repaint), tag, sizeOverride),
													   _defaultReactionShift)
		...
		if (j != end(_defaultReactionInStripMap)) {
			...
			return std::make_unique<StripEmoji>(std::move(result), _strip, -_stripPaintOneShift, j->second);
		}
		return result;
	};
}

Selector::Selector(not_null<QWidget *> parent,
	...
	: Selector(parent,
			   ...
			   std::move(show),
			   reactions,
			   (reactions.customAllowed			 ? ChatHelpers::EmojiListMode::FullReactions
					: reactions.stickers.empty() ? ChatHelpers::EmojiListMode::RecentReactions
...
Selector::Selector(not_null<QWidget *> parent,
				   ...
				   Fn<void(bool fast)> close,
				   bool child)
	: RpWidget(parent), _st(st), _show(std::move(show)), _reactions(reactions), _recent(std::move(recent)),
	  _listMode(mode), _paused(std::move(paused)), _jumpedToPremium([=] { close(false); }),
	  _cachedRound(QSize(2 * st::reactStripSkip + st::reactStripSize, st::reactStripHeight),
				   st::reactionCornerShadow,
				   st::reactStripHeight),
	  _strip(std::make_unique<Strip>(_st,
									 QRect(0, 0, st::reactStripSize, st::reactStripSize),
									 st::reactStripImage,
									 crl::guard(this, [=] { update(_inner); }),
									 std::move(iconFactory))),
	  _about(about.empty() ? nullptr : std::make_unique<Ui::FlatLabel>(this, rpl::single(about), _st.about)),
	  _size(st::reactStripSize), _skipx(countSkipLeft()), _skipy((st::reactStripHeight - st::reactStripSize) / 2) {
...
bool Selector::useTransparency() const { return _useTransparency; }

int Selector::recentCount() const { return int(_strip ? _reactions.recent.size() : _recent.size()); }

int Selector::countSkipLeft() const {
	const auto addedToMax = _reactions.customAllowed;
	const auto max = recentCount() + (addedToMax ? 1 : 0);
	return std::max((st::reactStripMinWidth - (max * _size)) / 2, st::reactStripSkip);
}

int Selector::countWidth(int desiredWidth, int maxWidth) {
	const auto addedToMax = _reactions.customAllowed;
	const auto max = recentCount() + (addedToMax ? 1 : 0);
	const auto desiredColumns = std::max((desiredWidth - 2 * _skipx + _size - 1) / _size, kMinNonTransparentColumns);
	const auto possibleColumns = std::min(desiredColumns, (maxWidth - 2 * _skipx) / _size);
	...
	const auto added =
		(_columns < max || _reactions.customAllowed) ? Strip::AddedButton::Expand : Strip::AddedButton::None;
	if (_strip) {
...
int Selector::effectPreviewHeight() const {
	...
	return st::previewMenu.shadow.extend.top() + HistoryView::Sticker::MessageEffectSize().height() +
		st::effectPreviewSend.height + st::previewMenu.shadow.extend.bottom();
}

QMargins Selector::marginsForShadow() const {
	const auto line = st::lineWidth;
	return useTransparency() ? st::reactionCornerShadow : QMargins(line, line, line, line);
}

int Selector::extendTopForCategories() const { return _reactions.customAllowed ? _st.footer : 0; }
...
int Selector::extendTopForCategoriesAndAbout(int width) const {
	if (_about) {
		...
		const auto desired =
			Ui::FindNiceTooltipWidth(std::min(available, _st.about.minWidth * 2), available, countAboutHeight);
...
int Selector::minimalHeight(int fullWidth) const {
	...
	if (const auto stickers = int(_reactions.stickers.size())) {
		...
		const auto listWidth =
			fullWidth - marginsForShadow().left() - marginsForShadow().right() - _st.margin.left() - _st.margin.right();
		const auto availableWidth = listWidth - (st::stickerPanPadding - _st.margin.left());
	...
	if (_listMode == ChatHelpers::EmojiListMode::MessageEffects) {
		inner += _st.searchMargin.top() + _st.search.height + _st.searchMargin.bottom();
	}
	return _skipy + std::min(inner, st::emojiPanMinHeight) + st::emojiPanRadius + _st.padding.bottom();
}

void Selector::setSpecialExpandTopSkip(int skip) { _specialExpandTopSkip = skip; }
...
void Selector::initGeometry(int innerTop) {
	...
	const auto width = _useTransparency ? (innerWidth + margins.left() + margins.right()) : parent.width();
	const auto forAbout = width - margins.left() - margins.right();
	_collapsedTopSkip = _useTransparency ? (extendTopForCategoriesAndAbout(forAbout) + _specialExpandTopSkip)
										 : opaqueExtendTopAbout(forAbout);
	_topAddOnExpand = _collapsedTopSkip - _aboutExtend;
	const auto height = margins.top() + _aboutExtend + innerHeight + margins.bottom();
	const auto left = style::RightToLeft() ? 0 : (parent.width() - width);
	const auto top = innerTop - margins.top() - (_useTransparency ? _collapsedTopSkip : 0);
	const auto add = _useTransparency ? (_st.icons.stripBubble.height() - margins.bottom()) : 0;
	_outer = QRect(0, _collapsedTopSkip - _aboutExtend, width, height);
	_outerWithBubble = _outer.marginsAdded({0, 0, 0, add});
	setGeometry(_outerWithBubble.marginsAdded({0, _outer.y(), 0, 0}).translated(left, top));
	_inner = _outer.marginsRemoved(margins + QMargins{0, _aboutExtend, 0, 0});
	if (_about) {
		_about->move(_inner.x() + (_inner.width() - _about->width()) / 2,
...
rpl::producer<> Selector::escapes() const { return _escapes.events(); }

void Selector::updateShowState(float64 progress, float64 opacity, bool appearing, bool toggling) {
	if (_useTransparency && _appearing && !appearing && !_paintBuffer.isNull()) {
		paintBackgroundToBuffer();
		if (_about && _about->isHidden()) {
			_about->show();
		}
	} else if (_useTransparency && !_appearing && appearing && _about) {
		_about->hide();
	}
...
void Selector::paintAppearing(QPainter &p) {
	...
	_strip->paint(q, {_skipx, _skipy}, {_size, 0}, {0, 0, appearedWidth, _inner.height()}, 1., false);
	...
	_cachedRound.overlayExpandedBorder(q, size, _appearProgress, radius, radius, 1.);
	q.setCompositionMode(QPainter::CompositionMode_Source);
	q.fillRect(QRect{0, size.height(), width(), height() - size.height()}, Qt::transparent);
	q.setCompositionMode(QPainter::CompositionMode_SourceOver);
	paintBubble(q, appearedWidth);
	q.end();

	p.drawImage(_outer.topLeft(), _paintBuffer, QRect(QPoint(), QSize(fullWidth, height()) * factor));
	...
	if (_about && _about->isHidden() && aboutRight > _about->x()) {
		...
		p.drawImage(
			_about->geometry().topLeft(), _aboutCache, QRect(QPoint(), QSize(aboutWidth, _about->height()) * factor));
	}
}
...
void Selector::paintCollapsed(QPainter &p) {
	...
	_strip->paint(p, _inner.topLeft() + QPoint(_skipx, _skipy), {_size, 0}, _inner, 1., false);
}
...
void Selector::paintExpanding(Painter &p, float64 progress) {
	...
	if (_footer) {
		_footer->paintExpanding(p, rects.categories, rects.radius, RectPart::BottomRight);
	}
	_list->paintExpanding(
		p, rects.list.marginsRemoved(_st.margin), rects.finalBottom, rects.expanding, progress, RectPart::TopRight);
	paintFadingExpandIcon(p, progress);
}

Selector::ExpandingRects Selector::updateExpandingRects(float64 progress) {
	progress = (progress >= kExpandDuration) ? 1. : (progress / kExpandDuration);
	...
	const auto radius = _reactions.customAllowed ? (radiusStart + progress * (radiusEnd - radiusStart)) : radiusStart;
	const auto margins = marginsForShadow();
	const auto expanding = anim::easeOutCirc(1., progress);
	const auto expandUp = anim::interpolate(0, _topAddOnExpand, expanding);
	const auto expandDown = anim::interpolate(0, (height() - _outer.y() - _outer.height()), expanding);
	const auto outer = _outer.marginsAdded({0, expandUp, 0, expandDown});
	const auto inner = outer.marginsRemoved(margins + QMargins{0, anim::interpolate(_aboutExtend, 0, expanding), 0, 0});
	const auto list = outer.marginsRemoved(
		margins + QMargins{0, anim::interpolate(_aboutExtend, extendTopForCategories(), expanding), 0, 0});
	_shadowTop = list.y();
	const auto categories = list.y() - inner.y();
	_shadowSkip = (_useTransparency && categories < radius)
		? int(base::SafeRound(radius - sqrt(categories * (2 * radius - categories))))
		: 0;

	if (!_useTransparency && _opaqueApplyHeightExpand) {
		Ui::PostponeCall(this, [=] { _opaqueApplyHeightExpand(y() + outer.y() + outer.height()); });
	}
...
void Selector::paintExpandingBg(QPainter &p, const ExpandingRects &rects) {
	if (_useTransparency) {
		const auto pattern = _cachedRound.validateFrame(rects.frame, 1., rects.radius);
...
void Selector::paintFadingExpandIcon(QPainter &p, float64 progress) {
	...
	const auto expandIconPosition = _inner.topLeft() + QPoint(_inner.width() - _size - _skipx, _skipy);
	const auto expandIconRect = QRect(expandIconPosition, QSize(_size, _size)).marginsRemoved({sub, sub, sub, sub});
	p.drawImage(expandIconRect, _expandIconCache);
	p.setOpacity(1.);
}

void Selector::paintNonTransparentExpandRect(QPainter &p, const QRect &inner) const {
	p.fillRect(inner, _st.bg);
	p.fillRect(
		inner.x(), inner.y() + inner.height(), inner.width(), st::lineWidth, st::defaultPopupMenu.shadow.fallback);
}
...
void Selector::finishExpand() {
	...
	if (_useTransparency) {
		auto q = QPainter(&_paintBuffer);
		q.setCompositionMode(QPainter::CompositionMode_Source);
		const auto pattern = _cachedRound.validateFrame(kFramesCount - 1, 1., st::emojiPanRadius);
...
void Selector::paintEvent(QPaintEvent *e) {
	...
	if (_strip && _appearing && _useTransparency) {
		...
		paintCollapsed(p);
	} else if (const auto progress = _expanding.value(kFullDuration); progress < kFullDuration) {
...
ChosenReaction Selector::lookupChosen(const Data::ReactionId &id) const {
	...
	result.globalGeometry = mapToGlobal(QRect(_inner.x() + rect.x() + (rect.width() - imageSize) / 2,
...
void Selector::expand() {
	...
	const auto margins = marginsForShadow();
	const auto heightLimit = _reactions.customAllowed ? st::emojiPanMaxHeight : minimalHeight(width());
	const auto opaqueAdded = _useTransparency ? 0 : _opaqueHeightExpand;
	const auto willBeHeight =
		std::min(parent.height() - y() + opaqueAdded, margins.top() + heightLimit + margins.bottom());
	...
	base::call_delayed(kExpandDelay,
					   this,
					   [this]
					   {
...
void Selector::createList() {
	...
	_unifiedFactoryOwner = std::make_unique<UnifiedFactoryOwner>(
		&_show->session(), _strip ? _reactions.recent : std::vector<Data::Reaction>(), _strip.get());
	_scroll = Ui::CreateChild<Ui::ScrollArea>(this,
											  !_useTransparency				 ? st::emojiScroll
												  : _reactions.customAllowed ? st::reactPanelScroll
	...
	auto lists = _scroll->setOwnedWidget(object_ptr<Ui::VerticalLayout>(_scroll));
	auto recentList = _strip ? _unifiedFactoryOwner->unifiedIdsList() : _recent;
	...
	if (effects) {
		...
		for (const auto &id : recentList) {
			const auto reactionId = _strip ? _unifiedFactoryOwner->lookupReactionId(id) : Data::ReactionId{id};
			if (free.contains(reactionId)) {
				freeEffects.insert(id);
			}
		}
	}
	_list = lists->add(object_ptr<EmojiListWidget>(lists,
												   EmojiListDescriptor{
													   .show = _show,
													   .mode = _listMode,
	...
	if (!_reactions.stickers.empty()) {
		auto descriptors =
			ranges::views::all(_reactions.stickers) |
			ranges::view::transform(
				[](const Data::Reaction &reaction)
				{ return ChatHelpers::StickerCustomRecentDescriptor{reaction.selectAnimation, reaction.title}; }) |
			ranges::to_vector;
		_stickers = lists->add(object_ptr<StickersListWidget>(lists,
	...
	rpl::merge(_list->customChosen(), (_stickers ? _stickers->chosen() : rpl::never<ChatHelpers::FileChosen>())) |
		rpl::start_with_next(
	...
	_list->jumpedToPremium() | rpl::start_with_next(_jumpedToPremium, _list->lifetime());

	const auto inner = rect().marginsRemoved(marginsForShadow());
	const auto footer = _reactions.customAllowed ? _list->createFooter().data() : nullptr;
	if ((_footer = static_cast<StickersListFooter *>(footer))) {
		_footer->setParent(this);
		_footer->hide();
		_footer->setGeometry(inner.x(), inner.y(), inner.width(), _footer->height());
		_shadowTop = _outer.y();
		_shadowSkip = _useTransparency ? (st::reactStripHeight / 2) : 0;
		_shadow = Ui::CreateChild<Ui::PlainShadow>(this);
		rpl::combine(_shadowTop.value(), _shadowSkip.value()) |
			rpl::start_with_next(
				[=](int top, int skip)
				{ _shadow->setGeometry(inner.x() + skip, top, inner.width() - 2 * skip, st::lineWidth); },
				_shadow->lifetime());
		_shadow->show();
	}
	...
	_scroll->scrollTopChanges() | rpl::start_with_next(updateVisibleTopBottom, lists->lifetime());
	...
	if (_stickers) {
		...
		_list->searchQueries() |
			rpl::start_with_next([=](std::vector<QString> &&query) { _stickers->applySearchQuery(std::move(query)); },
								 _stickers->lifetime());

		rpl::combine(_list->heightValue(), _stickers->heightValue()) |
			rpl::start_with_next([=] { InvokeQueued(lists, updateVisibleTopBottom); }, _stickers->lifetime());

		rpl::combine(_list->recentShownCount(), _stickers->recentShownCount()) |
			rpl::start_with_next(
				[=](int emoji, int stickers)
				{
		...
		_scroll->paintRequest() | rpl::filter([=] { return _showEmptySearch; }) |
			rpl::start_with_next(
				[=]
				{
					...
					p.drawText(_scroll->rect(), tr::lng_effect_none(tr::now), style::al_center);
				},
				_scroll->lifetime());
	} else {
...
bool AdjustMenuGeometryForSelector(not_null<Ui::PopupMenu *> menu,
	...
	const auto extend =
		useTransparency ? st::reactStripExtend : QMargins(0, st::lineWidth + st::reactStripHeight, 0, 0);
	...
	const auto categoriesAboutTop = selector->useTransparency() ? selector->extendTopForCategoriesAndAbout(width)
	...
	const auto minimalHeight =
		std::max(margins.top() + selector->minimalHeight(width) + margins.bottom(), selector->effectPreviewHeight());
	const auto willBeHeightWithoutBottomPadding = fullTop + height - menu->st().shadow.extend.top();
	const auto additionalPaddingBottom =
		(willBeHeightWithoutBottomPadding >= minimalHeight ? 0 : (minimalHeight - willBeHeightWithoutBottomPadding));
	menu->setAdditionalMenuPadding(
		QMargins(margins.left() + extend.left(), fullTop, margins.right() + extend.right(), additionalPaddingBottom),
		QMargins(margins.left(), margins.top(), margins.right(), std::min(additionalPaddingBottom, margins.bottom())));
	if (!menu->prepareGeometryFor(desiredPosition)) {
		return false;
	}
	const auto origin = menu->preparedOrigin();
	const auto expandDown =
		(origin == Ui::PanelAnimation::Origin::TopLeft) || (origin == Ui::PanelAnimation::Origin::TopRight);
	if (!useTransparency) {
		const auto expandBy = additionalPaddingBottom;
		selector->setOpaqueHeightExpand(expandBy,
										[=](int bottom)
										{
		...
		menu->setAdditionalMenuPadding(
			QMargins(margins.left() + extend.left(), fullTop, margins.right() + extend.right(), 0),
			QMargins(margins.left(), margins.top(), margins.right(), 0));
	...
	menu->setAdditionalMenuPadding(
		QMargins(
			margins.left() + extend.left(), fullTop + additionalPaddingBottom, margins.right() + extend.right(), 0),
		QMargins(margins.left(), margins.top(), margins.right(), 0));
...
AttachSelectorResult AttachSelectorToMenu(not_null<Ui::PopupMenu *> menu,
										  not_null<Window::SessionController *> controller,
										  QPoint desiredPosition,
	...
	const auto result = AttachSelectorToMenu(menu,
											 desiredPosition,
											 st::reactPanelEmojiPan,
											 controller->uiShow(),
	...
	selector->escapes() | rpl::start_with_next([=] { menu->hideMenu(); }, selector->lifetime());

	const auto weak = base::make_weak(controller);
	controller->enableGifPauseReason(Window::GifPauseReason::MediaPreview);
	QObject::connect(menu.get(),
					 &QObject::destroyed,
					 [weak]
					 {
						 if (const auto strong = weak.get()) {
							 strong->disableGifPauseReason(Window::GifPauseReason::MediaPreview);
						 }
					 });
...
auto AttachSelectorToMenu(not_null<Ui::PopupMenu *> menu,
						  QPoint desiredPosition,
						  const style::EmojiPan &st,
						  std::shared_ptr<ChatHelpers::Show> show,
						  const Data::PossibleItemReactionsRef &reactions,
						  TextWithEntities about,
						  IconFactory iconFactory,
						  Fn<bool()> paused) -> base::expected<not_null<Selector *>, AttachSelectorResult> {
	const auto settings = &AyuSettings::getInstance();
	if (!AyuUi::needToShowItem(settings->showReactionsPanelInContextMenu)) {
	...
	const auto selectorInnerTop =
		selector->useTransparency() ? (menu->preparedPadding().top() - st::reactStripExtend.top()) : st::lineWidth;
	menu->animatePhaseValue() |
		rpl::start_with_next(
	...
	menu->showStateValue() |
		rpl::start_with_next(
			[=](Ui::PopupMenu::ShowState state)
			{
				const auto origin = menu->preparedOrigin();
				using Origin = Ui::PanelAnimation::Origin;
				if (origin == Origin::BottomLeft || origin == Origin::BottomRight) {
					const auto add = state.appearing
						? (menu->rect().marginsRemoved(menu->preparedPadding()).height() - state.appearingHeight)
						: 0;
					selector->move(selector->x(), correctTop + add);
				}
				selector->updateShowState(
					state.widthProgress * state.heightProgress, state.opacity, state.appearing, state.toggling);
			},
			selector->lifetime());
...
TextWithEntities ItemReactionsAbout(not_null<HistoryItem *> item) {
	return !item->reactionsAreTags() ? TextWithEntities()
		: item->history()->session().premium()
		? TextWithEntities{tr::lng_add_tag_about(tr::now)}
		: tr::lng_subscribe_tag_about(tr::now,
									  lt_link,
									  Ui::Text::Link(tr::lng_subscribe_tag_link(tr::now), u"internal:about_tags"_q),
