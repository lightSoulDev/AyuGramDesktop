...
SendActionPainter::SendActionPainter(not_null<History *> history, MsgId rootId)
	: _history(history), _rootId(rootId), _weak(&_history->session()), _st(st::dialogsTextStyle),
	  _sendActionText(st::dialogsTextWidthMin) {}

void SendActionPainter::setTopic(Data::ForumTopic *topic) { _topic = topic; }

bool SendActionPainter::updateNeedsAnimating(not_null<UserData *> user, const MTPSendMessageAction &action) {
	...
	const auto emplaceAction = [&](Type type, crl::time duration, int progress = 0)
	...
	action.match([&](const MTPDsendMessageTypingAction &)
				 ...
				 { emplaceAction(Type::UploadVideo, kStatusShowClientsideUploadVideo, data.vprogress().v); },
				 [&](const MTPDsendMessageUploadAudioAction &data)
				 { emplaceAction(Type::UploadVoice, kStatusShowClientsideUploadVoice, data.vprogress().v); },
				 [&](const MTPDsendMessageUploadRoundAction &data)
				 { emplaceAction(Type::UploadRound, kStatusShowClientsideUploadRound, data.vprogress().v); },
				 [&](const MTPDsendMessageUploadPhotoAction &data)
				 { emplaceAction(Type::UploadPhoto, kStatusShowClientsideUploadPhoto, data.vprogress().v); },
				 [&](const MTPDsendMessageUploadDocumentAction &data)
				 { emplaceAction(Type::UploadFile, kStatusShowClientsideUploadFile, data.vprogress().v); },
				 [&](const MTPDsendMessageGamePlayAction &)
				 {
					 const auto i = _sendActions.find(user);
					 if ((i == end(_sendActions)) || (i->second.type == Type::PlayGame) || (i->second.until <= now)) {
						 emplaceAction(Type::PlayGame, kStatusShowClientsidePlayGame);
					 }
				 },
				 [&](const MTPDspeakingInGroupCallAction &)
				 { _speaking.emplace_or_assign(user, now + kStatusShowClientsideSpeaking); },
				 [&](const MTPDsendMessageHistoryImportAction &) {},
				 [&](const MTPDsendMessageChooseStickerAction &)
				 { emplaceAction(Type::ChooseSticker, kStatusShowClientsideChooseSticker); },
				 [&](const MTPDsendMessageEmojiInteraction &) { Unexpected("EmojiInteraction here."); },
				 [&](const MTPDsendMessageEmojiInteractionSeen &)
				 {
					 // #TODO interaction
				 },
				 [&](const MTPDsendMessageCancelAction &) { Unexpected("CancelAction here."); });
	return updateNeedsAnimating(now, true);
}

bool SendActionPainter::paint(
	Painter &p, int x, int y, int availableWidth, int outerWidth, style::color color, crl::time ms) {
	if (_sendActionAnimation) {
		const auto animationWidth = _sendActionAnimation.width();
		const auto extraAnimationWidth = _animationLeft ? animationWidth * 2 : 0;
		const auto left = (availableWidth < _animationLeft + extraAnimationWidth) ? 0 : _animationLeft;
		_sendActionAnimation.paint(p, color, left + x, y + st::normalFont->ascent, outerWidth, ms);
		// availableWidth should be the same
		// if an animation is in the middle of text.
		if (!left) {
			x += animationWidth;
			availableWidth -= _animationLeft ? extraAnimationWidth : animationWidth;
		}
		p.setPen(color);
...
void SendActionPainter::paintSpeaking(QPainter &p, int x, int y, int outerWidth, style::color color, crl::time ms) {
	if (_speakingAnimation) {
		_speakingAnimation.paint(p, color, x, y, outerWidth, ms);
	} else {
		Ui::SendActionAnimation::PaintSpeakingIdle(p, color, x, y, outerWidth);
	}
}
...
bool SendActionPainter::updateNeedsAnimating(crl::time now, bool force) {
	...
	if (force || sendActionChanged || speakingChanged) {
		...
		if (typingCount > 2) {
			...
			newTypingString = tr::lng_users_typing(tr::now,
												   lt_user,
			...
			for (const auto &[user, action] : _sendActions) {
				...
				newTypingString = sendActionString(action.type, isNamed ? user->firstName : QString());
				if (!newTypingString.isEmpty()) {
					...
					if (lang.supportChoosingStickerReplacement() && (action.type == Type::ChooseSticker)) {
						const auto index = newTypingString.size() - lang.rightIndexChoosingStickerReplacement(isNamed);
						animationLeft = Ui::Text::String(_st, newTypingString.mid(0, index)).maxWidth();
						...
						if (!_spacesCount) {
							...
							_spacesCount =
								base::SafeRound(_sendActionAnimation.widthNoMargins() / mf.horizontalAdvance(' '));
						}
						newTypingString = newTypingString.replace(index,
			...
			if (newTypingString.isEmpty()) {
				...
				if (playingCount > 2) {
					newTypingString = tr::lng_many_playing_game(tr::now, lt_count, playingCount);
				} else if (playingCount > 1) {
					newTypingString = tr::lng_users_playing_game(tr::now,
																 lt_user,
					...
					newTypingString = _history->peer->isUser()
						...
						: tr::lng_user_playing_game(tr::now, lt_user, begin(_sendActions)->first->firstName);
				}
		...
		if (_sendActionString != newTypingString) {
			...
			_sendActionText.setText(st::dialogsTextStyle, _sendActionString, Ui::NameTextOptions());
		}
	...
	if (force || sendActionChanged || (sendActionResult && !anim::Disabled())) {
		const auto height = std::max(st::normalFont->height, st::dialogsMiniPreviewTop + st::dialogsMiniPreview);
		_history->peer->owner().sendActionManager().updateAnimation({_topic ? ((Data::Thread *) _topic) : _history,
																	 0,
																	 _sendActionAnimation.width() + _animationLeft,
																	 height,
																	 (force || sendActionChanged)});
	}
	if (force || speakingChanged || (speakingResult && !anim::Disabled())) {
		_history->peer->owner().sendActionManager().updateSpeakingAnimation({_history});
	}
