...
[[nodiscard]] bool IsAttachedToPreviousInSavedMessages(not_null<HistoryItem *> previous,
	...
	const auto previousInfo = prevForwarded->savedFromHiddenSenderInfo ? prevForwarded->savedFromHiddenSenderInfo.get()
																	   : prevForwarded->originalHiddenSenderInfo.get();
	const auto itemInfo = forwarded->savedFromHiddenSenderInfo ? forwarded->savedFromHiddenSenderInfo.get()
...
[[nodiscard]] Window::SessionController *ContextOrSessionWindow(const ClickHandlerContext &context,
...
[[nodiscard]] TextSelection FindSearchQueryHighlight(const QString &text, const QString &query) {
	...
	const auto words = QStringView(lower).split(
		QRegularExpression(u"[\\W]"_q, QRegularExpression::UseUnicodePropertiesOption), Qt::SkipEmptyParts);
...
[[nodiscard]] TextSelection ApplyModificationsFrom(TextSelection result, const Ui::Text::String &text) {
	if (result.empty()) {
		return result;
	}
	for (const auto &modification : text.modifications()) {
		...
		const auto shiftTo = std::min(int(modification.skipped), result.to - modification.position);
		result.to -= shiftTo;
		if (modification.position <= result.from) {
			if (modification.added) {
				++result.from;
			}
			const auto shiftFrom = std::min(int(modification.skipped), result.from - modification.position);
...
std::unique_ptr<Ui::PathShiftGradient> MakePathShiftGradient(not_null<const Ui::ChatStyle *> st, Fn<void()> update) {
	return std::make_unique<Ui::PathShiftGradient>(
		st->msgServiceBg(), st->msgServiceBgSelected(), std::move(update), st->paletteChanged());
}

bool DefaultElementDelegate::elementUnderCursor(not_null<const Element *> view) { return false; }

SelectionModeResult DefaultElementDelegate::elementInSelectionMode(const Element *view) { return {}; }

bool DefaultElementDelegate::elementIntersectsRange(not_null<const Element *> view, int from, int till) { return true; }

void DefaultElementDelegate::elementStartStickerLoop(not_null<const Element *> view) {}

void DefaultElementDelegate::elementShowPollResults(not_null<PollData *> poll, FullMsgId context) {}

void DefaultElementDelegate::elementOpenPhoto(not_null<PhotoData *> photo, FullMsgId context) {}

void DefaultElementDelegate::elementOpenDocument(not_null<DocumentData *> document,
												 FullMsgId context,
												 bool showInMediaView) {}

void DefaultElementDelegate::elementCancelUpload(const FullMsgId &context) {}

void DefaultElementDelegate::elementShowTooltip(const TextWithEntities &text, Fn<void()> hiddenCallback) {}

bool DefaultElementDelegate::elementHideReply(not_null<const Element *> view) { return false; }

bool DefaultElementDelegate::elementShownUnread(not_null<const Element *> view) {
	return view->data()->unread(view->data()->history());
}

void DefaultElementDelegate::elementSendBotCommand(const QString &command, const FullMsgId &context) {}

void DefaultElementDelegate::elementSearchInList(const QString &query, const FullMsgId &context) {}

void DefaultElementDelegate::elementHandleViaClick(not_null<UserData *> bot) {}

bool DefaultElementDelegate::elementIsChatWide() { return false; }

void DefaultElementDelegate::elementReplyTo(const FullReplyTo &to) {}

void DefaultElementDelegate::elementStartInteraction(not_null<const Element *> view) {}

void DefaultElementDelegate::elementStartPremium(not_null<const Element *> view, Element *replacing) {}

void DefaultElementDelegate::elementCancelPremium(not_null<const Element *> view) {}

void DefaultElementDelegate::elementStartEffect(not_null<const Element *> view, Element *replacing) {}

QString DefaultElementDelegate::elementAuthorRank(not_null<const Element *> view) { return {}; }

bool DefaultElementDelegate::elementHideTopicButton(not_null<const Element *> view) { return true; }


SimpleElementDelegate::SimpleElementDelegate(not_null<Window::SessionController *> controller, Fn<void()> update)
	: _controller(controller), _pathGradient(MakePathShiftGradient(controller->chatStyle(), std::move(update))) {}
...
auto SimpleElementDelegate::elementPathShiftGradient() -> not_null<Ui::PathShiftGradient *> {
	return _pathGradient.get();
}

TextSelection UnshiftItemSelection(TextSelection selection, uint16 byLength) {
	return (selection == FullSelection) ? selection : ::unshiftSelection(selection, byLength);
}

TextSelection ShiftItemSelection(TextSelection selection, uint16 byLength) {
	return (selection == FullSelection) ? selection : ::shiftSelection(selection, byLength);
}

TextSelection UnshiftItemSelection(TextSelection selection, const Ui::Text::String &byText) {
	return UnshiftItemSelection(selection, byText.length());
}

TextSelection ShiftItemSelection(TextSelection selection, const Ui::Text::String &byText) {
...
QString DateTooltipText(not_null<Element *> view) {
	...
	if (const auto editedDate = view->displayedEditDate()) {
		dateText +=
			'\n' + tr::lng_edited_date(tr::now, lt_date, locale.toString(base::unixtime::parse(editedDate), format));
	}
	if (const auto forwarded = item->Get<HistoryMessageForwarded>()) {
		if (!forwarded->story && forwarded->psaType.isEmpty()) {
			dateText += '\n' +
				tr::lng_forwarded_date(
							tr::now, lt_date, locale.toString(base::unixtime::parse(forwarded->originalDate), format));
...
int UnreadBar::height() { return st::historyUnreadBarHeight + st::historyUnreadBarMargin; }

int UnreadBar::marginTop() { return st::lineWidth + st::historyUnreadBarMargin; }

void UnreadBar::paint(Painter &p, const PaintContext &context, int y, int w, bool chatWide) const {
	...
	y += marginTop();
	p.fillRect(0, y, w, height() - marginTop() - st::lineWidth, st->historyUnreadBarBg());
	p.fillRect(0, bottom - st::lineWidth, w, st::lineWidth, st->historyUnreadBarBorder());
	p.setFont(st::historyUnreadBarFont);
	p.setPen(st->historyUnreadBarFg());

	int maxwidth = w;
	if (chatWide) {
		maxwidth = qMin(maxwidth, st::msgMaxWidth + 2 * st::msgPhotoSkip + 2 * st::msgMargin.left());
	}
	w = maxwidth;

	const auto skip = st::historyUnreadBarHeight - 2 * st::lineWidth - st::historyUnreadBarFont->height;
	p.drawText((w - width) / 2, y + (skip / 2) + st::historyUnreadBarFont->ascent, text);
	if (previousTranslation != 0) {
		p.translate(previousTranslation, 0);
	}
}
...
void DateBadge::paint(Painter &p, not_null<const Ui::ChatStyle *> st, int y, int w, bool chatWide) const {
	ServiceMessagePainter::PaintDate(p, st, text, width, y, w, chatWide);
}

void ServicePreMessage::init(PreparedServiceText string) {
	text = Ui::Text::String(st::serviceTextStyle, string.text, kMarkupTextOptions, st::msgMinWidth);
...
int ServicePreMessage::resizeToWidth(int newWidth, bool chatWide) {
	width = newWidth;
	if (chatWide) {
		accumulate_min(width, st::msgMaxWidth + 2 * st::msgPhotoSkip + 2 * st::msgMargin.left());
	}
	...
	height = (contentWidth >= maxWidth) ? minHeight : text.countHeight(nwidth);
	height += st::msgServicePadding.top() + st::msgServicePadding.bottom() + st::msgServiceMargin.top() +
		st::msgServiceMargin.bottom();
	return height;
}

void ServicePreMessage::paint(Painter &p, const PaintContext &context, QRect g, bool chatWide) const {
	const auto top = g.top() - height - st::msgMargin.top();
	p.translate(0, top);

	const auto rect = QRect(0, 0, width, height) - st::msgServiceMargin;
	const auto trect = rect - st::msgServicePadding;

	ServiceMessagePainter::PaintComplexBubble(p, context.st, rect.left(), rect.width(), text, trect);
...
ClickHandlerPtr ServicePreMessage::textState(QPoint point, const StateRequest &request, QRect g) const {
	const auto top = g.top() - height - st::msgMargin.top();
	const auto rect = QRect(0, top, width, height) - st::msgServiceMargin;
	const auto trect = rect - st::msgServicePadding;
	if (trect.contains(point)) {
		auto textRequest = request.forText();
		textRequest.align = style::al_center;
		return text.getState(point - trect.topLeft(), trect.width(), textRequest).link;
	}
	return {};
}
...
void FakeBotAboutTop::init() {
	...
	text.setText(st::msgNameStyle, tr::lng_bot_description(tr::now), Ui::NameTextOptions());
	maxWidth = st::msgPadding.left() + text.maxWidth() + st::msgPadding.right();
	height = st::msgNameStyle.font->height + st::botDescSkip;
}

Element::Element(not_null<ElementDelegate *> delegate,
				 not_null<HistoryItem *> data,
				 Element *replacing,
				 Flag serviceFlag)
	: _delegate(delegate), _data(data),
	  _dateTime((IsItemScheduledUntilOnline(data) || data->shortcutId()) ? QDateTime() : ItemDateTime(data)),
	  _text(st::msgMinWidth), _flags(serviceFlag | Flag::NeedsResize |
									 (IsItemScheduledUntilOnline(data) ? Flag::ScheduledUntilOnline : Flag()) |
	...
	if (data->isFakeAboutView()) {
		...
		if (user && user->isBot() && !user->isRepliesChat() && !user->isVerifyCodes()) {
			AddComponents(FakeBotAboutTop::Bit());
		}
	}
}

bool Element::embedReactionsInBubble() const { return false; }

not_null<ElementDelegate *> Element::delegate() const { return _delegate; }

not_null<HistoryItem *> Element::data() const { return _data; }

not_null<History *> Element::history() const { return _data->history(); }

uint8 Element::colorIndex() const { return data()->colorIndex(); }

uint8 Element::contentColorIndex() const { return data()->contentColorIndex(); }

QDateTime Element::dateTime() const { return _dateTime; }

Media *Element::media() const { return _media.get(); }

Context Element::context() const { return _context; }

int Element::y() const { return _y; }

void Element::setY(int y) { _y = y; }
...
void Element::prepareCustomEmojiPaint(Painter &p, const PaintContext &context, const Ui::Text::String &text) const {
...
void Element::prepareCustomEmojiPaint(Painter &p,
...
void Element::repaint() const { history()->owner().requestViewRepaint(this); }

void Element::paintHighlight(Painter &p, const PaintContext &context, int geometryHeight) const {
...
void Element::paintCustomHighlight(
	Painter &p, const PaintContext &context, int y, int height, not_null<const HistoryItem *> item) const {
...
bool Element::isUnderCursor() const { return _delegate->elementUnderCursor(this); }
...
bool Element::pendingResize() const { return _flags & Flag::NeedsResize; }

bool Element::isAttachedToPrevious() const { return _flags & Flag::AttachedToPrevious; }

bool Element::isAttachedToNext() const { return _flags & Flag::AttachedToNext; }

bool Element::isBubbleAttachedToPrevious() const { return _flags & Flag::BubbleAttachedToPrevious; }

bool Element::isBubbleAttachedToNext() const { return _flags & Flag::BubbleAttachedToNext; }

bool Element::isTopicRootReply() const { return _flags & Flag::TopicRootReply; }
...
int Element::skipBlockHeight() const { return st::msgDateFont->height - st::msgDateDelta.y(); }

int Element::infoWidth() const { return 0; }

int Element::bottomInfoFirstLineWidth() const { return 0; }

bool Element::bottomInfoIsWide() const { return false; }

bool Element::isHiddenByGroup() const { return _flags & Flag::HiddenByGroup; }

bool Element::isHidden() const { return isMessageHidden(data()) || isHiddenByGroup(); }
...
int Element::AdditionalSpaceForSelectionCheckbox(not_null<const Element *> view, QRect countedGeometry) {
	...
	const auto diff = view->width() - (countedGeometry.x() + countedGeometry.width()) - st::msgPadding.right() -
		st::msgSelectionOffset - view->rightActionSize().value_or(QSize()).width();
	return (diff < 0) ? -(std::min(st::msgSelectionOffset, -diff)) : 0;
}
...
void Element::refreshMedia(Element *replacing) {
	...
	if (const auto media = item->media()) {
		if (media->canBeGrouped()) {
			if (const auto group = history()->owner().groups().find(item)) {
				if (group->items.front() != item) {
					...
					_media = std::make_unique<GroupedMedia>(this, group->items);
					if (!pendingResize()) {
						history()->owner().requestViewResize(this);
					}
				}
	...
	} else if (isOnlyCustomEmoji() && Core::App().settings().largeEmoji() && !item->isSponsored()) {
		_media = std::make_unique<UnwrappedMedia>(this, std::make_unique<CustomEmoji>(this, onlyCustomEmoji()));
	} else if (isIsolatedEmoji() && Core::App().settings().largeEmoji() && !item->isSponsored()) {
		...
		if (const auto sticker = emojiStickers->stickerForEmoji(emoji)) {
			auto content =
				std::make_unique<Sticker>(this, sticker.document, skipPremiumEffect, replacing, sticker.replacements);
			content->setEmojiSticker();
			_media = std::make_unique<UnwrappedMedia>(this, std::move(content));
		} else {
			_media = std::make_unique<UnwrappedMedia>(this, std::make_unique<LargeEmoji>(this, emoji));
		}
	} else {
		_media = nullptr;
	}
...
HistoryItem *Element::textItem() const { return _textItem; }

Ui::Text::IsolatedEmoji Element::isolatedEmoji() const { return _text.toIsolatedEmoji(); }

Ui::Text::OnlyCustomEmoji Element::onlyCustomEmoji() const { return _text.toOnlyCustomEmoji(); }

const Ui::Text::String &Element::text() const { return _text; }
...
OnlyEmojiAndSpaces Element::isOnlyEmojiAndSpaces() const {
	if (data()->Has<HistoryMessageTranslation>()) {
		...
		return _text.hasNotEmojiAndSpaces() ? OnlyEmojiAndSpaces::No : OnlyEmojiAndSpaces::Yes;
...
auto Element::contextDependentServiceText() -> TextWithLinks {
	...
	const auto topicUrl = u"internal:url:https://t.me/c/%1/%2"_q.arg(peerToChannel(peerId).bare).arg(topicRootId.bare);
	const auto fromLink = [&](int index) { return Ui::Text::Link(from->name(), index); };
	const auto placeholderLink = [&] { return Ui::Text::Link(tr::lng_action_topic_placeholder(tr::now), topicUrl); };
	const auto wrapTopic = [&](const QString &title, std::optional<DocumentId> iconId)
	{ return Ui::Text::Link(Data::ForumTopicIconWithTitle(topicRootId, iconId.value_or(0), title), topicUrl); };
	const auto wrapParentTopic = [&]
	{
		const auto forum = history()->asForum();
		if (!forum || forum->topicDeleted(topicRootId)) {
			return wrapTopic(tr::lng_deleted_message(tr::now), std::nullopt);
			...
			return wrapTopic(tr::lng_profile_loading(tr::now), std::nullopt);
		}
	};

	if (info->closed) {
		return {
			tr::lng_action_topic_closed(tr::now, lt_topic, wrapParentTopic(), Ui::Text::WithEntities),
		};
	} else if (info->reopened) {
		return {
			tr::lng_action_topic_reopened(tr::now, lt_topic, wrapParentTopic(), Ui::Text::WithEntities),
		};
	} else if (info->hidden) {
		return {
			tr::lng_action_topic_hidden(tr::now, lt_topic, wrapParentTopic(), Ui::Text::WithEntities),
		};
	} else if (info->unhidden) {
		return {
			tr::lng_action_topic_unhidden(tr::now, lt_topic, wrapParentTopic(), Ui::Text::WithEntities),
		};
		...
		return {
			tr::lng_action_topic_renamed(
				...
				placeholderLink(),
				lt_title,
				wrapTopic(info->title, (info->reiconed ? info->iconId : std::optional<DocumentId>())),
		...
		if (const auto iconId = info->iconId) {
			return {
				tr::lng_action_topic_icon_changed(tr::now,
												  lt_from,
												  fromLink(1),
												  lt_link,
												  placeholderLink(),
			...
			return {
				tr::lng_action_topic_icon_removed(
					tr::now, lt_from, fromLink(1), lt_link, placeholderLink(), Ui::Text::WithEntities),
				{from->createOpenLink()},
			};
		}
	} else {
...
void Element::validateText() {
	...
	if (media && media->storyExpired()) {
		...
		if (!storyMention) {
			if (_text.isEmpty()) {
				setTextWithLinks(Ui::Text::Italic(tr::lng_forwarded_story_expired(tr::now)));
			}
			return;
		}
	}
	...
	if (_text.isEmpty() == text.empty()) {
		...
		const auto &markedText = contextDependentText.text.empty() ? text : contextDependentText.text;
		const auto &customLinks =
			contextDependentText.text.empty() ? _textItem->customTextLinks() : contextDependentText.links;
...
void Element::setTextWithLinks(const TextWithEntities &text, const std::vector<ClickHandlerPtr> &links) {
	...
	if (_flags & Flag::ServiceMessage) {
		...
		if (!item->_text.empty() && _text.isEmpty()) {
			...
			_text.setMarkedText(st::messageTextStyle, {u":-("_q}, Ui::ItemTextOptions(item));
		}
		if (!item->media()) {
...
void Element::nextInBlocksRemoved() { setAttachToNext(false); }

bool Element::markSponsoredViewed(int shownFromTop) const {
	const auto sponsoredTextTop = height() - st::msgPadding.bottom() - st::historyViewButtonHeight;
...
bool Element::computeIsAttachToPrevious(not_null<Element *> previous) {
	...
	{
		const auto item = view->data();
		return !item->isService() && !item->isEmpty() && !item->isPostHidingAuthor() &&
			(!item->history()->peer->isMegagroup() || !view->hasOutLayout() || !item->from()->isChannel());
	};
	const auto item = data();
	if (!Has<DateBadge>() && !Has<UnreadBar>() && !Has<ServicePreMessage>()) {
		const auto prev = previous->data();
		const auto previousMarkup = prev->inlineReplyMarkup();
		const auto possible = (std::abs(prev->date() - item->date()) < kAttachMessageToPreviousSecondsDelta) &&
			mayBeAttached(this) && mayBeAttached(previous) &&
		...
		if (possible) {
			...
			if (peer->isSelf() || peer->isRepliesChat() || peer->isVerifyCodes() ||
				(forwarded && forwarded->imported) || (prevForwarded && prevForwarded->imported)) {
				return IsAttachedToPreviousInSavedMessages(prev, prevForwarded, item, forwarded);
			} else {
...
bool Element::displayDate() const { return Has<DateBadge>(); }

bool Element::isInOneDayWithPrevious() const { return !data()->isEmpty() && !displayDate(); }
	...
	setDisplayDate(
		[&]
		{
			...
			if (const auto previous = previousDisplayedInBlocks()) {
				const auto prev = previous->data();
				return prev->isEmpty() || (previous->dateTime().date() != dateTime().date());
			}
			return true;
		}());
}
...
void Element::setDisplayDate(bool displayDate) {
	...
	if (displayDate && !Has<DateBadge>()) {
		...
		Get<DateBadge>()->init(ItemDateText(item, (_flags & Flag::ScheduledUntilOnline)));
		setPendingResize();
...
bool Element::displayFromPhoto() const { return false; }

bool Element::hasFromPhoto() const { return false; }

bool Element::hasFromName() const { return false; }

bool Element::displayReply() const { return Has<Reply>(); }

bool Element::displayFromName() const { return false; }

TopicButton *Element::displayedTopicButton() const { return nullptr; }

bool Element::displayForwardedFrom() const { return false; }

bool Element::hasOutLayout() const { return false; }

bool Element::hasRightLayout() const { return hasOutLayout() && !_delegate->elementIsChatWide(); }

bool Element::drawBubble() const { return false; }

bool Element::hasBubble() const { return false; }

bool Element::unwrapped() const { return true; }

bool Element::hasFastReply() const { return false; }

bool Element::displayFastReply() const { return false; }

std::optional<QSize> Element::rightActionSize() const { return std::nullopt; }

void Element::drawRightAction(Painter &p, const PaintContext &context, int left, int top, int outerWidth) const {}

ClickHandlerPtr Element::rightActionLink(std::optional<QPoint> pressPoint) const { return ClickHandlerPtr(); }

TimeId Element::displayedEditDate() const { return TimeId(0); }

bool Element::toggleSelectionByHandlerClick(const ClickHandlerPtr &handler) const { return false; }

bool Element::allowTextSelectionByHandler(const ClickHandlerPtr &handler) const { return false; }

bool Element::usesBubblePattern(const PaintContext &context) const {
	return (context.selection != FullSelection) && hasOutLayout() && context.bubblesPattern &&
		!context.viewport.isEmpty() && !context.bubblesPattern->pixmap.size().isEmpty();
}

bool Element::hasVisibleText() const { return false; }

int Element::textualMaxWidth() const {
	return st::msgPadding.left() + (hasVisibleText() ? text().maxWidth() : 0) + st::msgPadding.right();
}

auto Element::verticalRepaintRange() const -> VerticalRepaintRange { return {.top = 0, .height = height()}; }

bool Element::hasHeavyPart() const { return (_flags & Flag::HeavyCustomEmoji); }
...
bool Element::isSignedAuthorElided() const { return false; }

void Element::setupReactions(Element *replacing) {
	refreshReactions();
	auto animations = replacing ? replacing->takeReactionAnimations()
								: base::flat_map<Data::ReactionId, std::unique_ptr<Ui::ReactionFlyAnimation>>();
	if (!animations.empty()) {
...
void Element::refreshReactions() {
	...
	if (!_reactions) {
		const auto handlerFactory = [=](ReactionId id)
		{
			...
			return std::make_shared<LambdaClickHandler>(
				[=](ClickContext context)
				{
					...
					if (id.paid()) {
						Payments::TryAddingPaidReaction(item, weak.get(), 1, std::nullopt, controller->uiShow());
						return;
					} else {
		...
		setReactions(std::make_unique<InlineList>(
			&history()->owner().reactions(), handlerFactory, [=] { customEmojiRepaint(); }, std::move(reactionsData)));
	} else {
...
void Element::setReactions(std::unique_ptr<Reactions::InlineList> list) {
	auto was = _reactions ? _reactions->computeTagsList() : std::vector<Data::ReactionId>();
	_reactions = std::move(list);
	auto now = _reactions ? _reactions->computeTagsList() : std::vector<Data::ReactionId>();
	if (!was.empty() || !now.empty()) {
		auto &owner = history()->owner();
		owner.viewTagsChanged(this, std::move(was), std::move(now));
	}
...
bool Element::updateReactions() {
	const auto wasReactions = _reactions ? _reactions->currentSize() : QSize();
	refreshReactions();
	const auto nowReactions = _reactions ? _reactions->currentSize() : QSize();
...
HistoryBlock *Element::block() { return _block; }

const HistoryBlock *Element::block() const { return _block; }
...
TextState Element::bottomInfoTextState(int right, int bottom, QPoint point, InfoDisplayType type) const {
	return TextState();
}

TextSelection Element::adjustSelection(TextSelection selection, TextSelectType type) const { return selection; }

SelectedQuote
Element::FindSelectedQuote(const Ui::Text::String &text, TextSelection selection, not_null<HistoryItem *> item) {
	...
	for (const auto &modification : text.modifications()) {
		if (modification.position >= selection.to) {
			...
			if (modification.added && modification.position < selection.from) {
				--modified.from;
			}
		}
	...
	result.text = result.text.mid(modified.from, modified.to - modified.from);
	...
	for (auto i = result.entities.begin(); i != result.entities.end();) {
		...
		if ((till <= modified.from) || (offset >= modified.to) || !ranges::contains(allowed, i->type())) {
...
TextSelection Element::FindSelectionFromQuote(const Ui::Text::String &text, const SelectedQuote &quote) {
	...
	if (quote.offset == kSearchQueryOffsetHint) {
		return ApplyModificationsFrom(FindSearchQueryHighlight(original.text, quote.text.text), text);
	}
	...
	const auto findTwoWays = [&](int offset)
	{
		...
		return before.empty() ? after : (offset - before.from < after.from - offset) ? before : after;
	};
...
Reactions::ButtonParameters Element::reactionButtonParameters(QPoint position, const TextState &reactionState) const {
	return {};
}

int Element::reactionsOptimalWidth() const { return 0; }

void Element::clickHandlerActiveChanged(const ClickHandlerPtr &handler, bool active) {
...
void Element::clickHandlerPressedChanged(const ClickHandlerPtr &handler, bool pressed) {
...
auto Element::takeReactionAnimations() -> base::flat_map<Data::ReactionId, std::unique_ptr<Ui::ReactionFlyAnimation>> {
	if (_reactions) {
...
auto Element::takeEffectAnimation() -> std::unique_ptr<Ui::ReactionFlyAnimation> { return nullptr; }

QRect Element::effectIconGeometry() const { return QRect(); }
...
void Element::Hovered(Element *view) { HoveredElement = view; }

Element *Element::Hovered() { return HoveredElement; }

void Element::Pressed(Element *view) { PressedElement = view; }

Element *Element::Pressed() { return PressedElement; }

void Element::HoveredLink(Element *view) { HoveredLinkElement = view; }

Element *Element::HoveredLink() { return HoveredLinkElement; }

void Element::PressedLink(Element *view) { PressedLinkElement = view; }

Element *Element::PressedLink() { return PressedLinkElement; }

void Element::Moused(Element *view) { MousedElement = view; }

Element *Element::Moused() { return MousedElement; }
...
int FindViewY(not_null<Element *> view, uint16 symbol, int yfrom) {
	...
	if (origin.y() < top || origin.y() + inner.height() > bottom || inner.height() <= 0) {
		return yfrom;
	}
	const auto fory = [&](int y) { return view->textState(origin + QPoint(0, y), request).symbol; };
	...
	while (ytill - yfrom >= 2 * single) {
		...
		if (found == symbol || symbolfrom > found || symboltill < found) {
			return middle;
