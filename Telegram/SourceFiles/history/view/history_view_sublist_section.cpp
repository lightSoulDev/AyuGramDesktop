...
SublistMemento::SublistMemento(not_null<Data::SavedSublist *> sublist) : _sublist(sublist) {
...
object_ptr<Window::SectionWidget> SublistMemento::createWidget(QWidget *parent,
	...
	auto result = object_ptr<SublistWidget>(parent, controller, _sublist);
	result->setInternalState(geometry, this);
	return result;
}

SublistWidget::SublistWidget(QWidget *parent,
							 not_null<Window::SessionController *> controller,
							 not_null<Data::SavedSublist *> sublist)
	: Window::SectionWidget(parent, controller, sublist->peer()), WindowListDelegate(controller), _sublist(sublist),
	  _history(sublist->owner().history(sublist->session().user())), _topBar(this, controller), _topBarShadow(this),
	...
	Window::ChatThemeValueFromPeer(controller, sublist->peer()) |
		rpl::start_with_next(
	...
	_topBar->searchRequest() | rpl::start_with_next([=] { searchInSublist(); }, _topBar->lifetime());
...
void SublistWidget::setupOpenChatButton() {
	...
	_openChatButton =
		std::make_unique<Ui::FlatButton>(this,
										 (_sublist->peer()->isBroadcast()  ? tr::lng_saved_open_channel(tr::now)
											  : _sublist->peer()->isUser() ? tr::lng_saved_open_chat(tr::now)
																		   : tr::lng_saved_open_group(tr::now)),
										 st::historyComposeButton);

	_openChatButton->setClickedCallback(
		[=] { controller()->showPeerHistory(_sublist->peer(), Window::SectionShow::Way::Forward); });
}
...
void SublistWidget::setupAboutHiddenAuthor() {
	...
	_aboutHiddenAuthor->paintRequest() |
		rpl::start_with_next(
			[=]
			{
...
void SublistWidget::cornerButtonsShowAtPosition(Data::MessagePosition position) { showAtPosition(position); }

Data::Thread *SublistWidget::cornerButtonsThread() { return nullptr; }

FullMsgId SublistWidget::cornerButtonsCurrentId() { return _lastShownAt; }

bool SublistWidget::cornerButtonsIgnoreVisibility() { return animatingShow(); }
...
bool SublistWidget::cornerButtonsUnreadMayBeShown() { return _inner->loadedAtBottomKnown(); }

bool SublistWidget::cornerButtonsHas(CornerButtonType type) { return (type == CornerButtonType::Down); }

void SublistWidget::showAtPosition(Data::MessagePosition position, FullMsgId originId) {
	showAtPosition(position, originId, {});
}

void SublistWidget::showAtPosition(Data::MessagePosition position,
								   FullMsgId originItemId,
								   const Window::SectionShow &params) {
	_lastShownAt = position.fullId;
	controller()->setActiveChatEntry(activeChat());
	_inner->showAtPosition(position, params, _cornerButtons.doneJumpFrom(position.fullId, originItemId));
}
...
not_null<Data::SavedSublist *> SublistWidget::sublist() const { return _sublist; }
...
void SublistWidget::checkActivation() { _inner->checkActivation(); }
...
bool SublistWidget::showInternal(not_null<Window::SectionMemento *> memento, const Window::SectionShow &params) {
...
void SublistWidget::setInternalState(const QRect &geometry, not_null<SublistMemento *> memento) {
	setGeometry(geometry);
	Ui::SendPendingMoveResizeEvents(this);
	restoreState(memento);
}

bool SublistWidget::searchInChatEmbedded(QString query, Dialogs::Key chat, PeerData *searchFrom) {
	...
	_composeSearch = std::make_unique<ComposeSearch>(this, controller(), _history, sublist->peer(), query);
	...
	_composeSearch->activations() |
		rpl::start_with_next(
			[=](ComposeSearch::Activation activation)
			{
...
bool SublistWidget::showMessage(PeerId peerId, const Window::SectionShow &params, MsgId messageId) {
...
void SublistWidget::saveState(not_null<SublistMemento *> memento) { _inner->saveState(memento->list()); }

void SublistWidget::restoreState(not_null<SublistMemento *> memento) { _inner->restoreState(memento->list()); }
...
void SublistWidget::showAnimatedHook(const Window::SectionSlideParams &params) {
...
bool SublistWidget::floatPlayerHandleWheelEvent(QEvent *e) { return _scroll->viewportEvent(e); }

QRect SublistWidget::floatPlayerAvailableRect() { return mapToGlobal(_scroll->geometry()); }

Context SublistWidget::listContext() { return Context::SavedSublist; }
...
void SublistWidget::listDeleteRequest() { confirmDeleteSelected(); }

void SublistWidget::listTryProcessKeyInput(not_null<QKeyEvent *> e) {}

rpl::producer<Data::MessagesSlice>
SublistWidget::listSource(Data::MessagePosition aroundId, int limitBefore, int limitAfter) {
	...
	return [=](auto consumer)
	{
		const auto pushSlice = [=]
		{
			...
			_topBar->setCustomTitle(result.fullCount
										? tr::lng_forum_messages(tr::now, lt_count_decimal, *result.fullCount)
										: tr::lng_contacts_loading(tr::now));
			const auto &messages = _sublist->messages();
			const auto i = ranges::lower_bound(
				messages, messageId, ranges::greater(), [](not_null<HistoryItem *> item) { return item->id; });
			...
			result.skippedBefore = result.fullCount ? (*result.fullCount - after - useBefore) : std::optional<int>();
...
bool SublistWidget::listAllowsMultiSelect() { return true; }

bool SublistWidget::listIsItemGoodForSelection(not_null<HistoryItem *> item) {
	return item->isRegular() && !item->isService();
}

bool SublistWidget::listIsLessInOrder(not_null<HistoryItem *> first, not_null<HistoryItem *> second) {
...
void SublistWidget::listMarkContentsRead(const base::flat_set<not_null<HistoryItem *>> &items) {}

MessagesBarData SublistWidget::listMessagesBar(const std::vector<not_null<Element *>> &elements) { return {}; }

void SublistWidget::listContentRefreshed() {}

void SublistWidget::listUpdateDateLink(ClickHandlerPtr &link, not_null<Element *> view) {}

bool SublistWidget::listElementHideReply(not_null<const Element *> view) { return false; }

bool SublistWidget::listElementShownUnread(not_null<const Element *> view) {
	return view->data()->unread(view->data()->history());
}

bool SublistWidget::listIsGoodForAroundPosition(not_null<const Element *> view) { return view->data()->isRegular(); }

void SublistWidget::listSendBotCommand(const QString &command, const FullMsgId &context) {}

void SublistWidget::listSearch(const QString &query, const FullMsgId &context) {
	const auto inChat = Data::SearchTagFromQuery(query) ? Dialogs::Key(_sublist) : Dialogs::Key();
	controller()->searchMessages(query, inChat);
}

void SublistWidget::listHandleViaClick(not_null<UserData *> bot) {}

not_null<Ui::ChatTheme *> SublistWidget::listChatTheme() { return _theme.get(); }

CopyRestrictionType SublistWidget::listCopyRestrictionType(HistoryItem *item) {
	return CopyRestrictionTypeFor(_history->peer, item);
}

CopyRestrictionType SublistWidget::listCopyMediaRestrictionType(not_null<HistoryItem *> item) {
	return CopyMediaRestrictionTypeFor(_history->peer, item);
}

CopyRestrictionType SublistWidget::listSelectRestrictionType() { return SelectRestrictionTypeFor(_history->peer); }

auto SublistWidget::listAllowedReactionsValue() -> rpl::producer<Data::AllowedReactions> {
...
void SublistWidget::listOpenPhoto(not_null<PhotoData *> photo, FullMsgId context) {
	controller()->openPhoto(photo, {context});
}

void SublistWidget::listOpenDocument(not_null<DocumentData *> document, FullMsgId context, bool showInMediaView) {
	controller()->openDocument(document, showInMediaView, {context});
}

void SublistWidget::listPaintEmpty(Painter &p, const Ui::ChatPaintContext &context) {}

QString SublistWidget::listElementAuthorRank(not_null<const Element *> view) { return {}; }

bool SublistWidget::listElementHideTopicButton(not_null<const Element *> view) { return true; }

History *SublistWidget::listTranslateHistory() { return _history; }

void SublistWidget::listAddTranslatedItems(not_null<TranslateTracker *> tracker) {}

void SublistWidget::confirmDeleteSelected() { ConfirmDeleteSelectedItems(_inner); }

void SublistWidget::confirmForwardSelected() { ConfirmForwardSelectedItems(_inner); }

void SublistWidget::clearSelected() { _inner->cancelSelection(); }
...
void SublistWidget::searchInSublist() { controller()->searchInChat(_sublist); }
