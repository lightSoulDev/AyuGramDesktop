...
constexpr auto kPreloadedScreensCountFull = kPreloadedScreensCount + 1 + kPreloadedScreensCount;
constexpr auto kClearUserpicsAfter = 50;

[[nodiscard]] std::unique_ptr<TranslateTracker> MaybeTranslateTracker(History *history) {
...
WindowListDelegate::WindowListDelegate(not_null<Window::SessionController *> window) : _window(window) {}

not_null<Window::SessionController *> WindowListDelegate::listWindow() { return _window; }

not_null<QWidget *> WindowListDelegate::listEmojiInteractionsParent() { return _window->content(); }

not_null<const Ui::ChatStyle *> WindowListDelegate::listChatStyle() { return _window->chatStyle(); }

rpl::producer<bool> WindowListDelegate::listChatWideValue() { return _window->adaptive().chatWideValue(); }

auto WindowListDelegate::listMakeReactionsManager(QWidget *wheelEventsTarget, Fn<void(QRect)> update)
	-> std::unique_ptr<Reactions::Manager> {
	return std::make_unique<Reactions::Manager>(wheelEventsTarget, std::move(update));
}

void WindowListDelegate::listVisibleAreaUpdated() { _window->floatPlayerAreaUpdated(); }

std::shared_ptr<Ui::Show> WindowListDelegate::listUiShow() { return _window->uiShow(); }

void WindowListDelegate::listShowPollResults(not_null<PollData *> poll, FullMsgId context) {
	_window->showPollResults(poll, context);
}

void WindowListDelegate::listCancelUploadLayer(not_null<HistoryItem *> item) { _window->cancelUploadLayer(item); }

bool WindowListDelegate::listAnimationsPaused() { return _window->isGifPausedAtLeastFor(Window::GifPauseReason::Any); }

auto WindowListDelegate::listSendingAnimation() -> Ui::MessageSendingAnimationController * {
	return &_window->sendingAnimation();
}

Ui::ChatPaintContext WindowListDelegate::listPreparePaintContext(Ui::ChatPaintContextArgs &&args) {
	return _window->preparePaintContext(std::move(args));
}

bool WindowListDelegate::listMarkingContentRead() { return _window->widget()->markingAsRead(); }

bool WindowListDelegate::listIgnorePaintEvent(QWidget *w, QPaintEvent *e) { return _window->contentOverlapped(w, e); }

bool WindowListDelegate::listShowReactPremiumError(not_null<HistoryItem *> item, const Data::ReactionId &id) {
	return Window::ShowReactPremiumError(_window, item, id);
}

auto WindowListDelegate::listFillSenderUserpicMenu(PeerId userpicPeerId) -> base::unique_qptr<Ui::PopupMenu> {
	return nullptr;
}

void WindowListDelegate::listWindowSetInnerFocus() { _window->widget()->setInnerFocus(); }

bool WindowListDelegate::listAllowsDragForward() { return _window->adaptive().isOneColumn(); }

void WindowListDelegate::listLaunchDrag(std::unique_ptr<QMimeData> data, Fn<void()> finished) {
	_window->widget()->launchDrag(std::move(data), std::move(finished));
}

ListWidget::MouseState::MouseState() : pointState(PointState::Outside) {}

ListWidget::MouseState::MouseState(FullMsgId itemId, int height, QPoint point, PointState pointState)
	: itemId(itemId), height(height), point(point), pointState(pointState) {}
...
ListWidget::ListWidget(QWidget *parent, not_null<Main::Session *> session, not_null<ListDelegate *> delegate)
	: RpWidget(parent), _delegate(delegate), _session(session),
	  _emojiInteractions(std::make_unique<EmojiInteractions>(this,
															 _delegate->listEmojiInteractionsParent(),
															 session,
															 [=](not_null<const Element *> view)
															 { return itemTop(view); })),
	  _context(_delegate->listContext()), _itemAverageHeight(itemMinimalHeight()),
	  _pathGradient(MakePathShiftGradient(_delegate->listChatStyle(), [=] { update(); })),
	  _reactionsManager(_delegate->listMakeReactionsManager(this, [=](QRect updated) { update(updated); })),
	  _translateTracker(MaybeTranslateTracker(_delegate->listTranslateHistory())),
	  _scrollDateCheck([this] { scrollDateCheck(); }), _applyUpdatedScrollState([this] { applyUpdatedScrollState(); }),
	...
	_session->downloaderTaskFinished() | rpl::start_with_next([=] { update(); }, lifetime());

	_session->data().peerDecorationsUpdated() | rpl::start_with_next([=] { update(); }, lifetime());

	_session->data().itemRemoved() |
		rpl::start_with_next([=](not_null<const HistoryItem *> item) { itemRemoved(item); }, lifetime());

	using MessageUpdateFlag = Data::MessageUpdate::Flag;
	_session->changes().realtimeMessageUpdates(MessageUpdateFlag::NewUnreadReaction) |
	...
	if (const auto history = _delegate->listTranslateHistory()) {
		_session->changes().historyUpdates(history, Data::HistoryUpdate::Flag::TranslatedTo) |
	...
	if (_reactionsManager) {
		...
		Reactions::SetupManagerList(_reactionsManager.get(), _reactionsItem.value());
	...
	_delegate->listChatWideValue() | rpl::start_with_next([=](bool wide) { _isChatWide = wide; }, lifetime());

	_selectScroll.scrolls() |
		rpl::start_with_next([=](int d) { delegate->listScrollTo(_visibleTop + d, false); }, lifetime());
}

Main::Session &ListWidget::session() const { return *_session; }

not_null<Window::SessionController *> ListWidget::controller() const { return _delegate->listWindow(); }

not_null<ListDelegate *> ListWidget::delegate() const { return _delegate; }

void ListWidget::refreshViewer() {
	_viewerLifetime.destroy();
	_refreshingViewer = true;
	_delegate->listSource(_aroundPosition, _idsLimit, _idsLimit) |
		rpl::start_with_next(
...
void ListWidget::setGeometryCrashAnnotations(not_null<Element *> view) {
	CrashReports::SetAnnotation("Geometry",
								u"size: %1x%2, visibleTop: %3, visibleBottom: %4, top: %5"_q.arg(width())
									.arg(height())
									.arg(_visibleTop)
									.arg(_visibleBottom)
									.arg(_itemsTop));
	const auto logItems = [&]
	{
		...
		for (const auto &some : _items) {
			items.push_back(
				u"(%1)%2=%3,%4,%5"_q.arg(index++).arg(top).arg(itemTop(some)).arg(some->y()).arg(some->height()));
			top += some->height();
		}
		return items.join(';');
	};
	CrashReports::SetAnnotation("Chosen", u"%1,%2,%3"_q.arg(itemTop(view)).arg(view->y()).arg(view->height()));
...
void ListWidget::refreshRows(const Data::MessagesSlice &old) {
	...
	const auto addedToEndFrom = (old.skippedAfter == 0 && (_slice.skippedAfter == 0) && !old.ids.empty())
		? ranges::find(_slice.ids, old.ids.back())
		: end(_slice.ids);
	const auto addedToEndCount = std::max(int(end(_slice.ids) - addedToEndFrom), 1) - 1;
...
std::optional<int> ListWidget::scrollTopForPosition(Data::MessagePosition position) const {
	...
	if (_visibleTop >= _visibleBottom) {
		...
		return std::nullopt;
	} else if (!_items.empty() && (_aroundPosition == position || _initialAroundPosition == position) &&
			   messageUnknown) {
	...
	} else if (_items.empty() || isBelowPosition(position) || isAbovePosition(position)) {
		return std::nullopt;
	}
...
std::optional<int> ListWidget::scrollTopForView(not_null<Element *> view) const {
	...
	} else if (const auto sel = _highlighter.state(view->data()).range; !sel.empty() && !IsSubGroupSelection(sel)) {
...
void ListWidget::scrollTo(int scrollTop, Data::MessagePosition attachPosition, int delta, AnimatedScroll type) {
	_scrollToAnimation.stop();
	if (!delta || _items.empty() || type == AnimatedScroll::None) {
		_delegate->listScrollTo(scrollTop);
		return;
	}
	const auto transition = (type == AnimatedScroll::Full) ? anim::sineInOut : anim::easeOutCubic;
	if (delta > 0 && scrollTop == height() - (_visibleBottom - _visibleTop)) {
		// Animated scroll to bottom.
		_scrollToAnimation.start(
			[=] { scrollToAnimationCallback(FullMsgId(), 0); }, -delta, 0, st::slideDuration, transition);
		return;
	}
	...
	_scrollToAnimation.start([=] { scrollToAnimationCallback(attachToId, relativeFinish); },
							 relativeStart,
							 relativeFinish,
							 st::slideDuration,
							 transition);
...
bool ListWidget::animatedScrolling() const { return _scrollToAnimation.animating(); }

void ListWidget::scrollToAnimationCallback(FullMsgId attachToId, int relativeTo) {
	if (!attachToId) {
		// Animated scroll to bottom.
		const auto current = int(base::SafeRound(_scrollToAnimation.value(0)));
		_delegate->listScrollTo(height() - (_visibleBottom - _visibleTop) + current);
		return;
	}
	...
	if (!attachToView) {
		...
		const auto current = int(base::SafeRound(_scrollToAnimation.value(relativeTo)));
...
void ListWidget::highlightMessage(FullMsgId itemId, const TextWithEntities &part, int partOffsetHint) {
	if (const auto view = viewForItem(itemId)) {
		_highlighter.highlight({view->data(), part, partOffsetHint});
	}
}

void ListWidget::showAroundPosition(Data::MessagePosition position, Fn<bool()> overrideInitialScroll) {
...
bool ListWidget::jumpToBottomInsteadOfUnread() const {
	...
	const auto firstReadMessage = [&]() -> Element *
	{
		for (const auto &view : ranges::views::reverse(_items)) {
			const auto item = view->data();
			if (item->isRegular() && (item->out() || !_delegate->listElementShownUnread(view))) {
				return view;
			}
		}
		return nullptr;
...
void ListWidget::showAtPosition(Data::MessagePosition position,
	...
	if (showAtUnread) {
		...
		showAroundPosition(position, [=] { return showAtPositionNow(position, params, done); });
	}
}

bool ListWidget::showAtPositionNow(Data::MessagePosition position,
								   const Window::SectionShow &params,
								   Fn<void(bool found)> done) {
	...
	if (position != Data::MaxMessagePosition && position != Data::UnreadMessagePosition) {
		const auto hasHighlight = !params.highlightPart.empty();
		highlightMessage(position.fullId, params.highlightPart, params.highlightPartOffsetHint);
		if (hasHighlight) {
	...
	if (done) {
		const auto found = !position.fullId.peer || !IsServerMsgId(position.fullId.msg) || viewForItem(position.fullId);
		done(found);
	}
	return true;
}

void ListWidget::computeScrollTo(int to, Data::MessagePosition position, anim::type animated) {
	const auto currentScrollHeight = (_visibleBottom - _visibleTop);
	const auto currentScrollTop = _visibleTop;
	const auto wanted = std::max(std::min(to, height() - currentScrollHeight), 0);
	...
	const auto type = (animated == anim::type::instant) ? AnimatedScroll::None
		: (std::abs(fullDelta) > limit)					? AnimatedScroll::Part
														: AnimatedScroll::Full;
...
void ListWidget::restoreScrollState() {
	if (_items.empty()) {
		return;
	} else if (_overrideInitialScroll && base::take(_overrideInitialScroll)()) {
	...
	if (!_scrollTopState.item) {
		...
		_scrollTopState.shift = st::lineWidth + st::historyUnreadBarMargin + _bar.element->displayedDateHeight();
	}
...
not_null<Element *> ListWidget::enforceViewForItem(not_null<HistoryItem *> item, ViewsMap &old) {
	if (const auto i = old.find(item); i != end(old)) {
		if (i->second) {
			return _views.emplace(item, base::take(i->second)).first->second.get();
		} else if (const auto j = _views.find(item); j != end(_views)) {
			return j->second.get();
		}
	}
	const auto &[i, ok] = _views.emplace(item, item->createView(this));
	return i->second.get();
}
...
Element *ListWidget::viewByPosition(Data::MessagePosition position) const {
	...
	return (position == Data::MinMessagePosition || position == Data::MaxMessagePosition ||
...
int ListWidget::findNearestItem(Data::MessagePosition position) const {
	...
	const auto after =
		ranges::find_if(_items, [&](not_null<Element *> view) { return (view->data()->position() >= position); });
	return (after == end(_items)) ? int(_items.size() - 1) : int(after - begin(_items));
}

HistoryItemsList ListWidget::collectVisibleItems() const {
	auto result = HistoryItemsList();
	const auto from =
		std::lower_bound(begin(_items),
						 end(_items),
						 _visibleTop,
						 [this](auto &elem, int top) { return this->itemTop(elem) + elem->height() <= top; });
	const auto to = std::lower_bound(begin(_items),
									 end(_items),
									 _visibleBottom,
...
void ListWidget::visibleTopBottomUpdated(int visibleTop, int visibleBottom) {
...
void ListWidget::applyUpdatedScrollState() { checkMoveToOtherViewer(); }
...
bool ListWidget::displayScrollDate() const { return (_visibleTop <= height() - 2 * (_visibleBottom - _visibleTop)); }
...
auto ListWidget::collectSelectedItems() const -> SelectedItems {
	...
	if (hasSelectedItems()) {
		...
		std::transform(_selected.begin(), _selected.end(), std::back_inserter(items), transformation);
	}
	return items;
}
...
MessageIdsList ListWidget::collectSelectedIds() const {
	...
	return ranges::views::all(selected) |
		ranges::views::transform([](const SelectedItem &item) { return item.msgId; }) | ranges::to_vector;
}

void ListWidget::pushSelectedItems() { _delegate->listSelectionChanged(collectSelectedItems()); }

void ListWidget::removeItemSelection(const SelectedMap::const_iterator &i) {
...
bool ListWidget::hasSelectedText() const { return (_selectedTextItem != nullptr) && !hasSelectedItems(); }

bool ListWidget::hasSelectedItems() const { return !_selected.empty(); }
...
bool ListWidget::overSelectedItems() const {
	if (_overState.pointState == PointState::GroupPart) {
		return _overItemExact && _selected.contains(_overItemExact->fullId());
	} else if (_overState.pointState == PointState::Inside) {
		return _overElement && isSelectedAsGroup(_selected, _overElement->data());
	}
	return false;
}

bool ListWidget::isSelectedGroup(const SelectedMap &applyTo, not_null<const Data::Group *> group) const {
...
bool ListWidget::isSelectedAsGroup(const SelectedMap &applyTo, not_null<HistoryItem *> item) const {
...
bool ListWidget::isGoodForSelection(SelectedMap &applyTo, not_null<HistoryItem *> item, int &totalCount) const {
...
bool ListWidget::addToSelection(SelectedMap &applyTo, not_null<HistoryItem *> item) const {
	if (isMessageHidden(item)) {
		return false;
	}

	const auto itemId = item->fullId();
	auto [iterator, ok] = applyTo.try_emplace(itemId, SelectionData());
	if (!ok) {
		return false;
	}
...
bool ListWidget::removeFromSelection(SelectedMap &applyTo, FullMsgId itemId) const { return applyTo.remove(itemId); }

void ListWidget::changeSelection(SelectedMap &applyTo, not_null<HistoryItem *> item, SelectAction action) const {
	const auto itemId = item->fullId();
	if (action == SelectAction::Invert) {
		action = applyTo.contains(itemId) ? SelectAction::Deselect : SelectAction::Select;
	}
...
void ListWidget::changeSelectionAsGroup(SelectedMap &applyTo, not_null<HistoryItem *> item, SelectAction action) const {
	const auto group = session().data().groups().find(item);
	if (!group) {
		return changeSelection(applyTo, item, action);
	}
	if (action == SelectAction::Invert) {
		action = isSelectedAsGroup(applyTo, item) ? SelectAction::Deselect : SelectAction::Select;
	}
	auto already = int(applyTo.size());
...
bool ListWidget::isItemUnderPressSelected() const { return itemUnderPressSelection() != _selected.end(); }

auto ListWidget::itemUnderPressSelection() -> SelectedMap::iterator {
	return (_pressState.itemId && _pressState.pointState != PointState::Outside) ? _selected.find(_pressState.itemId)
																				 : _selected.end();
}

bool ListWidget::isInsideSelection(not_null<const Element *> view,
	...
	} else if (_selectedTextItem && _selectedTextItem == view->data() && state.pointState != PointState::Outside) {
		StateRequest stateRequest;
		stateRequest.flags |= Ui::Text::StateRequest::Flag::LookupSymbol;
		const auto dragState = view->textState(state.point, stateRequest);
		if (dragState.cursor == CursorState::Text &&
			base::in_range(dragState.symbol, _selectedTextRange.from, _selectedTextRange.to)) {
			return true;
		}
	}
	return false;
...
auto ListWidget::itemUnderPressSelection() const -> SelectedMap::const_iterator {
	return (_pressState.itemId && _pressState.pointState != PointState::Outside) ? _selected.find(_pressState.itemId)
																				 : _selected.end();
}
...
bool ListWidget::isPressInSelectedText(TextState state) const {
	...
	if (!hasSelectedText() || !_selectedTextItem || _selectedTextItem->fullId() != _pressState.itemId) {
		return false;
	}
...
void ListWidget::setTextSelection(not_null<Element *> view, TextSelection selection) {
	clearSelected();
	const auto item = view->data();
	...
	_selectedText = (selection.from != selection.to) ? view->selectedText(selection) : TextForMimeData();
	repaintItem(view);
...
bool ListWidget::loadedAtTopKnown() const { return !!_slice.skippedBefore; }

bool ListWidget::loadedAtTop() const { return _slice.skippedBefore && (*_slice.skippedBefore == 0); }

bool ListWidget::loadedAtBottomKnown() const { return !!_slice.skippedAfter; }

bool ListWidget::loadedAtBottom() const { return _slice.skippedAfter && (*_slice.skippedAfter == 0); }

bool ListWidget::isEmpty() const {
	return loadedAtTop() && loadedAtBottom() && (_itemsHeight + _itemsRevealHeight == 0);
}

bool ListWidget::hasCopyRestriction(HistoryItem *item) const {
	return _delegate->listCopyRestrictionType(item) != CopyRestrictionType::None;
}

bool ListWidget::hasCopyMediaRestriction(not_null<HistoryItem *> item) const {
	return _delegate->listCopyMediaRestrictionType(item) != CopyRestrictionType::None;
}
...
bool ListWidget::showCopyRestriction(HistoryItem *item) {
	...
	_delegate->listUiShow()->showToast((type == CopyRestrictionType::Channel) ? tr::lng_error_nocopy_channel(tr::now)
...
bool ListWidget::hasSelectRestriction() const {
	return _delegate->listSelectRestrictionType() != CopyRestrictionType::None;
}

Element *ListWidget::lookupItemByY(int y) const { return strictFindItemByY(y); }

auto ListWidget::findViewForPinnedTracking(int top) const -> std::pair<Element *, int> {
	const auto findScrollTopItem = [&](int top) -> std::vector<not_null<Element *>>::const_iterator
	{
		if (!width() || _items.empty()) {
			return end(_items);
		}
		const auto first = ranges::lower_bound(_items, top, std::less<>(), &Element::y);
		return (first == end(_items) || (*first)->y() > top) ? first - 1 : first;
	};
	const auto findView = [&](int top) -> std::pair<std::vector<not_null<Element *>>::const_iterator, int>
	{
...
int ListWidget::itemMinimalHeight() const {
	return st::msgMarginTopAttached + st::msgPhotoSize + st::msgMargin.bottom();
}

void ListWidget::checkMoveToOtherViewer() {
	auto visibleHeight = (_visibleBottom - _visibleTop);
	if (width() <= 0 || visibleHeight <= 0 || _items.empty() || _aroundIndex < 0 || _scrollTopState.item) {
		return;
	}
	...
	auto preloadIdsLimit = preloadIdsLimitMin + (visibleHeight / _itemAverageHeight);
	...
	auto minScreenDelta = kPreloadedScreensCount - kPreloadIfLessThanScreens;
	auto minUniversalIdDelta = (minScreenDelta * visibleHeight) / _itemAverageHeight;
	...
	if (preloadTop && !topLoaded) {
		...
		const auto goodIndex = (goodAboveIndex >= 0) ? goodAboveIndex : findGoodBelow(topItemIndex);
		if (goodIndex >= 0) {
			preloadAroundMessage(goodIndex);
		}
	} else if (preloadBottom && !bottomLoaded) {
		const auto goodBelowIndex = findGoodBelow(bottomItemIndex);
		const auto goodIndex = (goodBelowIndex >= 0) ? goodBelowIndex : findGoodAbove(bottomItemIndex);
		if (goodIndex >= 0) {
			preloadAroundMessage(goodIndex);
		}
	}
...
QString ListWidget::tooltipText() const {
	const auto item = (_overElement && _mouseAction == MouseAction::None) ? _overElement->data().get() : nullptr;
...
QPoint ListWidget::tooltipPos() const { return _mousePosition; }

bool ListWidget::tooltipWindowActive() const { return Ui::AppInFocus() && Ui::InFocusChain(window()); }

Context ListWidget::elementContext() { return _delegate->listContext(); }

bool ListWidget::elementUnderCursor(not_null<const HistoryView::Element *> view) { return (_overElement == view); }

SelectionModeResult ListWidget::elementInSelectionMode(const HistoryView::Element *view) {
...
bool ListWidget::elementIntersectsRange(not_null<const Element *> view, int from, int till) {
...
void ListWidget::elementShowPollResults(not_null<PollData *> poll, FullMsgId context) {
	_delegate->listShowPollResults(poll, context);
}

void ListWidget::elementOpenPhoto(not_null<PhotoData *> photo, FullMsgId context) {
	_delegate->listOpenPhoto(photo, context);
}

void ListWidget::elementOpenDocument(not_null<DocumentData *> document, FullMsgId context, bool showInMediaView) {
...
void ListWidget::elementShowTooltip(const TextWithEntities &text, Fn<void()> hiddenCallback) {
	// Under the parent is supposed to be a scroll widget.
	_topToast.show(parentWidget(), &session(), text, hiddenCallback);
}

bool ListWidget::elementAnimationsPaused() { return _delegate->listAnimationsPaused(); }

bool ListWidget::elementHideReply(not_null<const Element *> view) { return _delegate->listElementHideReply(view); }

bool ListWidget::elementShownUnread(not_null<const Element *> view) { return _delegate->listElementShownUnread(view); }

void ListWidget::elementSendBotCommand(const QString &command, const FullMsgId &context) {
	_delegate->listSendBotCommand(command, context);
}

void ListWidget::elementSearchInList(const QString &query, const FullMsgId &context) {
	_delegate->listSearch(query, context);
}

void ListWidget::elementHandleViaClick(not_null<UserData *> bot) { _delegate->listHandleViaClick(bot); }

bool ListWidget::elementIsChatWide() { return _overrideIsChatWide.value_or(_isChatWide); }

not_null<Ui::PathShiftGradient *> ListWidget::elementPathShiftGradient() { return _pathGradient.get(); }

void ListWidget::elementReplyTo(const FullReplyTo &to) { replyToMessageRequestNotify(to, base::IsCtrlPressed()); }

void ListWidget::elementStartInteraction(not_null<const Element *> view) {}

void ListWidget::elementStartPremium(not_null<const Element *> view, Element *replacing) {
...
void ListWidget::elementCancelPremium(not_null<const Element *> view) { _emojiInteractions->cancelPremiumEffect(view); }

void ListWidget::elementStartEffect(not_null<const Element *> view, Element *replacing) {
	_emojiInteractions->playEffect(view);
}

QString ListWidget::elementAuthorRank(not_null<const Element *> view) { return _delegate->listElementAuthorRank(view); }
		...
		if (const auto height = view->height()) {
			...
			if (!_itemRevealAnimations.contains(view)) {
				...
				animation.animation.start(
					[=] { revealItemsCallback(); }, 0., 1., kItemRevealDuration, anim::easeOutCirc);
				if (view->data()->out()) {
...
void ListWidget::startMessageSendingAnimation(not_null<HistoryItem *> item) {
	const auto sendingAnimation = _delegate->listSendingAnimation();
	if (!sendingAnimation || !sendingAnimation->checkExpectedType(item)) {
		return;
	}

	auto globalEndTopLeft =
		rpl::merge(session().data().newItemAdded() | rpl::to_empty, geometryValue() | rpl::to_empty) |
		rpl::map(
			[=]() -> std::optional<QPoint>
			{
...
void ListWidget::showPremiumStickerTooltip(not_null<const HistoryView::Element *> view) {
...
void ListWidget::revealItemsCallback() {
	...
	for (auto i = begin(_itemRevealAnimations); i != end(_itemRevealAnimations);) {
		if (!i->second.animation.animating()) {
			...
			revealHeight += anim::interpolate(i->second.startHeight, 0, i->second.animation.value(1.));
			++i;
		}
	}
	if (_itemsRevealHeight != revealHeight) {
		...
		const auto nowHeight = _itemsTop + _itemsHeight + st::historyPaddingBottom;
...
int ListWidget::resizeGetHeight(int newWidth) {
	...
	if (newHeight > 0) {
		_itemAverageHeight = std::max(itemMinimalHeight(), newHeight / int(_items.size()));
	}
...
TextSelection ListWidget::computeRenderSelection(not_null<const SelectedMap *> selected,
	...
	if (const auto group = session().data().groups().find(item)) {
		...
		if (leaderSelection != FullSelection && leaderSelection != TextSelection()) {
...
TextSelection ListWidget::itemRenderSelection(not_null<const Element *> view) const {
	if (!_dragSelected.empty()) {
		const auto i = _dragSelected.find(view->data()->fullId());
		if (i != _dragSelected.end()) {
			return (_dragSelectAction == DragSelectAction::Selecting) ? FullSelection : TextSelection();
		}
	}
...
Ui::ChatPaintContext ListWidget::preparePaintContext(const QRect &clip) const {
...
bool ListWidget::markingContentsRead() const {
	return _showFinished && !_refreshingViewer && _delegate->listMarkingContentRead();
}

bool ListWidget::markingMessagesRead() const { return markingContentsRead() && !session().supportMode(); }
...
void ListWidget::checkActivation() {
	if (_resizePending || _visibleTop >= _visibleBottom || !markingMessagesRead()) {
		return;
	}
	for (const auto &view : ranges::views::reverse(_items)) {
...
void ListWidget::paintEvent(QPaintEvent *e) {
	...
	auto from = std::lower_bound(begin(_items),
								 end(_items),
								 clip.top(),
								 [this](auto &elem, int top) { return this->itemTop(elem) + elem->height() <= top; });
	auto to = std::lower_bound(begin(_items),
							   end(_items),
	...
	for (auto i = from; i != to; ++i) {
		...
		const auto item = view->data();
		const auto height = view->height();
		if (!sendingAnimation || !sendingAnimation->hasAnimatedMessage(item)) {
			if (_reactionsManager) {
		...
		const auto isUnread = _delegate->listElementShownUnread(view) && item->isRegular();
		const auto withReaction = context.reactionInfo && item->hasUnreadReaction();
		const auto yShown = [&](int y) { return (_visibleBottom >= y && _visibleTop <= y); };
		const auto markShown = (_context != Context::ChatPreview) &&
			(isSponsored		? view->markSponsoredViewed(_visibleBottom - top)
...
void ListWidget::paintUserpics(Painter &p, const Ui::ChatPaintContext &context, QRect clip) {
				...
				if (const auto from = item->displayFrom()) {
					...
					if (info->customUserpic.empty()) {
						info->emptyUserpic.paintCircle(
							p, st::historyPhotoLeft, userpicTop, view->width(), st::msgPhotoSize);
					} else {
						auto &userpic = _hiddenSenderUserpics[item->id];
						const auto valid = info->paintCustomUserpic(
							p, userpic, st::historyPhotoLeft, userpicTop, view->width(), st::msgPhotoSize);
						if (!valid) {
...
void ListWidget::paintDates(Painter &p, const Ui::ChatPaintContext &context, QRect clip) {
	if (_context == Context::ShortcutMessages) {
		return;
	}
...
void ListWidget::maybeMarkReactionsRead(not_null<HistoryItem *> item) {
	...
	if (top + reactionCenter < _visibleTop || top + view->height() > _visibleBottom) {
		return;
	}
...
TextForMimeData ListWidget::getSelectedText() const {
	...
	auto fullSize = 0;
	auto texts = std::vector<std::pair<not_null<HistoryItem *>, TextForMimeData>>();
	texts.reserve(selected.size());

	const auto wrapItem = [&](not_null<HistoryItem *> item, TextForMimeData &&unwrapped)
	{
		auto time = QString(", [%1]\n").arg(QLocale().toString(ItemDateTime(item), QLocale::ShortFormat));
		auto part = TextForMimeData();
		auto size = item->author()->name().size() + time.size() + unwrapped.expanded.size();
		part.reserve(size);
...
MessageIdsList ListWidget::getSelectedIds() const { return collectSelectedIds(); }

SelectedItems ListWidget::getSelectedItems() const { return collectSelectedItems(); }

const TextSelection &ListWidget::getSelectedTextRange(not_null<HistoryItem *> item) const { return _selectedTextRange; }
...
int ListWidget::findItemIndexByY(int y) const {
	...
	auto i = std::lower_bound(begin(_items),
							  end(_items),
							  y,
							  [this](auto &elem, int top) { return this->itemTop(elem) + elem->height() <= top; });
...
not_null<Element *> ListWidget::findItemByY(int y) const { return _items[findItemIndexByY(y)]; }

Element *ListWidget::strictFindItemByY(int y) const {
	if (_items.empty()) {
		return nullptr;
	}
	return (y >= _itemsTop && y < _itemsTop + _itemsHeight) ? findItemByY(y).get() : nullptr;
}

auto ListWidget::countScrollState() const -> ScrollTopState {
	if (_items.empty() || (_itemsKnownTillEnd && _visibleBottom == height())) {
		return {Data::MessagePosition(), 0};
	}
	const auto topItem = findItemByY(_visibleTop);
	return {topItem->data()->position(), _visibleTop - itemTop(topItem)};
}

void ListWidget::keyPressEvent(QKeyEvent *e) {
	const auto key = e->key();
	const auto hasModifiers = (Qt::NoModifier != (e->modifiers() & ~(Qt::KeypadModifier | Qt::GroupSwitchModifier)));
	...
	} else if (e == QKeySequence::Copy && (hasSelectedText() || hasSelectedItems()) && !showCopyRestriction() &&
			   !hasCopyRestrictionForSelected()) {
		TextUtilities::SetClipboardText(getSelectedText());
#ifdef Q_OS_MAC
	} else if (key == Qt::Key_E && e->modifiers().testFlag(Qt::ControlModifier) && !showCopyRestriction() &&
			   !hasCopyRestrictionForSelected()) {
	...
	} else if (!(e->modifiers() & ~Qt::ShiftModifier) && key != Qt::Key_Shift) {
...
auto ListWidget::scrollKeyEvents() const -> rpl::producer<not_null<QKeyEvent *>> { return _scrollKeyEvents.events(); }

void ListWidget::mouseDoubleClickEvent(QMouseEvent *e) {
	mouseActionStart(e->globalPos(), e->button());
	trySwitchToWordSelection();
	if (!ClickHandler::getActive() && !ClickHandler::getPressed() &&
		(_mouseCursorState == CursorState::None || _mouseCursorState == CursorState::Date) && _selected.empty() &&
		_overElement && _overElement->data()->isRegular()) {
		mouseActionCancel();
...
void ListWidget::trySwitchToWordSelection() {
	auto selectingSome = (_mouseAction == MouseAction::Selecting) && hasSelectedText();
	auto willSelectSome = (_mouseAction == MouseAction::None) && !hasSelectedItems();
	auto checkSwitchToWordSelection =
		_overElement && (_mouseSelectType == TextSelectType::Letters) && (selectingSome || willSelectSome);
...
void ListWidget::switchToWordSelection() {
	...
	if (_mouseAction == MouseAction::None) {
		...
		setTextSelection(_overElement, TextSelection(dragState.symbol, dragState.symbol));
	}
	mouseActionUpdate();
...
void ListWidget::contextMenuEvent(QContextMenuEvent *e) { showContextMenu(e); }
...
void ListWidget::showContextMenu(QContextMenuEvent *e, bool showFromTouch) {
	...
	if (link && !link->property(kSendReactionEmojiProperty).value<Data::ReactionId>().empty() && _reactionsManager &&
		_reactionsManager->showContextMenu(this, e, session().data().reactions().favoriteId())) {
		return;
	}
	const auto overItem = _overItemExact ? _overItemExact : _overElement ? _overElement->data().get() : nullptr;
	...
	const auto linkUserpicPeerId =
		(link && _overSenderUserpic) ? PeerId(link->property(kPeerLinkPeerIdProperty).toULongLong()) : PeerId();
	_whoReactedMenuLifetime.destroy();
	if (!clickedReaction.empty() && overItem && Api::WhoReactedExists(overItem, Api::WhoReactedList::One)) {
		HistoryView::ShowWhoReactedMenu(
			&_menu, e->globalPos(), this, overItem, clickedReaction, controller(), _whoReactedMenuLifetime);
		e->accept();
		return;
	} else if (!linkPhoneNumber.isEmpty()) {
		PhoneClickHandler(&session(), linkPhoneNumber)
			.onClick(prepareClickContext(Qt::LeftButton, _overItemExact ? _overItemExact->fullId() : FullMsgId()));
		return;
	...
	request.quote = (_overElement && _selectedTextItem == _overElement->data())
	...
	request.overSelection = (showFromTouch && hasSelection) ||
		(_overElement &&
		 isInsideSelection(_overElement, _overItemExact ? _overItemExact : _overElement->data().get(), _overState));
	...
	const auto reactItem =
		(_overElement && _overState.pointState != PointState::Outside) ? _overElement->data().get() : nullptr;
...
void ListWidget::reactionChosen(ChosenReaction reaction) {
	...
	if (!item) {
		...
		Payments::ShowPaidReactionDetails(controller(), item, viewForItem(item), HistoryReactionSource::Selector);
		return;
...
void ListWidget::onTouchScrollTimer() {
	...
	if (_touchScrollState == Ui::TouchScrollState::Acceleration && _touchWaitingAcceleration &&
		...
		const auto hasScrolled = _delegate->listScrollTo(_visibleTop - delta.y());
...
void ListWidget::touchScrollUpdated(const QPoint &screenPos) {
	_touchPos = screenPos;
	_delegate->listScrollTo(_visibleTop - (_touchPos - _touchPrevPos).y(), false);
	touchUpdateSpeed();
}

rpl::producer<bool> ListWidget::touchMaybeSelectingValue() const { return _touchMaybeSelecting.value(); }
...
void ListWidget::updateDragSelection() {
	if (!_overState.itemId || !_pressState.itemId || hasSelectRestriction()) {
		clearDragSelection();
		return;
	...
	const auto selectingUp = _delegate->listIsLessInOrder(overView->data(), pressItem);
...
void ListWidget::updateDragSelection(const Element *fromView,
	...
	const auto includeFrom = [&](not_null<const Element *> view, const MouseState &state)
	{
		const auto bottom = view->height() - view->marginBottom();
		return (state.point.y() < bottom - delta);
	};
	const auto includeTill = [&](not_null<const Element *> view, const MouseState &state)
	{
		const auto top = view->marginTop();
		return (state.point.y() >= top + delta);
	};
	const auto includeSingleItem =
		[&](not_null<const Element *> view, const MouseState &state1, const MouseState &state2)
	{
		const auto top = view->marginTop();
		const auto bottom = view->height() - view->marginBottom();
		const auto y1 = std::min(state1.point.y(), state2.point.y());
		const auto y2 = std::max(state1.point.y(), state2.point.y());
		return (y1 < bottom - delta && y2 >= top + delta) ? (y2 - y1 >= delta) : false;
	};

	const auto from = [&]
	{
		const auto result =
			fromView ? ranges::find(_items, fromView, [](auto view) { return view.get(); }) : end(_items);
		return (result == end(_items))									 ? begin(_items)
			: (fromView == tillView || includeFrom(fromView, fromState)) ? result
																		 : (result + 1);
	}();
	const auto till = [&]
	{
		if (fromView == tillView) {
			return (from == end(_items)) ? from : includeSingleItem(fromView, fromState, tillState) ? (from + 1) : from;
		}
		const auto result =
			tillView ? ranges::find(_items, tillView, [](auto view) { return view.get(); }) : end(_items);
		return (result == end(_items)) ? end(_items) : includeTill(tillView, tillState) ? (result + 1) : result;
	}();
	if (from < till) {
...
void ListWidget::updateDragSelection(std::vector<not_null<Element *>>::const_iterator from,
...
void ListWidget::ensureDragSelectAction(std::vector<not_null<Element *>>::const_iterator from,
										std::vector<not_null<Element *>>::const_iterator till) {
	...
	_dragSelectAction = _selected.contains(startId) ? DragSelectAction::Deselecting : DragSelectAction::Selecting;
	if (!_wasSelectedText && !_dragSelected.empty() && _dragSelectAction == DragSelectAction::Selecting) {
		_wasSelectedText = true;
		setFocus();
	}
}
...
void ListWidget::mouseActionStart(const QPoint &globalPosition, Qt::MouseButton button) {
	...
	if (_mouseAction == MouseAction::None && pressElement) {
		...
		if (_trippleClickStartTime != 0 && validStartPoint) {
			...
			if (dragState.cursor == CursorState::Text) {
				setTextSelection(pressElement, TextSelection(dragState.symbol, dragState.symbol));
				_mouseTextSymbol = dragState.symbol;
				_mouseAction = MouseAction::Selecting;
		...
		if (_mouseSelectType != TextSelectType::Paragraphs) {
			...
			if (isPressInSelectedText(dragState)) {
				...
				if (requiredToStartDragging(pressElement) && _pressState.pointState != PointState::Outside) {
					_mouseAction = MouseAction::PrepareDrag;
				} else {
					if (dragState.afterSymbol) ++_mouseTextSymbol;
					if (!hasSelectedItems() && _overState.pointState != PointState::Outside) {
						setTextSelection(pressElement, TextSelection(_mouseTextSymbol, _mouseTextSymbol));
						_mouseAction = MouseAction::Selecting;
...
Reactions::ButtonParameters ListWidget::reactionButtonParameters(not_null<const Element *> view,
																 QPoint position,
	...
	auto result = view->reactionButtonParameters(position, reactionState).translated({0, itemTop(view)});
...
void ListWidget::mouseActionFinish(const QPoint &globalPosition, Qt::MouseButton button) {
	mouseActionUpdate(globalPosition);
	...
	const auto toggleByHandler = [&](const ClickHandlerPtr &handler)
	{
		...
		return _overElement && _overElement->toggleSelectionByHandlerClick(handler);
	};

	auto activated = ClickHandler::unpressed();
	auto simpleSelectionChange = pressState.itemId && !_pressWasInactive && (button != Qt::RightButton) &&
		(_mouseAction == MouseAction::PrepareSelect || _mouseAction == MouseAction::PrepareDrag);
	auto needItemSelectionToggle =
		simpleSelectionChange && (!activated || toggleByHandler(activated)) && hasSelectedItems();
	auto needTextSelectionClear = simpleSelectionChange && hasSelectedText();

	_wasSelectedText = false;

	if (_mouseAction == MouseAction::Dragging || _mouseAction == MouseAction::Selecting || needItemSelectionToggle) {
		activated = nullptr;
	} else if (activated) {
		mouseActionCancel();
		ActivateClickHandler(
			window(), activated, {button, QVariant::fromValue(prepareClickHandlerContext(pressState.itemId))});
		return;
	}
	if (needItemSelectionToggle) {
		if (const auto item = session().data().message(pressState.itemId)) {
			...
			if (pressState.pointState == PointState::GroupPart) {
				changeSelection(_selected, _overItemExact ? _overItemExact : item, SelectAction::Invert);
			} else {
	...
	if (QGuiApplication::clipboard()->supportsSelection() && _selectedTextItem &&
		_selectedTextRange.from != _selectedTextRange.to && !hasCopyRestriction(_selectedTextItem)) {
		if (const auto view = viewForItem(_selectedTextItem)) {
			TextUtilities::SetClipboardText(view->selectedText(_selectedTextRange), QClipboard::Selection);
		}
	}
}
...
ClickContext ListWidget::prepareClickContext(Qt::MouseButton button, FullMsgId itemId) {
	return {
		button,
...
int ListWidget::SelectionViewOffset(not_null<const ListWidget *> inner, not_null<const Element *> view) {
...
void ListWidget::mouseActionUpdate() {
	...
	if (_reactionsManager) {
		_reactionsManager->updateButton(view ? reactionButtonParameters(view, itemPoint, reactionState)
	...
	auto inTextSelection = (_overState.pointState != PointState::Outside) &&
		(_overState.itemId == _pressState.itemId) && hasSelectedText();
	...
	if (overReaction) {
		...
		auto dragStartLength = [] { return QApplication::startDragDistance(); };
		if (_overState.itemId != _pressState.itemId || cursorDeltaLength() >= dragStartLength()) {
		...
		const auto dateHeight =
			st::msgServicePadding.bottom() + st::msgServiceFont->height + st::msgServicePadding.top();
		...
		if (_mouseAction == MouseAction::Selecting) {
			if (inTextSelection) {
				...
				if (_mouseSelectType != TextSelectType::Letters) {
					selection = view->adjustSelection(selection, _mouseSelectType);
				}
	...
	if (_pressState.pointState != PointState::Outside && ClickHandler::getPressed()) {
	...
	if (_mouseAction == MouseAction::Selecting) {
		_selectScroll.checkDeltaScroll(mousePosition, _visibleTop, _visibleBottom);
	} else {
		_selectScroll.cancel();
	}
}
...
style::cursor ListWidget::computeMouseCursor() const {
	...
	} else if (!hasSelectedItems() && (_mouseCursorState == CursorState::Text)) {
...
std::unique_ptr<QMimeData> ListWidget::prepareDrag() {
	...
	const auto uponSelected =
		pressedView && isInsideSelection(pressedView, _pressItemExact ? _pressItemExact : pressedItem, _pressState);
	...
	if (auto mimeData = TextUtilities::MimeDataFromText(selectedText)) {
		...
		if (uponSelected && !_delegate->listAllowsDragForward()) {
			...
			auto items = canForwardAll ? collectSelectedIds() : MessageIdsList();
			if (!items.empty()) {
		...
		const auto exactItem = _pressItemExact ? _pressItemExact : pressedItem;
		if (_mouseCursorState == CursorState::Date) {
			...
			if (pressedView->data()->allowsForward() && media->dragItemByHandler(pressedHandler)) {
				forwardIds = MessageIdsList(1, exactItem->fullId());
			}
		}
...
void ListWidget::performDrag() {
	if (auto mimeData = prepareDrag()) {
		...
		_delegate->listLaunchDrag(std::move(mimeData), crl::guard(this, [=] { mouseActionUpdate(QCursor::pos()); }));
	}
}

int ListWidget::itemTop(not_null<const Element *> view) const { return _itemsTop + view->y(); }
...
void ListWidget::repaintItem(const Element *view) {
	...
	const auto area = _reactionsManager ? _reactionsManager->lookupEffectArea(id) : std::nullopt;
	if (area) {
		update(*area);
	}
}
...
void ListWidget::refreshAttachmentsFromTill(int from, int till) {
	...
	const auto guard = gsl::finally([&] { updateSize(); });
	if (from == till) {
		return;
	}
	...
	for (auto i = from + 1; i != till; ++i) {
		...
		if (next->isHidden()) {
			...
			next->setDisplayDate(_context != Context::ShortcutMessages && nextDate.date() != viewDate.date());
...
void ListWidget::refreshItem(not_null<const Element *> view) {
	...
	if (index < int(_items.size())) {
		...
		const auto &[i, ok] = _views.emplace(item, item->createView(this, was.get()));
...
void ListWidget::itemRemoved(not_null<const HistoryItem *> item) {
	...
	saveScrollState();
	const auto guard = gsl::finally([&] { restoreScrollState(); });

	const auto view = i->second.get();
	_items.erase(ranges::remove(_items, view, [](auto view) { return view.get(); }), end(_items));
...
QPoint ListWidget::mapPointToItem(QPoint point, const Element *view) const {
	if (!view) {
		return QPoint();
	}
...
rpl::producer<FullMsgId> ListWidget::editMessageRequested() const { return _requestedToEditMessage.events(); }

void ListWidget::editMessageRequestNotify(FullMsgId item) const { _requestedToEditMessage.fire(std::move(item)); }

bool ListWidget::lastMessageEditRequestNotify() const {
	const auto now = base::unixtime::now();
	auto proj = [&](not_null<Element *> view) { return view->data()->allowsEdit(now) && !view->data()->isUploading(); };
	const auto &list = ranges::views::reverse(_items);
	const auto it = ranges::find_if(list, std::move(proj));
	if (it == end(list)) {
		return false;
	} else {
		const auto item = session().data().groups().findItemToEdit((*it)->data()).get();
		editMessageRequestNotify(item->fullId());
		return true;
	}
}

auto ListWidget::replyToMessageRequested() const -> rpl::producer<ReplyToMessageRequest> {
	return _requestedToReplyToMessage.events();
}

void ListWidget::replyToMessageRequestNotify(FullReplyTo to, bool forceAnotherChat) {
	_requestedToReplyToMessage.fire({std::move(to), forceAnotherChat});
}

rpl::producer<FullMsgId> ListWidget::readMessageRequested() const { return _requestedToReadMessage.events(); }

rpl::producer<FullMsgId> ListWidget::showMessageRequested() const { return _requestedToShowMessage.events(); }
...
void ListWidget::replyNextMessage(FullMsgId fullId, bool next) {
	...
	const auto replyFirst = [&] { reply(next ? nullptr : _items.back().get()); };
	if (!fullId) {
		replyFirst();
		return;
	}

	auto proj = [&](not_null<Element *> view) { return view->data()->fullId() == fullId; };
	const auto &list = ranges::views::reverse(_items);
	const auto it = ranges::find_if(list, std::move(proj));
	if (it == end(list)) {
		replyFirst();
...
void ListWidget::overrideIsChatWide(bool isWide) { _overrideIsChatWide = isWide; }
...
void ConfirmDeleteSelectedItems(not_null<ListWidget *> widget) {
	...
	const auto confirmed = crl::guard(widget, [=] { widget->cancelSelection(); });
	if (CanCreateModerateMessagesBox(historyItems)) {
		controller->show(Box(CreateModerateMessagesBox, historyItems, confirmed));
	} else {
		auto box = Box<DeleteMessagesBox>(&widget->session(), widget->getSelectedIds());
...
void ConfirmForwardSelectedItems(not_null<ListWidget *> widget) {
	...
	Window::ShowForwardMessagesBox(widget->controller(),
								   std::move(ids),
								   [=]
								   {
...
void ConfirmSendNowSelectedItems(not_null<ListWidget *> widget) {
	...
	Window::ShowSendNowMessagesBox(navigation, history, widget->getSelectedIds(), clearSelection);
}

CopyRestrictionType CopyRestrictionTypeFor(not_null<PeerData *> peer, HistoryItem *item) {
	return (peer->allowsForwarding() && (!item || !item->forbidsForward())) ? CopyRestrictionType::None
		: peer->isBroadcast()												? CopyRestrictionType::Channel
																			: CopyRestrictionType::Group;
}

CopyRestrictionType CopyMediaRestrictionTypeFor(not_null<PeerData *> peer, not_null<HistoryItem *> item) {
	if (const auto all = CopyRestrictionTypeFor(peer, item); all != CopyRestrictionType::None) {
		return all;
	}
	return !item->forbidsSaving() ? CopyRestrictionType::None
		: peer->isBroadcast()	  ? CopyRestrictionType::Channel
								  : CopyRestrictionType::Group;
}

CopyRestrictionType SelectRestrictionTypeFor(not_null<PeerData *> peer) {
	if (const auto chat = peer->asChat()) {
		return chat->canDeleteMessages() ? CopyRestrictionType::None : CopyRestrictionTypeFor(peer);
	} else if (const auto channel = peer->asChannel()) {
		return channel->canDeleteMessages() ? CopyRestrictionType::None : CopyRestrictionTypeFor(peer);
	}
