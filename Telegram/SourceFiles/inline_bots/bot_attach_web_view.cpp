...
[[nodiscard]] DocumentData *ResolveIcon(not_null<Main::Session *> session, const MTPDattachMenuBot &data) {
	for (const auto &icon : data.vicons().v) {
		const auto document = icon.match(
			[&](const MTPDattachMenuBotIcon &data) -> DocumentData *
			{
...
[[nodiscard]] PeerTypes ResolvePeerTypes(const QVector<MTPAttachMenuPeerType> &types) {
	auto result = PeerTypes();
	for (const auto &type : types) {
		result |= type.match([&](const MTPDattachMenuPeerTypeSameBotPM &) { return PeerType::SameBot; },
							 [&](const MTPDattachMenuPeerTypeBotPM &) { return PeerType::Bot; },
							 [&](const MTPDattachMenuPeerTypePM &) { return PeerType::User; },
							 [&](const MTPDattachMenuPeerTypeChat &) { return PeerType::Group; },
							 [&](const MTPDattachMenuPeerTypeBroadcast &) { return PeerType::Broadcast; });
	}
	return result;
}

[[nodiscard]] std::optional<AttachWebViewBot> ParseAttachBot(not_null<Main::Session *> session,
															 const MTPAttachMenuBot &bot) {
	auto result = bot.match(
		[&](const MTPDattachMenuBot &data)
		{
			const auto user = session->data().userLoaded(UserId(data.vbot_id()));
			const auto good = user && user->isBot() && user->botInfo->supportsAttachMenu;
			return good
			? AttachWebViewBot{
				.user = user,
...
[[nodiscard]] PeerTypes PeerTypesFromNames(const std::vector<QString> &names) {
	auto result = PeerTypes();
	for (const auto &name : names) {
		//, bots, groups, channels
		result |= (name == u"users"_q) ? PeerType::User
			: name == u"bots"_q		   ? PeerType::Bot
			: name == u"groups"_q	   ? PeerType::Group
			: name == u"channels"_q	   ? PeerType::Broadcast
									   : PeerType(0);
	}
	return result;
}

[[nodiscard]] Ui::LocationPickerConfig ResolveMapsConfig(not_null<Main::Session *> session) {
	const auto &appConfig = session->appConfig();
	auto map =
		appConfig.get<base::flat_map<QString, QString>>(u"tdesktop_config_map"_q, base::flat_map<QString, QString>());
	return {
...
[[nodiscard]] Window::SessionController *WindowForThread(base::weak_ptr<Window::SessionController> weak,
...
void ShowChooseBox(std::shared_ptr<Ui::Show> show,
	...
	*weak = show->show(
		Box<PeerListBox>(std::make_unique<ChooseRecipientBoxController>(session, std::move(done), std::move(filter)),
						 std::move(initBox)));
}

void ShowChooseBox(not_null<Window::SessionController *> controller,
				   PeerTypes types,
				   Fn<void(not_null<Data::Thread *>)> callback,
				   rpl::producer<QString> titleOverride = nullptr) {
	ShowChooseBox(controller->uiShow(), &controller->session(), types, std::move(callback), std::move(titleOverride));
}

void FillDisclaimerBox(not_null<Ui::GenericBox *> box, Fn<void(bool accepted)> done) {
	...
	Ui::ConfirmBox(box,
				   {
					   .text = tr::lng_mini_apps_disclaimer_text(tr::now, Ui::Text::RichLangValue),
					   .confirmed = callback,
					   .cancelled =
						   [=](Fn<void()> close)
					   {
	...
	auto checkView = std::make_unique<Ui::CheckView>(st::defaultCheck,
													 false,
													 [=]
													 {
														 if (*updateCheck) {
	...
	const auto row =
		box->addRow(object_ptr<Ui::Checkbox>(box.get(),
											 tr::lng_mini_apps_disclaimer_button(
												 lt_link,
												 rpl::single(Ui::Text::Link(tr::lng_mini_apps_disclaimer_link(tr::now),
	...
	row->setClickHandlerFilter(
		[=](const ClickHandlerPtr &link, Qt::MouseButton button)
		{
			ActivateClickHandler(row,
								 link,
	...
	const auto showError = Ui::CheckView::PrepareNonToggledError(check, box->lifetime());
...
WebViewContext ResolveContext(not_null<UserData *> bot, WebViewContext context) {
...
void FillBotUsepic(not_null<Ui::GenericBox *> box,
				   not_null<PeerData *> bot,
				   base::weak_ptr<Window::SessionController> weak) {
	auto aboutLabel = Ui::CreateLabelWithCustomEmoji(
		box->verticalLayout(),
		tr::lng_allow_bot_webview_details(
			lt_emoji, rpl::single(Ui::Text::IconEmoji(&st::textMoreIconEmoji)), Ui::Text::RichLangValue) |
			rpl::map([](TextWithEntities text) { return Ui::Text::Link(std::move(text), u"internal:"_q); }),
		Core::TextContext({.session = &bot->session()}),
		st::defaultFlatLabel);
	const auto userpic = Ui::CreateChild<Ui::UserpicButton>(box->verticalLayout(), bot, st::infoPersonalChannelUserpic);
	Ui::AddSkip(box->verticalLayout());
	aboutLabel->setClickHandlerFilter(
		[=](auto &&...)
		{
			if (const auto strong = weak.get()) {
				strong->showPeerHistory(bot->id, Window::SectionShow::Way::Forward);
				return true;
			}
			return false;
		});
	...
	const auto titleLabel =
		Ui::CreateChild<Ui::FlatLabel>(title, rpl::single(bot->name()), box->getDelegate()->style().title);
	const auto icon = bot->isVerified() ? &st::infoVerifiedCheck : nullptr;
	title->resize(titleLabel->width() + (icon ? icon->width() : 0), titleLabel->height());
	title->widthValue() | rpl::distinct_until_changed() |
		rpl::start_with_next([=](int w) { titleLabel->resizeToWidth(w - (icon ? icon->width() + st::lineWidth : 0)); },
							 title->lifetime());
	if (icon) {
		title->paintRequest() |
			rpl::start_with_next(
				[=]
				{
					...
					icon->paint(p,
								std::min(titleLabel->textMaxWidth() + st::lineWidth,
...
std::unique_ptr<Ui::RpWidget>
MakeEmojiSetStatusPreview(not_null<QWidget *> parent, not_null<PeerData *> peer, not_null<DocumentData *> document) {
	const auto emoji = Ui::CreateChild<Ui::PaddingWrap<Ui::FlatLabel>>(
		parent,
		object_ptr<Ui::FlatLabel>(
			parent,
			rpl::single(Ui::Text::SingleCustomEmoji(Data::SerializeCustomEmojiId(document->id),
	...
	auto result = Info::BotStarRef::MakePeerBubbleButton(parent, peer, emoji);
...
bool CheckEmojiStatusPremium(not_null<UserData *> bot) {
	...
	const auto window = ChatHelpers::ResolveWindowDefault()(&bot->session());
	if (window) {
...
void ConfirmEmojiStatusAccessBox(not_null<Ui::GenericBox *> box, not_null<UserData *> bot, Fn<void(bool)> done) {
	...
	const auto statusIcon = ChatHelpers::GenerateLocalTgsSticker(&bot->session(), u"hello_status"_q);
	statusIcon->overrideEmojiUsesTextColor(true);

	auto ownedSet = MakeEmojiSetStatusPreview(box, bot->session().user(), statusIcon);
	box->addRow(object_ptr<Ui::RpWidget>::fromRaw(ownedSet.release()));
	...
	box->addRow(
		object_ptr<Ui::FlatLabel>(box,
								  tr::lng_bot_emoji_status_access_text(
									  lt_bot, rpl::single(name), lt_name, rpl::single(name), Ui::Text::RichLangValue),
...
void ConfirmEmojiStatusBox(not_null<Ui::GenericBox *> box,
	...
	const auto preview = box->addRow(object_ptr<Ui::RpWidget>::fromRaw(owned.release()));
	...
	box->addRow(object_ptr<Ui::FlatLabel>(box, tr::lng_bot_emoji_status_title(), st::botEmojiStatusTitle));
	AddSkip(box->verticalLayout());

	box->addRow(object_ptr<Ui::FlatLabel>(
		box,
		tr::lng_bot_emoji_status_text(lt_bot, rpl::single(Ui::Text::Bold(bot->name())), Ui::Text::RichLangValue),
		st::botEmojiStatusText));

	AddSkip(box->verticalLayout(), 2 * st::defaultVerticalListSkip);

	auto ownedSet = MakeEmojiSetStatusPreview(box, document->session().user(), document);
	box->addRow(object_ptr<Ui::RpWidget>::fromRaw(ownedSet.release()));
	...
	box->addButton(tr::lng_bot_emoji_status_confirm(),
				   [=]
				   {
					   ...
					   document->owner().emojiStatuses().set({document->id},
...
{
public:
	BotAction(not_null<Ui::RpWidget *> parent,
...
BotAction::BotAction(not_null<Ui::RpWidget *> parent,
					 std::shared_ptr<Ui::Show> show,
					 const style::Menu &st,
					 const AttachWebViewBot &bot,
					 Fn<void()> callback)
	: ItemBase(parent, st), _show(std::move(show)), _dummyAction(new QAction(parent)), _st(st), _bot(bot),
	  _icon(this, _bot.media), _height(_st.itemPadding.top() + _st.itemStyle.font->height + _st.itemPadding.bottom()) {
	setAcceptBoth(false);
...
bool BotAction::isEnabled() const { return true; }

not_null<QAction *> BotAction::action() const { return _dummyAction; }
...
void BotAction::contextMenuEvent(QContextMenuEvent *e) {
	...
	QObject::connect(_menu, &QObject::destroyed, [=] { _forceShown.fire(false); });
...
QImage BotAction::prepareRippleMask() const { return Ui::RippleAnimation::RectMask(size()); }

int BotAction::contentHeight() const { return _height; }

rpl::producer<bool> BotAction::forceShown() const { return _forceShown.events(); }
...
MenuBotIcon::MenuBotIcon(QWidget *parent, std::shared_ptr<Data::DocumentMedia> media)
	: RpWidget(parent), _media(std::move(media)) {
...
bool PeerMatchesTypes(not_null<PeerData *> peer, not_null<UserData *> bot, PeerTypes types) {
	if (const auto user = peer->asUser()) {
		return (user == bot) ? (types & PeerType::SameBot)
			: user->isBot()	 ? (types & PeerType::Bot)
							 : (types & PeerType::User);
	} else if (peer->isBroadcast()) {
		return (types & PeerType::Broadcast);
	}
...
WebViewInstance::WebViewInstance(WebViewDescriptor &&descriptor)
	: _parentShow(descriptor.parentShow				  ? std::move(descriptor.parentShow)
					  : descriptor.context.controller ? descriptor.context.controller.get()->uiShow()
													  : nullptr),
	  _session(&descriptor.bot->session()), _bot(descriptor.bot),
	  _context(ResolveContext(_bot, std::move(descriptor.context))), _button(std::move(descriptor.button)),
...
Main::Session &WebViewInstance::session() const { return *_session; }

not_null<UserData *> WebViewInstance::bot() const { return _bot; }

WebViewSource WebViewInstance::source() const { return _source; }
...
void WebViewInstance::resolve() {
	v::match(
		...
		[&](WebViewSourceSwitch) { confirmOpen([=] { requestSimple(); }); },
		[&](WebViewSourceLinkApp data)
		{
			resolveApp(
				data.appname, data.token, (_context.maySkipConfirmation ? ConfirmType::None : ConfirmType::Always));
		},
		[&](WebViewSourceLinkBotProfile) { confirmOpen([=] { requestMain(); }); },
		[&](WebViewSourceLinkAttachMenu data) { requestWithMenuAdd(); },
		[&](WebViewSourceMainMenu) { requestWithMainMenuDisclaimer(); },
		[&](WebViewSourceAttachMenu) { requestWithMenuAdd(); },
		[&](WebViewSourceBotMenu)
		{
			if (!openAppFromBotMenuLink()) {
				confirmOpen([=] { requestButton(); });
			}
		},
		[&](WebViewSourceGame game) { showGame(); },
		[&](WebViewSourceBotProfile)
		{
			if (_context.maySkipConfirmation) {
				requestMain();
			} else {
				confirmOpen([=] { requestMain(); });
			}
		});
}
...
bool WebViewInstance::openAppFromBotMenuLink() {
	...
	const auto params = qthelp::url_parse_params(local.mid(prefix.size()), qthelp::UrlParamNameTransform::ToLower);
	const auto domainParam = params.value(u"domain"_q);
	const auto appnameParam = params.value(u"appname"_q);
	const auto webChannelPreviewLink = (domainParam == u"s"_q) && !appnameParam.isEmpty();
...
void WebViewInstance::resolveApp(const QString &appname, const QString &startparam, ConfirmType confirmType) {
	const auto already = _session->data().findBotApp(_bot->id, appname);
	_requestId = _session->api()
					 .request(MTPmessages_GetBotApp(MTP_inputBotAppShortName(_bot->inputUser, MTP_string(appname)),
													MTP_long(already ? already->hash : 0)))
					 .done(
						 [=](const MTPmessages_BotApp &result)
						 {
							 _requestId = 0;
							 const auto &data = result.data();
							 const auto received = _session->data().processBotApp(_bot->id, data.vapp());
							 ...
							 const auto confirm = data.is_inactive() || (confirmType != ConfirmType::None);
							 const auto writeAccess = result.data().is_request_write_access();
							 const auto forceConfirmation = data.is_inactive() || (confirmType == ConfirmType::Always);
							 ...
							 const auto done = crl::guard(this,
														  [=](Result value, auto)
														  {
															  if (value == Result::Cancelled) {
																  ...
																  confirmAppOpen(
																	  writeAccess,
																	  [=](bool allowWrite) { requestApp(allowWrite); },
																	  forceConfirmation);
															  } else {
																  requestApp(false);
															  }
...
void WebViewInstance::confirmOpen(Fn<void()> done) {
	if (_bot->isVerified() || _session->local().isPeerTrustedOpenWebView(_bot->id)) {
		done();
		return;
	}
	const auto callback = [=](Fn<void()> close)
			...
			Ui::ConfirmBox(
				box,
				{
					.text = tr::lng_profile_open_app_about(
						tr::now,
						lt_terms,
						Ui::Text::Link(tr::lng_profile_open_app_terms(tr::now), tr::lng_mini_apps_tos_url(tr::now)),
...
void WebViewInstance::confirmAppOpen(bool writeAccess, Fn<void(bool allowWrite)> done, bool forceConfirmation) {
	if (!forceConfirmation && (_bot->isVerified() || _session->local().isPeerTrustedOpenWebView(_bot->id))) {
		done(writeAccess);
		return;
	}
	_parentShow->show(Box(
		[=](not_null<Ui::GenericBox *> box)
		{
			...
			Ui::ConfirmBox(box,
						   {
							   tr::lng_profile_open_app_about(tr::now,
															  lt_terms,
															  Ui::Text::Link(tr::lng_profile_open_app_terms(tr::now),
																			 tr::lng_mini_apps_tos_url(tr::now)),
															  Ui::Text::RichLangValue),
				...
				(*allowed) = box->addRow(
					object_ptr<Ui::Checkbox>(box,
											 tr::lng_url_auth_allow_messages(
												 tr::now, lt_bot, Ui::Text::Bold(_bot->name()), Ui::Text::WithEntities),
											 true,
											 st::urlAuthCheckbox),
...
void WebViewInstance::requestButton() {
	...
	_requestId = _session->api()
					 .request(MTPmessages_RequestWebView(
						 MTP_flags(Flag::f_theme_params | (_context.fullscreen ? Flag::f_fullscreen : Flag(0)) |
								   (_button.url.isEmpty() ? Flag(0) : Flag::f_url) |
								   (_button.startCommand.isEmpty() ? Flag(0) : Flag::f_start_param) |
								   (v::is<WebViewSourceBotMenu>(_source) ? Flag::f_from_bot_menu : Flag(0)) |
						 ...
						 action.mtpReplyTo(),
						 (action.options.sendAs ? action.options.sendAs->input : MTP_inputPeerEmpty())))
					 .done(
...
void WebViewInstance::requestSimple() {
	...
	_requestId = _session->api()
					 .request(MTPmessages_RequestSimpleWebView(
						 MTP_flags(Flag::f_theme_params | (_context.fullscreen ? Flag::f_fullscreen : Flag(0)) |
								   (v::is<WebViewSourceSwitch>(_source) ? (Flag::f_url | Flag::f_from_switch_webview)
...
void WebViewInstance::requestMain() {
	...
	_requestId =
		_session->api()
			.request(MTPmessages_RequestMainWebView(
				MTP_flags(Flag::f_theme_params | (_context.fullscreen ? Flag::f_fullscreen : Flag(0)) |
						  (_button.startCommand.isEmpty() ? Flag() : Flag::f_start_param) |
						  (v::is<WebViewSourceLinkBotProfile>(_source)
							   ? (v::get<WebViewSourceLinkBotProfile>(_source).compact ? Flag::f_compact : Flag(0))
							   : Flag(0))),
...
void WebViewInstance::requestApp(bool allowWrite) {
	...
	const auto flags = Flag::f_theme_params | (_context.fullscreen ? Flag::f_fullscreen : Flag(0)) |
		(_appStartParam.isEmpty() ? Flag(0) : Flag::f_start_param) | (allowWrite ? Flag::f_write_allowed : Flag(0));
	_requestId =
		_session->api()
			.request(MTPmessages_RequestAppWebView(MTP_flags(flags),
												   _context.action->history->peer->input,
...
void WebViewInstance::requestWithMainMenuDisclaimer() {
	...
	_session->attachWebView().acceptMainMenuDisclaimer(_parentShow, _bot, done);
}

void WebViewInstance::requestWithMenuAdd() {
	using Result = AttachWebView::AddToMenuResult;
	const auto done = crl::guard(this,
								 [=](Result value, PeerTypes types)
								 {
									 if (value == Result::Cancelled) {
										 close();
									 } else if (value == Result::Unsupported) {
...
void WebViewInstance::show(ShowArgs &&args) {
	auto title = args.title.isEmpty() ? Info::Profile::NameValue(_bot) : rpl::single(args.title);
	auto titleBadge = _bot->isVerified() ? object_ptr<Ui::RpWidget>(_parentShow->toastParent()) : nullptr;
	if (titleBadge) {
	...
	const auto attached = ranges::find(bots, not_null{_bot}, &AttachWebViewBot::user);
	const auto hasOpenBot = v::is<WebViewSourceMainMenu>(_source) || (_context.action->history->peer != _bot);
	const auto hasRemoveFromMenu = (attached != end(bots)) && (!attached->inactive || attached->inMainMenu) &&
		(v::is<WebViewSourceMainMenu>(_source) || v::is<WebViewSourceAttachMenu>(_source) ||
		 v::is<WebViewSourceLinkAttachMenu>(_source));
	const auto buttons = (hasOpenBot ? Button::OpenBot : Button::None) |
		(!hasRemoveFromMenu			? Button::None
			 : attached->inMainMenu ? Button::RemoveFromMainMenu
	...
	const auto allowClipboardRead = v::is<WebViewSourceMainMenu>(_source) || v::is<WebViewSourceAttachMenu>(_source) ||
		(attached != end(bots) && (attached->inAttachMenu || attached->inMainMenu));
...
void WebViewInstance::close() { _session->attachWebView().close(this); }
...
void WebViewInstance::started(uint64 queryId) {
	...
	_session->data().webViewResultSent() |
		rpl::filter([=](const Data::Session::WebViewResultSent &sent) { return (sent.queryId == queryId); }) |
		rpl::start_with_next([=] { close(); }, _panel->lifetime());

	const auto action = *_context.action;
	base::timer_each(kProlongTimeout) |
		rpl::start_with_next(
			[=]
			{
				using Flag = MTPmessages_ProlongWebView::Flag;
				_session->api().request(base::take(_prolongId)).cancel();
				_prolongId = _session->api()
								 .request(MTPmessages_ProlongWebView(
									 MTP_flags(Flag(0) | (action.replyTo ? Flag::f_reply_to : Flag(0)) |
											   (action.options.sendAs ? Flag::f_send_as : Flag(0)) |
											   (action.options.silent ? Flag::f_silent : Flag(0))),
									 action.history->peer->input,
									 _bot->inputUser,
...
Webview::ThemeParams WebViewInstance::botThemeParams() {
	...
	if (const auto info = _bot->botInfo.get()) {
		...
		const auto &title = night ? info->botAppColorTitleNight : info->botAppColorTitleDay;
		const auto &body = night ? info->botAppColorBodyNight : info->botAppColorBodyDay;
		if (title.alpha() == 255) {
...
auto WebViewInstance::botDownloads(bool forceCheck) -> const std::vector<Ui::BotWebView::DownloadsEntry> & {
	return _session->attachWebView().downloads().list(_bot, forceCheck);
}

void WebViewInstance::botDownloadsAction(uint32 id, Ui::BotWebView::DownloadsAction type) {
...
bool WebViewInstance::botHandleLocalUri(QString uri, bool keepOpen) {
	...
	crl::on_main(
		[=]
		{
			...
			const auto window = !bot->session().windows().empty() ? bot->session().windows().front().get() : nullptr;
...
void WebViewInstance::botHandleInvoice(QString slug) {
	...
	Payments::CheckoutProcess::Start(&_bot->session(), slug, reactivate, nonPanelPaymentFormFactory(reactivate));
}

auto WebViewInstance::nonPanelPaymentFormFactory(Fn<void(Payments::CheckoutResult)> reactivate)
	...
	return [=](Payments::NonPanelPaymentForm form)
	{
		...
		v::match(
			form,
			[&](const CreditsFormDataPtr &form)
			{
				if (const auto strong = panel.get()) {
					ProcessCreditsPayment(uiShow(), strong->toastParent().get(), form, reactivate);
				}
			},
			...
			[&](RealFormPresentedNotification) { _panel->hideForPayment(); });
	};
}

void WebViewInstance::botHandleMenuButton(Ui::BotWebView::MenuButton button) {
	Expects(_panel != nullptr);
	...
	switch (button) {
		...
		case Button::RemoveFromMainMenu: {
			const auto &bots = _session->attachWebView().attachBots();
			const auto attached = ranges::find(bots, _bot, &AttachWebViewBot::user);
			const auto name = (attached != end(bots)) ? attached->name : _bot->name();
			const auto done = crl::guard(this,
										 [=]
										 {
			...
			_panel->showBox(Ui::MakeConfirmBox({
				(main ? tr::lng_bot_remove_from_side_menu_sure : tr::lng_bot_remove_from_menu_sure)(
					tr::now, lt_bot, Ui::Text::Bold(name), Ui::Text::WithEntities),
				done,
			}));
		} break;
		case Button::ShareGame: {
			const auto itemId =
				v::is<WebViewSourceGame>(_source) ? v::get<WebViewSourceGame>(_source).messageId : FullMsgId();
			if (!_panel || !itemId) {
				return;
...
bool WebViewInstance::botValidateExternalLink(QString uri) {
	...
	const auto allowed = _session->appConfig().get<std::vector<QString>>("web_app_allowed_protocols",
...
void WebViewInstance::botSendData(QByteArray data) {
	...
	if (!button || !button->simple || _context.action->history->peer != _bot || _dataSent) {
		return;
	}
	_dataSent = true;
	_session->api()
		.request(MTPmessages_SendWebViewData(
			_bot->inputUser, MTP_long(base::RandomValue<uint64>()), MTP_string(_button.text), MTP_bytes(data)))
		.done([session = _session](const MTPUpdates &result) { session->api().applyUpdates(result); })
		.send();
	botClose();
}

void WebViewInstance::botSwitchInlineQuery(std::vector<QString> chatTypes, QString query) {
	const auto controller = _context.controller.get();
	const auto types = PeerTypesFromNames(chatTypes);
	if (!_bot || !_bot->isBot() || _bot->botInfo->inlinePlaceholder.isEmpty() || !controller) {
		return;
	} else if (!types) {
		if (_context.dialogsEntryState.key.owningHistory()) {
			controller->switchInlineQuery(_context.dialogsEntryState, _bot, query);
		}
	} else {
		const auto bot = _bot;
		const auto done = [=](not_null<Data::Thread *> thread) { controller->switchInlineQuery(thread, bot, query); };
		ShowChooseBox(controller, types, done, tr::lng_inline_switch_choose());
	}
	botClose();
}
...
void WebViewInstance::botCheckWriteAccess(Fn<void(bool allowed)> callback) {
	_session->api()
		.request(MTPbots_CanSendMessage(_bot->inputUser))
		.done([=](const MTPBool &result) { callback(mtpIsTrue(result)); })
		.fail([=] { callback(false); })
		.send();
}
...
void WebViewInstance::botAllowWriteAccess(Fn<void(bool allowed)> callback) {
	_session->api()
		.request(MTPbots_AllowSendMessage(_bot->inputUser))
		.done(
			[session = _session, callback](const MTPUpdates &result)
			{
...
void WebViewInstance::botRequestEmojiStatusAccess(Fn<void(bool allowed)> callback) {
	if (_bot->botInfo->canManageEmojiStatus) {
		callback(true);
	} else if (const auto panel = _panel.get()) {
		const auto bot = _bot;
		panel->showBox(Box(ConfirmEmojiStatusAccessBox,
						   bot,
						   [=](bool ok)
						   {
							   if (!ok) {
							   ...
							   session->api()
								   .request(MTPbots_ToggleUserEmojiStatusPermission(bot->inputUser, MTP_bool(true)))
								   .done([=] { callback(true); })
								   .fail([=] { callback(false); })
								   .send();
						   }));
...
void WebViewInstance::botSharePhone(Fn<void(bool shared)> callback) {
	...
	_session->api().shareContact(_session->user(), action, std::move(callback));
}

void WebViewInstance::botInvokeCustomMethod(Ui::BotWebView::CustomMethodRequest request) {
	const auto callback = request.callback;
	_session->api()
		.request(MTPbots_InvokeWebViewCustomMethod(
			_bot->inputUser, MTP_string(request.method), MTP_dataJSON(MTP_bytes(request.params))))
		.done([=](const MTPDataJSON &result) { callback(result.data().vdata().v); })
		.fail([=](const MTP::Error &error) { callback(base::make_unexpected(error.type())); })
		.send();
}

void WebViewInstance::botSendPreparedMessage(Ui::BotWebView::SendPreparedMessageRequest request) {
	const auto bot = _bot;
	...
	_session->api()
		.request(MTPmessages_GetPreparedInlineMessage(bot->inputUser, MTP_string(request.id)))
		.done(
			[=](const MTPmessages_PreparedInlineMessage &result)
			{
				const auto panel = weak.get();
				const auto &data = result.data();
				bot->owner().processUsers(data.vusers());
				const auto parsed =
					std::shared_ptr<Result>(Result::Create(&bot->session(), data.vquery_id().v, data.vresult()));
				if (!parsed || !panel) {
					callback(u"UNKNOWN_ERROR"_q);
					return;
				}
				...
				const auto send = [=](std::vector<not_null<Data::Thread *>> list, Api::SendOptions options)
				{
					if (state->sent) {
						return;
					}
					...
					for (const auto &thread : list) {
						bot->session().api().sendInlineResult(
							bot, parsed.get(), Api::SendAction(thread, options), std::nullopt, done);
					}
				};
				auto box = Box(
					PreparedPreviewBox,
					...
					[=]
					{
						...
						auto box = Window::PrepareChooseRecipientBox(
							&bot->session(), chosen, tr::lng_inline_switch_choose(), nullptr, types, send);
					...
					[=](not_null<Data::Thread *> thread)
					{
						...
						state->send = [=](Api::SendOptions options)
						{
							...
							const auto checked = state->sendPayment.check(
								uiShow(), strong->peer(), 1, options.starsApproved, withPaymentApproved);
							if (!checked) {
								return;
							}
							state->send = nullptr;
		...
		.fail([=] { callback(u"MESSAGE_EXPIRED"_q); })
		.send();
}

void WebViewInstance::botSetEmojiStatus(Ui::BotWebView::SetEmojiStatusRequest request) {
	const auto bot = _bot;
	const auto panel = _panel.get();
	const auto callback = request.callback;
	...
	_session->data().customEmojiManager().resolve(request.customEmojiId) |
		rpl::start_with_next_error(
			[=](not_null<DocumentData *> document)
			{
				...
				const auto done = [=](bool success) { callback(success ? QString() : u"USER_DECLINED"_q); };
				panel->showBox(Box(ConfirmEmojiStatusBox, bot, document, duration, done));
			},
			[=] { callback(u"SUGGESTED_EMOJI_INVALID"_q); },
			panel->lifetime());
}

void WebViewInstance::botDownloadFile(Ui::BotWebView::DownloadFileRequest request) {
	const auto callback = request.callback;
	...
	_bot->session()
		.api()
		.request(MTPbots_CheckDownloadFileParams(_bot->inputUser, MTP_string(request.name), MTP_string(request.url)))
		.done(
			[=]
			{
				_panel->showBox(Box(DownloadFileBox,
									DownloadBoxArgs{
...
void WebViewInstance::botOpenPrivacyPolicy() {
	...
	const auto makeOtherContext = [=](bool forceWindow)
	{
		return QVariant::fromValue(ClickHandlerContext{
			.sessionWindow = (forceWindow ? WindowForThread(weak, bot->owner().history(bot)) : weak),
			.peer = bot,
		});
	};
	const auto sendCommand = [=]
	{
		...
		BotCommandClickHandler('/' + original).onClick(ClickContext{Qt::LeftButton, makeOtherContext(true)});
		return true;
	};
	const auto openUrl = [=](const QString &url)
	{ Core::App().iv().openWithIvPreferred(&_bot->session(), url, makeOtherContext(false)); };
	if (const auto info = _bot->botInfo.get()) {
...
std::shared_ptr<Main::SessionShow> WebViewInstance::uiShow() {
	...
	public:
		...
		[[nodiscard]] bool valid() const override { return _that && (_that->_panel != nullptr); }
...
AttachWebView::AttachWebView(not_null<Main::Session *> session)
	: _session(session), _downloads(std::make_unique<Downloads>(session)), _refreshTimer([=] { requestBots(); }) {
...
void AttachWebView::openByUsername(not_null<Window::SessionController *> controller,
	...
	if (botUsername.isEmpty() ||
		(_botUsername == botUsername && _startCommand == startCommand && _fullScreenRequested == fullscreen)) {
		return;
	}
	cancel();
...
void AttachWebView::close(not_null<WebViewInstance *> instance) {
	const auto i = ranges::find(_instances, instance.get(), &std::unique_ptr<WebViewInstance>::get);
	if (i != end(_instances)) {
...
void AttachWebView::loadPopularAppBots() {
	...
	_popularAppBotsRequestId = _session->api()
								   .request(MTPbots_GetPopularAppBots(MTP_string(), MTP_int(kPopularAppBotsLimit)))
								   .done(
									   [=](const MTPbots_PopularAppBots &result)
									   {
...
auto AttachWebView::popularAppBots() const -> const std::vector<not_null<UserData *>> & { return _popularAppBots; }

rpl::producer<> AttachWebView::popularAppBotsLoaded() const { return _popularAppBotsLoaded.changes() | rpl::to_empty; }
...
void AttachWebView::requestBots(Fn<void()> callback) {
	...
	_botsRequestId = _session->api()
						 .request(MTPmessages_GetAttachMenuBots(MTP_long(_botsHash)))
						 .done(
							 [=](const MTPAttachMenuBots &result)
							 {
								 _botsRequestId = 0;
...
bool AttachWebView::showMainMenuNewBadge(const AttachWebViewBot &bot) const {
	return bot.inMainMenu && bot.disclaimerRequired && !disclaimerAccepted(bot);
}

void AttachWebView::requestAddToMenu(not_null<UserData *> bot, Fn<void(AddToMenuResult, PeerTypes supported)> done) {
	...
	process.requestId = _session->api()
							.request(MTPmessages_GetAttachMenuBot(bot->inputUser))
							.done(
								[=](const MTPAttachMenuBotsBot &result)
								{
									...
									const auto i = ranges::find(_attachBots, not_null(bot), &AttachWebViewBot::user);
									...
									if (parsed->inactive) {
										confirmAddToMenu(*parsed,
														 [=](bool added)
														 {
															 const auto result = added ? AddToMenuResult::Added
							...
							.fail([=] { finish(AddToMenuResult::Unsupported, {}); })
							.send();
}

void AttachWebView::removeFromMenu(std::shared_ptr<Ui::Show> show, not_null<UserData *> bot) {
	toggleInMenu(bot,
				 ToggledState::Removed,
				 [=](bool success)
				 {
					 if (success) {
...
void AttachWebView::resolveUsername(std::shared_ptr<Ui::Show> show, Fn<void(not_null<PeerData *>)> done) {
	...
	_session->api().request(base::take(_requestId)).cancel();
	_requestId = _session->api()
					 .request(MTPcontacts_ResolveUsername(MTP_flags(0), MTP_string(_botUsername), MTP_string()))
					 .done(
					 ...
					 .fail(
						 [=](const MTP::Error &error)
						 {
							 _requestId = 0;
							 if (error.code() == 400) {
								 show->showToast(tr::lng_username_not_found(tr::now, lt_user, _botUsername));
							 }
						 })
					 .send();
}
...
void AttachWebView::open(WebViewDescriptor &&descriptor) {
	for (const auto &instance : _instances) {
		if (instance->bot() == descriptor.bot && instance->source() == descriptor.source) {
			instance->activate();
			return;
		}
	}
	_instances.push_back(std::make_unique<WebViewInstance>(std::move(descriptor)));
	_instances.back()->activate();
}

void AttachWebView::acceptMainMenuDisclaimer(std::shared_ptr<Ui::Show> show,
											 not_null<UserData *> bot,
	...
	show->show(Box(FillDisclaimerBox,
				   crl::guard(this,
							  [=](bool accepted)
							  {
								  if (accepted) {
...
void AttachWebView::confirmAddToMenu(AttachWebViewBot bot, Fn<void(bool added)> callback) {
	const auto active = Core::App().activeWindow();
	if (!active) {
		if (callback) {
			callback(false);
	...
	active->show(Box(
		[=](not_null<Ui::GenericBox *> box)
		{
			...
			{
				const auto state = (disclaimer || ((*allowed) && (*allowed)->checked())) ? ToggledState::AllowedToWrite
																						 : ToggledState::Added;
				toggleInMenu(bot.user,
							 state,
							 [=](bool success)
							 {
								 if (callback) {
								 ...
								 if (const auto strong = weak.get()) {
									 strong->showToast((bot.inMainMenu ? tr::lng_bot_add_to_side_menu_done
			...
			if (disclaimer) {
				...
				box->addRow(object_ptr<Ui::FixedHeightWidget>(box, st::boxRowPadding.left()));
				box->addRow(object_ptr<Ui::FlatLabel>(
					box,
					tr::lng_bot_will_be_added(lt_bot, rpl::single(Ui::Text::Bold(bot.name)), Ui::Text::WithEntities),
					st::boxLabel));
			} else {
				Ui::ConfirmBox(box, {
				(bot.inMainMenu
				...
				if (bot.requestWriteAccess) {
					(*allowed) = box->addRow(
						object_ptr<Ui::Checkbox>(box,
												 tr::lng_url_auth_allow_messages(
													 tr::now, lt_bot, Ui::Text::Bold(bot.name), Ui::Text::WithEntities),
												 true,
												 st::urlAuthCheckbox),
						style::margins(st::boxRowPadding.left(),
									   (disclaimer ? st::boxPhotoCaptionSkip : st::boxRowPadding.left()),
									   st::boxRowPadding.right(),
									   st::boxRowPadding.left()));
					(*allowed)->setAllowTextLines();
				}
...
void AttachWebView::toggleInMenu(not_null<UserData *> bot, ToggledState state, Fn<void(bool success)> callback) {
	using Flag = MTPmessages_ToggleBotInAttachMenu::Flag;
	_session->api()
		.request(MTPmessages_ToggleBotInAttachMenu(
			MTP_flags((state == ToggledState::AllowedToWrite) ? Flag::f_write_allowed : Flag()),
			bot->inputUser,
...
void ChooseAndSendLocation(not_null<Window::SessionController *> controller,
	...
	state->send = [=](Data::InputVenue venue, Api::SendAction action)
	{
		if (const auto strong = weak.get()) {
			...
			const auto checked = state->sendPayment.check(
				strong, action.history->peer, 1, action.options.starsApproved, withPaymentApproved);
			if (!checked) {
				return;
			}
		}
	...
	const auto callback = [=](Data::InputVenue venue) { state->send(venue, action); };
...
std::unique_ptr<Ui::DropdownMenu> MakeAttachBotsMenu(not_null<QWidget *> parent,
													 not_null<Window::SessionController *> controller,
													 not_null<PeerData *> peer,
													 Fn<Api::SendAction()> actionFactory,
													 Fn<void(bool)> attach) {
	auto result = std::make_unique<Ui::DropdownMenu>(parent, st::dropdownMenuWithIcons);
	const auto bots = &peer->session().attachWebView();
	const auto raw = result.get();
	auto minimal = 0;
	if (Data::CanSend(peer, ChatRestriction::SendPhotos, false)) {
		++minimal;
		raw->addAction(tr::lng_attach_photo_or_video(tr::now), [=] { attach(true); }, &st::menuIconPhoto);
	}
	const auto fileTypes = ChatRestriction::SendVideos | ChatRestriction::SendGifs | ChatRestriction::SendStickers |
		ChatRestriction::SendMusic | ChatRestriction::SendFiles;
	if (Data::CanSendAnyOf(peer, fileTypes)) {
		++minimal;
		raw->addAction(tr::lng_attach_document(tr::now), [=] { attach(false); }, &st::menuIconFile);
	}
	if (peer->canCreatePolls()) {
		...
		raw->addAction(
			...
			[=]
			{
				...
				const auto source = action.options.scheduled ? Api::SendType::Scheduled : Api::SendType::Normal;
				const auto sendMenuType = (action.replyTo.topicRootId || action.history->peer->starsPerMessageChecked())
					? SendMenu::Type::SilentOnly
					: SendMenu::Type::Scheduled;
				const auto flag = PollData::Flags();
				const auto replyTo = action.replyTo;
				Window::PeerMenuCreatePoll(controller, peer, replyTo, flag, flag, source, {sendMenuType});
			},
	...
	if (Data::CanSendAnyOf(peer, locationType) && Ui::LocationPicker::Available(config)) {
		raw->addAction(
			tr::lng_maps_point(tr::now),
			[=] { ChooseAndSendLocation(controller, config, actionFactory()); },
			&st::menuIconAddress);
	}
	const auto addBots = Data::CanSend(peer, ChatRestriction::SendInline) && !peer->starsPerMessageChecked();
	for (const auto &bot : bots->attachBots()) {
		if (!addBots || !bot.inAttachMenu || !PeerMatchesTypes(peer, bot.user, bot.types)) {
			continue;
		}
		const auto callback = [=]
		{
		...
		auto action = base::make_unique_q<BotAction>(raw, controller->uiShow(), raw->menu()->st(), bot, callback);
		action->forceShown() |
			rpl::start_with_next(
				[=](bool shown)
				{
