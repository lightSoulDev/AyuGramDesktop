...
bool IsGlobalLink(const QString &link) {
	return link.startsWith(u"http://"_q, Qt::CaseInsensitive) || link.startsWith(u"https://"_q, Qt::CaseInsensitive);
}

QByteArray NoFileDescription(Data::File::SkipReason reason) {
	using SkipReason = Data::File::SkipReason;
	switch (reason) {
		case SkipReason::Unavailable: return "Unavailable, please try again later.";
...
auto CalculateThumbSize(int maxWidth, int maxHeight, int minWidth, int minHeight, bool expandForRetina = false) {
	return [=](QSize largeSize)
	{
		...
		const auto smallSize = (checkWidth > maxWidth || checkHeight > maxHeight)
			? largeSize.scaled(maxWidth, maxHeight, Qt::KeepAspectRatio)
			: largeSize;
		const auto retinaSize = QSize(smallSize.width() & ~0x01, smallSize.height() & ~0x01);
		return (retinaSize.width() < kPhotoMinWidth || retinaSize.height() < kPhotoMinHeight) ? QSize() : retinaSize;
	};
}
...
QByteArray JoinList(const QByteArray &separator, const std::vector<QByteArray> &list) {
	if (list.empty()) {
		return QByteArray();
	} else if (list.size() == 1) {
		return list[0];
...
QByteArray
FormatCustomEmoji(const Data::Utf8String &custom_emoji, const QByteArray &text, const QString &relativeLinkBase) {
	return (custom_emoji.isEmpty() ? "<a href=\"\" onclick=\"return ShowNotLoadedEmoji();\">"
...
QByteArray FormatText(const std::vector<Data::TextPart> &data,
	...
	return JoinList(QByteArray(),
					ranges::views::all(data) |
						ranges::views::transform(
							[&](const Data::TextPart &part)
							{
								...
								switch (part.type) {
									...
									case Type::Hashtag:
										return "<a href=\"\" "
											...
											SerializeString('"' + text.mid(1) + '"') + ")\">" + text + "</a>";
									case Type::BotCommand:
										return "<a href=\"\" "
											   "onclick=\"return ShowBotCommand(" +
											SerializeString('"' + text.mid(1) + '"') + ")\">" + text + "</a>";
									case Type::Url: return "<a href=\"" + text + "\">" + text + "</a>";
									case Type::Email: return "<a href=\"mailto:" + text + "\">" + text + "</a>";
									...
									case Type::TextUrl:
										return "<a href=\"" + SerializeString(part.additional) + "\">" + text + "</a>";
									case Type::MentionName:
										return "<a href=\"\" "
											   "onclick=\"return ShowMentionName()\">" +
											text + "</a>";
									case Type::Phone: return "<a href=\"tel:" + text + "\">" + text + "</a>";
									case Type::Cashtag:
										return "<a href=\"\" "
									...
									case Type::Blockquote: return "<blockquote>" + text + "</blockquote>";
									case Type::BankCard: return text;
									case Type::Spoiler:
...
bool DisplayDate(TimeId date, TimeId previousDate) {
	...
	return QDateTime::fromSecsSinceEpoch(date).date() != QDateTime::fromSecsSinceEpoch(previousDate).date();
}
...
QByteArray FormatDateText(TimeId date) {
	...
	return Data::NumberToString(parsed.day()) + ' ' + month(parsed.month()) + ' ' + Data::NumberToString(parsed.year());
}

QByteArray FormatTimeText(TimeId date) {
	const auto parsed = QDateTime::fromSecsSinceEpoch(date).time();
	return Data::NumberToString(parsed.hour(), 2) + ':' + Data::NumberToString(parsed.minute(), 2);
}
...
QByteArray PeersMap::wrapUserName(UserId userId) const {
	...
	return result.isEmpty() ? QByteArray("Deleted Account") : SerializeString(result);
}
...
QByteArray HtmlContext::pushTag(const QByteArray &tag, std::map<QByteArray, QByteArray> &&attributes) {
	auto data = Tag();
	data.name = tag;
	auto empty = false;
	...
	auto result = (data.block ? ("\n" + indent()) : QByteArray()) + "<" + data.name + inner + (empty ? "/" : "") + ">" +
		(data.block ? "\n" : "");
	if (!empty) {
...
QByteArray HtmlContext::popTag() {
	...
	_tags.pop_back();
	return (data.block ? ("\n" + indent()) : QByteArray()) + "</" + data.name + ">" + (data.block ? "\n" : "");
}
...
public:
	...
	[[nodiscard]] QByteArray pushTag(const QByteArray &tag, std::map<QByteArray, QByteArray> &&attributes = {});
	[[nodiscard]] QByteArray popTag();
	[[nodiscard]] QByteArray indent() const;

	[[nodiscard]] QByteArray pushDiv(const QByteArray &className, const QByteArray &style = {});
	...
	[[nodiscard]] QByteArray pushStoriesListEntry(const StoryData &story,
												  const QByteArray &name,
												  ...
												  const QString &internalLinksDomain,
												  const QString &link = QString());
	[[nodiscard]] QByteArray pushSessionListEntry(int apiId,
												  const QByteArray &name,
												  const QByteArray &subname,
												  std::initializer_list<QByteArray> details,
												  const QByteArray &info = QByteArray());

	[[nodiscard]] QByteArray pushHeader(const QByteArray &header, const QString &path = QString());
	[[nodiscard]] QByteArray
	pushSection(const QByteArray &label, const QByteArray &type, int count, const QString &path);
	[[nodiscard]] QByteArray pushAbout(const QByteArray &text, bool withDivider = false);
	[[nodiscard]] QByteArray pushServiceMessage(int messageId,
												const Data::DialogInfo &dialog,
												const QString &basePath,
												const QByteArray &text,
												const Data::Photo *photo = nullptr);
	[[nodiscard]] std::pair<MessageInfo, QByteArray>
	pushMessage(const Data::Message &message,
...
private:
	...
	[[nodiscard]] QByteArray pushGenericListEntry(const QString &link,
	...
	[[nodiscard]] bool messageNeedsWrap(const Data::Message &message, const MessageInfo *previous) const;
	[[nodiscard]] bool forwardedNeedsWrap(const Data::Message &message, const MessageInfo *previous) const;

	[[nodiscard]] MediaData prepareMediaData(const Data::Message &message,
											 const QString &basePath,
											 const PeersMap &peers,
											 const QString &internalLinksDomain) const;
	[[nodiscard]] QByteArray pushMedia(const Data::Message &message,
									   const QString &basePath,
									   const PeersMap &peers,
									   const QString &internalLinksDomain,
									   Fn<QByteArray(int messageId, QByteArray text)> wrapMessageLink);
	[[nodiscard]] QByteArray pushGenericMedia(const MediaData &data);
	[[nodiscard]] QByteArray pushStickerMedia(const Data::Document &data, const QString &basePath);
	[[nodiscard]] QByteArray pushAnimatedMedia(const Data::Document &data, const QString &basePath);
	[[nodiscard]] QByteArray pushVideoFileMedia(const Data::Document &data, const QString &basePath);
	[[nodiscard]] QByteArray pushPhotoMedia(const Data::Photo &data, const QString &basePath);
	[[nodiscard]] QByteArray pushPoll(const Data::Poll &data);
	[[nodiscard]] QByteArray pushGiveaway(const PeersMap &peers, const Data::GiveawayStart &data);
...
QByteArray ComposeName(const UserpicData &data, const QByteArray &empty) {
	return ((data.firstName.isEmpty() && data.lastName.isEmpty()) ? empty : (data.firstName + ' ' + data.lastName));
}

QString WriteUserpicThumb(const QString &basePath,
						  const QString &largePath,
						  const UserpicData &userpic,
						  const QString &postfix = "_thumb") {
	return Data::WriteImageThumb(basePath, largePath, userpic.pixelSize * 2, userpic.pixelSize * 2, postfix);
}

HtmlWriter::Wrap::Wrap(const QString &path, const QString &base, Stats *stats) : _file(path, stats) {
...
QByteArray HtmlWriter::Wrap::pushTag(const QByteArray &tag, std::map<QByteArray, QByteArray> &&attributes) {
...
QByteArray HtmlWriter::Wrap::pushDiv(const QByteArray &className, const QByteArray &style) {
	return style.isEmpty() ? _context.pushTag("div", {{"class", className}})
						   : _context.pushTag("div", {{"class", className}, {"style", style}});
}
...
QByteArray HtmlWriter::Wrap::pushUserpic(const UserpicData &userpic) {
	...
	if (!userpic.largeLink.isEmpty()) {
		result.append(pushTag("a", {{"class", "userpic_link"}, {"href", relativePath(userpic.largeLink).toUtf8()}}));
	}
	...
	if (!userpic.imageLink.isEmpty()) {
		...
		result.append(pushTag(
			"div",
			{{"class", "userpic userpic" + Data::NumberToString(userpic.colorIndex + 1)}, {"style", sizeStyle}}));
		...
		auto character = [](const QByteArray &from)
		{
			...
			return utf.isEmpty() ? QByteArray() : SerializeString(utf.mid(0, 1).toUtf8());
		};
...
QByteArray HtmlWriter::Wrap::pushStoriesListEntry(const StoryData &story,
												  const QByteArray &name,
												  const QByteArrayList &details,
												  const QByteArray &info,
												  const std::vector<Data::TextPart> &caption,
	...
	if (!story.imageLink.isEmpty()) {
		const auto sizeStyle = "width: " + Data::NumberToString(kStoryThumbWidth) +
	...
	const auto text = caption.empty() ? QByteArray() : FormatText(caption, internalLinksDomain, _base);
	if (!text.isEmpty()) {
...
QByteArray HtmlWriter::Wrap::pushSessionListEntry(int apiId,
												  const QByteArray &name,
												  const QByteArray &subname,
												  std::initializer_list<QByteArray> details,
												  const QByteArray &info) {
	const auto link = QString();
	auto userpic = UserpicData{Data::ApplicationColorIndex(apiId), kEntryUserpicSize};
	userpic.firstName = name;
	return pushGenericListEntry(link, userpic, name, subname, details, info);
}

QByteArray HtmlWriter::Wrap::pushGenericListEntry(const QString &link,
												  const UserpicData &userpic,
												  const QByteArray &name,
												  const QByteArray &subname,
												  std::initializer_list<QByteArray> details,
												  const QByteArray &info) {
	auto result = link.isEmpty() ? pushDiv("entry clearfix")
								 : pushTag("a",
										   {
...
QByteArray HtmlWriter::Wrap::pushHeader(const QByteArray &header, const QString &path) {
	auto result = pushDiv("page_header");
	result.append(path.isEmpty() ? pushDiv("content")
								 : pushTag("a",
										   {
...
QByteArray
HtmlWriter::Wrap::pushSection(const QByteArray &header, const QByteArray &type, int count, const QString &link) {
...
QByteArray HtmlWriter::Wrap::pushAbout(const QByteArray &text, bool withDivider) {
	auto result = pushDiv(withDivider ? "page_about details with_divider" : "page_about details");
...
QByteArray HtmlWriter::Wrap::pushServiceMessage(int messageId,
	...
	auto result = pushTag("div", {{"class", "message service"}, {"id", "message" + Data::NumberToString(messageId)}});
...
auto HtmlWriter::Wrap::pushMessage(const Data::Message &message,
								   const MessageInfo *previous,
								   const Data::DialogInfo &dialog,
								   const QString &basePath,
								   const PeersMap &peers,
	...
	const auto wrapReplyToLink = [&](const QByteArray &text) { return wrapMessageLink(message.replyToMsgId, text); };

	using DialogType = Data::DialogInfo::Type;
	const auto isChannel = (dialog.type == DialogType::PrivateChannel) || (dialog.type == DialogType::PublicChannel);
	const auto serviceFrom = peers.wrapPeerName(message.fromId);
	const auto serviceText = v::match(
		message.action.content,
		[&](const ActionChatCreate &data)
		{
			return serviceFrom + " created group &laquo;" + SerializeString(data.title) + "&raquo;" +
				(data.userIds.empty() ? QByteArray() : " with members " + peers.wrapUserNames(data.userIds));
		},
		[&](const ActionChatEditTitle &data)
		{
			return isChannel
				? ("Channel title changed to &laquo;" + SerializeString(data.title) + "&raquo;")
				: (serviceFrom + " changed group title to &laquo;" + SerializeString(data.title) + "&raquo;");
		},
		[&](const ActionChatEditPhoto &data)
		{ return isChannel ? QByteArray("Channel photo changed") : (serviceFrom + " changed group photo"); },
		[&](const ActionChatDeletePhoto &data)
		{ return isChannel ? QByteArray("Channel photo removed") : (serviceFrom + " removed group photo"); },
		[&](const ActionChatAddUser &data) { return serviceFrom + " invited " + peers.wrapUserNames(data.userIds); },
		[&](const ActionChatDeleteUser &data) { return serviceFrom + " removed " + peers.wrapUserName(data.userId); },
		[&](const ActionChatJoinedByLink &data)
		{ return serviceFrom + " joined group by link from " + peers.wrapUserName(data.inviterId); },
		[&](const ActionChannelCreate &data)
		{ return "Channel &laquo;" + SerializeString(data.title) + "&raquo; created"; },
		[&](const ActionChatMigrateTo &data) { return serviceFrom + " converted this group to a supergroup"; },
		[&](const ActionChannelMigrateFrom &data)
		{
			return serviceFrom + " converted a basic group to this supergroup " + "&laquo;" +
				SerializeString(data.title) + "&raquo;";
		},
		[&](const ActionPinMessage &data) { return serviceFrom + " pinned " + wrapReplyToLink("this message"); },
		[&](const ActionHistoryClear &data) { return QByteArray("History cleared"); },
		[&](const ActionGameScore &data)
		{ return serviceFrom + " scored " + NumberToString(data.score) + " in " + wrapReplyToLink("this game"); },
		[&](const ActionPaymentSent &data)
		{
			...
			auto result = "You have successfully transferred " + amount + " for " + wrapReplyToLink("this invoice");
		...
		[&](const ActionScreenshotTaken &data) { return serviceFrom + " took a screenshot"; },
		...
		[&](const ActionBotAllowed &data)
		{
			return data.attachMenu ? "You allowed this bot to message you "
				...
				: data.fromRequest ? "You allowed this bot to message you in his web-app."_q
				: data.app.isEmpty()
				? ("You allowed this bot to message you when you opened " + SerializeString(data.app))
				: ("You allowed this bot to message you when you logged in on " + SerializeString(data.domain));
		},
				...
				list.push_back(
					[&]
					{
						...
						switch (type) {
							...
							case Type::PassportRegistration: return "Passport registration";
							case Type::TemporaryRegistration: return "Temporary registration";
		...
		[&](const ActionContactSignUp &data) { return serviceFrom + " joined Telegram"; },
		...
		[&](const ActionPhoneNumberRequest &data) { return serviceFrom + " requested your phone number"; },
		[&](const ActionGroupCall &data)
		{
			const auto durationText =
				(data.duration ? (" (" + QString::number(data.duration) + " seconds)") : QString()).toUtf8();
			return isChannel ? ("Voice chat" + durationText) : (serviceFrom + " started voice chat" + durationText);
		},
		[&](const ActionInviteToGroupCall &data)
		{ return serviceFrom + " invited " + peers.wrapUserNames(data.userIds) + " to the voice chat"; },
		[&](const ActionSetMessagesTTL &data)
		{
			const auto periodText = (data.period == 7 * 86400) ? "7 days"
				: (data.period == 86400)					   ? "24 hours"
															   : QByteArray();
			return isChannel
				? (data.period ? "New messages will auto-delete in " + periodText : "New messages will not auto-delete")
				: (data.period ? (serviceFrom + " has set messages to auto-delete in " + periodText)
		...
		[&](const ActionGroupCallScheduled &data)
		{
			...
			return isChannel ? ("Voice chat scheduled for " + dateText)
		...
		[&](const ActionSetChatTheme &data)
		{
			...
			return isChannel ? ("Channel theme was changed to " + data.emoji).toUtf8()
							 : (serviceFrom + " changed chat theme to " + data.emoji).toUtf8();
		},
		[&](const ActionChatJoinedByRequest &data) { return serviceFrom + " joined group by request"; },
		[&](const ActionWebViewDataSent &data)
		{
			return "You have just successfully transferred data from the &laquo;" + SerializeString(data.text) +
		...
		[&](const ActionGiftPremium &data)
		{
			...
			return serviceFrom + " sent you a gift for " + data.cost + ": Telegram Premium for " +
				QString::number(data.months).toUtf8() + " months.";
		},
		[&](const ActionTopicCreate &data)
		{ return serviceFrom + " created topic &laquo;" + SerializeString(data.title) + "&raquo;"; },
		[&](const ActionTopicEdit &data)
		{
			auto parts = QList<QByteArray>();
			if (!data.title.isEmpty()) {
				parts.push_back("title to &laquo;" + SerializeString(data.title) + "&raquo;");
			}
			if (data.iconEmojiId) {
				parts.push_back("icon to &laquo;" + QString::number(*data.iconEmojiId).toUtf8() + "&raquo;");
			}
			return serviceFrom + " changed topic " + parts.join(',');
		},
		[&](const ActionSuggestProfilePhoto &data) { return serviceFrom + " suggests to use this photo"; },
		[&](const ActionRequestedPeer &data) { return "requested: "_q /* + data.peerId*/; },
		[&](const ActionSetChatWallPaper &data)
		{
			return serviceFrom +
				(data.same ? (" set " + wrapReplyToLink("the same background") + " for this chat")
						   : " set a new background for this chat");
		},
		[&](const ActionGiftCode &data)
		{
			return data.unclaimed
				? ("This is an unclaimed Telegram Premium for " + NumberToString(data.months) +
				   (data.months > 1 ? " months" : "month") + " prize in a giveaway organized by a channel.")
				: data.viaGiveaway
				? ("You won a Telegram Premium for " + NumberToString(data.months) +
				   (data.months > 1 ? " months" : "month") + " prize in a giveaway organized by a channel.")
				: ("You've received a Telegram Premium for " + NumberToString(data.months) +
				   (data.months > 1 ? " months" : "month") + " gift from a channel.");
		},
		...
		[&](const ActionGiveawayResults &data)
		{
			return !data.winners					? "No winners of the giveaway could be selected."
				: (data.credits && data.unclaimed)	? "Some winners of the giveaway were randomly selected by "
													  "Telegram and received their prize."
				: (!data.credits && data.unclaimed) ? "Some winners of the giveaway were randomly selected by "
													  "Telegram and received private messages with giftcodes."
				: (data.credits && !data.unclaimed) ? NumberToString(data.winners) +
		...
		{
			return serviceFrom + " boosted the group " + QByteArray::number(data.boosts) +
				(data.boosts > 1 ? " times" : " time");
		},
		[&](const ActionPaymentRefunded &data)
		{
			const auto amount = FormatMoneyAmount(data.amount, data.currency);
			auto result = peers.wrapPeerName(data.peerId) + " refunded back " + amount;
			return result;
		},
		[&](const ActionGiftStars &data)
		{
			...
			return serviceFrom + " sent you a gift for " + data.cost + ": " + QString::number(data.credits).toUtf8() +
				" Telegram Stars.";
		},
		[&](const ActionPrizeStars &data)
		{
			return "You won a prize in a giveaway organized by " + peers.wrapPeerName(data.peerId) +
				".\n Your prize is " + QString::number(data.amount).toUtf8() + " Telegram Stars.";
		},
		[&](const ActionStarGift &data)
		{ return serviceFrom + " sent you a gift of " + QByteArray::number(data.stars) + " Telegram Stars."; },
		[](v::null_t) { return QByteArray(); });

	if (!serviceText.isEmpty()) {
		const auto &content = message.action.content;
		const auto photo = v::is<ActionChatEditPhoto>(content) ? &v::get<ActionChatEditPhoto>(content).photo
			: v::is<ActionSuggestProfilePhoto>(content)		   ? &v::get<ActionSuggestProfilePhoto>(content).photo
															   : nullptr;
		return {info, pushServiceMessage(message.id, dialog, basePath, serviceText, photo)};
	}
	info.type = MessageInfo::Type::Default;

	const auto wrap = messageNeedsWrap(message, previous);
	const auto fromPeerId = message.fromId;
	const auto showForwardedInfo = message.forwarded && !message.showForwardedAsOriginal;
	auto forwardedUserpic = UserpicData();
	if (message.forwarded) {
		forwardedUserpic.colorIndex =
			message.forwardedFromId ? PeerColorIndex(message.forwardedFromId) : PeerColorIndex(message.id);
	...
	const auto className = wrap ? "message default clearfix" : "message default clearfix joined";
	auto block = pushTag("div", {{"class", className}, {"id", "message" + NumberToString(message.id)}});
	if (wrap) {
	...
	if (wrap) {
		...
		if (!via.isEmpty() && (!message.forwarded || message.showForwardedAsOriginal)) {
	...
	if (showForwardedInfo) {
		...
		if (forwardedWrap) {
			block.append(pushDiv("from_name"));
			block.append(SerializeString(ComposeName(forwardedUserpic, "Deleted Account")));
			if (!via.isEmpty()) {
				block.append(" via @" + via);
			}
			block.append(pushTag(
				"span",
				{{"class", "date details"}, {"title", FormatDateTime(message.forwardedDate, true)}, {"inline", ""}}));
	...
	if (!message.inlineButtonRows.empty()) {
		...
		for (const auto &row : message.inlineButtonRows) {
			...
			for (const auto &button : row) {
				...
				const auto content = (!button.data.isEmpty() ? (u"Data: "_q + button.data + endline) : QString()) +
					(!button.forwardText.isEmpty() ? (u"Forward text: "_q + button.forwardText + endline) : QString()) +
					(u"Type: "_q + HistoryMessageMarkupButton::TypeToString(button));
				const auto link = (button.type == Type::Url) ? button.data : QByteArray();
				const auto onclick =
					(button.type != Type::Url) ? ("return ShowTextCopied('" + content + "');").toUtf8() : QByteArray();
				...
				if (&button != &row.back()) {
					block.append(pushTag("div", {{"class", "bot_button_column_separator"}}));
					block.append(popTag());
				}
			}
			block.append(popTag());
	...
	if (!message.reactions.empty()) {
		...
		for (const auto &reaction : message.reactions) {
			...
			switch (reaction.type) {
				case Reaction::Type::Emoji: block.append(SerializeString(reaction.emoji.toUtf8())); break;
				case Reaction::Type::CustomEmoji:
					block.append(FormatCustomEmoji(reaction.documentId, "\U0001F44B", _base));
					break;
				case Reaction::Type::Paid: block.append(SerializeString("\u2B50")); break;
			}
			block.append(popTag());
			if (!reaction.recent.empty()) {
				...
				for (const auto &recent : reaction.recent) {
					...
					block.append(pushUserpic(UserpicData({
						...
						.pixelSize = 20,
						.firstName = peer.user() ? peer.user()->info.firstName : peer.name(),
...
bool HtmlWriter::Wrap::messageNeedsWrap(const Data::Message &message, const MessageInfo *previous) const {
	if (!previous) {
		return true;
		...
		return true;
			   ...
			   ((message.forwardedFromId || !message.forwardedFromName.isEmpty()) ? 1 : kJoinWithinSeconds)) {
		return true;
	}
	return false;
}

QByteArray HtmlWriter::Wrap::pushMedia(const Data::Message &message,
									   const QString &basePath,
									   const PeersMap &peers,
									   const QString &internalLinksDomain,
									   Fn<QByteArray(int messageId, QByteArray text)> wrapMessageLink) {
	const auto data = prepareMediaData(message, basePath, peers, internalLinksDomain);
...
QByteArray HtmlWriter::Wrap::pushGenericMedia(const MediaData &data) {
	...
	if (data.link.isEmpty()) {
		...
		result.append(
			pushTag("a",
					{{"class", "media clearfix pull_left block_link " + data.classes},
					 {"href", (IsGlobalLink(data.link) ? data.link.toUtf8() : relativePath(data.link).toUtf8())}}));
	}
	if (data.thumb.isEmpty()) {
		...
		result.append(
			pushTag("img", {{"class", "thumb pull_left"}, {"src", relativePath(data.thumb).toUtf8()}, {"empty", ""}}));
	}
	result.append(pushDiv("body"));
	if (!data.title.isEmpty()) {
...
QByteArray HtmlWriter::Wrap::pushStickerMedia(const Data::Document &data, const QString &basePath) {
	using namespace Data;

	const auto &[thumb, size] =
		WriteImageThumb(basePath,
	...
	auto result = pushDiv("media_wrap clearfix");
	result.append(pushTag(
		"a", {{"class", "sticker_wrap clearfix pull_left"}, {"href", relativePath(data.file.relativePath).toUtf8()}}));
	const auto sizeStyle =
		"width: " + NumberToString(size.width() / 2) + "px; height: " + NumberToString(size.height() / 2) + "px";
	result.append(pushTag(
		"img", {{"class", "sticker"}, {"style", sizeStyle}, {"src", relativePath(thumb).toUtf8()}, {"empty", ""}}));
	result.append(popTag());
	result.append(popTag());
	return result;
}

QByteArray HtmlWriter::Wrap::pushAnimatedMedia(const Data::Document &data, const QString &basePath) {
	using namespace Data;

	auto size = QSize(data.width, data.height);
	auto thumbSize = CalculateThumbSize(kPhotoMaxWidth, kPhotoMaxHeight, kPhotoMinWidth, kPhotoMinHeight, true)(size);
	if (data.thumb.file.relativePath.isEmpty() || data.file.relativePath.isEmpty() || !thumbSize.width() ||
		!thumbSize.height()) {
		auto generic = MediaData();
	...
	auto result = pushDiv("media_wrap clearfix");
	result.append(pushTag(
		"a", {{"class", "animated_wrap clearfix pull_left"}, {"href", relativePath(data.file.relativePath).toUtf8()}}));
	result.append(pushDiv("video_play_bg"));
	result.append(pushDiv("gif_play"));
	result.append("GIF");
	result.append(popTag());
	result.append(popTag());
	const auto sizeStyle = "width: " + NumberToString(thumbSize.width() / 2) +
...
QByteArray HtmlWriter::Wrap::pushVideoFileMedia(const Data::Document &data, const QString &basePath) {
	using namespace Data;
	...
	result.append(pushTag(
		"a",
		{{"class", "video_file_wrap clearfix pull_left"}, {"href", relativePath(data.file.relativePath).toUtf8()}}));
	result.append(pushDiv("video_play_bg"));
...
QByteArray HtmlWriter::Wrap::pushPhotoMedia(const Data::Photo &data, const QString &basePath) {
	using namespace Data;

	const auto &[thumb, size] =
		WriteImageThumb(basePath,
						data.image.file.relativePath,
						CalculateThumbSize(kPhotoMaxWidth, kPhotoMaxHeight, kPhotoMinWidth, kPhotoMinHeight));
	if (thumb.isEmpty()) {
		auto generic = MediaData();
		generic.title = "Photo";
		generic.status = Ui::FormatImageSizeText(QSize(data.image.width, data.image.height)).toUtf8();
	...
	result.append(pushTag(
		"a",
		{{"class", "photo_wrap clearfix pull_left"}, {"href", relativePath(data.image.file.relativePath).toUtf8()}}));
	const auto sizeStyle =
		"width: " + NumberToString(size.width() / 2) + "px; height: " + NumberToString(size.height() / 2) + "px";
	result.append(pushTag(
		"img", {{"class", "photo"}, {"style", sizeStyle}, {"src", relativePath(thumb).toUtf8()}, {"empty", ""}}));
	result.append(popTag());
	result.append(popTag());
	return result;
}
...
QByteArray HtmlWriter::Wrap::pushPoll(const Data::Poll &data) {
	...
	const auto details = [&](const Poll::Answer &answer)
	{
		...
		return " <span class=\"details\">" + votes(answer.votes) + ", chosen vote</span>";
	};
...
QByteArray HtmlWriter::Wrap::pushGiveaway(const PeersMap &peers, const Data::GiveawayStart &data) {
	auto result = pushDiv("media_wrap clearfix");
	result.append(pushDiv("media_giveaway"));

	result.append(pushDiv("section_title bold"));
	result.append((data.quantity > 1) ? SerializeString("Giveaway Prizes") : SerializeString("Giveaway Prize"));
	result.append(popTag());
	...
	result.append(pushDiv("section_body"));
	if (data.credits > 0) {
		result.append("<b>" + Data::NumberToString(data.credits) +
					  (SerializeString(data.credits == 1 ? (" Star") : (" Stars"))) + "</b> " +
					  SerializeString("will be distributed ") +
					  ((data.quantity == 1) ? SerializeString("to ") + "<b>" + Data::NumberToString(data.quantity) +
							   "</b> " + SerializeString("winner.")
											: SerializeString("among ") + "<b>" + Data::NumberToString(data.quantity) +
							   "</b> " + SerializeString("winners.")));
	} else {
		result.append(
			"<b>" + Data::NumberToString(data.quantity) + "</b> " +
			SerializeString((data.quantity > 1) ? "Telegram Premium Subscriptions" : "Telegram Premium Subscription") +
			" for <b>" + Data::NumberToString(data.months) + "</b> " + (data.months > 1 ? "months." : "month."));
	}
	result.append(popTag());
...

	{
		...
		if (const auto count = countries.size()) {
			...
			for (auto i = 1; i != count; ++i) {
				united = ((i + 1 == count) ? u"%1 and %2"_q : u"%1, %2"_q).arg(united, countries[i]);
			}
			result.append(pushDiv("section_body"));
...
QByteArray HtmlWriter::Wrap::pushGiveaway(const PeersMap &peers,
	...
	result.append(pushDiv("section_title bold"));
	result.append((data.winnersCount > 1) ? SerializeString("Winners Selected!") : SerializeString("Winner Selected!"));
	result.append(popTag());

	result.append(pushDiv("section_body"));
	result.append("<b>" + Data::NumberToString(data.winnersCount) + "</b> " +
				  SerializeString((data.winnersCount > 1) ? "winners" : "winner") + " of the " +
				  wrapMessageLink(data.launchId, "Giveaway") + " was randomly selected by Telegram.");
	result.append(popTag());

	result.append(pushDiv("section_title bold"));
	result.append((data.winnersCount > 1) ? SerializeString("Winners") : SerializeString("Winner"));
	result.append(popTag());
	...
	const auto andMore = [&, size = data.winners.size()]
	{
		if (data.winnersCount > size) {
			return SerializeString(" and ") + Data::NumberToString(data.winnersCount - size) +
	...
	const auto prize = [&, singleStar = (data.credits == 1)]
	{
		if (data.credits && data.winnersCount == 1) {
			return SerializeString("The winner received ") + "<b>" + Data::NumberToString(data.credits) + "</b>" +
				SerializeString(singleStar ? " Star." : " Stars.");
		} else if (data.credits && data.winnersCount > 1) {
			return SerializeString("All winners received ") + "<b>" + Data::NumberToString(data.credits) + "</b>" +
			...
			return SerializeString("The winner received their gift link in a private message.");
		} else if (data.winnersCount > 1) {
			return SerializeString("All winners received gift links in private messages.");
		}
		return QByteArray();
	}();
	result.append(prize);
...
MediaData HtmlWriter::Wrap::prepareMediaData(const Data::Message &message,
											 const QString &basePath,
											 const PeersMap &peers,
											 const QString &internalLinksDomain) const {
	using namespace Data;
	...
	if (const auto call = std::get_if<ActionPhoneCall>(&action.content)) {
		...
		result.title = peers.peer(message.out ? message.peerId : message.selfId).name();
		result.status = [&]
		{
	...
	v::match(
		...
		[&](const Document &data)
		{
			...
			if (data.isSticker) {
				...
				result.title = (data.songPerformer.isEmpty() || data.songTitle.isEmpty())
		...
		[&](const Game &data)
		{
			...
			if (data.botId != 0 && !data.shortName.isEmpty()) {
				...
				if (bot.isBot && !bot.username.isEmpty()) {
					const auto link = internalLinksDomain.toUtf8() + bot.username + "?game=" + data.shortName;
					result.link = link;
					result.status = link;
				}
			}
...
bool HtmlWriter::Wrap::forwardedNeedsWrap(const Data::Message &message, const MessageInfo *previous) const {
	Expects(message.forwarded);

	if (messageNeedsWrap(message, previous)) {
		return true;
	} else if (!message.forwardedFromId || message.forwardedFromId != previous->forwardedFromId) {
		return true;
	} else if (!peerIsUser(message.forwardedFromId)) {
		return true;
	} else if (abs(message.forwardedDate - previous->forwardedDate) > kJoinWithinSeconds) {
		return true;
	}
	return false;
}
...
QString HtmlWriter::Wrap::relativePath(const QString &path) const { return _base + path; }

QString HtmlWriter::Wrap::relativePath(const Data::File &file) const { return relativePath(file.relativePath); }
...
QByteArray HtmlWriter::Wrap::composeStart() {
	...
	result.append(_context.pushTag("link", {{"href", _base + "css/style.css"}, {"rel", "stylesheet"}, {"empty", ""}}));
...
Result HtmlWriter::start(const Settings &settings, const Environment &environment, Stats *stats) {
	...
	//	return result;
	// }
	const auto copy = [&](const QString &filename) { return copyFile(":/export/" + filename, filename); };
	const auto files = {
		"css/style.css",
		"images/back.png",
...
Result HtmlWriter::writeDelayedPersonal(const QString &userpicPath) {
	...
	const auto result = writePreparedPersonal(*base::take(_delayedPersonalInfo), userpicPath);
	if (!result) {
		return result;
	}
	if (_userpicsCount) {
...
Result HtmlWriter::writePreparedPersonal(const Data::PersonalInfo &data, const QString &userpicPath) {
	Expects(_summary != nullptr);
	...
	userpic.imageLink = WriteUserpicThumb(_settings.path, userpicPath, userpic, "_info");
	...
	const auto pushRows = [&](QByteArray name, std::vector<std::pair<QByteArray, QByteArray>> &&values)
	{
...
Result HtmlWriter::writeUserpicsStart(const Data::UserpicsInfo &data) {
	...
	auto block = _userpics->pushHeader("Profile pictures", mainFileRelativePath());
	...
	for (const auto &userpic : data.list) {
		...
		const auto status = [&]() -> Data::Utf8String
		{
			switch (file.skipReason) {
				case SkipReason::Unavailable: return "(Photo unavailable, please try again later)";
				case SkipReason::FileSize:
		...
		block.append(_userpics->pushListEntry(data,
...
QString HtmlWriter::userpicsFilePath() const { return "lists/profile_pictures.html"; }

void HtmlWriter::pushUserpicsSection() {
	pushSection(kUserpicsPriority, "Profile pictures", "photos", _userpicsCount, userpicsFilePath());
}
...
Result HtmlWriter::writeStoriesSlice(const Data::StoriesSlice &data) {
	...
	for (const auto &story : data.list) {
		...
		status.append(
			...
			{
				switch (file.skipReason) {
					case SkipReason::Unavailable: return "(Story unavailable, please try again later)";
					case SkipReason::FileSize:
		...
		const auto &image =
			story.thumb().file.relativePath.isEmpty() ? story.file().relativePath : story.thumb().file.relativePath;
		data.imageLink = Data::WriteImageThumb(_settings.path, image, kStoryThumbWidth * 2, kStoryThumbHeight * 2);
		const auto info = (story.date > 0) ? Data::FormatDateTime(story.date) : QByteArray();
		block.append(_stories->pushStoriesListEntry(data,
...
void HtmlWriter::pushStoriesSection() {
	pushSection(kStoriesPriority, "Stories archive", "stories", _storiesCount, storiesFilePath());
}
...
Result HtmlWriter::writeSavedContacts(const Data::ContactsList &data) {
	...
	for (const auto index : Data::SortedContactsIndices(data)) {
		...
		auto userpic = UserpicData{Data::ContactColorIndex(contact), kEntryUserpicSize};
		...
		block.append(file->pushListEntry(userpic,
	...
	pushSection(kContactsPriority, "Contacts", "contacts", data.list.size(), filename);
	return Result::Success();
}

Result HtmlWriter::writeFrequentContacts(const Data::ContactsList &data) {
	const auto size = data.correspondents.size() + data.inlineBots.size() + data.phoneCalls.size();
	if (!size) {
		return Result::Success();
	}
	...
	block.append(file->pushDiv("entry_list"));
	const auto writeList = [&](const std::vector<Data::TopPeer> &peers, Data::Utf8String category)
	{
		for (const auto &top : peers) {
			...
			auto userpic = UserpicData{Data::PeerColorIndex(top.peer.id()), kEntryUserpicSize};
			userpic.firstName = name;
	...
	pushSection(kFrequentContactsPriority, "Frequent contacts", "frequent", size, filename);
	return Result::Success();
}
...
Result HtmlWriter::writeSessions(const Data::SessionsList &data) {
	...
	for (const auto &session : data.list) {
		block.append(file->pushSessionListEntry(
			...
			((session.applicationName.isEmpty() ? Data::Utf8String("Unknown") : session.applicationName) + ' ' +
			 session.applicationVersion),
			(session.deviceModel + ", " + session.platform + ' ' + session.systemVersion),
			{(session.ip + " \xE2\x80\x93 " + session.region +
			  ((session.region.isEmpty() || session.country.isEmpty()) ? QByteArray() : QByteArray(", ")) +
			  session.country),
	...
	pushSection(kSessionsPriority, "Sessions", "sessions", data.list.size(), filename);
	return Result::Success();
}
...
Result HtmlWriter::writeWebSessions(const Data::SessionsList &data) {
	...
	for (const auto &session : data.webList) {
		block.append(
			file->pushSessionListEntry(Data::DomainApplicationId(session.domain),
									   (session.domain.isEmpty() ? Data::Utf8String("Unknown") : session.domain),
									   session.platform + ", " + session.browser,
									   {session.ip + " \xE2\x80\x93 " + session.region,
										"Last active: " + Data::FormatDateTime(session.lastActive),
										"Created: " + Data::FormatDateTime(session.created)},
									   (session.botUsername.isEmpty() ? QByteArray() : ('@' + session.botUsername))));
	}
	if (const auto result = file->writeBlock(block); !result) {
		return result;
	} else if (const auto closed = file->close(); !closed) {
	...
	pushSection(kWebSessionsPriority, "Web sessions", "web", data.webList.size(), filename);
	return Result::Success();
}
...
Result HtmlWriter::writeDialogsStart(const Data::DialogsInfo &data) {
	...
	pushSection(kChatsPriority, "Chats", "chats", data.chats.size() + data.left.size(), "lists/chats.html");
...
Result HtmlWriter::writeDialogSlice(const Data::MessagesSlice &data) {
	...
	const auto messageLinkWrapper = [&](int messageId, QByteArray text) { return wrapMessageLink(messageId, text); };
	auto oldIndex = (_messagesCount > 0) ? ((_messagesCount - 1) / kMessagesInFile) : 0;
	...
	for (const auto &message : data.list) {
		...
		if (oldIndex != newIndex) {
			if (const auto result = _chat->writeBlock(block); !result) {
				...
				_lastMessageIdsPerFile.push_back(saved ? saved->id : _lastMessageInfo->id);
				block = QByteArray();
				_lastMessageInfo = nullptr;
				previous = nullptr;
				saved = std::nullopt;
		...
		if (DisplayDate(date, previous ? previous->date : 0)) {
			block.append(_chat->pushServiceMessage(--_dateMessageId, _dialog, _settings.path, FormatDateText(date)));
		}
		const auto &[info, content] = _chat->pushMessage(message,
														 previous,
														 _dialog,
														 _settings.path,
														 data.peers,
...
Result HtmlWriter::writeEmptySinglePeer() {
	...
	return _chat->writeBlock(
		_chat->pushServiceMessage(--_dateMessageId, _dialog, _settings.path, "No exported messages"));
}
...
Result HtmlWriter::writeDialogEnd() {
	...
	const auto CountString = [](int count, bool outgoing) -> QByteArray
	{
		...
		return Data::NumberToString(count) + (outgoing ? " outgoing messages" : " messages");
	};
	auto userpic =
		UserpicData{((_dialog.type == Type::Self || _dialog.type == Type::Replies || _dialog.type == Type::VerifyCodes)
...
Result HtmlWriter::validateDialogsMode(bool isLeftChannel) {
	...
	_dialogsMode = mode;
	auto block = _chats->pushAbout(isLeftChannel ? _environment.aboutLeftChats : _environment.aboutChats);
...
Result HtmlWriter::writeDialogOpening(int index) {
	const auto name = (_dialog.name.isEmpty() && _dialog.lastName.isEmpty()) ? QByteArray("Deleted Account")
																			 : (_dialog.name + ' ' + _dialog.lastName);
	auto block = _chat->pushHeader(name, _settings.onlySinglePeer() ? QString() : _dialogsRelativePath);
	...
	if (index > 0) {
		...
		block.append(_chat->pushTag("a", {{"class", "pagination block_link"}, {"href", previousPath.toUtf8()}}));
...
void HtmlWriter::pushSection(
	int priority, const QByteArray &label, const QByteArray &type, int count, const QString &path) {
...
Result HtmlWriter::writeSections() {
	...
	if (_savedSections.empty()) {
		...
		auto block = _summary->pushDiv(_summaryNeedDivider ? "sections with_divider" : "sections");
	...
	auto block = QByteArray();
	ranges::sort(_savedSections, std::less<>(), [](const SavedSection &data) { return data.priority; });
	for (const auto &section : base::take(_savedSections)) {
		block.append(
			_summary->pushSection(section.label, section.type, section.count, _summary->relativePath(section.path)));
	}
	return _summary->writeBlock(block);
}
...
QByteArray HtmlWriter::wrapMessageLink(int messageId, QByteArray text) {
	const auto it =
		ranges::find_if(_lastMessageIdsPerFile, [&](int maxMessageId) { return messageId <= maxMessageId; });
	if (it == end(_lastMessageIdsPerFile)) {
		return "<a href=\"#go_to_message" + Data::NumberToString(messageId) + "\" onclick=\"return GoToMessage(" +
			Data::NumberToString(messageId) + ")\">" + text + "</a>";
	} else {
		const auto index = it - begin(_lastMessageIdsPerFile);
		return "<a href=\"" + messagesFile(index).toUtf8() + "#go_to_message" + Data::NumberToString(messageId) +
			"\">" + text + "</a>";
	}
}
...
Result HtmlWriter::switchToNextChatFile(int index) {
	...
	auto next = _chat->pushTag("a", {{"class", "pagination block_link"}, {"href", nextPath.toUtf8()}});
...
Result HtmlWriter::finish() {
	...
	block.append(_summary->pushAbout(_environment.aboutTelegram, _summaryNeedDivider));
	if (const auto result = _summary->writeBlock(block); !result) {
		return result;
	}
...
Result HtmlWriter::copyFile(const QString &source, const QString &relativePath) const {
	return File::Copy(source, pathWithRelativePath(relativePath), _stats);
}

QString HtmlWriter::mainFilePath() {
	return pathWithRelativePath(_settings.onlySinglePeer() ? messagesFile(0) : mainFileRelativePath());
}

QString HtmlWriter::mainFileRelativePath() const { return "export_results.html"; }

QString HtmlWriter::pathWithRelativePath(const QString &path) const { return _settings.path + path; }

QString HtmlWriter::messagesFile(int index) const {
	return "messages" + (index > 0 ? QString::number(index + 1) : QString()) + ".html";
}

std::unique_ptr<HtmlWriter::Wrap> HtmlWriter::fileWithRelativePath(const QString &path) const {
	return std::make_unique<Wrap>(pathWithRelativePath(path), _settings.path, _stats);
}
