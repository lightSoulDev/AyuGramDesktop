...
SparseIdsList::Slice::Slice(base::flat_set<MsgId> &&messages, MsgRange range)
	: messages(std::move(messages)), range(range) {}

template<typename Range>
void SparseIdsList::Slice::merge(const Range &moreMessages, MsgRange moreNoSkipRange) {
	...
	range = {qMin(range.from, moreNoSkipRange.from), qMax(range.till, moreNoSkipRange.till)};
}

template<typename Range>
SparseIdsList::AddResult SparseIdsList::uniteAndAdd(SparseIdsSliceUpdate &update,
	...
	_slices.modify(uniteFrom, [&](Slice &slice) { slice.merge(messages, noSkipRange); });
	const auto firstToErase = uniteFrom + 1;
	if (firstToErase != uniteTill) {
		for (auto it = firstToErase; it != uniteTill; ++it) {
			_slices.modify(uniteFrom, [&](Slice &slice) { slice.merge(it->messages, it->range); });
		}
...
template<typename Range>
SparseIdsList::AddResult
SparseIdsList::addRangeItemsAndCountNew(SparseIdsSliceUpdate &update, const Range &messages, MsgRange noSkipRange) {
	// Expects(noSkipRange.from <= noSkipRange.till);
	if (!(noSkipRange.from <= noSkipRange.till)) {
		return {0}; // ViGram: fix crash when using `saveDeletedMessages`
	}

	if (noSkipRange.from == noSkipRange.till && std::begin(messages) == std::end(messages)) {
		return {0};
	}
	auto uniteFrom = ranges::lower_bound(
		_slices, noSkipRange.from, std::less<>(), [](const Slice &slice) { return slice.range.till; });
	auto uniteTill = ranges::upper_bound(
		_slices, noSkipRange.till, std::less<>(), [](const Slice &slice) { return slice.range.from; });
	if (uniteFrom < uniteTill) {
		return uniteAndAdd(update, uniteFrom, uniteTill, messages, noSkipRange);
	}

	auto sliceMessages = base::flat_set<MsgId>{std::begin(messages), std::end(messages)};
	auto slice = _slices.emplace(std::move(sliceMessages), noSkipRange).first;
...
template<typename Range>
void SparseIdsList::addRange(const Range &messages,
							 MsgRange noSkipRange,
	...
	const auto result = addRangeItemsAndCountNew(update, messages, noSkipRange);
	if (count) {
		_count = count;
	...
	if (_slices.size() == 1) {
		if (_count && _slices.front().messages.size() >= *_count) {
			_slices.modify(_slices.begin(), [&](Slice &slice) { slice.range = {0, ServerMaxMsgId}; });
		}
...
void SparseIdsList::addExisting(MsgId messageId, MsgRange noSkipRange) {
	auto range = {messageId};
	addRange(range, noSkipRange, std::nullopt);
}

void SparseIdsList::addSlice(std::vector<MsgId> &&messageIds, MsgRange noSkipRange, std::optional<int> count) {
	addRange(messageIds, noSkipRange, count);
}

void SparseIdsList::removeOne(MsgId messageId) {
	auto slice =
		ranges::lower_bound(_slices, messageId, std::less<>(), [](const Slice &slice) { return slice.range.till; });
	if (slice != _slices.end() && slice->range.from <= messageId) {
		_slices.modify(slice, [messageId](Slice &slice) { return slice.messages.remove(messageId); });
	}
	if (_count) {
		--*_count;
	}
...
void SparseIdsList::invalidateBottom() {
	if (!_slices.empty()) {
		...
		if (last.range.till == ServerMaxMsgId) {
			_slices.modify(_slices.end() - 1,
						   [](Slice &slice)
						   { slice.range.till = slice.messages.empty() ? slice.range.from : slice.messages.back(); });
		}
	}
	_count = std::nullopt;
}

rpl::producer<SparseIdsListResult> SparseIdsList::query(SparseIdsListQuery &&query) const {
...
SparseIdsListResult SparseIdsList::snapshot(const SparseIdsListQuery &query) const {
	auto slice = query.aroundId
		? ranges::lower_bound(
			  _slices, query.aroundId, std::less<>(), [](const Slice &slice) { return slice.range.till; })
		: _slices.end();
	if (slice != _slices.end() && slice->range.from <= query.aroundId) {
...
rpl::producer<SparseIdsSliceUpdate> SparseIdsList::sliceUpdated() const { return _sliceUpdated.events(); }

SparseIdsListResult SparseIdsList::queryFromSlice(const SparseIdsListQuery &query, const Slice &slice) const {
	auto result = SparseIdsListResult{};
	...
	if (_count) {
		...
		if (!result.skippedBefore && result.skippedAfter) {
			result.skippedBefore = *result.count - *result.skippedAfter - int(result.messageIds.size());
		} else if (!result.skippedAfter && result.skippedBefore) {
			result.skippedAfter = *result.count - *result.skippedBefore - int(result.messageIds.size());
		}
	}
	return result;
}
