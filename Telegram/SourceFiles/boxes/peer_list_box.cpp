...
PaintRoundImageCallback PaintUserpicCallback(not_null<PeerData *> peer, bool respectSavedMessagesChat) {
...
PeerListContentDelegateShow::PeerListContentDelegateShow(std::shared_ptr<Main::SessionShow> show) : _show(show) {}

auto PeerListContentDelegateShow::peerListUiShow() -> std::shared_ptr<Main::SessionShow> { return _show; }

PeerListBox::PeerListBox(QWidget *,
						 std::unique_ptr<PeerListController> controller,
						 Fn<void(not_null<PeerListBox *>)> init)
	: _show(Main::MakeSessionShow(uiShow(), &controller->session())), _controller(std::move(controller)),
	  _init(std::move(init)) {
...
void PeerListBox::createMultiSelect() {
	...
	auto entity =
		object_ptr<Ui::MultiSelect>(this,
									(_controller->selectSt() ? *_controller->selectSt() : st::defaultMultiSelect),
									tr::lng_participant_filter());
	_select.create(this, std::move(entity));
	_select->heightValue() | rpl::start_with_next([this] { updateScrollSkips(); }, lifetime());
	_select->entity()->setSubmittedCallback([=](Qt::KeyboardModifiers) { content()->submitted(); });
	_select->entity()->setQueryChangedCallback(
		[=](const QString &query)
		{
	...
	_select->entity()->setItemRemovedCallback(
		[=](uint64 itemId)
		{
...
void PeerListBox::showFinished() { _controller->showFinished(); }
...
void PeerListBox::prepare() {
	setContent(setInnerWidget(object_ptr<PeerListContent>(this, _controller.get()), st::boxScroll));
	content()->resizeToWidth(_controller->contentWidth());

	_controller->setDelegate(this);

	_controller->boxHeightValue() |
		rpl::start_with_next([=](int height) { setDimensions(_controller->contentWidth(), height); }, lifetime());
	...
	content()->scrollToRequests() |
		rpl::start_with_next([this](Ui::ScrollToRequest request) { scrollToY(request.ymin, request.ymax); },
							 lifetime());
...
void PeerListBox::paintEvent(QPaintEvent *e) {
	...
	const auto fill = QRect(0, _addedTopScrollSkip, width(), height() - _addedTopScrollSkip);
...
void PeerListBox::peerListSetForeignRowChecked(not_null<PeerListRow *> row, bool checked, anim::type animated) {
	if (checked) {
...
void PeerListBox::peerListScrollToTop() { scrollToY(0); }
...
std::shared_ptr<Main::SessionShow> PeerListBox::peerListUiShow() { return _show; }
...
const style::PeerList &PeerListController::computeListSt() const { return _listSt ? *_listSt : st::peerListBox; }

const style::MultiSelect &PeerListController::computeSelectSt() const {
	return _selectSt ? *_selectSt : st::defaultMultiSelect;
}

bool PeerListController::hasComplexSearch() const { return (_searchController != nullptr); }
...
void PeerListController::peerListSearchRefreshRows() { delegate()->peerListRefreshRows(); }
...
void PeerListController::sortByName() {
	...
	const auto predicate = [&](const PeerListRow &a, const PeerListRow &b) { return (key(a).compare(key(b)) < 0); };
	delegate()->peerListSortRows(predicate);
}

base::unique_qptr<Ui::PopupMenu> PeerListController::rowContextMenu(QWidget *parent, not_null<PeerListRow *> row) {
	return nullptr;
}

std::unique_ptr<PeerListRow> PeerListController::createSearchRow(PeerListRowId id) {
...
std::unique_ptr<PeerListState> PeerListController::saveState() const { return delegate()->peerListSaveState(); }

void PeerListController::restoreState(std::unique_ptr<PeerListState> state) {
	delegate()->peerListRestoreState(std::move(state));
}

int PeerListController::contentWidth() const { return st::boxWideWidth; }

rpl::producer<int> PeerListController::boxHeightValue() const { return rpl::single(st::boxMaxListHeight); }

int PeerListController::descriptionTopSkipMin() const { return computeListSt().item.height; }

void PeerListBox::addSelectItem(not_null<PeerData *> peer, anim::type animated) {
	const auto respect = !_controller->savedMessagesChatStatus().isEmpty();
	const auto text = (respect && peer->isSelf()) ? tr::lng_saved_short(tr::now)
		: (respect && peer->isRepliesChat())	  ? tr::lng_replies_messages(tr::now)
		: (respect && peer->isVerifyCodes())	  ? tr::lng_verification_codes(tr::now)
												  : peer->shortName();
	addSelectItem(peer->id.value,
				  text,
				  (peer->isForum() ? ForceRoundUserpicCallback(peer) : PaintUserpicCallback(peer, respect)),
				  animated);
}

void PeerListBox::addSelectItem(not_null<PeerListRow *> row, anim::type animated) {
	addSelectItem(row->id(), row->generateShortName(), row->generatePaintUserpicCallback(true), animated);
}

void PeerListBox::addSelectItem(uint64 itemId,
								const QString &text,
	...
	const auto &activeBg =
		(_controller->selectSt() ? *_controller->selectSt() : st::defaultMultiSelect).item.textActiveBg;
	if (animated == anim::type::instant) {
		_select->entity()->addItemInBunch(itemId, text, activeBg, std::move(paintUserpic));
	} else {
		_select->entity()->addItem(itemId, text, activeBg, std::move(paintUserpic));
	}
}
...
int PeerListBox::peerListSelectedRowsCount() { return _select ? _select->entity()->getItemsCount() : 0; }
...
auto PeerListBox::collectSelectedRows() -> std::vector<not_null<PeerData *>> {
...
rpl::producer<> PeerListBox::noSearchSubmits() const { return content()->noSearchSubmits(); }

PeerListRow::PeerListRow(not_null<PeerData *> peer) : PeerListRow(peer, peer->id.value) {}

PeerListRow::PeerListRow(not_null<PeerData *> peer, PeerListRowId id) : _id(id), _peer(peer) {}

PeerListRow::PeerListRow(PeerListRowId id) : _id(id) {}

PeerListRow::~PeerListRow() = default;

bool PeerListRow::checked() const { return _checkbox && _checkbox->checked(); }
...
void PeerListRow::refreshStatus() {
	if (!_initialized || special() || _statusType == StatusType::Custom || _statusType == StatusType::CustomActive) {
		return;
	}
	...
	if (auto user = peer()->asUser()) {
		if (!_savedMessagesStatus.isEmpty()) {
			...
			_statusValidTill = crl::now() + Data::OnlineChangeTimeout(user, time);
		}
...
crl::time PeerListRow::refreshStatusTime() const { return _statusValidTill; }

void PeerListRow::refreshName(const style::PeerListItem &st) {
	if (!_initialized) {
		return;
	}
	const auto text = !_savedMessagesStatus.isEmpty() ? tr::lng_saved_messages(tr::now)
		: _isRepliesMessagesChat					  ? tr::lng_replies_messages(tr::now)
		: _isVerifyCodesChat						  ? tr::lng_verification_codes(tr::now)
													  : generateName();
	_name.setText(st.nameStyle, text, Ui::NameTextOptions());
}

int PeerListRow::elementsCount() const { return 1; }
...
bool PeerListRow::elementDisabled(int element) const { return (element == 1) && rightActionDisabled(); }

bool PeerListRow::elementOnlySelect(int element) const { return false; }

void PeerListRow::elementAddRipple(int element, QPoint point, Fn<void()> updateCallback) {
	if (element == 1) {
		rightActionAddRipple(point, std::move(updateCallback));
	}
}

void PeerListRow::elementsStopLastRipple() { rightActionStopLastRipple(); }

void PeerListRow::elementsPaint(Painter &p, int outerWidth, bool selected, int selectedElement) {
	const auto geometry = elementGeometry(1, outerWidth);
	if (!geometry.isEmpty()) {
		rightActionPaint(p, geometry.x(), geometry.y(), outerWidth, selected, (selectedElement == 1));
	}
}

QString PeerListRow::generateName() { return peer()->name(); }

QString PeerListRow::generateShortName() {
	return !_savedMessagesStatus.isEmpty() ? tr::lng_saved_short(tr::now)
		: _isRepliesMessagesChat		   ? tr::lng_replies_messages(tr::now)
		: _isVerifyCodesChat			   ? tr::lng_verification_codes(tr::now)
										   : peer()->shortName();
}
...
PaintRoundImageCallback PeerListRow::generatePaintUserpicCallback(bool forceRound) {
...
auto PeerListRow::generateNameFirstLetters() const -> const base::flat_set<QChar> & {
	return peer()->nameFirstLetters();
}

auto PeerListRow::generateNameWords() const -> const base::flat_set<QString> & { return peer()->nameWords(); }

const style::PeerListItem &PeerListRow::computeSt(const style::PeerListItem &st) const { return st; }
...
int PeerListRow::paintNameIconGetWidth(Painter &p,
									   Fn<void()> repaint,
									   crl::time now,
									   int nameLeft,
									   int nameTop,
									   ...
									   int outerWidth,
									   bool selected) {
	if (_skipPeerBadge || special() || !_savedMessagesStatus.isEmpty() || _isRepliesMessagesChat ||
		_isVerifyCodesChat) {
		return 0;
	}
	return _badge.drawGetWidth(
		...
		{
			.peer = peer(),
			.rectForName = QRect(nameLeft, nameTop, availableWidth, st::semiboldFont->height),
			.nameWidth = nameWidth,
			.outerWidth = outerWidth,
			.verified = &(selected ? st::dialogsVerifiedIconOver : st::dialogsVerifiedIcon),
			.exteraOfficial = &(selected ? st::dialogsExteraOfficialIcon.over : st::dialogsExteraOfficialIcon.icon),
			.exteraSupporter = &(selected ? st::dialogsExteraSupporterIcon.over : st::dialogsExteraSupporterIcon.icon),
			.premium = &(selected ? st::dialogsPremiumIcon.over : st::dialogsPremiumIcon.icon),
			.scam = &(selected ? st::dialogsScamFgOver : st::dialogsScamFg),
			.premiumFg = &(selected ? st::dialogsVerifiedIconBgOver : st::dialogsVerifiedIconBg),
...
void PeerListRow::paintStatusText(
	...
	auto statusHasOnlineColor =
		(_statusType == PeerListRow::StatusType::Online) || (_statusType == PeerListRow::StatusType::CustomActive);
...
void PeerListRow::addRipple(const style::PeerListItem &st,
	...
	if (!_ripple) {
		...
		_ripple = std::make_unique<Ui::RippleAnimation>(
			st.button.ripple, std::move(mask), std::forward<UpdateCallback>(updateCallback));
	}
	_ripple->add(point);
}
...
void PeerListRow::paintRipple(Painter &p, const style::PeerListItem &st, int x, int y, int outerWidth) {
	if (_ripple) {
...
void PeerListRow::paintUserpic(Painter &p, const style::PeerListItem &st, int x, int y, int outerWidth) {
...
void PeerListRow::paintDisabledCheckUserpic(
...

	{
		...
		if (peer()->forum()) {
			const auto radius = userpicDiameter * Ui::ForumUserpicRadiusMultiplier();
...
float64 PeerListRow::checkedRatio() { return _checkbox ? _checkbox->checkedAnimationRatio() : 0.; }
...
bool PeerListRow::useForumLikeUserpic() const { return !special() && peer()->isForum(); }

void PeerListRow::createCheckbox(const style::RoundImageCheckbox &st, Fn<void()> updateCallback) {
	const auto generateRadius = [=](int size)
	{ return useForumLikeUserpic() ? int(size * Ui::ForumUserpicRadiusMultiplier()) : std::optional<int>(); };
	_checkbox = std::make_unique<Ui::RoundImageCheckbox>(
		st, std::move(updateCallback), generatePaintUserpicCallback(false), generateRadius);
}
...
void PeerListRow::setCustomizedCheckSegments(std::vector<Ui::OutlineSegment> segments) {
	Expects(_checkbox != nullptr);

	_checkbox->setCustomizedSegments(std::move(segments));
}

void PeerListRow::finishCheckedAnimation() { _checkbox->setChecked(_checkbox->checked(), anim::type::instant); }

PeerListContent::PeerListContent(QWidget *parent, not_null<PeerListController *> controller)
	: RpWidget(parent), _st(controller->computeListSt()), _controller(controller), _rowHeight(_st.item.height) {
	_controller->session().downloaderTaskFinished() | rpl::start_with_next([=] { update(); }, lifetime());

	using UpdateFlag = Data::PeerUpdate::Flag;
	_controller->session().changes().peerUpdates(UpdateFlag::Name | UpdateFlag::Photo | UpdateFlag::EmojiStatus) |
		rpl::start_with_next(
	...
	style::PaletteChanged() | rpl::start_with_next([=] { invalidatePixmapsCache(); }, lifetime());
...
void PeerListContent::setMode(Mode mode) {
	...
	_mode = mode;
	switch (_mode) {
		case Mode::Default: _rowHeight = _st.item.height; break;
		case Mode::Custom: _rowHeight = _controller->customRowHeight(); break;
	}
...
void PeerListContent::changeCheckState(not_null<PeerListRow *> row, bool checked, anim::type animated) {
	row->setChecked(checked, _st.item.checkbox, animated, [=] { updateRow(row); });
}
...
std::optional<QPoint> PeerListContent::lastRowMousePosition() const {
	...
	return (index >= 0 && index == _selected.index.value) ? QPoint(point.x(), rowsPointY) : std::optional<QPoint>();
}
...
void PeerListContent::removeRow(not_null<PeerListRow *> row) {
	...
	removeFromSearchIndex(row);
	_filterResults.erase(ranges::remove(_filterResults, row), end(_filterResults));
	_hiddenRows.remove(row);
...
int PeerListContent::fullRowsCount() const { return _rows.size(); }
...
int PeerListContent::searchRowsCount() const { return _searchRows.size(); }
...
void PeerListContent::initDecorateWidget(Ui::RpWidget *widget) {
	if (widget) {
		...
		widget->events() |
			rpl::filter([=](not_null<QEvent *> e) { return (e->type() == QEvent::Enter) && widget->isVisible(); }) |
			rpl::start_with_next([=] { mouseLeftGeometry(); }, widget->lifetime());
		widget->heightValue() | rpl::skip(1) |
			rpl::start_with_next([=] { resizeToWidth(width()); }, widget->lifetime());
	}
}
	...
	if (_searchMode != mode) {
		...
		if (_controller->hasComplexSearch()) {
			if (_mode == Mode::Custom) {
				if (!_searchLoading) {
					setSearchLoading(
						object_ptr<Ui::FlatLabel>(this, tr::lng_contacts_loading(tr::now), st::membersAbout));
				}
			} else {
				if (!_loadingAnimation) {
					_loadingAnimation = Ui::CreateLoadingPeerListItemWidget(this, _st.item, 2);
				}
			}
		} else {
			clearSearchRows();
...
void PeerListContent::paintEvent(QPaintEvent *e) {
	...
	if (count > 0) {
		...
		for (auto index = from; index != to; ++index) {
			...
			if (repaintAfter > 0 && (repaintAfterMin < 0 || repaintAfterMin > repaintAfter)) {
...
int PeerListContent::resizeGetHeight(int newWidth) {
	...
	const auto labelTop = rowsTop() + std::max(shownRowsCount() * _rowHeight, _controller->descriptionTopSkipMin());
	...
	if (_searchNoResults) {
		...
		_searchNoResults->moveToLeft(
			st::contactsPadding.left(), labelTop + st::membersAboutLimitPadding.top(), newWidth);
		_searchNoResults->setVisible(!hideAll && showingSearch() && _filterResults.empty() &&
	...
	if (_searchLoading) {
		...
		_searchLoading->setVisible(!hideAll && showingSearch() && _filterResults.empty() &&
								   _controller->isSearchLoading());
	}
	if (_loadingAnimation) {
		_loadingAnimation->resizeToWidth(newWidth);
		_loadingAnimation->moveToLeft(0, rowsTop(), newWidth);
		_loadingAnimation->setVisible(!hideAll && showingSearch() && _filterResults.empty() &&
									  _controller->isSearchLoading());
	}
	const auto label = labelHeight();
	const auto belowTop = (label > 0 || rowsCount > 0) ? (labelTop + label + _st.padding.bottom()) : _aboveHeight;
	_belowHeight = 0;
	if (_belowWidget) {
...
void PeerListContent::enterEventHook(QEnterEvent *e) { setMouseTracking(true); }
...
void PeerListContent::mouseMoveEvent(QMouseEvent *e) { handleMouseMove(e->globalPos()); }
...
void PeerListContent::handleMouseMove(QPoint globalPosition) {
	...
	if (_trackPressStart &&
		((*_trackPressStart - globalPosition).manhattanLength() > QApplication::startDragDistance())) {
		_trackPressStart = {};
...
void PeerListContent::mousePressEvent(QMouseEvent *e) {
	...
	_trackPressStart = {};
	if (const auto row = getRow(_selected.index)) {
		const auto updateCallback = [this, row, hint = _selected.index] { updateRow(row, hint); };
		if (_selected.element) {
			const auto elementRect = getElementRect(row, _selected.index, _selected.element);
			if (!elementRect.isEmpty()) {
				row->elementAddRipple(_selected.element,
			...
			if (_mode == Mode::Custom) {
				...
				const auto maskGenerator = [&] { return Ui::RippleAnimation::RectMask(QSize(width(), _rowHeight)); };
...
void PeerListContent::mouseReleaseEvent(QMouseEvent *e) { mousePressReleased(e->button()); }
...
void PeerListContent::showRowMenu(not_null<PeerListRow *> row,
								  bool highlightRow,
								  Fn<void(not_null<Ui::PopupMenu *>)> destroyed) {
	const auto index = findRowIndex(row);
	showRowMenu(index, row, QCursor::pos(), highlightRow, std::move(destroyed));
}

bool PeerListContent::showRowMenu(RowIndex index,
								  PeerListRow *row,
								  QPoint globalPos,
								  bool highlightRow,
								  Fn<void(not_null<Ui::PopupMenu *>)> destroyed) {
	...
	raw->setDestroyedCallback(crl::guard(this,
										 [=]
										 {
											 if (highlightRow) {
												 setContexted(Selected());
...
crl::time PeerListContent::paintRow(Painter &p, crl::time now, RowIndex index) {
	...
	const auto refreshStatusIn = (refreshStatusAt > 0) ? std::max(refreshStatusAt - now, crl::time(1)) : 0;

	const auto peer = row->special() ? nullptr : row->peer().get();
	const auto active = (_contexted.index.value >= 0) ? _contexted : (_pressed.index.value >= 0) ? _pressed : _selected;
	const auto selected = (active.index == index) && (!active.element || !row->elementOnlySelect(active.element));
	...
	const auto &bg = selected ? st.button.textBgOver : st.button.textBg;
	if (opacity < 1.) {
	...
	row->paintUserpic(p, st, st.photoPosition.x(), st.photoPosition.y(), outerWidth);

	p.setPen(st::contactsNameFg);

	const auto skipRight = st.photoPosition.x();
	const auto rightActionSize = row->rightActionSize();
	const auto rightActionMargins = rightActionSize.isEmpty() ? QMargins() : row->rightActionMargins();
	...
	if (!rightActionSize.isEmpty() && (namey < rightActionMargins.top() + rightActionSize.height()) &&
		(namey + st.nameStyle.font->height > rightActionMargins.top())) {
		namew -= rightActionMargins.left() + rightActionSize.width() + rightActionMargins.right() - skipRight;
	}
	const auto statusx = st.statusPosition.x();
	const auto statusy = st.statusPosition.y();
	auto statusw = outerWidth - statusx - skipRight;
	if (!rightActionSize.isEmpty() && (statusy < rightActionMargins.top() + rightActionSize.height()) &&
		(statusy + st::contactsStatusFont->height > rightActionMargins.top())) {
		statusw -= rightActionMargins.left() + rightActionSize.width() + rightActionMargins.right() - skipRight;
	}
	namew -= row->paintNameIconGetWidth(
		p, [=] { updateRow(row); }, now, namex, namey, name.maxWidth(), namew, width(), selected);
	...
	row->elementsPaint(p, width(), selected, (active.index == index) ? active.element : 0);
...
PeerListContent::SkipResult PeerListContent::selectSkip(int direction) {
	...
	enumerateShownRows(
		[&firstEnabled, &lastEnabled, &index](not_null<PeerListRow *> row)
		{
			if (!row->disabled()) {
				if (firstEnabled < 0) {
					firstEnabled = index;
	...
	// Snap the index.
	newSelectedIndex = std::clamp(newSelectedIndex, firstEnabled - 1, lastEnabled);
	...
	if (_controller->overrideKeyboardNavigation(direction, _selected.index.value, newSelectedIndex)) {
		return {_selected.index.value, _selected.index.value};
	}
...
rpl::producer<int> PeerListContent::selectedIndexValue() const { return _selectedIndex.value(); }

int PeerListContent::selectedIndex() const { return _selectedIndex.current(); }

bool PeerListContent::hasSelection() const { return _selected.index.value >= 0; }

bool PeerListContent::hasPressed() const { return _pressed.index.value >= 0; }

void PeerListContent::clearSelection() { setSelected(Selected()); }
...
void PeerListContent::searchQueryChanged(QString query) {
	...
	if (_normalizedSearchQuery != normalizedQuery) {
		...
		if (_controller->searchInLocal() && !searchWordsList.isEmpty()) {
			...
			if (minimalList) {
				auto searchWordInNames = [](not_null<PeerListRow *> row, const QString &searchWord)
				{
				...
				auto allSearchWordsInNames = [&](not_null<PeerListRow *> row)
				{
					for (const auto &searchWord : searchWordsList) {
...
std::unique_ptr<PeerListState> PeerListContent::saveState() const {
	...
	result->controllerState = std::make_unique<PeerListController::SavedStateBase>();
...
void PeerListContent::restoreState(std::unique_ptr<PeerListState> state) {
...
void PeerListContent::dragLeft() { clearSelection(); }

void PeerListContent::setIgnoreHiddenRowsOnSearch(bool value) { _ignoreHiddenRowsOnSearch = value; }

void PeerListContent::visibleTopBottomUpdated(int visibleTop, int visibleBottom) {
...
auto PeerListContent::saveSelectedData(Selected from) -> SelectedSaved {
...
auto PeerListContent::restoreSelectedData(SelectedSaved from) -> Selected {
...
void PeerListContent::selectByMouse(QPoint globalPosition) {
	...
	if (selected.index.value >= 0) {
		...
		if (row->disabled() ||
			(customMode &&
			 !_controller->customRowSelectionPoint(row, point.x(), rowsPointY - (selected.index.value * _rowHeight)))) {
			selected = Selected();
		} else if (!customMode) {
...
QRect PeerListContent::getElementRect(not_null<PeerListRow *> row, RowIndex index, int element) const {
...
int PeerListContent::rowsTop() const { return _aboveHeight + _st.padding.top(); }
...
void PeerListContent::updateRow(not_null<PeerListRow *> row, RowIndex hint) { updateRow(findRowIndex(row, hint)); }
...
void PeerListContentDelegate::peerListShowRowMenu(not_null<PeerListRow *> row,
												  bool highlightRow,
												  Fn<void(not_null<Ui::PopupMenu *>)> destroyed) {
