...
[[nodiscard]] std::optional<QColor> ComputeImageColor(const style::icon &lockIcon, const QImage &frame, RectPart part) {
	if (frame.isNull() || frame.format() != QImage::Format_ARGB32_Premultiplied) {
		return {};
	}
	auto sr = int64();
	auto sg = int64();
	...
	const auto skipx = (part == RectPart::TopLeft || part == RectPart::Left || part == RectPart::BottomLeft) ? 0
		: (part == RectPart::Top || part == RectPart::Center || part == RectPart::Bottom)
		? (frame.width() - width) / 2
		: std::max(frame.width() - width - radius, 0);
	const auto skipy = (part == RectPart::TopLeft || part == RectPart::Top || part == RectPart::TopRight) ? 0
		: (part == RectPart::Left || part == RectPart::Center || part == RectPart::Right)
		? (frame.height() - height) / 2
		: std::max(frame.height() - height - radius, 0);
	const auto perline = frame.bytesPerLine();
	const auto addperline = perline - (width * 4);
	auto bits = static_cast<const uchar *>(frame.bits()) + perline * skipy + sizeof(uint32) * skipx;
...
[[nodiscard]] QColor ComputeLockColor(const style::icon &lockIcon, const QImage &frame, RectPart part) {
	return ComputeImageColor(lockIcon, frame, part).value_or(st::windowSubTextFg->c);
}

void ValidatePremiumLockBg(const style::icon &lockIcon, QImage &image, const QImage &frame, RectPart part) {
	if (!image.isNull()) {
		return;
	}
	const auto factor = style::DevicePixelRatio();
	const auto size = lockIcon.size();
	image = QImage(size * factor, QImage::Format_ARGB32_Premultiplied);
	image.setDevicePixelRatio(factor);
	auto p = QPainter(&image);
	const auto color = ComputeLockColor(lockIcon, frame, part);
	p.fillRect(QRect(QPoint(), size), anim::color(color, st::windowSubTextFg, kGrayLockOpacity));
	p.end();
...
void ValidatePremiumStarFg(const style::icon &lockIcon, QImage &image) {
	...
	const auto outer = QRectF(QPointF(), size).marginsRemoved({skip, skip, skip, skip});
	p.setBrush(st::premiumButtonFg);
	p.setPen(Qt::NoPen);
	star.render(&p, outer);
}

[[nodiscard]] TextForMimeData PrepareTextFromEmoji(not_null<DocumentData *> document) {
	...
	return {
		...
		.rich =
			{
				text,
				{EntityInText(EntityType::CustomEmoji, 0, text.size(), Data::SerializeCustomEmojiId(document))},
			},
	};
}
...
StickerPremiumMark::StickerPremiumMark(not_null<Main::Session *> session, const style::icon &lockIcon, RectPart part)
	: _lockIcon(lockIcon), _part(part) {
	...
	Data::AmPremiumValue(session) | rpl::start_with_next([=](bool premium) { _premium = premium; }, _lifetime);
}

void StickerPremiumMark::paint(
	QPainter &p, const QImage &frame, QImage &backCache, QPoint position, QSize singleSize, int outerWidth) {
	validateLock(frame, backCache);
	const auto &bg = frame.isNull() ? _lockGray : backCache;
	const auto factor = style::DevicePixelRatio();
	const auto radius = st::roundRadiusSmall;
	const auto shiftx = (_part == RectPart::Center) ? (singleSize.width() - (bg.width() / factor)) / 2
													: (singleSize.width() - (bg.width() / factor) - radius);
	const auto shifty = (_part == RectPart::Center) ? (singleSize.height() - (bg.height() / factor)) / 2
...
void StickerPremiumMark::validateLock(const QImage &frame, QImage &backCache) {
	auto &image = frame.isNull() ? _lockGray : backCache;
	ValidatePremiumLockBg(_lockIcon, image, frame, _part);
}

void StickerPremiumMark::validateStar() { ValidatePremiumStarFg(_lockIcon, _star); }
...
public:
	...
	[[nodiscard]] bool reorderState() const { return _dragging.enabled; }
	...
	[[nodiscard]] Data::StickersType setType() const {
		return (_setFlags & SetFlag::Emoji) ? Data::StickersType::Emoji
			: (_setFlags & SetFlag::Masks)	? Data::StickersType::Masks
											: Data::StickersType::Stickers;
	}

	[[nodiscard]] bool amSetCreator() const { return _amSetCreator; }
...
private:
	...
	[[nodiscard]] Ui::MessageSendingAnimationFrom messageSentAnimationInfo(int index,
																		   not_null<DocumentData *> document) const;
	[[nodiscard]] QSize boundingBoxSize() const;

	void paintSticker(Painter &p, int index, QPoint position, bool paused, crl::time now) const;
	...
	void clipCallback(Media::Clip::Notification notification, not_null<DocumentData *> document, int index);
	void setupEmoji(int index);
	[[nodiscard]] not_null<Ui::Text::CustomEmoji *> resolveCustomEmoji(not_null<DocumentData *> document);
	...
	void chosen(int index, not_null<DocumentData *> sticker, Api::SendOptions options);
	...
	base::flat_map<not_null<DocumentData *>, std::unique_ptr<Ui::Text::CustomEmoji>> _customEmoji;
...
StickerSetBox::StickerSetBox(QWidget *parent,
							 std::shared_ptr<ChatHelpers::Show> show,
							 const StickerSetIdentifier &set,
							 Data::StickersType type)
	: _show(std::move(show)), _session(&_show->session()), _set(set), _type(type) {}
...
QPointer<Ui::BoxContent> StickerSetBox::Show(std::shared_ptr<ChatHelpers::Show> show,
											 not_null<DocumentData *> document) {
	if (const auto sticker = document->sticker()) {
		if (sticker->set) {
			auto box = Box<StickerSetBox>(show, sticker->set, sticker->setType);
...
void StickerSetBox::prepare() {
	...
	_inner = setInnerWidget(object_ptr<Inner>(this, _show, _set, _type), st::stickersScroll);
	_session->data().stickers().updated(_type) | rpl::start_with_next([=] { updateButtons(); }, lifetime());

	setDimensions(st::boxWideWidth,
				  (_type == Data::StickersType::Emoji ? st::emojiSetMaxHeight : st::stickersMaxHeight));

	updateTitleAndButtons();

	_inner->updateControls() | rpl::start_with_next([=] { updateTitleAndButtons(); }, lifetime());
	...
	_inner->errors() | rpl::start_with_next([=](Error error) { handleError(error); }, lifetime());
	...
	_inner->setArchived() |
		rpl::start_with_next(
			[=](uint64 setId)
			{
				...
				showToast((type == Data::StickersType::Masks) ? tr::lng_masks_has_been_archived(tr::now)
															  : tr::lng_stickers_has_been_archived(tr::now));

				auto &order = (type == Data::StickersType::Masks) ? _session->data().stickers().maskSetsOrderRef()
...
void StickerSetBox::addStickers() { _inner->install(); }

void StickerSetBox::copyStickersLink() {
	const auto part = _inner->isEmojiSet() ? u"addemoji"_q : "addstickers";
	const auto url = _session->createInternalLinkFull(part + '/' + _inner->shortName());
	QGuiApplication::clipboard()->setText(url);
}

void StickerSetBox::handleError(Error error) {
	const auto guard = gsl::finally(crl::guard(this, [=] { closeBox(); }));

	switch (error) {
		case Error::NotFound: _show->showBox(Ui::MakeInformBox(tr::lng_stickers_not_found(tr::now))); break;
...
void ChangeSetNameBox(not_null<Ui::GenericBox *> box,
	...
	box->addRow(object_ptr<Ui::FlatLabel>(box, tr::lng_stickers_box_edit_name_about(), st::boxLabel));
	...
	const auto wrap = box->addRow(object_ptr<Ui::FixedHeightWidget>(box, st::editStickerSetNameField.heightMin));
	auto owned =
		object_ptr<Ui::InputField>(wrap, st::editStickerSetNameField, tr::lng_stickers_context_edit_name(), wasName);
	...
	const auto save = [=, show = box->uiShow()]
	{
		...
		if ((Ui::ComputeRealUnicodeCharactersCount(text) > kMaxSetNameLength) || text.isEmpty()) {
			field->showError();
			return;
		}
		const auto buttonWidth = state->saveButton ? state->saveButton->width() : 0;
		state->requestId = data->session()
							   .api()
							   .request(MTPstickers_RenameStickerSet(Data::InputStickerSet(input), MTP_string(text)))
							   .done(
								   [=](const TLStickerSet &result)
								   {
									   result.match(
										   [&](const MTPDmessages_stickerSet &d)
										   {
	...
	state->saveButton = box->addButton(rpl::conditional(state->requestId.value() | rpl::map(rpl::mappers::_1 > 0),
														rpl::single(QString()),
														tr::lng_box_done()),
									   save);
	if (const auto saveButton = state->saveButton) {
		using namespace Info::Statistics;
		const auto loadingAnimation =
			InfiniteRadialAnimationWidget(saveButton, saveButton->height() / 2, &st::editStickerSetNameLoading);
		AddChildToWidgetCenter(saveButton, loadingAnimation);
		loadingAnimation->showOn(state->requestId.value() | rpl::map(rpl::mappers::_1 > 0));
	}
...
void StickerSetBox::updateButtons() {
	...
	if (_inner->reorderState()) {
		...
		{
			copyStickersLink();
			showToast(type == Data::StickersType::Emoji ? tr::lng_stickers_copied_emoji(tr::now)
		...
		const auto fillSetCreatorMenu = [&]
		{
			...
			return Filler(
				[=, show = _show, set = _set](not_null<Ui::PopupMenu *> menu)
				{
		...
		if (_inner->notInstalled()) {
			if (!_session->premium() && _session->premiumPossible() && _inner->premiumEmojiSet()) {
				const auto &st = st::premiumPreviewDoubledLimitsBox;
				setStyle(st);
				auto button = CreateUnlockButton(this, tr::lng_premium_unlock_emoji());
				button->resizeToWidth(st::boxWideWidth - st.buttonPadding.left() - st.buttonPadding.left());
				...
				auto addText = (type == Data::StickersType::Emoji) ? tr::lng_stickers_add_emoji()
					: (type == Data::StickersType::Masks)		   ? tr::lng_stickers_add_masks()
				...
				top->setClickedCallback(
					[=]
					{
						*menu = base::make_unique_q<Ui::PopupMenu>(top, st::popupMenuWithIcons);
						...
						(*menu)->addAction(((type == Data::StickersType::Emoji) ? tr::lng_stickers_share_emoji
			...
			auto shareText = (type == Data::StickersType::Emoji) ? tr::lng_stickers_share_emoji()
				: (type == Data::StickersType::Masks)			 ? tr::lng_stickers_share_masks()
			...
			if (!_inner->shortName().isEmpty()) {
				const auto top = addTopButton(st::infoTopBarMenu);
				const auto archive = [=] { _inner->archiveStickers(); };
				const auto remove = [=]
				{
					const auto session = &_show->session();
					auto box = ChatHelpers::MakeConfirmRemoveSetBox(session, st::boxLabel, _inner->setId());
					if (box) {
				...
				top->setClickedCallback(
					[=]
					{
						*menu = base::make_unique_q<Ui::PopupMenu>(top, st::popupMenuWithIcons);
						if (type == Data::StickersType::Emoji) {
							(*menu)->addAction(
								tr::lng_custom_emoji_remove_pack_button(tr::now), remove, &st::menuIconRemove);
						} else {
							if (fillSetCreatorMenu) {
								fillSetCreatorMenu(*menu);
							}
							(*menu)->addAction((type == Data::StickersType::Masks
...
StickerSetBox::Inner::Inner(QWidget *parent,
							std::shared_ptr<ChatHelpers::Show> show,
							const StickerSetIdentifier &set,
							Data::StickersType type)
	: RpWidget(parent), _show(std::move(show)), _session(&_show->session()), _api(&_session->mtp()), _setId(set.id),
	  _setAccessHash(set.accessHash), _setShortName(set.shortName),
	  _pathGradient(
		  std::make_unique<Ui::PathShiftGradient>(st::windowBgRipple, st::windowBgOver, [=] { repaintItems(); })),
	  _premiumMark(_session, st::stickersPremiumLock), _updateItemsTimer([=] { updateItems(); }), _input(set),
	  _padding((type == Data::StickersType::Emoji) ? st::emojiSetPadding : st::stickersPadding),
	  _previewTimer([=] { showPreview(); }) {
	setAttribute(Qt::WA_OpaquePaintEvent);

	_api.request(MTPmessages_GetStickerSet(Data::InputStickerSet(_input),
										   MTP_int(0) // hash
										   ))
		.done([=](const TLStickerSet &result) { applySet(result); })
		.fail(
			[=]
			{
				_loaded = true;
	...
	_session->downloaderTaskFinished() | rpl::start_with_next([=] { updateItems(); }, lifetime());
...
void StickerSetBox::Inner::applySet(const TLStickerSet &set) {
	...
	set.match(
		[&](const MTPDmessages_stickerSet &data)
		{
...

			{
				...
				_setTitle = _session->data().stickers().getSetTitle(set);
				...
				_setThumbnail = [&]
				{
					if (const auto thumbs = set.vthumbs()) {
						for (const auto &thumb : thumbs->v) {
							const auto result = Images::FromPhotoSize(_session, set, thumb);
							if (result.location.valid()) {
								_setThumbnailType = Data::ThumbnailTypeFromPhotoSize(thumb);
								return result;
							}
						}
					}
				...
				if (it != sets.cend()) {
					...
					const auto clientFlags =
						set->flags & (SetFlag::Featured | SetFlag::NotLoaded | SetFlag::Unread | SetFlag::Special);
	...
	resize(_padding.left() + _perRow * _singleSize.width(),
...
rpl::producer<uint64> StickerSetBox::Inner::setInstalled() const { return _setInstalled.events(); }

rpl::producer<uint64> StickerSetBox::Inner::setArchived() const { return _setArchived.events(); }

rpl::producer<> StickerSetBox::Inner::updateControls() const { return _updateControls.events(); }

rpl::producer<StickerSetBox::Error> StickerSetBox::Inner::errors() const { return _errors.events(); }
...
void StickerSetBox::Inner::installDone(const MTPmessages_StickerSetInstallResult &result) {
	...
	if (wasArchived && type != Data::StickersType::Emoji) {
		const auto index = ((type == Data::StickersType::Masks) ? stickers.archivedMaskSetsOrderRef()
																: stickers.archivedSetsOrderRef())
							   .indexOf(_setId);
		if (index >= 0) {
			((type == Data::StickersType::Masks) ? stickers.archivedMaskSetsOrderRef()
												 : stickers.archivedSetsOrderRef())
				.removeAt(index);
		}
	}
	...
	if (it == sets.cend()) {
		it = sets.emplace(_setId,
						  std::make_unique<StickersSet>(&_session->data(),
														_setId,
														_setAccessHash,
														_setHash,
														_setTitle,
	...
	auto &order = (type == Data::StickersType::Emoji) ? stickers.emojiSetsOrderRef()
		: (type == Data::StickersType::Masks)		  ? stickers.maskSetsOrderRef()
...
void StickerSetBox::Inner::mouseMoveEvent(QMouseEvent *e) {
	...
	if (_dragging.index >= 0 && _dragging.index < _pack.size() && _dragging.lastSelected >= 0 && !draggedAnimating) {
		for (auto i = 0; i < _pack.size(); i++) {
			...
			} else if ((i < _dragging.index) && (i >= _dragging.lastSelected)) {
				if (entry.shift == 0) {
					entry.shift = 1;
				} else if (entry.shift == -1) {
					entry.shift = 0;
			...
			if (wasShift != entry.shift) {
				...
				const auto ratio = [&]
				{
					...
					const auto ratioX = std::min(toX, fromX) / std::max(toX, fromX);
					const auto fromY = entry.yAnimation.value(toY);
					const auto ratioY = std::min(toY, fromY) / std::max(toY, fromY);
					return (ratioX == 1.) ? ratioY : (ratioY == 1.) ? ratioX : std::max(ratioX, ratioY);
				}();
				if (!entry.animation.animating()) {
					entry.animation.stop();
					entry.animation.start([=] { update(); }, fromPoint.x(), toX, kStickerMoveDuration);
				} else {
					entry.animation.change(toX, kStickerMoveDuration * (1. - ratio), anim::linear);
				}
				if (!entry.yAnimation.animating()) {
					entry.yAnimation.stop();
					entry.yAnimation.start([=] { update(); }, fromPoint.y(), toY, kStickerMoveDuration);
				} else {
					entry.yAnimation.change(toY, kStickerMoveDuration * (1. - ratio), anim::linear);
				}
			}
		}
		update();
...
void StickerSetBox::Inner::showPreviewAt(QPoint globalPos) {
	...
	if (index >= 0 && index < _pack.size() && index != _previewShown) {
		_previewShown = index;
		_show->showMediaPreview(Data::FileOriginStickerSet(_setId, _setAccessHash), _pack[_previewShown]);
	}
}

void StickerSetBox::Inner::leaveEventHook(QEvent *e) { setSelected(-1); }

void StickerSetBox::Inner::requestReorder(not_null<DocumentData *> document, int index) {
	if (!_apiReorder) {
		_apiReorder.emplace(&_session->mtp());
	}
	_reorderRequests.emplace_back(
		[document, index, this]
		{
			_apiReorder->request(MTPstickers_ChangeStickerPosition(document->mtpInput(), MTP_int(index)))
				.done(
					[this, document](const TLStickerSet &result)
					{
						result.match(
							[&](const MTPDmessages_stickerSet &d)
							{
								document->owner().stickers().feedSetFull(d);
								document->owner().stickers().notifyUpdated(Data::StickersType::Stickers);
							},
							[](const auto &) {});
				...
				.fail([show = _show](const MTP::Error &error) { show->showToast(error.type()); })
				.send();
		});
	...
	if (_dragging.index >= 0 && !isDraggedAnimating()) {
		...
		entry.animation.start(
			[finish, toPos, this](float64 value)
			{
				...
				if (value >= toPos.x() && index >= 0 && !_shiftAnimations[index].yAnimation.animating()) {
					finish();
				}
				update();
			},
		...
		entry.yAnimation.start(
			...
			{
				const auto index = _dragging.index;
				if (value >= toPos.y() && index >= 0 && !_shiftAnimations[index].animation.animating()) {
					finish();
				}
				update();
			},
...
void StickerSetBox::Inner::chosen(int index, not_null<DocumentData *> sticker, Api::SendOptions options) {
	const auto animation =
		options.scheduled ? Ui::MessageSendingAnimationFrom() : messageSentAnimationInfo(index, sticker);
...
auto StickerSetBox::Inner::messageSentAnimationInfo(int index, not_null<DocumentData *> document) const
	...
	const auto size = ChatHelpers::ComputeStickerSize(document, boundingBoxSize());
	const auto innerPos = QPoint((rect.width() - size.width()) / 2, (rect.height() - size.height()) / 2);
	return {
		.type = Ui::MessageSendingAnimationFrom::Type::Sticker,
		.localId = _session->data().nextLocalMessageId(),
		.globalStartGeometry = mapToGlobal(QRect(rect.topLeft() + innerPos, size)),
	};
}

void StickerSetBox::Inner::contextMenuEvent(QContextMenuEvent *e) {
	const auto index = stickerFromGlobalPos(e->globalPos());
	if (index < 0 || index >= _pack.size() || setType() == Data::StickersType::Masks) {
		return;
	}
	_previewTimer.cancel();
	...
	if (setType() == Data::StickersType::Emoji) {
		...
		const auto send = crl::guard(this, [=](Api::SendOptions options) { chosen(index, document, options); });
		...
		SendMenu::FillSendMenu(_menu.get(),
		...
		{ Api::ToggleFavedSticker(show, document, Data::FileOriginStickerSet(Data::Stickers::FavedSetId, 0)); };
		const auto isFaved = document->owner().stickers().isFaved(document);
		_menu->addAction((isFaved ? tr::lng_faved_stickers_remove : tr::lng_faved_stickers_add)(tr::now),
						 toggleFavedSticker,
						 (isFaved ? &st::menuIconUnfave : &st::menuIconFave));
		if (amSetCreator()) {
			const auto addAction = Ui::Menu::CreateAddActionCallback(_menu.get());
			addAction({
				.text = tr::lng_stickers_context_delete(tr::now),
				.handler = [index, this, show = _show]
				{ show->showBox(Box([=](not_null<Ui::GenericBox *> box) { fillDeleteStickerBox(box, index); })); },
				.icon = &st::menuIconDeleteAttention,
				.isAttention = true,
			});
		}

		SendMenu::AttachSendMenuEffect(_menu.get(), _show, details, SendMenu::DefaultCallback(_show, send));
	}
	if (_menu->empty()) {
		_menu = nullptr;
	} else {
...
void StickerSetBox::Inner::fillDeleteStickerBox(not_null<Ui::GenericBox *> box, int index) {
		...
		rpl::start_with_next(
			[=]
			{
				...
				if (const auto strong = weak.data()) {
					const auto paused =
						On(PowerSaving::kStickersPanel) || show->paused(ChatHelpers::PauseReason::Layer);
			...
			sticker->lifetime());
	const auto label =
		Ui::CreateChild<Ui::FlatLabel>(line, tr::lng_stickers_context_delete(), box->getDelegate()->style().title);
	line->widthValue() |
		rpl::start_with_next(
			[=](int width)
			{
				...
				label->resizeToWidth(width - rect::right(sticker) - skip - st::boxRowPadding.right());
				label->moveToLeft(rect::right(sticker) + skip, ((sticker->height() - label->height()) / 2));
			},
			label->lifetime());
	...
	box->addRow(object_ptr<Ui::FlatLabel>(container, tr::lng_stickers_context_delete_sure(), st::boxLabel));
	const auto save = [=]
	{
		if (state->requestId.current()) {
			return;
		...
		const auto buttonWidth = state->saveButton ? state->saveButton->width() : 0;
		state->requestId = document->owner()
							   .session()
							   .api()
							   .request(MTPstickers_RemoveStickerFromSet(document->mtpInput()))
							   .done(
								   [=](const TLStickerSet &result)
								   {
									   result.match(
	...
	box->addButton(tr::lng_close(),
				   [=]
				   {
					   document->owner().session().api().request(state->requestId.current()).cancel();
					   box->closeBox();
				   });
}
...
void StickerSetBox::Inner::setSelected(int selected) {
	if (_selected != selected) {
		...
		setCursor((_selected < 0) ? style::cur_default : _dragging.enabled ? style::cur_sizeall : style::cur_pointer);
	}
}
...
bool StickerSetBox::Inner::isDraggedAnimating() const {
	...
	return (it == _shiftAnimations.end()) ? false
										  : (it->second.animation.animating() || it->second.yAnimation.animating());
}

not_null<Lottie::MultiPlayer *> StickerSetBox::Inner::getLottiePlayer() {
	if (!_lottiePlayer) {
		_lottiePlayer = std::make_unique<Lottie::MultiPlayer>(Lottie::Quality::Default, Lottie::MakeFrameRenderer());
		_lottiePlayer->updates() | rpl::start_with_next([=] { updateItems(); }, lifetime());
	}
...
void StickerSetBox::Inner::paintEvent(QPaintEvent *e) {
	...
	int32 from = qFloor(e->rect().top() / _singleSize.height()),
		  to = qFloor(e->rect().bottom() / _singleSize.height()) + 1;

	_pathGradient->startFrame(0, width(), width() / 2);

	const auto indexUnderCursor =
		(_dragging.index >= 0 && _dragging.index < _elements.size()) ? stickerFromGlobalPos(QCursor::pos()) : -2;
	const auto lastIndex = indexUnderCursor >= 0 ? indexUnderCursor : _dragging.lastSelected;

	const auto now = crl::now();
	const auto paused = On(PowerSaving::kStickersPanel) || _show->paused(ChatHelpers::PauseReason::Layer);
		...
		for (int32 j = 0; j < _perRow; ++j) {
			...
			if (lastIndex >= 0) {
				...
				if (it != _shiftAnimations.end()) {
					...
					const auto pos = QPoint(entry.animation.value(toPos.x()), entry.yAnimation.value(toPos.y()));
			...
			const auto pos =
				QPoint(_padding.left() + j * _singleSize.width(), _padding.top() + i * _singleSize.height());
			paintSticker(p, index, pos, paused, now);
		}
	}
	if (_dragging.index >= 0 && _dragging.index < _elements.size()) {
		const auto pos = isDraggedAnimating() ? QPoint(_shiftAnimations[_dragging.index].animation.value(0),
...
bool StickerSetBox::Inner::isEmojiSet() const { return (_setFlags & Data::StickersSetFlag::Emoji); }

uint64 StickerSetBox::Inner::setId() const { return _setId; }

QSize StickerSetBox::Inner::boundingBoxSize() const {
	if (isEmojiSet()) {
		using namespace Data;
		const auto size = FrameSizeFromTag(CustomEmojiSizeTag::Large) / style::DevicePixelRatio();
		return {size, size};
	}
	return QSize(_singleSize.width() - st::roundRadiusSmall * 2, _singleSize.height() - st::roundRadiusSmall * 2);
}

void StickerSetBox::Inner::visibleTopBottomUpdated(int visibleTop, int visibleBottom) {
	...
	if (visibleTop >= rowsTop + singleHeight && visibleTop < rowsBottom) {
		...
		const auto pauseRows = std::min(pauseHeight / singleHeight, _rowsCount);
		pauseInRows(0, pauseRows);
	}
	if (visibleBottom > rowsTop && visibleBottom + singleHeight <= rowsBottom) {
...
void StickerSetBox::Inner::setupLottie(int index) {
	...
	element.lottie = ChatHelpers::LottieAnimationFromDocument(getLottiePlayer(),
...
void StickerSetBox::Inner::setupWebm(int index) {
	...
	auto callback = [=](Media::Clip::Notification notification) { clipCallback(notification, document, index); };
	element.webm = Media::Clip::MakeReader(
		element.documentMedia->owner()->location(), element.documentMedia->bytes(), std::move(callback));
}

void StickerSetBox::Inner::clipCallback(Media::Clip::Notification notification,
										not_null<DocumentData *> document,
										int index) {
	const auto i = (index < _elements.size() && _elements[index].document == document)
	...
	switch (notification) {
		case Notification::Reinit: {
			...
			if (webm->state() == State::Error) {
				...
				const auto size = ChatHelpers::ComputeStickerSize(i->document, boundingBoxSize());
...
not_null<Ui::Text::CustomEmoji *> StickerSetBox::Inner::resolveCustomEmoji(not_null<DocumentData *> document) {
	const auto i = _customEmoji.find(document);
	if (i != end(_customEmoji)) {
		return i->second.get();
	}
	auto emoji = document->session().data().customEmojiManager().create(
		document, [=] { customEmojiRepaint(); }, Data::CustomEmojiManager::SizeTag::Large);
	return _customEmoji.emplace(document, std::move(emoji)).first->second.get();
}
...
void StickerSetBox::Inner::shakeTransform(QPainter &p, int index, QPoint position, crl::time now) const {
	...
	const auto diff = ((index % 2) ? 0 : kShakeYDuration / 2) + (now - _shakeAnimation.started());
	const auto pX = (diff % kShakeXDuration) / float64(kShakeXDuration);
	const auto pY = (diff % kShakeYDuration) / float64(kShakeYDuration);
	const auto pA = (diff % kShakeADuration) / float64(kShakeADuration);
	...
	// 0, -kMaxA, 0, kMaxA, 0.
	const auto angle = (pA < kAStep) ? anim::interpolateF(0., -kMaxA, pA / kAStep)
		: (pA < kAStep * 2.)		 ? anim::interpolateF(-kMaxA, 0, (pA - kAStep) / kAStep)
		: (pA < kAStep * 3.)		 ? anim::interpolateF(0, kMaxA, (pA - kAStep * 2.) / kAStep)
		: (pA < kAStep * 4.)		 ? anim::interpolateF(kMaxA, 0, (pA - kAStep * 3.) / kAStep)
									 : anim::interpolateF(0, 0., (pA - kAStep * 4.) / kAStep);

	// 0, kMaxTranslation, 0, -kMaxTranslation, 0.
	const auto x = (pX < kXStep) ? anim::interpolateF(0., kMaxTranslation, pX / kXStep)
		: (pX < kXStep * 2.)	 ? anim::interpolateF(kMaxTranslation, 0, (pX - kXStep) / kXStep)
		: (pX < kXStep * 3.)	 ? anim::interpolateF(0, -kMaxTranslation, (pX - kXStep * 2.) / kXStep)
		: (pX < kXStep * 4.)	 ? anim::interpolateF(-kMaxTranslation, 0, (pX - kXStep * 3.) / kXStep)
								 : anim::interpolateF(0, 0., (pX - kXStep * 4.) / kXStep);

	// 0, kMaxTranslation, -kMaxTranslation, 0.
	const auto y = (pY < kYStep) ? anim::interpolateF(0., kMaxTranslation, pY / kYStep)
		: (pY < kYStep * 2.)	 ? anim::interpolateF(kMaxTranslation, 0, (pY - kYStep) / kYStep)
		: (pY < kYStep * 3.)	 ? anim::interpolateF(0, -kMaxTranslation, (pY - kYStep * 2.) / kYStep)
								 : anim::interpolateF(-kMaxTranslation, 0, (pY - kYStep * 3) / kYStep);

	const auto center = position + QPoint(_singleSize.width() / 2, _singleSize.height() / 2);
...
void StickerSetBox::Inner::paintSticker(Painter &p, int index, QPoint position, bool paused, crl::time now) const {
	if (_dragging.index != index) {
		const auto over = _elements[index].overAnimation.value((index == _selected) ? 1. : 0.);
		if (over) {
			p.setOpacity(over);
			Ui::FillRoundRect(
				p,
				QRect(rtl() ? QPoint(width() - position.x() - _singleSize.width(), position.y()) : position,
					  _singleSize),
				st::emojiPanHover,
	...
	const auto size = ChatHelpers::ComputeStickerSize(document, boundingBoxSize());
	const auto ppos =
		position + QPoint((_singleSize.width() - size.width()) / 2, (_singleSize.height() - size.height()) / 2);
	...
	if (element.emoji) {
		...
		p.drawImage(QRect(ppos, lottieFrame.size() / style::DevicePixelRatio()), lottieFrame);
		...
		if (premium) {
			lottieFrame = pixmap.toImage().convertToFormat(QImage::Format_ARGB32_Premultiplied);
		}
	} else {
		ChatHelpers::PaintStickerThumbnailPath(p, media.get(), QRect(ppos, size), _pathGradient.get());
	}
	...
	if (premium) {
		_premiumMark.paint(p, lottieFrame, element.premiumLock, position, _singleSize, width());
	}
	if (hasShake) {
		p.resetTransform();
	}
...
bool StickerSetBox::Inner::loaded() const { return _loaded && !_pack.isEmpty(); }

bool StickerSetBox::Inner::premiumEmojiSet() const {
	return (_setFlags & SetFlag::Emoji) && !_pack.empty() && _pack.front()->isPremiumEmoji();
}
...
bool StickerSetBox::Inner::notInstalled() const {
	...
	const auto &sets = _session->data().stickers().sets();
	const auto it = sets.find(_setId);
	if ((it == sets.cend()) || !(it->second->flags & SetFlag::Installed) || (it->second->flags & SetFlag::Archived)) {
		return !_pack.empty();
	}
	return false;
}

bool StickerSetBox::Inner::official() const { return _loaded && _setShortName.isEmpty(); }
...
QString StickerSetBox::Inner::shortName() const { return _setShortName; }

void StickerSetBox::Inner::install() {
	if (_installRequest) {
		return;
	}
	_installRequest = _api.request(MTPmessages_InstallStickerSet(Data::InputStickerSet(_input), MTP_bool(false)))
						  .done([=](const MTPmessages_StickerSetInstallResult &result) { installDone(result); })
						  .fail([=] { _errors.fire(Error::NotFound); })
						  .send();
}

void StickerSetBox::Inner::archiveStickers() {
	_api.request(MTPmessages_InstallStickerSet(Data::InputStickerSet(_input), MTP_boolTrue()))
		.done(
		...
		.fail([=] { _show->showToast(Lang::Hard::ServerError()); })
		.send();
}
...
void StickerSetBox::Inner::updateItems() {
	...
	const auto delay = std::max(_lastScrolledAt + kMinAfterScrollDelay - now, _lastUpdatedAt + kMinRepaintDelay - now);
	if (delay <= 0) {
		repaintItems(now);
	} else if (!_updateItemsTimer.isActive() || _updateItemsTimer.remainingTime() > kMinRepaintDelay) {
