...
[[nodiscard]] int CurrentCount() {
	return std::clamp(Core::App().settings().notificationsCount(), 1, kMaxNotificationsCount);
}
...
{
public:
	NotificationsCount(QWidget *parent, not_null<Window::SessionController *> controller);
...
void AddTypeButton(not_null<Ui::VerticalLayout *> container,
	...
	const auto button = AddButtonWithIcon(container, std::move(label), st::settingsNotificationType, {icon});
	button->setClickedCallback([=] { showOther(NotificationsType::Id(type)); });

	const auto session = &controller->session();
	const auto settings = &session->data().notifySettings();
	const auto &st = st::settingsNotificationType;
	auto status = rpl::combine(NotificationsEnabledForTypeValue(session, type),
							   rpl::single(type) |
								   rpl::then(settings->exceptionsUpdates() | rpl::filter(rpl::mappers::_1 == type))) |
		rpl::map(
					  [=](bool enabled, const auto &)
					  {
						  const auto count = int(settings->exceptions(type).size());
						  return !count ? tr::lng_notification_click_to_change()
										: (enabled ? tr::lng_notification_on : tr::lng_notification_off)(
											  lt_exceptions,
											  tr::lng_notification_exceptions(lt_count, rpl::single(float64(count))));
					  }) |
		rpl::flatten_latest();
	const auto details =
		Ui::CreateChild<Ui::FlatLabel>(button.get(), std::move(status), st::settingsNotificationTypeDetails);
	details->show();
	details->moveToLeft(st.padding.left(), st.padding.top() + st.height - details->height());
	...
	const auto checkView = button->lifetime().make_state<Ui::ToggleView>(
		st.toggle, NotificationsEnabledForType(session, type), [=] { toggleButton->update(); });
	...
	button->geometryValue() |
		rpl::start_with_next(
			[=](const QRect &r)
			{
				...
				separator->setGeometry(toggleButton->x() - st::lineWidth,
				...
				if (!count) {
					...
					controller->show(Box(
						[=](not_null<Ui::GenericBox *> box)
						{
							...
							{
								switch (type) {
									case Type::User: return tr::lng_notification_about_private_chats;
									case Type::Group: return tr::lng_notification_about_groups;
									case Type::Broadcast: return tr::lng_notification_about_channels;
								}
							...
							Ui::ConfirmBox(
								box,
								{
									.text = phrase(lt_count, rpl::single(float64(count)), Ui::Text::RichLangValue),
									.confirmed =
										[=](auto close)
									{
										toggle();
							...
							box->addLeftButton(tr::lng_notification_exceptions_view(),
											   [=]
											   {
												   box->closeBox();
...
NotificationsCount::NotificationsCount(QWidget *parent, not_null<Window::SessionController *> controller)
	: _controller(controller), _chosenCorner(Core::App().settings().notificationsCorner()), _oldCount(CurrentCount()) {
...
void NotificationsCount::paintEvent(QPaintEvent *e) {
	...
	for (int corner = 0; corner != 4; ++corner) {
		...
		auto sampleLeft = isLeft
			? (screenRect.x() + st::notificationsSampleSkip)
			: (screenRect.x() + screenRect.width() - st::notificationsSampleSkip - st::notificationSampleSize.width());
...
void NotificationsCount::setCount(int count) {
	...
	if (count != Core::App().settings().notificationsCount()) {
		...
		Core::App().notifications().notifySettingsChanged(ChangeType::MaxCount);
	}
}

int NotificationsCount::getContentLeft() const { return (width() - st::notificationsBoxMonitor.width()) / 2; }

QRect NotificationsCount::getScreenRect() const { return getScreenRect(width()); }
...
QRect NotificationsCount::getScreenRect(int width) const {
	...
	return QRect(
		screenLeft, screenTop, st::notificationsBoxScreenSize.width(), st::notificationsBoxScreenSize.height());
}
...
void NotificationsCount::prepareNotificationSampleSmall() {
	...
	auto sampleImage = QImage(QSize(width, height) * style::DevicePixelRatio(), QImage::Format_ARGB32_Premultiplied);
...
void NotificationsCount::prepareNotificationSampleUserpic() {
	if (_notificationSampleUserpic.isNull()) {
		_notificationSampleUserpic =
			Ui::PixmapFromImage(Window::LogoNoMargin().scaled(st::notifyPhotoSize * style::DevicePixelRatio(),
															  st::notifyPhotoSize * style::DevicePixelRatio(),
															  Qt::IgnoreAspectRatio,
															  Qt::SmoothTransformation));
		_notificationSampleUserpic.setDevicePixelRatio(style::DevicePixelRatio());
	}
}
...
void NotificationsCount::prepareNotificationSampleLarge() {
	...
	auto sampleImage =
		QImage(w * style::DevicePixelRatio(), h * style::DevicePixelRatio(), QImage::Format_ARGB32_Premultiplied);
	sampleImage.setDevicePixelRatio(style::DevicePixelRatio());
	sampleImage.fill(st::notificationBg->c);
	{
		Painter p(&sampleImage);
		...
		p.fillRect(st::notifyBorderWidth,
				   h - st::notifyBorderWidth,
				   w - st::notifyBorderWidth,
				   st::notifyBorderWidth,
				   st::notifyBorder->b);
		...
		auto rectForName = style::rtlrect(st::notifyPhotoPos.x() + st::notifyPhotoSize + st::notifyTextLeft,
										  st::notifyTextTop,
										  itemWidth,
		...
		auto notifyTitle = st::msgNameFont->elided(u"ViGram Desktop"_q, rectForName.width());
		p.drawText(rectForName.left(), rectForName.top() + st::msgNameFont->ascent, notifyTitle);

		st::notifyClose.icon.paint(p,
								   w - st::notifyClosePos.x() - st::notifyClose.width +
									   st::notifyClose.iconPosition.x(),
								   st::notifyClosePos.y() + st::notifyClose.iconPosition.y(),
								   w);
	}
...
void NotificationsCount::mouseMoveEvent(QMouseEvent *e) {
	...
	auto topRight = style::rtlrect(
		screenRect.x() + screenRect.width() - cornerWidth, screenRect.y(), cornerWidth, cornerHeight, width());
	auto bottomRight = style::rtlrect(screenRect.x() + screenRect.width() - cornerWidth,
									  screenRect.y() + screenRect.height() - cornerHeight,
									  cornerWidth,
									  cornerHeight,
									  width());
	auto bottomLeft = style::rtlrect(
		screenRect.x(), screenRect.y() + screenRect.height() - cornerHeight, cornerWidth, cornerHeight, width());
...
void NotificationsCount::leaveEventHook(QEvent *e) { clearOverCorner(); }
...
void NotificationsCount::setOverCorner(ScreenCorner corner) {
	if (_isOverCorner) {
		...
		Core::App().notifications().notifySettingsChanged(ChangeType::DemoIsShown);
	}
	_overCorner = corner;
...
void NotificationsCount::clearOverCorner() {
	if (_isOverCorner) {
		...
		Core::App().notifications().notifySettingsChanged(ChangeType::DemoIsHidden);
...
void NotificationsCount::mouseReleaseEvent(QMouseEvent *e) {
	...
	if (isDownCorner && _isOverCorner && _downCorner == _overCorner && _downCorner != _chosenCorner) {
		...
		if (_chosenCorner != Core::App().settings().notificationsCorner()) {
			...
			Core::App().saveSettingsDelayed();
			Core::App().notifications().notifySettingsChanged(ChangeType::Corner);
		}
	}
}
...
NotificationsCount::SampleWidget::SampleWidget(NotificationsCount *owner, const QPixmap &cache)
	: _owner(owner), _cache(cache) {
	setFixedSize(cache.width() / cache.devicePixelRatio(), cache.height() / cache.devicePixelRatio());
...
void NotificationsCount::SampleWidget::startAnimation() {
	_opacity.start([=] { animationCallback(); }, _hiding ? 1. : 0., _hiding ? 0. : 1., st::notifyFastAnim);
}
...
NotifyPreview::NotifyPreview(bool nameShown, bool previewShown)
	: _nameShown(nameShown), _previewShown(previewShown), _roundRect(st::boxRadius, st::msgInBg),
	  _userpic(u":/gui/icons/settings/dino.svg"_q), _logo(Window::LogoNoMargin()) {
	const auto ratio = style::DevicePixelRatio();
	_logo = _logo.scaledToWidth(st::notifyPreviewUserpicSize * ratio, Qt::SmoothTransformation);
	_logo.setDevicePixelRatio(ratio);

	_name.setText(st::defaultSubsectionTitle.style, tr::lng_notification_preview_title(tr::now));
	_title.setText(st::defaultSubsectionTitle.style, AppName.utf16());

	_text.setText(st::boxTextStyle, tr::lng_notification_preview_text(tr::now));
	_preview.setText(st::boxTextStyle, tr::lng_notification_preview(tr::now));
}

void NotifyPreview::setNameShown(bool shown) { _nameShown = shown; }

void NotifyPreview::setPreviewShown(bool shown) { _previewShown = shown; }

int NotifyPreview::resizeGetHeight(int newWidth) {
	_width = newWidth;
	_height =
		st::notifyPreviewUserpicPosition.y() + st::notifyPreviewUserpicSize + st::notifyPreviewUserpicPosition.y();
	const auto available = _width - st::notifyPreviewTextPosition.x() - st::notifyPreviewUserpicPosition.x();
...
void NotifyPreview::paint(Painter &p, int x, int y) {
	...
	const auto userpic =
		QRect(st::notifyPreviewUserpicPosition, QSize{st::notifyPreviewUserpicSize, st::notifyPreviewUserpicSize});
...
NotifyViewCheckboxes SetupNotifyViewOptions(not_null<Window::SessionController *> controller,
											not_null<Ui::VerticalLayout *> container,
											bool nameShown,
											bool previewShown) {
	using namespace rpl::mappers;

	auto wrap = container->add(object_ptr<Ui::SlideWrap<>>(container, object_ptr<Ui::RpWidget>(container)));
	const auto widget = wrap->entity();

	const auto makeCheckbox = [&](const QString &text, bool checked)
	{
		return Ui::MakeChatServiceCheckbox(widget, text, st::backgroundCheckbox, st::backgroundCheck, checked)
			.release();
	};
	const auto name = makeCheckbox(tr::lng_notification_show_name(tr::now), nameShown);
	const auto preview = makeCheckbox(tr::lng_notification_show_text(tr::now), previewShown);

	const auto view = widget->lifetime().make_state<NotifyPreview>(nameShown, previewShown);
	widget->widthValue() | rpl::filter(_1 >= (st::historyMinimalWidth / 2)) |
		rpl::start_with_next(
			[=](int width)
			{
				...
				const auto checksTop = height - (margins.bottom() + name->height()) / 2;
	...
	widget->paintRequest() |
		rpl::start_with_next(
			[=](QRect rect)
			{
				Window::SectionWidget::PaintBackground(controller,
													   controller->defaultChatTheme().get(), // #TODO themes
													   widget,
													   rect);

				Painter p(widget);
				view->paint(p, st::notifyPreviewMargins.left(), st::notifyPreviewMargins.top());
			},
			widget->lifetime());
...
void SetupAdvancedNotifications(not_null<Window::SessionController *> controller,
								not_null<Ui::VerticalLayout *> container) {
	if (Platform::IsWindows()) {
		const auto skipInFocus =
			container->add(object_ptr<Button>(container, tr::lng_settings_skip_in_focus(), st::settingsButtonNoIcon))
				->toggleOn(rpl::single(Core::App().settings().skipToastsInFocus()));

		skipInFocus->toggledChanges() |
			rpl::filter([](bool checked) { return (checked != Core::App().settings().skipToastsInFocus()); }) |
			rpl::start_with_next(
				[=](bool checked)
				{
					...
					if (checked && Platform::Notifications::SkipToastForCustom()) {
						...
						Core::App().notifications().notifySettingsChanged(Change::DesktopEnabled);
					}
				},
	...
	Ui::AddSkip(container, st::settingsCheckboxesSkip);
	Ui::AddSubsectionTitle(container, tr::lng_settings_notifications_position());
	Ui::AddSkip(container, st::settingsCheckboxesSkip);

	const auto position = container->add(object_ptr<NotificationsCount>(container, controller));

	Ui::AddSkip(container, st::settingsCheckboxesSkip);
	Ui::AddSubsectionTitle(container, tr::lng_settings_notifications_count());

	const auto count =
		container->add(object_ptr<Ui::SettingsSlider>(container, st::settingsSlider), st::settingsBigScalePadding);
	for (int i = 0; i != kMaxNotificationsCount; ++i) {
		count->addSection(QString::number(i + 1));
	}
	count->setActiveSectionFast(CurrentCount() - 1);
	count->sectionActivated() |
		rpl::start_with_next([=](int section) { position->setCount(section + 1); }, count->lifetime());
	Ui::AddSkip(container, st::settingsCheckboxesSkip);
}

void SetupMultiAccountNotifications(not_null<Window::SessionController *> controller,
									not_null<Ui::VerticalLayout *> container) {
	...
	const auto fromAll =
		container->add(object_ptr<Button>(container, tr::lng_settings_notify_all(), st::settingsButtonNoIcon))
			->toggleOn(rpl::single(Core::App().settings().notifyFromAll()));
	fromAll->toggledChanges() |
		rpl::filter([](bool checked) { return (checked != Core::App().settings().notifyFromAll()); }) |
		rpl::start_with_next(
			[=](bool checked)
			{
...
void SetupNotificationsContent(not_null<Window::SessionController *> controller,
							   not_null<Ui::VerticalLayout *> container,
							   Fn<void(Type)> showOther) {
	using namespace rpl::mappers;
	...
	const auto session = &controller->session();
	const auto checkbox = [&](rpl::producer<QString> label, IconDescriptor &&descriptor, rpl::producer<bool> checked)
	{
		auto result = CreateButtonWithIcon(container, std::move(label), st::settingsButton, std::move(descriptor));
		result->toggleOn(std::move(checked));
		return result;
	};
	const auto addCheckbox = [&](rpl::producer<QString> label, IconDescriptor &&descriptor, rpl::producer<bool> checked)
	{ return container->add(checkbox(std::move(label), std::move(descriptor), std::move(checked))); };
	const auto &settings = Core::App().settings();
	const auto desktopToggles = container->lifetime().make_state<rpl::event_stream<bool>>();
	const auto desktop = addCheckbox(tr::lng_settings_desktop_notify(),
									 {&st::menuIconNotifications},
									 desktopToggles->events_starting_with(settings.desktopNotify()));

	const auto flashbounceToggles = container->lifetime().make_state<rpl::event_stream<bool>>();
	const auto flashbounce = addCheckbox((Platform::IsWindows()	  ? tr::lng_settings_alert_windows
											  : Platform::IsMac() ? tr::lng_settings_alert_mac
																  : tr::lng_settings_alert_linux)(),
										 {&st::menuIconDockBounce},
										 flashbounceToggles->events_starting_with(settings.flashBounceNotify()));

	const auto soundAllowed = container->lifetime().make_state<rpl::event_stream<bool>>();
	const auto allowed = [=] { return Core::App().settings().soundNotify(); };
	const auto sound = addCheckbox(
		tr::lng_settings_sound_allowed(), {&st::menuIconUnmute}, soundAllowed->events_starting_with(allowed()));

	Ui::AddSkip(container);

	const auto checkboxes = SetupNotifyViewOptions(controller,
												   container,
	...
	const auto previewDivider = container->add(
		object_ptr<Ui::SlideWrap<Ui::BoxContentDivider>>(container, object_ptr<Ui::BoxContentDivider>(container)));
	...
	const auto addType = [&](Data::DefaultNotify type) { AddTypeButton(container, controller, type, showOther); };
	...
	auto joinSilent = rpl::single(session->api().contactSignupSilentCurrent().value_or(false)) |
		rpl::then(session->api().contactSignupSilent());
	const auto joined =
		addCheckbox(tr::lng_settings_events_joined(), {&st::menuIconInvite}, std::move(joinSilent) | rpl::map(!_1));
	joined->toggledChanges() |
		...
		rpl::start_with_next([=](bool enabled) { session->api().saveContactSignupSilent(!enabled); },
							 joined->lifetime());

	const auto pinned =
		addCheckbox(tr::lng_settings_events_pinned(),
					{&st::menuIconPin},
					rpl::single(settings.notifyAboutPinned()) | rpl::then(settings.notifyAboutPinnedChanges()));
	pinned->toggledChanges() |
		rpl::filter([=](bool notify) { return (notify != Core::App().settings().notifyAboutPinned()); }) |
		rpl::start_with_next(
			[=](bool notify)
			{
	...
	Ui::AddDivider(container);
	Ui::AddSkip(container, st::settingsCheckboxesSkip);
	Ui::AddSubsectionTitle(container, tr::lng_settings_notifications_calls_title());
	...
	acceptCalls->toggledChanges() |
		rpl::filter([=](bool toggled) { return (toggled == authorizations->callsDisabledHere()); }) |
		rpl::start_with_next([=](bool toggled) { authorizations->toggleCallsDisabledHere(!toggled); },
							 container->lifetime());
	...
	const auto muted =
		container->add(object_ptr<Button>(container, tr::lng_settings_include_muted(), st::settingsButtonNoIcon));
	muted->toggleOn(rpl::single(settings.includeMutedCounter()));
	const auto mutedFolders = session->data().chatsFilters().has()
		? container->add(
			  object_ptr<Button>(container, tr::lng_settings_include_muted_folders(), st::settingsButtonNoIcon))
		: nullptr;
	if (mutedFolders) {
		mutedFolders->toggleOn(rpl::single(settings.includeMutedCounterFolders()));
	}
	const auto count =
		container->add(object_ptr<Button>(container, tr::lng_settings_count_unread(), st::settingsButtonNoIcon));
	count->toggleOn(rpl::single(settings.countUnreadMessages()));

	auto nativeText = [&]
	{
		if (!Platform::Notifications::Supported() || Platform::Notifications::Enforced()) {
	...
	const auto native = [&]() -> Ui::SettingsButton *
	{
		...
		return container->add(object_ptr<Button>(container, std::move(nativeText), st::settingsButtonNoIcon))
	...
	const auto advancedSlide = !Platform::Notifications::Enforced()
		...
		: nullptr;
	const auto advancedWrap = advancedSlide ? advancedSlide->entity() : nullptr;
	if (advancedWrap) {
	...
	sound->toggledChanges() |
		rpl::filter([](bool checked) { return (checked != Core::App().settings().soundNotify()); }) |
		rpl::start_with_next(
			[=](bool checked)
			{
	...
	name->checkedChanges() |
		...
		rpl::filter([=](NotifyView value) { return (value != Core::App().settings().notifyView()); }) |
		rpl::start_with_next(
			[=](NotifyView value)
			{
	...
	flashbounce->toggledChanges() |
		rpl::filter([](bool checked) { return (checked != Core::App().settings().flashBounceNotify()); }) |
		rpl::start_with_next(
			[=](bool checked)
			{
	...
	muted->toggledChanges() |
		rpl::filter([=](bool checked) { return (checked != Core::App().settings().includeMutedCounter()); }) |
		rpl::start_with_next(
			[=](bool checked)
			{
	...
	if (mutedFolders) {
		mutedFolders->toggledChanges() |
			rpl::filter([=](bool checked)
						{ return (checked != Core::App().settings().includeMutedCounterFolders()); }) |
			rpl::start_with_next(
				[=](bool checked)
				{
	...
	count->toggledChanges() |
		rpl::filter([=](bool checked) { return (checked != Core::App().settings().countUnreadMessages()); }) |
		rpl::start_with_next(
			[=](bool checked)
			{
		...
		rpl::start_with_next(
			[=](Change change)
			{
				if (change == Change::DesktopEnabled) {
					...
					previewWrap->toggle(Core::App().settings().desktopNotify(), anim::type::normal);
					previewDivider->toggle(!Core::App().settings().desktopNotify(), anim::type::normal);
					...
					flashbounceToggles->fire(Core::App().settings().flashBounceNotify());
				}
			},
			desktop->lifetime());
	...
	if (native) {
		native->toggledChanges() |
			rpl::filter([](bool checked) { return (checked != Core::App().settings().nativeNotifications()); }) |
			rpl::start_with_next(
				[=](bool checked)
				{
					...
					if (advancedSlide) {
						advancedSlide->toggle(!Core::App().settings().nativeNotifications(), anim::type::normal);
					}
				},
				native->lifetime());
	}
...
void SetupNotifications(not_null<Window::SessionController *> controller,
						not_null<Ui::VerticalLayout *> container,
						Fn<void(Type)> showOther) {
...
Notifications::Notifications(QWidget *parent, not_null<Window::SessionController *> controller) : Section(parent) {
	setupContent(controller);
}

rpl::producer<QString> Notifications::title() { return tr::lng_settings_section_notify(); }

void Notifications::setupContent(not_null<Window::SessionController *> controller) {
