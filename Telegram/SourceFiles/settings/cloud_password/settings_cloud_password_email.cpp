...
rpl::producer<QString> Email::title() { return tr::lng_settings_cloud_password_email_title(); }
...
void Email::setupContent() {
	...
	SetupHeader(content,
				...
				showFinishes(),
				hasRecovery ? tr::lng_settings_cloud_password_manage_email_change()
	...
	const auto wrap = AddWrappedField(content, tr::lng_cloud_password_email(), currentStepDataEmail);
	const auto newInput = wrap->entity();
	const auto error = AddError(content, nullptr);
	newInput->changes() | rpl::start_with_next([=] { error->hide(); }, newInput->lifetime());
	AddSkipInsteadOfField(content);

	const auto send = [=](Fn<void()> close)
	{
		// ViGram: idk weird crash
		if (_requestLifetime) {
		...
		_requestLifetime =
			(setOnly ? cloudPassword().setEmail(data.currentPassword, data.email)
					 : cloudPassword().set(
						   data.currentPassword, data.password, data.hint, !data.email.isEmpty(), data.email)) |
			rpl::start_with_next_error_done(
				[=](Api::CloudPassword::SetOk d)
				{
					_requestLifetime.destroy();
				...
				[=]
				{
					...
					empty.currentPassword =
						stepData().password.isEmpty() ? stepData().currentPassword : stepData().password;
	...
	const auto skip = AddLinkButton(wrap, tr::lng_cloud_password_skip_email());
	skip->setClickedCallback([=] { confirm(QString()); });
	skip->setVisible(!setOnly);

	const auto button = AddDoneButton(content, tr::lng_settings_cloud_password_save());
...
Type CloudPasswordEmailId() { return CloudPassword::Email::Id(); }
