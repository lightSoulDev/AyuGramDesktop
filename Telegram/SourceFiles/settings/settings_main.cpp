...
{
public:
	Cover(QWidget *parent, not_null<Window::SessionController *> controller, not_null<UserData *> user);
	~Cover();
...
Cover::Cover(QWidget *parent, not_null<Window::SessionController *> controller, not_null<UserData *> user)
	: FixedHeightWidget(parent,
						st::settingsPhotoTop + st::infoProfileCover.photo.size.height() + st::settingsPhotoBottom),
	  ...
	  _name(this, st::infoProfileCover.name), _id(this, st::defaultFlatLabel),
	...
	_badge.setPremiumClickCallback([=] { _emojiStatusPanel.show(_controller, _badge.widget(), _badge.sizeTag()); });
	rpl::combine(_badge.updated(), _exteraBadge.updated()) |
...
void Cover::setupChildGeometry() {
	...
	widthValue() |
		rpl::start_with_next(
			[=](int newWidth)
			{
				_userpic->moveToLeft(st::settingsPhotoLeft, st::settingsPhotoTop, newWidth);
		...
		rpl::start_with_next(
			[=](const TextWithEntities &value)
			{
				_username->setMarkedText(
					Ui::Text::Link(value.text.isEmpty() ? tr::lng_settings_username_add(tr::now) : value.text));
	...
	_username->overrideLinkClickHandler(
		[=]
		{
			...
			if (username.isEmpty()) {
				...
				QGuiApplication::clipboard()->setText(_user->session().createInternalLinkFull(username));
...
void Cover::refreshNameGeometry(int newWidth) {
	...
	auto nameWidth = newWidth - nameLeft - st::infoProfileCover.rightSkip;
	...
	const auto exteraBadgeLeft =
		badgeLeft + (_badge.widget() ? (_badge.widget()->width() + st::infoVerifiedCheckPosition.x()) : 0);
...
void Cover::refreshIdGeometry(int newWidth) {
	...
	const auto idWidth = newWidth - idLeft - st::infoProfileCover.rightSkip;
...
[[nodiscard]] not_null<Ui::SettingsButton *>
AddPremiumStar(not_null<Ui::SettingsButton *> button, bool credits, Fn<bool()> isPaused) {
	const auto stops = credits ? Ui::Premium::CreditsIconGradientStops() : Ui::Premium::ButtonGradientStops();

	const auto ministarsContainer = Ui::CreateChild<Ui::RpWidget>(button);
	const auto &buttonSt = button->st();
	const auto fullHeight = buttonSt.height + rect::m::sum::v(buttonSt.padding);
	using MiniStars = Ui::Premium::ColoredMiniStars;
	const auto ministars = button->lifetime().make_state<MiniStars>(ministarsContainer, false);
	ministars->setColorOverride(stops);

	const auto isPausedValue = button->lifetime().make_state<rpl::variable<bool>>(isPaused());
	isPausedValue->value() |
		rpl::start_with_next([=](bool value) { ministars->setPaused(value); }, ministarsContainer->lifetime());
	...
	auto star = [&]
	{
		...
		auto image = QImage(size * factor, QImage::Format_ARGB32_Premultiplied);
	...
	button->sizeValue() |
		rpl::start_with_next(
			[=](const QSize &s)
			{
				badge->moveToLeft(button->st().iconLeft + (st::menuIconShop.width() - badge->width()) / 2,
								  (s.height() - badge->height()) / 2);
				ministarsContainer->moveToLeft(badge->x() - (fullHeight - badge->height()) / 2, 0);
			},
			badge->lifetime());
...
void SetupPowerSavingButton(not_null<Window::Controller *> window, not_null<Ui::VerticalLayout *> container) {
	const auto button =
		AddButtonWithIcon(container, tr::lng_settings_power_menu(), st::settingsButton, {&st::menuIconPowerUsage});
	button->setClickedCallback([=] { window->show(Box(PowerSavingBox)); });
}

void SetupLanguageButton(not_null<Window::Controller *> window, not_null<Ui::VerticalLayout *> container) {
	const auto button =
		AddButtonWithLabel(container,
						   tr::lng_settings_language(),
						   rpl::single(Lang::GetInstance().id()) | rpl::then(Lang::GetInstance().idChanges()) |
...
void SetupSections(not_null<Window::SessionController *> controller,
				   not_null<Ui::VerticalLayout *> container,
				   Fn<void(Type)> showOther) {
	Ui::AddDivider(container);

	const auto addSection = [&](rpl::producer<QString> label, Type type, IconDescriptor &&descriptor)
	{
	...
	addSection(tr::ayu_AyuPreferences(), Ayu::Id(), {.icon = &st::menuIconPremium});
	Ui::AddSkip(container);
	Ui::AddDivider(container);
	Ui::AddSkip(container);
	...
	if (controller->session().supportMode()) {
		...
		addSection(tr::lng_settings_my_account(), Information::Id(), {&st::menuIconProfile});
	}

	addSection(tr::lng_settings_section_notify(), Notifications::Id(), {&st::menuIconNotifications});
	addSection(tr::lng_settings_section_privacy(), PrivacySecurity::Id(), {&st::menuIconLock});
	addSection(tr::lng_settings_section_chat_settings(), Chat::Id(), {&st::menuIconChatBubble});

	const auto preload = [=] { controller->session().data().chatsFilters().requestSuggested(); };
	const auto account = &controller->session().account();
	const auto slided =
		container
			->add(object_ptr<Ui::SlideWrap<Ui::SettingsButton>>(
				container,
				CreateButtonWithIcon(
					container, tr::lng_settings_section_filters(), st::settingsButton, {&st::menuIconShowInFolder})))
			->setDuration(0);
	if (controller->session().data().chatsFilters().has() || controller->session().settings().dialogsFiltersEnabled()) {
		slided->show(anim::type::instant);
		preload();
	} else {
		const auto enabled = [=]
		{
			const auto result = account->appConfig().get<bool>(u"dialog_filters_enabled"_q, false);
			if (result) {
				preload();
			}
			return result;
		...
		slided->toggleOn(rpl::single(rpl::empty) | rpl::then(account->appConfig().refreshed()) | rpl::map(enabled) |
						 rpl::before_next(preloadIfEnabled));
	}
	slided->entity()->setClickedCallback([=] { showOther(Folders::Id()); });

	addSection(tr::lng_settings_advanced(), Advanced::Id(), {&st::menuIconManage});
	addSection(tr::lng_settings_section_devices(), Calls::Id(), {&st::menuIconUnmute});
...
void SetupPremium(not_null<Window::SessionController *> controller,
				  not_null<Ui::VerticalLayout *> container,
				  Fn<void(Type)> showOther) {
	...
	const auto isPaused = Window::PausedIn(controller, Window::GifPauseReason::Any);

	AddPremiumStar(AddButtonWithIcon(container, tr::lng_premium_summary_title(), st::settingsButton), false, isPaused)
		->addClickHandler(
			[=]
			{
		...
		AddPremiumStar(
			AddButtonWithLabel(
				...
				controller->session().credits().balanceValue() |
					rpl::map([=](StarsAmount c) { return c ? Lang::FormatStarsAmountToShort(c).string : QString(); }),
				st::settingsButton),
			true,
			isPaused)
			->addClickHandler(
	...
	const auto button =
		AddButtonWithIcon(container, tr::lng_business_title(), st::settingsButton, {.icon = &st::menuIconShop});
	button->addClickHandler([=] { showOther(BusinessId()); });
	Ui::NewBadge::AddToRight(button);

	if (controller->session().premiumCanBuy()) {
		const auto button = AddButtonWithIcon(
			container, tr::lng_settings_gift_premium(), st::settingsButton, {.icon = &st::menuIconGiftPremium});
		button->addClickHandler([=] { Ui::ChooseStarGiftRecipient(controller); });
	}
	Ui::AddSkip(container);
}
...
void SetupInterfaceScale(not_null<Window::Controller *> window, not_null<Ui::VerticalLayout *> container, bool icon) {
	if (!HasInterfaceScale()) {
		return;
	}

	const auto toggled = Ui::CreateChild<rpl::event_stream<bool>>(container.get());

	const auto switched = (cConfigScale() == style::kScaleAuto);
	const auto button = AddButtonWithIcon(container,
	...
	const auto valueFromScale = [=](int scale)
	{
		...
		return ((result == valuesCount) ? (result - 1) : result) / float64(valuesCount - 1);
	};
	auto sliderWithLabel = MakeSliderWithLabel(container,
											   st::settingsScale,
											   st::settingsScaleLabel,
											   st::normalFont->spacew * 2,
											   st::settingsScaleLabel.style.font->width("300%"),
											   true);
	container->add(std::move(sliderWithLabel.widget), icon ? st::settingsScalePadding : st::settingsBigScalePadding);
		...
		if (cEvalScale(scale) != cEvalScale(cConfigScale())) {
			...
			const auto cancelled =
				crl::guard(button,
						   [=](Fn<void()> close)
						   {
							   base::call_delayed(st::defaultSettingsSlider.duration,
												  button,
	...
	{
		scale = cEvalScale(scale);
		const auto show = *shown ? ScalePreviewShow::Update : ScalePreviewShow::Show;
		*shown = true;
		for (auto i = 0; i != valuesCount; ++i) {
			if (values[i] <= scale && (i + 1 == valuesCount || values[i + 1] > scale)) {
	...
	button->toggledValue() |
		rpl::map([](bool checked) { return checked ? style::kScaleAuto : cEvalScale(cConfigScale()); }) |
		rpl::start_with_next([=](int scale) { setScale(scale, setScale); }, button->lifetime());
...
void SetupHelp(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	Ui::AddDivider(container);
	Ui::AddSkip(container);

	AddButtonWithIcon(container, tr::lng_settings_faq(), st::settingsButton, {&st::menuIconFaq})
		->addClickHandler([=] { OpenFaq(controller); });

	AddButtonWithIcon(container, tr::lng_settings_features(), st::settingsButton, {&st::menuIconEmojiObjects})
		->setClickedCallback([=] { UrlClickHandler::Open(tr::lng_telegram_features_url(tr::now)); });

	const auto button =
		AddButtonWithIcon(container, tr::lng_settings_ask_question(), st::settingsButton, {&st::menuIconDiscussion});
...
void Main::fillTopBarMenu(const Ui::Menu::MenuCallback &addAction) {
	...
	if (!_controller->session().supportMode()) {
		addAction(tr::lng_settings_information(tr::now), [=] { showOther(Information::Id()); }, &st::menuIconEdit);
	}
...
void Main::keyPressEvent(QKeyEvent *e) {
	crl::on_main(this, [=, text = e->text()] { CodesFeedString(_controller, text); });
...
void Main::setupContent(not_null<Window::SessionController *> controller) {
	...
	content->add(object_ptr<Cover>(content, controller, controller->session().user()));
