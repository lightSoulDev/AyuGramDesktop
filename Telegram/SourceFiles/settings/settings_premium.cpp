...
[[nodiscard]] Data::PremiumSubscriptionOptions SubscriptionOptionsForRows(Data::PremiumSubscriptionOptions result) {
	for (auto &option : result) {
		const auto total = option.costTotal;
		const auto perMonth = option.costPerMonth;

		option.costTotal = tr::lng_premium_gift_per(tr::now, lt_cost, perMonth);
		option.costPerMonth = tr::lng_premium_subscribe_total(tr::now, lt_cost, total);
...
[[nodiscard]] int TopTransitionSkip() {
	return (st::settingsButton.padding.top() + st::settingsPremiumRowTitlePadding.top()) / 2;
}
...
struct Data
{
	...
	explicit operator bool() const { return peerId != 0; }
};

[[nodiscard]] QString Serialize(const Data &gift) {
	return QString::number(gift.peerId.value) + ':' + QString::number(gift.months) + ':' +
...
void SendAppLog(not_null<Main::Session *> session, const QString &type, const MTPJSONValue &data) {
	const auto now = double(base::unixtime::now()) + (QTime::currentTime().msec() / 1000.);
	session->api()
		.request(MTPhelp_SaveAppLog(
			MTP_vector<MTPInputAppEvent>(1, MTP_inputAppEvent(MTP_double(now), MTP_string(type), MTP_long(0), data))))
		.send();
}

[[nodiscard]] QString ResolveRef(const QString &ref) { return ref.isEmpty() ? "settings" : ref; }

void SendScreenShow(not_null<Window::SessionController *> controller,
	...
	auto values = QVector<MTPJSONObjectValue>{
		MTP_jsonObjectValue(MTP_string("premium_promo_order"),
							MTP_jsonArray(MTP_vector<MTPJSONValue>(std::move(list)))),
		MTP_jsonObjectValue(MTP_string("source"), MTP_jsonString(MTP_string(ResolveRef(ref)))),
	};
	const auto data = MTP_jsonObject(MTP_vector<MTPJSONObjectValue>(std::move(values)));
	SendAppLog(&controller->session(), "premium.promo_screen_show", data);
}

void SendScreenAccept(not_null<Window::SessionController *> controller) {
	SendAppLog(&controller->session(), "premium.promo_screen_accept", MTP_jsonNull());
}

class EmojiStatusTopBar final
{
public:
	EmojiStatusTopBar(not_null<DocumentData *> document, Fn<void(QRect)> callback, QSizeF size);
...
EmojiStatusTopBar::EmojiStatusTopBar(not_null<DocumentData *> document, Fn<void(QRect)> callback, QSizeF size)
	: _rect(QPointF(), size) {
	...
	rpl::single() | rpl::then(document->owner().session().downloaderTaskFinished()) |
		rpl::start_with_next(
			[=]
			{
				if (!_media->loaded()) {
				...
				_lifetime.destroy();
				if (sticker->isLottie()) {
					_player = std::make_unique<HistoryView::LottiePlayer>(ChatHelpers::LottiePlayerFromDocument(
						_media.get(),
					...
					_player = std::make_unique<HistoryView::WebmPlayer>(
						_media->owner()->location(), _media->bytes(), size.toSize());
				} else if (sticker) {
...
void EmojiStatusTopBar::setPaused(bool paused) { _paused = paused; }

void EmojiStatusTopBar::paint(QPainter &p) {
	if (_player && _player->ready()) {
		const auto frame =
			_player->frame(_rect.size().toSize(),
						   (_media->owner()->emojiUsesTextColor() ? st::profileVerifiedCheckBg->c : QColor(0, 0, 0, 0)),
						   false,
						   crl::now(),
...
{
public:
	TopBarUser(not_null<QWidget *> parent,
...
private:
	void updateTitle(DocumentData *document, TextWithEntities name, not_null<Window::SessionController *> controller);
...
TopBarUser::TopBarUser(not_null<QWidget *> parent,
					   not_null<Window::SessionController *> controller,
					   not_null<PeerData *> peer,
					   rpl::producer<> showFinished)
	: TopBarAbstract(parent, st::userPremiumCover), _content(this), _title(_content, st::settingsPremiumUserTitle),
	  _about(_content, st::userPremiumCover.about), _ministars(_content, true),
	  _smallTop({
		  .widget = object_ptr<Ui::RpWidget>(this),
		  .text = Ui::Text::String(st::boxTitle.style, tr::lng_premium_summary_title(tr::now)),
	  }) {
	_starRect = TopBarAbstract::starRect(1., 1.);

	rpl::single() | rpl::then(style::PaletteChanged()) |
		rpl::start_with_next(
			[=]
			{
	...
	auto documentValue = Info::Profile::EmojiStatusIdValue(peer) |
		rpl::map(
							 [=](EmojiStatusId id) -> DocumentData *
							 {
								 const auto documentId = id.collectible ? id.collectible->documentId : id.documentId;
								 const auto document =
									 documentId ? controller->session().data().document(documentId).get() : nullptr;
								 return (document && document->sticker()) ? document : nullptr;
							 });

	rpl::combine(std::move(documentValue), Info::Profile::NameValue(peer)) |
		rpl::start_with_next(
			[=](DocumentData *document, const QString &name)
			{
				if (document) {
					_emojiStatus = std::make_unique<EmojiStatusTopBar>(
						document, [=](QRect r) { _content->update(std::move(r)); }, HistoryView::Sticker::EmojiSize());
					_imageStar = QImage();
				} else {
	...
	rpl::combine(_title->sizeValue(), _about->sizeValue(), _content->sizeValue()) |
		rpl::start_with_next(
			[=](const QSize &titleSize, const QSize &aboutSize, const QSize &size)
			{
				const auto rect = TopBarAbstract::starRect(1., 1.);
				const auto &padding = st::settingsPremiumUserTitlePadding;
				_title->moveToLeft((size.width() - titleSize.width()) / 2, rect.top() + rect.height() + padding.top());
				_about->moveToLeft((size.width() - aboutSize.width()) / 2,
								   _title->y() + titleSize.height() + padding.bottom());

				const auto aboutBottom = _about->y() + _about->height();
				const auto height = (aboutBottom > st::settingsPremiumUserHeight) ? aboutBottom + padding.bottom()
	...
	const auto smallTopShadow = Ui::CreateChild<Ui::FadeShadow>(_smallTop.widget.data());
	smallTopShadow->setDuration(st::infoTopBarDuration);
	rpl::combine(rpl::single(false) | rpl::then(std::move(showFinished) | rpl::map_to(true)), sizeValue()) |
		rpl::start_with_next(
			[=](bool showFinished, const QSize &size)
			{
				_content->resize(size.width(), maximumHeight());
				...
				smallTopShadow->moveToLeft(0, _smallTop.widget->height() - smallTopShadow->height());
				...
				if (_smallTop.shown != shown) {
					...
					if (!showFinished) {
						...
						_smallTop.animation.start([=] { _smallTop.widget->update(); },
												  _smallTop.shown ? 0. : 1.,
												  _smallTop.shown ? 1. : 0.,
	...
	_smallTop.widget->paintRequest() |
		rpl::start_with_next(
			[=]
			{
				...
				_smallTop.text.drawLeft(p, _smallTop.position.x(), _smallTop.position.y(), width(), width());
			},
			lifetime());
...
void TopBarUser::updateTitle(DocumentData *document,
							 TextWithEntities name,
							 not_null<Window::SessionController *> controller) {
	if (!document) {
		return _title->setMarkedText(
			tr::lng_premium_summary_user_title(tr::now, lt_user, std::move(name), Ui::Text::WithEntities));
	}
	...
	const auto text = (set->thumbnailDocumentId ? QChar('0') : QChar()) + set->title;
	const auto linkIndex = 1;
	const auto entityEmojiData = Data::SerializeCustomEmojiId(set->thumbnailDocumentId);
	...
	auto title = (setId == coloredId)
		? tr::lng_premium_emoji_status_title_colored(tr::now, lt_user, std::move(name), Ui::Text::WithEntities)
		: tr::lng_premium_emoji_status_title(tr::now,
											 lt_user,
											 std::move(name),
											 lt_link,
											 {
	...
	_title->setMarkedText(std::move(title), Core::TextContext({.session = &controller->session()}));
	auto link = std::make_shared<LambdaClickHandler>(
		[=, stickerSetIdentifier = stickerInfo->set]
		{
			setPaused(true);
			const auto box = controller->show(
				Box<StickerSetBox>(controller->uiShow(), stickerSetIdentifier, Data::StickersType::Emoji));

			box->boxClosing() | rpl::start_with_next(crl::guard(this, [=] { setPaused(false); }), box->lifetime());
		});
	_title->setLink(linkIndex, std::move(link));
}

void TopBarUser::updateAbout(DocumentData *document) const {
	_about->setMarkedText((document ? tr::lng_premium_emoji_status_about
									: tr::lng_premium_summary_user_about)(tr::now, Ui::Text::RichLangValue));
}
...
void TopBarUser::setTextPosition(int x, int y) { _smallTop.position = {x, y}; }

rpl::producer<int> TopBarUser::additionalHeight() const { return rpl::never<int>(); }
...
{
public:
	Premium(QWidget *parent, not_null<Window::SessionController *> controller);

	[[nodiscard]] rpl::producer<QString> title() override;

	[[nodiscard]] QPointer<Ui::RpWidget> createPinnedToTop(not_null<QWidget *> parent) override;
	[[nodiscard]] QPointer<Ui::RpWidget> createPinnedToBottom(not_null<Ui::RpWidget *> parent) override;
...
Premium::Premium(QWidget *parent, not_null<Window::SessionController *> controller)
	: Section(parent), _controller(controller), _ref(ResolveRef(controller->premiumRef())),
	  _radioGroup(std::make_shared<Ui::RadiobuttonGroup>()) {
	setupContent();
	_controller->session().api().premium().reload();
}

rpl::producer<QString> Premium::title() { return tr::lng_premium_summary_title(); }

bool Premium::hasFlexibleTopBar() const { return true; }

rpl::producer<> Premium::sectionShowBack() { return _showBack.events(); }

void Premium::setStepDataReference(std::any &data) {
	const auto my = std::any_cast<SectionCustomTopBarData>(&data);
	if (my) {
		_backToggles = std::move(my->backButtonEnables) | rpl::map_to(true);
		_wrap = std::move(my->wrapValue);
	}
}

void Premium::setupSubscriptionOptions(not_null<Ui::VerticalLayout *> container) {
	...
	Ui::Premium::AddGiftOptions(content,
								_radioGroup,
	...
	auto toggleOn = rpl::combine(Data::AmPremiumValue(&_controller->session()),
								 apiPremium->statusTextValue() |
									 rpl::map([=] { return apiPremium->subscriptionOptions().size() < 2; })) |
		rpl::map([=](bool premium, bool noOptions) { return !premium && !noOptions; });
	options->toggleOn(rpl::duplicate(toggleOn), anim::type::instant);
	skip->toggleOn(std::move(toggleOn) | rpl::map([](bool value) { return !value; }), anim::type::instant);
}
...
QPointer<Ui::RpWidget> Premium::createPinnedToTop(not_null<QWidget *> parent) {
	auto title = _controller->session().premium() ? tr::lng_premium_summary_title()
												  : rpl::conditional(Data::AmPremiumValue(&_controller->session()),
	...
	auto about = [&]() -> rpl::producer<TextWithEntities>
	{
		...
		if (gift) {
			...
			if (const auto peer = data.peer(gift.peerId)) {
				return (gift.me ? tr::lng_premium_summary_subtitle_gift_me
								: tr::lng_premium_summary_subtitle_gift)(lt_count,
		...
		return rpl::conditional(Data::AmPremiumValue(&_controller->session()),
	...
	{
		if (peerWithPremium) {
			return Ui::CreateChild<TopBarUser>(parent.get(), _controller, peerWithPremium, _showFinished.events());
		}
		...
		return Ui::CreateChild<Ui::Premium::TopBar>(parent.get(),
	...
	_wrap.value() |
		rpl::start_with_next([=](Info::Wrap wrap) { content->setRoundEdges(wrap == Info::Wrap::Layer); },
							 content->lifetime());
	...
	content->additionalHeight() |
		rpl::start_with_next(
			[=](int additionalHeight)
			{
				...
				content->setMaximumHeight(calculateMaximumHeight() + additionalHeight);
				if (wasMax) {
					content->resize(content->width(), content->maximumHeight());
				}
			},
			...
			[=](Info::Wrap wrap)
			{
				...
				_back = base::make_unique_q<Ui::FadeWrap<Ui::IconButton>>(
					...
					object_ptr<Ui::IconButton>(
						content,
						isEmojiStatus ? (isLayer ? st::infoTopBarBack : st::infoLayerTopBarBack)
									  : (isLayer ? st::settingsPremiumLayerTopBarBack : st::settingsPremiumTopBarBack)),
					st::infoTopBarScale);
				_back->setDuration(0);
				_back->toggleOn(isLayer ? _backToggles.value() | rpl::type_erased() : rpl::single(true));
				_back->entity()->addClickHandler([=] { _showBack.fire({}); });
				_back->toggledValue() |
					rpl::start_with_next(
						[=](bool toggled)
						{
							const auto &st = isLayer ? st::infoLayerTopBar : st::infoTopBar;
							content->setTextPosition(toggled ? st.back.width : st.titlePosition.x(),
													 st.titlePosition.y());
						},
						_back->lifetime());
				...
				if (!isLayer) {
					...
					_close = base::make_unique_q<Ui::IconButton>(
						content, isEmojiStatus ? st::infoTopBarClose : st::settingsPremiumTopBarClose);
					...
					content->widthValue() |
						rpl::start_with_next([=] { _close->moveToRight(0, 0); }, _close->lifetime());
				}
			},
			content->lifetime());
...
void Premium::showFinished() { _showFinished.fire({}); }

QPointer<Ui::RpWidget> Premium::createPinnedToBottom(not_null<Ui::RpWidget *> parent) {
	...
	auto buttonText = [&]() -> std::optional<rpl::producer<QString>>
	{
		if (emojiStatusData) {
			...
			if (const auto peer = data.peer(emojiStatusData.peerId)) {
				return Info::Profile::EmojiStatusIdValue(peer) |
					rpl::map(
						   [=](EmojiStatusId id)
						   {
							   return id ? tr::lng_premium_emoji_status_button() : _buttonText.value();
	...
	_subscribe = CreateSubscribeButton({
		...
		[=, options = session->api().premium().subscriptionOptions()]
		{
			...
			return (value < options.size() && value >= 0) ? options[value].botUrl : QString();
		},
	});
#if 0
	if (emojiStatusData) {
...
#endif
	{
		const auto callback = [=](int value)
		{
			...
			auto text = tr::lng_premium_subscribe_button(tr::now, lt_cost, options[value].costPerMonth);
	...
	_showFinished.events() | rpl::take(1) |
		rpl::start_with_next([=] { _subscribe->startGlareAnimation(); }, _subscribe->lifetime());
	...
	rpl::combine(_subscribe->heightValue(), Data::AmPremiumValue(session), session->premiumPossibleValue()) |
		rpl::start_with_next(
			[=](int buttonHeight, bool premium, bool premiumPossible)
			{
				const auto padding = st::settingsPremiumButtonPadding;
				const auto finalHeight = !premiumPossible ? 0
					: !premium							  ? (padding.top() + buttonHeight + padding.bottom())
														  : 0;
...
{
	object_ptr<AbstractSection> create(not_null<QWidget *> parent,
									   not_null<Window::SessionController *> controller,
									   not_null<Ui::ScrollArea *> scroll,
									   rpl::producer<Container> containerValue) const final override {
		return object_ptr<Premium>(parent, controller);
	}
	bool hasCustomTopBar() const final override { return true; }
...
Type PremiumId() { return Premium::Id(); }

void ShowPremium(not_null<Main::Session *> session, const QString &ref) {
	const auto active = Core::App().activeWindow();
	const auto controller = (active && active->isPrimary()) ? active->sessionController() : nullptr;
...
void ShowPremium(not_null<Window::SessionController *> controller, const QString &ref) {
...
void ShowGiftPremium(not_null<Window::SessionController *> controller, not_null<PeerData *> peer, int months, bool me) {
	ShowPremium(controller, Ref::Gift::Serialize({peer->id, months, me}));
}

void ShowEmojiStatusPremium(not_null<Window::SessionController *> controller, not_null<PeerData *> peer) {
...
void StartPremiumPayment(not_null<Window::SessionController *> controller, const QString &ref) {
	const auto session = &controller->session();
	const auto username = session->appConfig().get<QString>(u"premium_bot_username"_q, QString());
	const auto slug = session->appConfig().get<QString>(u"premium_invoice_slug"_q, QString());
	if (!username.isEmpty()) {
...
void ShowPremiumPromoToast(std::shared_ptr<ChatHelpers::Show> show, TextWithEntities textWithLink, const QString &ref) {
	ShowPremiumPromoToast(
		show,
		...
		std::move(textWithLink),
		ref);
}

void ShowPremiumPromoToast(std::shared_ptr<Main::SessionShow> show,
	...
	(*toast) = show->showToast({
		...
		.filter = crl::guard(&show->session(),
							 [=](const ClickHandlerPtr &, Qt::MouseButton button)
							 {
								 if (button == Qt::LeftButton) {
									 if (const auto strong = toast->get()) {
										 strong->hideAnimated();
										 (*toast) = nullptr;
										 if (const auto controller = resolveWindow(&show->session())) {
...
not_null<Ui::RoundButton *> CreateLockedButton(not_null<QWidget *> parent,
											   rpl::producer<QString> text,
											   const style::RoundButton &st,
											   rpl::producer<bool> locked) {
	const auto result = Ui::CreateChild<Ui::RoundButton>(parent.get(), rpl::single(QString()), st);

	const auto labelSt = result->lifetime().make_state<style::FlatLabel>(st::defaultFlatLabel);
	labelSt->style.font = st.style.font;
	labelSt->textFg = st.textFg;

	const auto label = Ui::CreateChild<Ui::FlatLabel>(result, std::move(text), *labelSt);
	...
	rpl::combine(result->widthValue(), label->widthValue(), std::move(locked)) |
		rpl::start_with_next(
			[=](int outer, int inner, bool locked)
			{
				if (locked) {
					icon->show();
					inner += icon->width();
					label->move((outer - inner) / 2 + icon->width(), st::similarChannelsLock.textTop);
					icon->move((outer - inner) / 2, st::similarChannelsLock.textTop);
				} else {
					icon->hide();
					label->move((outer - inner) / 2, st::similarChannelsLock.textTop);
				}
			},
			result->lifetime());
...
not_null<Ui::GradientButton *> CreateSubscribeButton(SubscribeButtonArgs &&args) {
	...
	return CreateSubscribeButton(std::move(show), std::move(resolve), std::move(args));
}

not_null<Ui::GradientButton *>
CreateSubscribeButton(std::shared_ptr<::Main::SessionShow> show,
					  Fn<Window::SessionController *(not_null<::Main::Session *>)> resolveWindow,
					  SubscribeButtonArgs &&args) {
	const auto result = Ui::CreateChild<Ui::GradientButton>(
		args.parent.get(), args.gradientStops ? base::take(*args.gradientStops) : Ui::Premium::ButtonGradientStops());

	result->setClickedCallback(
		[show, resolveWindow, promo = args.showPromo, computeRef = args.computeRef, computeBotUrl = args.computeBotUrl]
		{
			const auto window = resolveWindow(&show->session());
			if (!window) {
				return;
			} else if (promo) {
	...
	const auto computeCost = [=]
	{
		...
		return Ui::FillAmountAndCurrency(valid ? amount : 500, valid ? currency : "USD");
	};

	const auto label = Ui::CreateChild<Ui::FlatLabel>(
		result,
		args.text ? base::take(*args.text)
				  : tr::lng_premium_summary_button(lt_cost, premium->statusTextValue() | rpl::map(computeCost)),
		st::premiumPreviewButtonLabel);
	label->setAttribute(Qt::WA_TransparentForMouseEvents);
	rpl::combine(result->widthValue(), label->widthValue()) |
		rpl::start_with_next([=](int outer, int width)
							 { label->moveToLeft((outer - width) / 2, st::premiumPreviewBox.button.textTop, outer); },
							 label->lifetime());

	return result;
}

std::vector<PremiumFeature> PremiumFeaturesOrder(not_null<Main::Session *> session) {
	const auto mtpOrder = session->appConfig().get<Order>("premium_promo_order", FallbackOrder());
	return ranges::views::all(mtpOrder) |
		...
		ranges::views::filter([](PremiumFeature type) { return (type != PremiumFeature::kCount); }) | ranges::to_vector;
}

void AddSummaryPremium(not_null<Ui::VerticalLayout *> content,
					   not_null<Window::SessionController *> controller,
					   const QString &ref,
	...
	const auto addRow = [&](Entry &entry)
	{
		...
		const auto button = Ui::CreateChild<Ui::SettingsButton>(content.get(), rpl::single(QString()));

		const auto label = content->add(
			object_ptr<Ui::FlatLabel>(content, std::move(entry.title) | Ui::Text::ToBold(), stLabel), titlePadding);
		label->setAttribute(Qt::WA_TransparentForMouseEvents);
		...
		content->sizeValue() |
			rpl::start_with_next([=](const QSize &s) { dummy->resize(s.width(), iconSize.height()); },
								 dummy->lifetime());

		label->geometryValue() |
			rpl::start_with_next([=](const QRect &r) { dummy->moveToLeft(0, r.y() + (r.height() - labelAscent)); },
								 dummy->lifetime());

		rpl::combine(content->widthValue(), label->heightValue(), description->heightValue()) |
			rpl::start_with_next(
				[=, topPadding = titlePadding, bottomPadding = descriptionPadding](
					int width, int topHeight, int bottomHeight)
				{
					button->resize(width,
								   topPadding.top() + topHeight + topPadding.bottom() + bottomPadding.top() +
									   bottomHeight + bottomPadding.bottom());
				},
				button->lifetime());
		label->topValue() |
			rpl::start_with_next([=, padding = titlePadding.top()](int top) { button->moveToLeft(0, top - padding); },
								 button->lifetime());
		const auto arrow = Ui::CreateChild<Ui::IconButton>(button, st::backButton);
		arrow->setIconOverride(&st::settingsPremiumArrow, &st::settingsPremiumArrowOver);
		...
		button->sizeValue() |
			rpl::start_with_next(
				[=](const QSize &s)
				{
					...
					arrow->moveToRight(-point.x(), point.y() + (s.height() - arrow->height()) / 2);
				},
				arrow->lifetime());
	...
	for (auto i = 0; i < int(icons.size()); i++) {
		...
		auto resultGradient = QLinearGradient(QPointF(), QPointF(0, pointBottom - pointTop));

		resultGradient.setColorAt(.0, anim::gradient_color_at(gradient, ratioTop));
		resultGradient.setColorAt(.1, anim::gradient_color_at(gradient, ratioBottom));

		const auto brush = QBrush(resultGradient);
		AddButtonIcon(iconContainer, stDefault, {.icon = icons[i], .backgroundBrush = brush});
	}

	Ui::AddSkip(content, descriptionPadding.bottom());
}

std::unique_ptr<Ui::RpWidget> MakeEmojiStatusPreview(not_null<QWidget *> parent, not_null<DocumentData *> document) {
	auto result = std::make_unique<Ui::RpWidget>(parent);

	const auto raw = result.get();
	const auto size = HistoryView::Sticker::EmojiSize();
	const auto emoji =
		raw->lifetime().make_state<EmojiStatusTopBar>(document, [=](QRect r) { raw->update(std::move(r)); }, size);
	raw->paintRequest() |
		rpl::start_with_next(
			[=]
			{
	...
	raw->sizeValue() |
		rpl::start_with_next([=](QSize size) { emoji->setCenter(QPointF(size.width() / 2., size.height() / 2.)); },
							 raw->lifetime());
