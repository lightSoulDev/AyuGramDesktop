...
private:
	...
	{
	public:
		Button(not_null<QWidget *> parent, std::vector<QColor> &&colors, bool selected);
	...
	void show(not_null<const Scheme *> scheme, std::vector<QColor> &&colors, int selected);
...
void PaintCustomButton(QPainter &p, const std::vector<QColor> &colors) {
	...
	{
		const auto where = QPointF{size * (1. + center.x()) / 2, size * (1. + center.y()) / 2};
		p.setBrush(colors[index]);
		p.drawEllipse(where.x() - smallSize, where.y() - smallSize, 2 * smallSize, 2 * smallSize);
	};
...
ColorsPalette::Button::Button(not_null<QWidget *> parent, std::vector<QColor> &&colors, bool selected)
	...
	_widget.paintRequest() | rpl::start_with_next([=] { paint(); }, _widget.lifetime());
}

void ColorsPalette::Button::moveToLeft(int x, int y) { _widget.moveToLeft(x, y); }

void ColorsPalette::Button::update(std::vector<QColor> &&colors, bool selected) {
	if (_colors != colors) {
	...
	if (_selected != selected) {
		...
		_selectedAnimation.start(
			[=] { _widget.update(); }, _selected ? 0. : 1., _selected ? 1. : 0., st::defaultRadio.duration * 2);
	}
}

rpl::producer<> ColorsPalette::Button::clicks() const { return _widget.clicks() | rpl::to_empty; }
...
void ColorsPalette::Button::paint() {
	...
	if (_colors.size() == 1) {
		PaintRoundColorButton(
			p, st::settingsAccentColorSize, _colors.front(), _selectedAnimation.value(_selected ? 1. : 0.));
...
ColorsPalette::ColorsPalette(not_null<Ui::VerticalLayout *> container)
	: _outer(container->add(object_ptr<Ui::SlideWrap<>>(container, object_ptr<Ui::RpWidget>(container)))) {
	_outer->hide(anim::type::instant);

	const auto inner = _outer->entity();
	inner->widthValue() | rpl::start_with_next([=] { updateInnerGeometry(); }, inner->lifetime());
}
...
void ColorsPalette::show(Type type) {
	...
	const auto color = AyuFeatures::MessageShot::isChoosingTheme()
		...
		: Core::App().settings().themesAccentColors().get(type);
	const auto current = color.value_or(scheme->accentColor);
	const auto i = ranges::find(list, current);
	if (i == end(list)) {
		list.back() = current;
	}
	const auto selected = std::clamp(int(i - begin(list)), 0, int(list.size()) - 1);
...
void ColorsPalette::show(not_null<const Scheme *> scheme, std::vector<QColor> &&colors, int selected) {
	...
	const auto pushButton = [&](std::vector<QColor> &&colors)
	{
		...
		if (_buttons.size() > index) {
			...
			_buttons.push_back(std::make_unique<Button>(inner, std::move(colors), chosen));
	...
	for (const auto &color : colors) {
		...
		if (clicks) {
			std::move(clicks) | rpl::map([=] { return _buttons[index - 1]->color(); }) |
				rpl::start_with_next([=](QColor color) { _selected.fire_copy(color); }, inner->lifetime());
		}
	}

	auto clicks = pushButton(std::move(colors));
	if (clicks) {
		std::move(clicks) | rpl::start_with_next([=] { selectCustom(scheme); }, inner->lifetime());
	}
}
...
void ColorsPalette::selectCustom(not_null<const Scheme *> scheme) {
	...
	const auto colorizer = Window::Theme::ColorizerFrom(*scheme, scheme->accentColor);
	Ui::show(Box(
		[=](not_null<Ui::GenericBox *> box)
		{
			const auto editor = box->addRow(object_ptr<ColorEditor>(box, ColorEditor::Mode::HSL, (*selected)->color()));
			...
			editor->submitRequests() | rpl::start_with_next(save, editor->lifetime());
...
rpl::producer<QColor> ColorsPalette::selected() const { return _selected.events(); }
...
void ColorsPalette::updateInnerGeometry() {
	...
	const auto skip = (width - size * _buttons.size()) / float64(_buttons.size() - 1);
...
void PaintRoundColorButton(QPainter &p, int size, QBrush brush, float64 selected) {
...
{
public:
	BackgroundRow(QWidget *parent, not_null<Window::SessionController *> controller);
...
void ChooseFromFile(not_null<Window::SessionController *> controller, not_null<QWidget *> parent);

BackgroundRow::BackgroundRow(QWidget *parent, not_null<Window::SessionController *> controller)
	: RpWidget(parent), _controller(controller),
	  _chooseFromGallery(this, tr::lng_settings_bg_from_gallery(tr::now), st::settingsLink),
	  _chooseFromFile(this, tr::lng_settings_bg_from_file(tr::now), st::settingsLink),
	  _radial([=](crl::time now) { radialAnimationCallback(now); }) {
	updateImage();

	_chooseFromGallery->addClickHandler([=] { controller->show(Box<BackgroundBox>(controller)); });
	...
	Window::Theme::Background()->updates() |
		rpl::filter(
			[](const Update &update)
			{
				return (update.type == Update::Type::New || update.type == Update::Type::Start ||
						update.type == Update::Type::Changed);
			}) |
		rpl::start_with_next([=] { updateImage(); }, lifetime());
}
...
void BackgroundRow::paintEvent(QPaintEvent *e) {
	...
	if (radial) {
		...
		if (!backThumb) {
			...
			const auto &pix = backThumb->pix(st::settingsBackgroundThumb, {.options = Images::Option::Blur});
		...
		const auto inner = QRect(QPoint(outer.x() + (outer.width() - st::radialSize.width()) / 2,
...
float64 BackgroundRow::radialProgress() const { return _controller->content()->chatBackgroundProgress(); }
...
void BackgroundRow::updateImage() {
	...
	const auto preparePattern = [&]
	{
		...
		return Ui::GenerateBackgroundImage({fullsize, fullsize},
	...
	{
		auto result = QImage(QSize{fullsize, fullsize}, QImage::Format_ARGB32_Premultiplied);
		...
		p.drawImage(QRect(0, 0, size, size), prepared, QRect((w - s) / 2, (h - s) / 2, s, s));
		p.end();
		return result;
	};
	auto back = (paper.isPattern() || !background.gradientForFill().isNull()) ? preparePattern() : prepareNormal();
	_background = Ui::PixmapFromImage(Images::Round(std::move(back), ImageRoundRadius::Small));
...
void ChooseFromFile(not_null<Window::SessionController *> controller, not_null<QWidget *> parent) {
	auto filters = QStringList(u"Theme files (*.tdesktop-theme *.tdesktop-palette *"_q +
		...
		crl::guard(controller,
				   [=](const FileDialog::OpenResult &result)
				   {
					   ...
					   if (!result.paths.isEmpty()) {
						   ...
						   if (hasExtension(qstr(".tdesktop-theme")) || hasExtension(qstr(".tdesktop-palette"))) {
					   ...
					   local.setLocalImageAsThumbnail(std::make_shared<Image>(std::move(image)));
					   controller->show(Box<BackgroundPreviewBox>(controller, local));
				   });
	FileDialog::GetOpenPath(
		parent.get(), tr::lng_choose_image(tr::now), filters.join(u";;"_q), crl::guard(parent, callback));
}

void SetupStickersEmoji(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	...
	container->add(object_ptr<Ui::OverrideMargins>(
		container, std::move(wrap), QMargins(0, 0, 0, st::settingsCheckbox.margin.bottom())));

	const auto checkbox = [&](const QString &label, bool checked)
	{ return object_ptr<Ui::Checkbox>(container, label, checked, st::settingsCheckbox); };
	const auto add = [&](const QString &label, bool checked, auto &&handle)
	{
		inner->add(checkbox(label, checked), st::settingsCheckboxPadding)->checkedChanges() |
			rpl::start_with_next(std::move(handle), inner->lifetime());
	};
	const auto addSliding = [&](const QString &label, bool checked, auto &&handle, rpl::producer<bool> shown)
	{
		inner->add(
				 object_ptr<Ui::SlideWrap<Ui::Checkbox>>(inner, checkbox(label, checked), st::settingsCheckboxPadding))
				->setDuration(0)
	...
	add(tr::lng_settings_large_emoji(tr::now),
	...
	add(tr::lng_settings_replace_emojis(tr::now),
	...
	const auto suggestEmoji = inner->lifetime().make_state<rpl::variable<bool>>(Core::App().settings().suggestEmoji());
	add(tr::lng_settings_suggest_emoji(tr::now),
	...
	add(tr::lng_settings_suggest_by_emoji(tr::now),
	...
	add(tr::lng_settings_loop_stickers(tr::now),
	...
	AddButtonWithIcon(container, tr::lng_stickers_you_have(), st::settingsButton, {&st::menuIconStickers})
		->addClickHandler(
			[=] { controller->show(Box<StickersBox>(controller->uiShow(), StickersBox::Section::Installed)); });

	AddButtonWithIcon(container, tr::lng_emoji_manage_sets(), st::settingsButton, {&st::menuIconEmoji})
		->addClickHandler([=] { controller->show(Box<Ui::Emoji::ManageSetsBox>(session)); });

	Ui::AddSkip(container, st::settingsCheckboxesSkip);
}

void SetupMessages(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	Ui::AddDivider(container);
	Ui::AddSkip(container);
	...
	auto wrap = object_ptr<Ui::VerticalLayout>(container);
	const auto inner = wrap.data();
	container->add(object_ptr<Ui::OverrideMargins>(container, std::move(wrap), QMargins(0, skip, 0, skip)));

	const auto groupSend = std::make_shared<Ui::RadioenumGroup<SendByType>>(Core::App().settings().sendSubmitWay());
	const auto addSend = [&](SendByType value, const QString &text)
	{
		inner->add(object_ptr<Ui::Radioenum<SendByType>>(inner, groupSend, value, text, st::settingsSendType),
				   st::settingsSendTypePadding);
	};
	addSend(SendByType::Enter, tr::lng_settings_send_enter(tr::now));
	addSend(SendByType::CtrlEnter,
			(Platform::IsMac() ? tr::lng_settings_send_cmdenter(tr::now) : tr::lng_settings_send_ctrlenter(tr::now)));

	groupSend->setChangedCallback(
		[=](SendByType value)
		{
	...
	const auto groupQuick = std::make_shared<Ui::RadioenumGroup<Quick>>(Core::App().settings().chatQuickAction());
	const auto addQuick = [&](Quick value, const QString &text)
	{
		return inner->add(object_ptr<Ui::Radioenum<Quick>>(inner, groupQuick, value, text, st::settingsSendType),
						  st::settingsSendTypePadding);
	};
	addQuick(Quick::Reply, tr::lng_settings_chat_quick_action_reply(tr::now));
	const auto react = addQuick(Quick::React, tr::lng_settings_chat_quick_action_react(tr::now));

	const auto buttonRight = Ui::CreateSimpleCircleButton(inner, st::stickersRemove.ripple);
	...
	auto idValue = rpl::single(reactions.favoriteId()) |
		rpl::then(reactions.favoriteUpdates() |
				  rpl::map([=] { return controller->session().data().reactions().favoriteId(); })) |
	...
	react->geometryValue() |
		rpl::start_with_next(
			[=](const QRect &r)
			{
				...
				buttonRight->moveToRight(st::settingsButtonRightSkip, r.y() + (r.height() - rightSize.height()) / 2);
			},
			buttonRight->lifetime());

	groupQuick->setChangedCallback(
		[=](Quick value)
		{
	...
	inner->add(object_ptr<Ui::Checkbox>(inner,
...
void SetupArchive(not_null<Window::SessionController *> controller,
	...
	AddButtonWithIcon(container, tr::lng_settings_shortcuts(), st::settingsButton, {&st::menuIconShortcut})
		->addClickHandler([=] { showOther(Shortcuts::Id()); });

	PreloadArchiveSettings(&controller->session());
	AddButtonWithIcon(container, tr::lng_context_archive_settings(), st::settingsButton, {&st::menuIconArchive})
		->addClickHandler([=] { controller->show(Box(Settings::ArchiveSettingsBox, controller)); });
}

void SetupExport(not_null<Window::SessionController *> controller,
				 not_null<Ui::VerticalLayout *> container,
				 Fn<void(Type)> showOther) {
	AddButtonWithIcon(container, tr::lng_settings_export_data(), st::settingsButton, {&st::menuIconExport})
		->addClickHandler(
			[=]
			{
				const auto session = &controller->session();
				controller->window().hideSettingsAndLayer();
				base::call_delayed(st::boxDuration, session, [=] { Core::App().exportManager().start(session); });
			});

	AddButtonWithIcon(container, tr::lng_settings_experimental(), st::settingsButton, {&st::menuIconExperimental})
		->addClickHandler([=] { showOther(Experimental::Id()); });
}

void SetupLocalStorage(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	AddButtonWithIcon(container, tr::lng_settings_manage_local_storage(), st::settingsButton, {&st::menuIconStorage})
		->addClickHandler([=] { LocalStorageBox::Show(controller); });
}

void SetupDataStorage(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	using namespace rpl::mappers;
	...
	SetupConnectionType(&controller->window(), &controller->session().account(), container);

#ifndef OS_WIN_STORE
	const auto showpath = container->lifetime().make_state<rpl::event_stream<bool>>();

	const auto path = container->add(object_ptr<Ui::SlideWrap<Button>>(
		container,
		CreateButtonWithIcon(container, tr::lng_download_path(), st::settingsButton, {&st::menuIconShowInFolder})));
	auto pathtext = Core::App().settings().downloadPathValue() |
		rpl::map(
						[](const QString &text)
						{
							if (text.isEmpty()) {
								return Core::App().canReadDefaultDownloadPath() ? tr::lng_download_path_default(tr::now)
	...
	CreateRightLabel(path->entity(), std::move(pathtext), st::settingsButton, tr::lng_download_path());
	path->entity()->addClickHandler([=] { controller->show(Box<DownloadPathBox>(controller)); });
#endif // OS_WIN_STORE

	SetupLocalStorage(controller, container);

	AddButtonWithIcon(container, tr::lng_downloads_section(), st::settingsButton, {&st::menuIconDownload})
		->setClickedCallback([=] { controller->showSection(Info::Downloads::Make(controller->session().user())); });

	const auto ask =
		container->add(object_ptr<Ui::SettingsButton>(container, tr::lng_download_path_ask(), st::settingsButtonNoIcon))
			->toggleOn(rpl::single(Core::App().settings().askDownloadPath()));

	ask->toggledValue() |
		rpl::filter([](bool checked) { return (checked != Core::App().settings().askDownloadPath()); }) |
		rpl::start_with_next(
			[=](bool checked)
			{
...
void SetupAutoDownload(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	Ui::AddDivider(container);
	Ui::AddSkip(container);
	...
	const auto add = [&](rpl::producer<QString> label, Source source, IconDescriptor &&descriptor)
	{
		AddButtonWithIcon(container, std::move(label), st::settingsButton, std::move(descriptor))
			->addClickHandler([=] { controller->show(Box<AutoDownloadBox>(&controller->session(), source)); });
	};
	...
	add(tr::lng_media_auto_in_channels(), Source::Channel, {&st::menuIconChannel});

	Ui::AddSkip(container, st::settingsCheckboxesSkip);
}

void SetupChatBackground(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	Ui::AddDivider(container);
	Ui::AddSkip(container);

	Ui::AddSubsectionTitle(container, tr::lng_settings_section_background());

	container->add(object_ptr<BackgroundRow>(container, controller), st::settingsBackgroundPadding);

	const auto skipTop = st::settingsCheckbox.margin.top();
	const auto skipBottom = st::settingsCheckbox.margin.bottom();
	auto wrap = object_ptr<Ui::VerticalLayout>(container);
	const auto inner = wrap.data();
	container->add(object_ptr<Ui::OverrideMargins>(container, std::move(wrap), QMargins(0, skipTop, 0, skipBottom)));
	...
	const auto tile = inner->add(object_ptr<Ui::SlideWrap<Ui::Checkbox>>(
		inner,
		object_ptr<Ui::Checkbox>(inner, tr::lng_settings_bg_tile(tr::now), background->tile(), st::settingsCheckbox),
		st::settingsSendTypePadding));
	const auto adaptive = inner->add(
		object_ptr<Ui::SlideWrap<Ui::Checkbox>>(inner,
	...
	tile->entity()->checkedChanges() |
		rpl::start_with_next([=](bool checked) { background->setTile(checked); }, tile->lifetime());

	const auto shown = [=] { return !background->paper().isPattern() && !background->colorForFill(); };
	tile->toggle(shown(), anim::type::instant);

	using Update = const Window::Theme::BackgroundUpdate;
	background->updates() |
		rpl::filter([](const Update &update)
					{ return (update.type == Update::Type::Changed) || (update.type == Update::Type::New); }) |
		rpl::start_with_next(
			[=]
			{
	...
	adaptive->toggleOn(
		controller->adaptive().chatLayoutValue() |
		rpl::map([](Window::Adaptive::ChatLayout layout) { return (layout == Window::Adaptive::ChatLayout::Wide); }));
...
void SetupDefaultThemes(not_null<Window::Controller *> window, not_null<Ui::VerticalLayout *> container) {
	using Type = Window::Theme::EmbeddedType;
	using Scheme = Window::Theme::EmbeddedScheme;
	using Check = Window::Theme::CloudListCheck;
	using namespace Window::Theme;

	const auto block = container->add(object_ptr<Ui::FixedHeightWidget>(container));
	const auto palette = Ui::CreateChild<ColorsPalette>(container.get(), container.get());
	...
	const auto apply = [=](const Scheme &scheme)
	{
		...
		const auto currentlyIsCustom = (chosen() == Type(-1)) && !Background()->themeObject().cloud.id;
		const auto keep = [=]
		{
	...
	const auto schemeClicked = [=](const Scheme &scheme, Qt::KeyboardModifiers modifiers)
	{
		if (AyuFeatures::MessageShot::isChoosingTheme()) {
	...
	auto buttons = ranges::views::all(kSchemesList) |
		ranges::views::transform(
					   [&](const Scheme &scheme)
					   {
						   ...
						   const auto result = Ui::CreateChild<Ui::Radioenum<Type>>(
							   block, group, scheme.type, QString(), st::settingsTheme, std::move(check));
						   rpl::duplicate(scheme.name) |
							   rpl::start_with_next([=](const QString &themeName) { result->setText(themeName); },
													result->lifetime());
						   result->addClickHandler([=] { schemeClicked(scheme, result->clickModifiers()); });
		...
		rpl::start_with_next(
			[=](QColor color)
			{
				...
				if (Background()->editingTheme()) {
					...
					window->show(Ui::MakeInformBox(tr::lng_theme_editor_cant_change_theme()));
					return;
				}
				const auto type = chosen();
				const auto scheme = ranges::find(kSchemesList, type, &Scheme::type);
...
void SetupThemeOptions(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	using namespace Window::Theme;
...
void SetupCloudThemes(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	using namespace Window::Theme;
	using namespace rpl::mappers;

	const auto wrap =
		container
			->add(object_ptr<Ui::SlideWrap<Ui::VerticalLayout>>(container, object_ptr<Ui::VerticalLayout>(container)))
			->setDuration(0);
	...
	const auto title = AddSubsectionTitle(inner, tr::lng_settings_bg_cloud_themes());
	const auto showAll = Ui::CreateChild<Ui::LinkButton>(inner, tr::lng_settings_bg_show_all(tr::now));

	rpl::combine(title->topValue(), inner->widthValue(), showAll->widthValue()) |
		rpl::start_with_next([=](int top, int outerWidth, int width)
							 { showAll->moveToRight(st::defaultSubsectionTitlePadding.left(), top, outerWidth); },
							 showAll->lifetime());
	...
	inner->add(list->takeWidget(),
			   style::margins(st::settingsButtonNoIcon.padding.left(), 0, st::settingsButtonNoIcon.padding.right(), 0));

	list->allShown() | rpl::start_with_next([=](bool shown) { showAll->setVisible(!shown); }, showAll->lifetime());
	...
	AddButtonWithIcon(edit, tr::lng_settings_bg_theme_edit(), st::settingsButton, {&st::menuIconPalette})
		->addClickHandler([=] { StartEditor(&controller->window(), Background()->themeObject().cloud); });

	editWrap->toggleOn(rpl::single(BackgroundUpdate(BackgroundUpdate::Type::ApplyingTheme, Background()->tile())) |
...
void SetupThemeSettings(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	Ui::AddDivider(container);
	Ui::AddSkip(container, st::settingsPrivacySkip);

	Ui::AddSubsectionTitle(container, tr::lng_settings_theme_settings());

	AddPeerColorButton(container, controller->uiShow(), controller->session().user(), st::settingsColorButton);
	...
	if (settings->systemDarkMode().has_value()) {
		auto label =
			...
			rpl::map(
				[=](bool enabled)
				{ return enabled ? tr::lng_settings_auto_night_mode_on() : tr::lng_settings_auto_night_mode_off(); }) |
			rpl::flatten_latest();
		AddButtonWithLabel(container,
			...
			->setClickedCallback(
				[=]
				{
					...
					if (now && Window::Theme::Background()->editingTheme()) {
						controller->show(Ui::MakeInformBox(tr::lng_theme_editor_cant_change_theme()));
					} else {
	...
	const auto family = container->lifetime().make_state<rpl::variable<QString>>(settings->customFontFamily());
	auto label = family->value() |
		rpl::map(
					 [](QString family)
					 {
						 return family.isEmpty()				  ? tr::lng_font_default(tr::now)
							 : (family == style::SystemFontTag()) ? tr::lng_font_system(tr::now)
																  : family;
					 });
	AddButtonWithLabel(
		container, tr::lng_settings_font_family(), std::move(label), st::settingsButton, {&st::menuIconFont})
		->setClickedCallback(
			[=]
			{
				...
				const auto theme =
					std::shared_ptr<Ui::ChatTheme>(Window::Theme::DefaultChatThemeOn(container->lifetime()));
				const auto generateBg = [=]
				{
					const auto size = st::boxWidth;
					const auto ratio = style::DevicePixelRatio();
					auto result = QImage(QSize(size, size) * ratio, QImage::Format_ARGB32_Premultiplied);
					auto p = QPainter(&result);
				...
				controller->show(Box(Ui::ChooseFontBox, generateBg, family->current(), save));
			});

	Ui::AddSkip(container, st::settingsCheckboxesSkip);
}

void SetupSupportSwitchSettings(not_null<Window::SessionController *> controller,
								not_null<Ui::VerticalLayout *> container) {
	using SwitchType = Support::SwitchSettings;
	const auto group =
		std::make_shared<Ui::RadioenumGroup<SwitchType>>(controller->session().settings().supportSwitch());
	const auto add = [&](SwitchType value, const QString &label)
	{
		container->add(object_ptr<Ui::Radioenum<SwitchType>>(container, group, value, label, st::settingsSendType),
					   st::settingsSendTypePadding);
	};
...
void SetupSupportChatsLimitSlice(not_null<Window::SessionController *> controller,
								 not_null<Ui::VerticalLayout *> container) {
	...
	const auto best =
		ranges::min_element(options, std::less<>(), [&](const Option &option) { return std::abs(option.days - days); });

	const auto group = std::make_shared<Ui::RadiobuttonGroup>(best->days);
	for (const auto &option : options) {
		container->add(object_ptr<Ui::Radiobutton>(container, group, option.days, option.label, st::settingsSendType),
					   st::settingsSendTypePadding);
	}
	group->setChangedCallback(
		[=](int days)
		{
			controller->session().settings().setSupportChatsTimeSlice(days * kDayDuration);
			controller->session().saveSettingsDelayed();
		});
}

void SetupSupport(not_null<Window::SessionController *> controller, not_null<Ui::VerticalLayout *> container) {
	Ui::AddSkip(container);
	...
	inner->add(object_ptr<Ui::Checkbox>(inner,
		...
		rpl::start_with_next(
			[=](bool checked)
			{
				controller->session().settings().setSupportTemplatesAutocomplete(checked);
				controller->session().saveSettingsDelayed();
			},
			inner->lifetime());
...
rpl::producer<QString> Chat::title() { return tr::lng_settings_section_chat_settings(); }
...
void Chat::setupContent(not_null<Window::SessionController *> controller) {
	...
	auto updateOnTick = rpl::single() | rpl::then(base::timer_each(60 * crl::time(1000)));
