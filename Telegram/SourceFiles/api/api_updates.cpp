...
void ProcessScheduledMessageWithElapsedTime(not_null<Main::Session *> session,
											bool needToAdd,
...
bool HasForceLogoutNotification(const MTPUpdates &updates) {
	const auto checkUpdate = [](const MTPUpdate &update)
	{
		...
		return IsForceLogoutNotification(update.c_updateServiceNotification());
	};
	...
	switch (updates.type()) {
		case mtpc_updates: return checkVector(updates.c_updates().vupdates());
		case mtpc_updatesCombined: return checkVector(updates.c_updatesCombined().vupdates());
		case mtpc_updateShort: return checkUpdate(updates.c_updateShort().vupdate());
	}
	return false;
}

bool ForwardedInfoDataLoaded(not_null<Main::Session *> session, const MTPMessageFwdHeader &header) {
	return header.match(
...
bool MentionUsersLoaded(not_null<Main::Session *> session, const MTPVector<MTPMessageEntity> &entities) {
...
DataIsLoadedResult AllDataLoadedForMessage(not_null<Main::Session *> session, const MTPMessage &message) {
	return message.match(
		[&](const MTPDmessage &message)
		{
			if (const auto fromId = message.vfrom_id()) {
				if (!message.is_post() && !session->data().peerLoaded(peerFromMTP(*fromId))) {
		...
		[&](const MTPDmessageService &message)
		{
			...
			return message.vaction().match(
				...
				[](const auto &) { return DataIsLoadedResult::Ok; });
		},
		[](const MTPDmessageEmpty &message) { return DataIsLoadedResult::Ok; });
}

} // namespace

Updates::Updates(not_null<Main::Session *> session)
	: _session(session), _noUpdatesTimer([=] { sendPing(); }), _onlineTimer([=] { updateOnline(); }), _ptsWaiter(this),
	  _byPtsTimer([=] { getDifferenceByPts(); }), _bySeqTimer([=] { getDifference(); }),
	  _byMinChannelTimer([=] { getDifference(); }), _failDifferenceTimer([=] { getDifferenceAfterFail(); }),
	  _idleFinishTimer([=] { checkIdleFinish(); }) {
	_ptsWaiter.setRequesting(true);

	session->account().mtpUpdates() |
		rpl::start_with_next([=](const MTPUpdates &updates) { mtpUpdateReceived(updates); }, _lifetime);

	session->account().mtpNewSessionCreated() | rpl::start_with_next([=] { mtpNewSessionCreated(); }, _lifetime);

	api().request(MTPupdates_GetState()).done([=](const MTPupdates_State &result) { stateDone(result); }).send();
				...
				if (const auto list = _pendingSpeakingCallParticipants.take(peer)) {
					if (const auto call = peer->groupCall()) {
						for (const auto &[participantPeerId, when] : *list) {
							call->applyActiveUpdate(participantPeerId,
													Data::LastSpokeTimes{.anything = when, .voice = when},
...
Main::Session &Updates::session() const { return *_session; }

ApiWrap &Updates::api() const { return _session->api(); }

void Updates::checkLastUpdate(bool afterSleep) {
	const auto now = crl::now();
	const auto skip = afterSleep ? kNoUpdatesAfterSleepTimeout : kNoUpdatesTimeout;
...
void Updates::feedUpdateVector(const MTPVector<MTPUpdate> &updates, SkipUpdatePolicy policy) {
	auto list = updates.v;
	const auto hasGroupCallParticipantUpdates =
		ranges::contains(list, mtpc_updateGroupCallParticipants, &MTPUpdate::type);
	if (hasGroupCallParticipantUpdates) {
		ranges::stable_sort(list,
							std::less<>(),
							[](const MTPUpdate &entry)
							{
	...
	for (const auto &entry : std::as_const(list)) {
		...
		if ((policy == SkipUpdatePolicy::SkipMessageIds && type == mtpc_updateMessageID) ||
			(policy == SkipUpdatePolicy::SkipExceptGroupCallParticipants && type != mtpc_updateGroupCallParticipants)) {
			continue;
		}
		feedUpdate(entry);
	}
...
void Updates::checkForSentToScheduled(const MTPUpdates &updates) {
	updates.match([&](const MTPDupdates &data) { applyConvertToScheduledOnSend(data.vupdates(), true); },
				  [&](const MTPDupdatesCombined &data) { applyConvertToScheduledOnSend(data.vupdates(), true); },
				  [](const auto &) {});
}
...
void Updates::channelDifferenceDone(not_null<ChannelData *> channel, const MTPupdates_ChannelDifference &difference) {
	_channelFailDifferenceTimeout.remove(channel);

	const auto timeout = difference.match([&](const auto &data) { return data.vtimeout().value_or_empty(); });
	const auto isFinal = difference.match([&](const auto &data) { return data.is_final(); });
	difference.match([&](const MTPDupdates_channelDifferenceEmpty &data) { channel->ptsInit(data.vpts().v); },
					 [&](const MTPDupdates_channelDifferenceTooLong &data)
					 {
						 ...
						 session().data().applyDialogs(
							 nullptr, data.vmessages().v, QVector<MTPDialog>(1, data.vdialog()));
	...
	if (!isFinal) {
		...
		channel->ptsSetWaitingForShortPoll(timeout ? (timeout * crl::time(1000)) : kWaitForChannelGetDifference);
	} else {
		channel->ptsSetWaitingForShortPoll(-1);
	}
}

void Updates::feedChannelDifference(const MTPDupdates_channelDifference &data) {
	session().data().processUsers(data.vusers());
	session().data().processChats(data.vchats());
	...
	session().data().processMessages(data.vnew_messages(), NewMessageType::Unread);
	feedUpdateVector(data.vother_updates(), SkipUpdatePolicy::SkipMessageIds);
	_handlingChannelDifference = false;
}

void Updates::channelDifferenceFail(not_null<ChannelData *> channel, const MTP::Error &error) {
	LOG(("RPC Error in getChannelDifference: %1 %2: %3")
			.arg(QString::number(error.code()), error.type(), error.description()));
...
bool Updates::whenGetDiffChanged(ChannelData *channel,
								 int32 ms,
...
bool Updates::updateAndApply(int32 pts, int32 ptsCount, const MTPUpdates &updates) {
	return _ptsWaiter.updateAndApply(nullptr, pts, ptsCount, updates);
}

bool Updates::updateAndApply(int32 pts, int32 ptsCount, const MTPUpdate &update) {
	return _ptsWaiter.updateAndApply(nullptr, pts, ptsCount, update);
}

bool Updates::updateAndApply(int32 pts, int32 ptsCount) { return _ptsWaiter.updateAndApply(nullptr, pts, ptsCount); }

void Updates::feedDifference(const MTPVector<MTPUser> &users,
...
void Updates::getDifferenceByPts() {
	...
	for (auto i = _whenGetDiffByPts.begin(); i != _whenGetDiffByPts.cend();) {
		if (i->second > now) {
			...
			getChannelDifference(i->first, ChannelDifferenceRequest::PtsGapOrShortPoll);
...
void Updates::getDifference() {
	...
	api()
		.request(MTPupdates_GetDifference(MTP_flags(0),
		...
		.done([=](const MTPupdates_Difference &result) { differenceDone(result); })
		.fail([=](const MTP::Error &error) { differenceFail(error); })
		.send();
}

void Updates::getChannelDifference(not_null<ChannelData *> channel, ChannelDifferenceRequest from) {
	...
	api()
		.request(MTPupdates_GetChannelDifference(MTP_flags(flags),
												 channel->inputChannel,
												 filter,
												 MTP_int(channel->pts()),
												 MTP_int(kChannelGetDifferenceLimit)))
		.done([=](const MTPupdates_ChannelDifference &result) { channelDifferenceDone(channel, result); })
		.fail([=](const MTP::Error &error) { channelDifferenceFail(channel, error); })
		.send();
}

void Updates::sendPing() { _session->mtp().ping(); }
			...
			[=](PeerData *peer)
			{
				...
				if (was != peer) {
					...
					if (const auto channel = peer ? peer->asChannel() : nullptr) {
						channel->ptsSetWaitingForShortPoll(kWaitForChannelGetDifference);
					}
				}
			},
...
bool Updates::inActiveChats(not_null<PeerData *> peer) const {
	return ranges::contains(_activeChats, peer.get(), [](const auto &pair) { return pair.second.peer; });
}
...
void Updates::channelRangeDifferenceSend(not_null<ChannelData *> channel, MsgRange range, int32 pts) {
	Expects(range.from < range.till);

	const auto limit = range.till - range.from;
	const auto filter = MTP_channelMessagesFilter(
		MTP_flags(0), MTP_vector<MTPMessageRange>(1, MTP_messageRange(MTP_int(range.from), MTP_int(range.till - 1))));
	const auto requestId =
		api()
			.request(MTPupdates_GetChannelDifference(MTP_flags(MTPupdates_GetChannelDifference::Flag::f_force),
													 channel->inputChannel,
													 filter,
													 MTP_int(pts),
													 MTP_int(limit)))
			...
			.fail([=] { _rangeDifferenceRequests.remove(channel); })
			.send();
	_rangeDifferenceRequests.emplace(channel, requestId);
}

void Updates::channelRangeDifferenceDone(not_null<ChannelData *> channel,
										 MsgRange range,
										 const MTPupdates_ChannelDifference &result) {
	...
	switch (result.type()) {
		...
		case mtpc_updates_channelDifferenceTooLong: {
			...
			nextRequestPts = d.vdialog().match([&](const MTPDdialog &data) { return data.vpts().value_or_empty(); },
											   [&](const MTPDdialogFolder &data) { return 0; });
			isFinal = d.is_final();
		} break;
...
void Updates::mtpNewSessionCreated() {
	Core::App().checkAutoLock();
	_updatesSeq = 0;
	MTP_LOG(0, ("getDifference { after new_session_created }%1").arg(_session->mtp().isTestMode() ? " TESTMODE" : ""));
	getDifference();
}
...
void Updates::mtpUpdateReceived(const MTPUpdates &updates) {
	...
	_lastUpdateTime = crl::now();
	_noUpdatesTimer.callOnce(kNoUpdatesTimeout);
	if (!requestingDifference() || HasForceLogoutNotification(updates)) {
...
void Updates::applyConvertToScheduledOnSend(const MTPVector<MTPUpdate> &other, bool skipScheduledCheck) {
	for (const auto &update : other.v) {
		update.match(
			[&](const MTPDupdateNewScheduledMessage &data)
			{
				...
				for (const auto &updateId : other.v) {
					updateId.match(
						[&](const MTPDupdateMessageID &dataId)
						{
							if (dataId.vid().v == id) {
...
void Updates::applyGroupCallParticipantUpdates(const MTPUpdates &updates) {
	updates.match(
		[&](const MTPDupdates &data)
		{
			session().data().processUsers(data.vusers());
			session().data().processChats(data.vchats());
			feedUpdateVector(data.vupdates(), SkipUpdatePolicy::SkipExceptGroupCallParticipants);
		},
...
int32 Updates::pts() const { return _ptsWaiter.current(); }

void Updates::updateOnline(crl::time lastNonIdleTime) { updateOnline(lastNonIdleTime, false); }

bool Updates::isIdle() const { return _isIdle.current(); }

rpl::producer<bool> Updates::isIdleValue() const { return _isIdle.value(); }

void Updates::updateOnline(crl::time lastNonIdleTime, bool gotOtherOffline) {
	if (!lastNonIdleTime) {
		lastNonIdleTime = Core::App().lastNonIdleTime();
	}
	crl::on_main(&session(), [=] { Core::App().checkAutoLock(lastNonIdleTime); });

	// ViGram sendOnlinePackets
	...
	auto ms = crl::now();
	if (isOnline != _lastWasOnline || (isOnline && _lastSetOnline + config.onlineUpdatePeriod <= ms) ||
		...
		_lastSetOnline = ms;
		if (!Core::Quitting()) {
			_onlineRequest = api().request(MTPaccount_UpdateStatus(MTP_bool(!isOnline))).send();
		} else {
			_onlineRequest = api()
								 .request(MTPaccount_UpdateStatus(MTP_bool(!isOnline)))
								 .done([=] { Core::App().quitPreventFinished(); })
								 .fail([=] { Core::App().quitPreventFinished(); })
								 .send();
		}

		const auto self = session().user();
		const auto onlineFor = (config.onlineUpdatePeriod / 1000);
		self->updateLastseen(Data::LastseenStatus::OnlineTill(base::unixtime::now() + (isOnline ? onlineFor : -1)));
		session().changes().peerUpdated(self, Data::PeerUpdate::Flag::OnlineStatus);
...
void Updates::checkIdleFinish(crl::time lastNonIdleTime) {
	...
	if (crl::now() - lastNonIdleTime < _session->serverConfig().offlineIdleTimeout) {
...
bool Updates::lastWasOnline() const { return _lastWasOnline; }

crl::time Updates::lastSetOnline() const { return _lastSetOnline; }
...
void Updates::handleSendActionUpdate(PeerId peerId, MsgId rootId, PeerId fromId, const MTPSendMessageAction &action) {
	const auto history = session().data().historyLoaded(peerId);
	if (!history) {
		return;
	}
	const auto peer = history->peer;
	const auto from = (fromId == session().userPeerId()) ? session().user().get() : session().data().peerLoaded(fromId);
	...
	session().data().sendActionManager().registerFor(history, rootId, from->asUser(), action, when);
}

void Updates::handleEmojiInteraction(not_null<PeerData *> peer, const MTPDsendMessageEmojiInteraction &data) {
	const auto json = data.vinteraction().match([&](const MTPDdataJSON &data) { return data.vdata().v; });
	handleEmojiInteraction(peer, data.vmsg_id().v, qs(data.vemoticon()), ChatHelpers::EmojiInteractions::Parse(json));
}

void Updates::handleSpeakingInCall(not_null<PeerData *> peer,
	...
	if (call) {
		call->applyActiveUpdate(
			participantPeerId, Data::LastSpokeTimes{.anything = now, .voice = now}, participantPeerLoaded);
	} else {
		const auto chat = peer->asChat();
		const auto channel = peer->asChannel();
		const auto active =
			chat ? (chat->flags() & ChatDataFlag::CallActive) : (channel->flags() & ChannelDataFlag::CallActive);
		if (active) {
			_pendingSpeakingCallParticipants.emplace(peer).first->second[participantPeerId] = now;
...
void Updates::handleEmojiInteraction(not_null<PeerData *> peer,
									 MsgId messageId,
	...
	window->emojiInteractions().startIncoming(peer, messageId, emoticon, std::move(bunch));
}

void Updates::handleEmojiInteraction(not_null<PeerData *> peer, const QString &emoticon) {
	if (session().windows().empty()) {
		return;
	}
	const auto window = session().windows().front();
...
void Updates::applyUpdatesNoPtsCheck(const MTPUpdates &updates) {
	switch (updates.type()) {
		case mtpc_updateShortMessage: {
			...
			_session->data().addNewMessage(
				MTP_message(MTP_flags(flags),
							d.vid(),
							(d.is_out() ? peerToMTP(_session->userPeerId()) : MTP_peerUser(d.vuser_id())),
		...
		case mtpc_updateShortChatMessage: {
			...
			const auto flags = mtpCastFlags(d.vflags().v) | MTPDmessage::Flag::f_from_id;
			_session->data().addNewMessage(MTP_message(MTP_flags(flags),
													   d.vid(),
...
void Updates::applyUpdateNoPtsCheck(const MTPUpdate &update) {
	switch (update.type()) {
		case mtpc_updateNewMessage: {
			...
			if (needToAdd) {
				_session->data().addNewMessage(d.vmessage(), MessageFlags(), NewMessageType::Unread);
			}
		} break;
		...
		case mtpc_updateReadHistoryInbox: {
			...
			if (const auto history = _session->data().historyLoaded(peer)) {
				...
				history->applyInboxReadUpdate(folderId, d.vmax_id().v, d.vstill_unread_count().v);
			}
		} break;
		...
		case mtpc_updateDeleteChannelMessages: {
			...
			_session->data().processMessagesDeleted(peerFromChannel(d.vchannel_id().v), d.vmessages().v);
		} break;
...
void Updates::applyUpdates(const MTPUpdates &updates, uint64 sentMessageRandomId) {
	...
	switch (updates.type()) {
		...
		case mtpc_updateShortMessage: {
			...
			if (!session().data().userLoaded(d.vuser_id()) || (viaBotId && !session().data().userLoaded(*viaBotId)) ||
		...
		case mtpc_updateShortChatMessage: {
			...
			const auto entities = d.ventities();
			const auto fwd = d.vfwd_from();
			if (!chat || noFrom || (viaBotId && !session().data().userLoaded(*viaBotId)) ||
				(entities && !MentionUsersLoaded(&session(), *entities)) ||
				(fwd && !ForwardedInfoDataLoaded(&session(), *fwd))) {
				MTP_LOG(0,
						("getDifference "
		...
		case mtpc_updateShortSentMessage: {
			...
			if (!IsServerMsgId(d.vid().v)) {
				...
				{ return sent.peerId ? owner.message(sent.peerId, d.vid().v) : nullptr; };
				if (const auto id = owner.messageIdByRandomId(randomId)) {
					const auto local = owner.message(id);
					if (local && local->isScheduled()) {
						session().scheduledMessages().sendNowSimpleMessage(d, local);
					}
				}
				...
				if (const auto item = lookupMessage()) {
					...
					if (list && !MentionUsersLoaded(&session(), *list)) {
						session().api().requestMessageData(item->history()->peer, item->id, nullptr);
					}
...
void Updates::feedUpdate(const MTPUpdate &update) {
	switch (update.type()) {
			...
			if (const auto id = session().data().messageIdByRandomId(randomId)) {
				...
				if (const auto local = owner.message(id)) {
					if (local->isScheduled()) {
						...
						const auto existing = session().data().message(id.peer, newId);
						if (existing && !local->mainView()) {
							...
							if (existing) {
								...
								session().topPeers().increment(local->history()->peer, local->date());
							}
		...
		case mtpc_updateChannelReadMessagesContents: {
			...
			session().api().unreadThings().mediaAndMentionsRead(unknownReadIds, channel);
		} break;
		...
		case mtpc_updateMessageReactions: {
			...
			if (const auto history = session().data().historyLoaded(peer)) {
				...
				if (item) {
					...
					const auto hasUnreadReaction = Data::Reactions::HasUnread(d.vreactions());
		...
		case mtpc_updateReadChannelInbox: {
			...
			if (const auto history = session().data().historyLoaded(peer)) {
				history->applyInboxReadUpdate(
					d.vfolder_id().value_or_empty(), d.vmax_id().v, d.vstill_unread_count().v, d.vpts().v);
			}
		} break;
		...
		case mtpc_updateUserTyping: {
			...
			handleSendActionUpdate(peerFromUser(d.vuser_id()), 0, peerFromUser(d.vuser_id()), d.vaction());
		} break;

		case mtpc_updateChatUserTyping: {
			auto &d = update.c_updateChatUserTyping();
			handleSendActionUpdate(peerFromChat(d.vchat_id()), 0, peerFromMTP(d.vfrom_id()), d.vaction());
		} break;

		case mtpc_updateChannelUserTyping: {
			const auto &d = update.c_updateChannelUserTyping();
			handleSendActionUpdate(peerFromChannel(d.vchannel_id()),
		...
		case mtpc_updateUserName: {
			...
			if (const auto user = session().data().userLoaded(d.vuser_id())) {
				...
				const auto first = contact ? user->firstName : qs(d.vfirst_name());
				const auto last = contact ? user->lastName : qs(d.vlast_name());
				// #TODO usernames
				const auto username =
					d.vusernames().v.isEmpty() ? QString() : qs(d.vusernames().v.front().data().vusername());
				user->setName(TextUtilities::SingleLine(first),
		...
		case mtpc_updateNotifySettings: {
			...
			session().data().notifySettings().apply(d.vpeer(), d.vnotify_settings());
		} break;
			...
			if (const auto user = session().data().userLoaded(d.vuser_id())) {
				...
				if (newPhone != user->phone()) {
					...
					user->setName(
						user->firstName,
						user->lastName,
						((user->isContact() || user->isServiceUser() || user->isSelf() || user->phone().isEmpty())
							 ? QString()
							 : Ui::FormatPhone(user->phone())),
						user->username());

					session().changes().peerUpdated(user, Data::PeerUpdate::Flag::PhoneNumber);
				}
			}
		} break;
		...
		case mtpc_updatePeerWallpaper: {
			...
			if (const auto peer = session().data().peerLoaded(peerFromMTP(d.vpeer()))) {
				if (const auto paper = d.vwallpaper()) {
					peer->setWallPaper(Data::WallPaper::Create(&session(), *paper), d.is_wallpaper_overridden());
				} else {
					peer->setWallPaper({});
				}
			}
		...
		case mtpc_updateBotCommands: {
			...
			if (const auto peer = session().data().peerLoaded(peerFromMTP(d.vpeer()))) {
				...
				const auto commands = Data::BotCommands{
					...
					.commands = ranges::views::all(d.vcommands().v) | ranges::views::transform(Data::BotCommandFromTL) |
						ranges::to_vector,
				};

				if (const auto user = peer->asUser()) {
					if (user->isBot() && user->id == peerFromUser(botId)) {
						const auto equal = ranges::equal(user->botInfo->commands, commands.commands);
		...
		case mtpc_updateServiceNotification: {
			...
			const auto text = TextWithEntities{qs(d.vmessage()), Api::EntitiesFromMTP(&session(), d.ventities().v)};
			if (IsForceLogoutNotification(d)) {
				...
				session().data().serviceNotification(text, d.vmedia(), d.is_invert_media());
		...
		case mtpc_updatePrivacy: {
			...
			const auto allChatsLoaded = [&](const MTPVector<MTPlong> &ids)
			{
				for (const auto &chatId : ids.v) {
					if (!session().data().chatLoaded(chatId) && !session().data().channelLoaded(chatId)) {
						return false;
					}
				}
				return true;
			...
			const auto allLoaded = [&]
			{
				for (const auto &rule : d.vrules().v) {
					const auto loaded = rule.match([&](const MTPDprivacyValueAllowChatParticipants &data)
			...
			session().api().userPrivacy().apply(d.vkey().type(), d.vrules(), allLoaded());
		} break;

		case mtpc_updatePinnedDialogs: {
			const auto &d = update.c_updatePinnedDialogs();
			const auto folderId = d.vfolder_id().value_or_empty();
			const auto loaded = !folderId || (session().data().folderLoaded(folderId) != nullptr);
			const auto folder = folderId ? session().data().folder(folderId).get() : nullptr;
			const auto done = [&]
			{
				...
				const auto notLoaded = [&](const MTPDialogPeer &peer)
				{
					return peer.match([&](const MTPDdialogPeer &data)
									  { return !session().data().historyLoaded(peerFromMTP(data.vpeer())); },
		...
		case mtpc_updatePinnedSavedDialogs: {
			session().data().savedMessages().apply(update.c_updatePinnedSavedDialogs());
		} break;

		case mtpc_updateSavedDialogPinned: {
			session().data().savedMessages().apply(update.c_updateSavedDialogPinned());
		} break;
		...
		case mtpc_updateChannel: {
			...
			if (const auto channel = session().data().channelLoaded(d.vchannel_id())) {
				...
				if (channel->amIn()) {
					if (channel->isMegagroup() && !channel->amCreator() && !channel->hasAdminRights()) {
						channel->updateFullForced();
					}
					const auto history = channel->owner().history(channel);
					history->requestChatListMessage();
					if (!history->folderKnown() || (!history->unreadCountKnown() && !history->isForum())) {
						history->owner().histories().requestDialogEntry(history);
					}
		...
		case mtpc_updateReadChannelDiscussionInbox: {
			...
			const auto id = FullMsgId(peerFromChannel(d.vchannel_id()), d.vtop_msg_id().v);
			...
			if (const auto broadcastId = d.vbroadcast_id()) {
				if (const auto post = session().data().message(peerFromChannel(*broadcastId), d.vbroadcast_post()->v)) {
					post->setCommentsInboxReadTill(readTillId);
				}
			}
		} break;
			...
			if (const auto peer = session().data().peerLoaded(peerId)) {
				if (const auto forum = peer->forum()) {
					const auto done = [&]
					{
						...
						const auto &order = list->v;
						const auto notLoaded = [&](const MTPint &topicId) { return !forum->topicFor(topicId.v); };
						if (!ranges::none_of(order, notLoaded)) {
							return false;
						}
		...
		case mtpc_updateStickerSetsOrder: {
			...
			const auto localSize = isEmoji ? stickers.emojiSetsOrder().size()
				: isMasks				   ? stickers.maskSetsOrder().size()
			...
			if ((result.size() != localSize) || (result.size() != order.size())) {
				...
				stickers.notifyUpdated(isEmoji		 ? Data::StickersType::Emoji
										   : isMasks ? Data::StickersType::Masks
		...
		case mtpc_updateMoveStickerSetToTop: {
			...
			auto &order = isEmoji ? stickers.emojiSetsOrderRef() : stickers.setsOrderRef();
			...
			if (i == order.end()) {
				...
				stickers.notifyUpdated(isEmoji ? Data::StickersType::Emoji : Data::StickersType::Stickers);
			}
		} break;
		...
		case mtpc_updateDraftMessage: {
			...
			data.vdraft().match(
				[&](const MTPDdraftMessage &data) { Data::ApplyPeerCloudDraft(&session(), peerId, topicRootId, data); },
				[&](const MTPDdraftMessageEmpty &data)
				{ Data::ClearPeerCloudDraft(&session(), peerId, topicRootId, data.vdate().value_or_empty()); });
		} break;
