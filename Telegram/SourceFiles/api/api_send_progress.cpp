...
SendProgressManager::SendProgressManager(not_null<Main::Session *> session)
	: _session(session), _stopTypingTimer([=] { cancelTyping(base::take(_stopTypingHistory)); }) {}

void SendProgressManager::cancel(not_null<History *> history, SendProgressType type) { cancel(history, 0, type); }

void SendProgressManager::cancel(not_null<History *> history, MsgId topMsgId, SendProgressType type) {
...
void SendProgressManager::update(not_null<History *> history, SendProgressType type, int progress) {
	update(history, 0, type, progress);
}

void SendProgressManager::update(not_null<History *> history, MsgId topMsgId, SendProgressType type, int progress) {
	const auto peer = history->peer;
	if (peer->isSelf() || (peer->isChannel() && !peer->isMegagroup() && type != SendProgressType::Speaking)) {
		return;
	}
...
bool SendProgressManager::updated(const Key &key, bool doing) {
	...
	if (doing) {
		const auto sendEach =
			(key.type == SendProgressType::Speaking) ? kSendMySpeakingInterval : kSendMyTypingInterval;
		if (i == end(_updated)) {
...
void SendProgressManager::send(const Key &key, int progress) {
	...
	// ViGram sendUploadProgress
	const auto settings = &AyuSettings::getInstance();
	if (!settings->sendUploadProgress) {
		DEBUG_LOG(("[ViGram] Don't send upload progress"));
		return;
	}
	...
	const auto requestId =
		_session->api()
			.request(MTPmessages_SetTyping(
				MTP_flags(key.topMsgId ? MTPmessages_SetTyping::Flag::f_top_msg_id : MTPmessages_SetTyping::Flag(0)),
				key.history->peer->input,
				MTP_int(key.topMsgId),
				action))
			.done([=](const MTPBool &result, mtpRequestId requestId) { done(requestId); })
			.send();
