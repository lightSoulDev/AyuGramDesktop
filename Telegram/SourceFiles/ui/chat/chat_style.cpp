...
void EnsureCorners(CornersPixmaps &corners,
				   int radius,
...
void EnsureBlockquoteCache(std::unique_ptr<Text::QuotePaintCache> &cache, Fn<ColorIndexValues()> values) {
	if (cache) {
		return;
	}
...
void EnsurePreCache(std::unique_ptr<Text::QuotePaintCache> &cache,
					const style::color &color,
...
not_null<const MessageStyle *> ChatPaintContext::messageStyle() const { return &st->messageStyle(outbg, selected()); }

not_null<const MessageImageStyle *> ChatPaintContext::imageStyle() const { return &st->imageStyle(selected()); }

not_null<Text::QuotePaintCache *> ChatPaintContext::quoteCache(uint8 colorIndex) const {
	return !outbg ? st->coloredQuoteCache(selected(), colorIndex).get()
...
int HistoryServiceMsgInvertedShrink() {
	static const auto result = [] { return (HistoryServiceMsgInvertedRadius() * 2) / 3; }();
	return result;
}
...
int BackgroundEmojiData::CacheIndex(bool selected, bool outbg, bool inbubble, uint8 colorIndexPlusOne) {
	const auto base = colorIndexPlusOne ? (colorIndexPlusOne - 1)
										: (kColorIndexCount +
										   (!inbubble	? 0
												: outbg ? 1
														: 2));
...
int ColorPatternIndex(const ColorIndicesCompressed &indices, uint8 colorIndex, bool dark) {
	Expects(colorIndex >= 0 && colorIndex < kColorIndexCount);

	if (!indices.colors || colorIndex < kSimpleColorIndexCount) {
		return 0;
	}
...
ChatStyle::ChatStyle(rpl::producer<ColorIndicesCompressed> colorIndices) {
	if (colorIndices) {
		_colorIndicesLifetime = std::move(colorIndices) |
			rpl::start_with_next([=](ColorIndicesCompressed &&indices) { _colorIndices = std::move(indices); });
	}
	...
	make(_historyViewsSendingInvertedIcon, st::historyViewsSendingInvertedIcon);
	...
	make(&MessageStyle::channelBadgeIcon,
	...
	make(&MessageStyle::historyTranscribeIcon,
	...
	make(&MessageStyle::historyTranscribeLock,
	...
	make(&MessageStyle::historyTranscribeHide,
	...
	make(&MessageImageStyle::msgDateImgBg, st::msgDateImgBg, st::msgDateImgBgSelected);
	make(&MessageImageStyle::msgServiceBg, st::msgServiceBg, st::msgServiceBgSelected);
	make(&MessageImageStyle::msgShadow, st::msgInShadow, st::msgInShadowSelected);
	make(&MessageImageStyle::historyFileThumbRadialFg,
		 st::historyFileThumbRadialFg,
		 st::historyFileThumbRadialFgSelected);
	make(&MessageImageStyle::historyFileThumbPlay, st::historyFileThumbPlay, st::historyFileThumbPlaySelected);
	make(&MessageImageStyle::historyFileThumbWaiting, st::historyFileThumbWaiting, st::historyFileThumbWaitingSelected);
	make(&MessageImageStyle::historyFileThumbDownload,
		 st::historyFileThumbDownload,
		 st::historyFileThumbDownloadSelected);
	make(&MessageImageStyle::historyFileThumbCancel, st::historyFileThumbCancel, st::historyFileThumbCancelSelected);
	make(&MessageImageStyle::historyFileThumbPause, st::historyFileThumbPause, st::historyFileThumbPauseSelected);
	make(&MessageImageStyle::historyVideoDownload, st::historyVideoDownload, st::historyVideoDownloadSelected);
	make(&MessageImageStyle::historyVideoCancel, st::historyVideoCancel, st::historyVideoCancelSelected);
	make(&MessageImageStyle::historyVideoMessageMute, st::historyVideoMessageMute, st::historyVideoMessageMuteSelected);
	make(&MessageImageStyle::historyVideoMessageTtlIcon,
		 st::historyVideoMessageTtlIcon,
		 st::historyVideoMessageTtlIconSelected);
	make(&MessageImageStyle::historyPageEnlarge, st::historyPageEnlarge, st::historyPageEnlargeSelected);
	make(&MessageStyle::historyVoiceMessageTTL,
	...
	make(&MessageStyle::liveLocationLongIcon,
...
ChatStyle::ChatStyle(not_null<const style::palette *> isolated) : ChatStyle(rpl::producer<ColorIndicesCompressed>()) {
	assignPalette(isolated);
}

ChatStyle::~ChatStyle() = default;

void ChatStyle::apply(not_null<ChatTheme *> theme) { applyCustomPalette(theme->palette()); }

void ChatStyle::updateDarkValue() {
	const auto withBg = [&](const QColor &color) { return CountContrast(windowBg()->c, color); };
...
void ChatStyle::applyCustomPalette(const style::palette *palette) {
	...
	if (palette) {
		...
		style::PaletteChanged() |
			rpl::start_with_next([=] { assignPalette(style::main_palette::get()); }, _defaultPaletteChangeLifetime);
	}
}
...
std::span<Text::SpecialColor> ChatStyle::highlightColors() const {
	if (_highlightColors.empty()) {
		const auto push = [&](const style::color &color) { _highlightColors.push_back({&color->p, &color->p}); };
...
void ChatStyle::assignPalette(not_null<const style::palette *> palette) {
	*static_cast<style::palette *>(this) = *palette;
	...
	for (auto &style : _messageStyles) {
		...
		style.textPalette.linkAlwaysActive = style.semiboldPalette.linkAlwaysActive =
...
const CornersPixmaps &ChatStyle::serviceBgCornersNormal() const {
	EnsureCorners(_serviceBgCornersNormal, HistoryServiceMsgRadius(), msgServiceBg());
...
const CornersPixmaps &ChatStyle::serviceBgCornersInverted() const {
	if (_serviceBgCornersInverted.p[0].isNull()) {
		_serviceBgCornersInverted = PrepareInvertedCornerPixmaps(HistoryServiceMsgInvertedRadius(), msgServiceBg());
	}
...
const MessageStyle &ChatStyle::messageStyle(bool outbg, bool selected) const {
	...
	EnsureCorners(result.msgBgCornersSmall, BubbleRadiusSmall(), result.msgBg, &result.msgShadow);
	EnsureCorners(result.msgBgCornersLarge, BubbleRadiusLarge(), result.msgBg, &result.msgShadow);
	const auto &replyBar = result.msgReplyBarColor->c;
	for (auto i = 0; i != kColorPatternsCount; ++i) {
		EnsureBlockquoteCache(result.replyCache[i], [&] { return SimpleColorIndexValues(replyBar, i); });
		if (!result.quoteCache[i]) {
			result.quoteCache[i] = std::make_unique<Text::QuotePaintCache>(*result.replyCache[i]);
		}
	}

	const auto preBgOverride = [&] { return _dark ? QColor(0, 0, 0, 192) : std::optional<QColor>(); };
	EnsurePreCache(
		result.preCache, (selected ? result.textPalette.selectMonoFg : result.textPalette.monoFg), preBgOverride);
	return result;
}
...
const MessageImageStyle &ChatStyle::imageStyle(bool selected) const {
	...
	EnsureCorners(
		result.msgDateImgBgCorners, (st::msgDateImgPadding.y() * 2 + st::normalFont->height) / 2, result.msgDateImgBg);
	EnsureCorners(result.msgServiceBgCornersSmall, BubbleRadiusSmall(), result.msgServiceBg);
	EnsureCorners(result.msgServiceBgCornersLarge, BubbleRadiusLarge(), result.msgServiceBg);
	EnsureCorners(result.msgShadowCornersSmall, BubbleRadiusSmall(), result.msgShadow);
	EnsureCorners(result.msgShadowCornersLarge, BubbleRadiusLarge(), result.msgShadow);
...
int ChatStyle::colorPatternIndex(uint8 colorIndex) const {
	...
	if (!_colorIndices.colors || colorIndex < kSimpleColorIndexCount) {
		return 0;
	}
...
ColorIndexValues ChatStyle::computeColorIndexValues(bool selected, uint8 colorIndex) const {
	...
	auto &colors = _dark ? data.dark : data.light;
	if (!colors[0]) {
		return computeColorIndexValues(selected, colorIndex % kSimpleColorIndexCount);
	}
	const auto color = [&](int index)
	{
		const auto v = colors[index];
		return v ? QColor((v >> 16) & 0xFF, (v >> 8) & 0xFF, v & 0xFF) : QColor(0, 0, 0, 0);
	};
	auto result = ColorIndexValues{.outlines = {color(0), color(1), color(2)}};
...
not_null<Text::QuotePaintCache *> ChatStyle::serviceQuoteCache(bool twoColored) const {
	const auto index = (twoColored ? 1 : 0);
	const auto &service = msgServiceFg()->c;
	EnsureBlockquoteCache(_serviceQuoteCache[index], [&] { return SimpleColorIndexValues(service, twoColored); });
	return _serviceQuoteCache[index].get();
}

not_null<Text::QuotePaintCache *> ChatStyle::serviceReplyCache(bool twoColored) const {
	const auto index = (twoColored ? 1 : 0);
	const auto &service = msgServiceFg()->c;
	EnsureBlockquoteCache(_serviceReplyCache[index], [&] { return SimpleColorIndexValues(service, twoColored); });
	return _serviceReplyCache[index].get();
}

const ColorIndexValues &ChatStyle::coloredValues(bool selected, uint8 colorIndex) const {
	Expects(colorIndex >= 0 && colorIndex < kColorIndexCount);
...
const style::TextPalette &ChatStyle::coloredTextPalette(bool selected, uint8 colorIndex) const {
	Expects(colorIndex >= 0 && colorIndex < kColorIndexCount);
	...
	if (!result.linkFg) {
		...
		make(result.data, (selected ? st::inReplyTextPaletteSelected : st::inReplyTextPalette));
...
not_null<BackgroundEmojiData *> ChatStyle::backgroundEmojiData(uint64 id) const { return &_backgroundEmojis[id]; }

not_null<Text::QuotePaintCache *> ChatStyle::coloredQuoteCache(bool selected, uint8 colorIndex) const {
	return coloredCache(_coloredQuoteCaches, selected, colorIndex);
}

not_null<Text::QuotePaintCache *> ChatStyle::coloredReplyCache(bool selected, uint8 colorIndex) const {
	return coloredCache(_coloredReplyCaches, selected, colorIndex);
}

not_null<Text::QuotePaintCache *>
ChatStyle::coloredCache(ColoredQuotePaintCaches &caches, bool selected, uint8 colorIndex) const {
	Expects(colorIndex >= 0 && colorIndex < kColorIndexCount);

	const auto shift = (selected ? kColorIndexCount : 0);
	auto &cache = caches[shift + colorIndex];
	EnsureBlockquoteCache(cache, [&] { return coloredValues(selected, colorIndex); });
	return cache.get();
}

const CornersPixmaps &ChatStyle::msgBotKbOverBgAddCornersSmall() const {
	EnsureCorners(_msgBotKbOverBgAddCornersSmall, BubbleRadiusSmall(), msgBotKbOverBgAdd());
	return _msgBotKbOverBgAddCornersSmall;
}

const CornersPixmaps &ChatStyle::msgBotKbOverBgAddCornersLarge() const {
	EnsureCorners(_msgBotKbOverBgAddCornersLarge, BubbleRadiusLarge(), msgBotKbOverBgAdd());
	return _msgBotKbOverBgAddCornersLarge;
}

const CornersPixmaps &ChatStyle::msgSelectOverlayCorners(CachedCornerRadius radius) const {
	const auto index = static_cast<int>(radius);
	Assert(index >= 0 && index < int(CachedCornerRadius::kCount));

	EnsureCorners(_msgSelectOverlayCorners[index], CachedCornerRadiusValue(radius), msgSelectOverlay());
...
MessageStyle &ChatStyle::messageIn() { return messageStyleRaw(false, false); }

MessageStyle &ChatStyle::messageInSelected() { return messageStyleRaw(false, true); }

MessageStyle &ChatStyle::messageOut() { return messageStyleRaw(true, false); }

MessageStyle &ChatStyle::messageOutSelected() { return messageStyleRaw(true, true); }

MessageImageStyle &ChatStyle::imageStyleRaw(bool selected) const { return _imageStyles[selected ? 1 : 0]; }

MessageImageStyle &ChatStyle::image() { return imageStyleRaw(false); }

MessageImageStyle &ChatStyle::imageSelected() { return imageStyleRaw(true); }

void ChatStyle::make(style::color &my, const style::color &original) const {
	my = _colors[style::main_palette::indexOfColor(original)];
}

void ChatStyle::make(style::icon &my, const style::icon &original) const { my = original.withPalette(*this); }

void ChatStyle::make(style::TextPalette &my, const style::TextPalette &original) const {
...
void ChatStyle::make(style::TwoIconButton &my, const style::TwoIconButton &original) const {
	my = original;
...
void ChatStyle::make(style::ScrollArea &my, const style::ScrollArea &original) const {
	my = original;
...
void ChatStyle::make(Type MessageStyle::*my,
...
template<typename Type>
void ChatStyle::make(Type MessageImageStyle::*my, const Type &original, const Type &originalSelected) {
	make(image().*my, original);
	make(imageSelected().*my, originalSelected);
}

uint8 DecideColorIndex(uint64 id) { return id % kSimpleColorIndexCount; }
...
QColor FromNameFg(not_null<const ChatStyle *> st, bool selected, uint8 colorIndex) {
	return st->coloredValues(selected, colorIndex).name;
}

void FillComplexOverlayRect(QPainter &p, QRect rect, const style::color &color, const CornersPixmaps &corners) {
	using namespace Images;

	const auto pix = corners.p;
	const auto fillRect = [&](QRect rect) { p.fillRect(rect, color); };
	if (pix[kTopLeft].isNull() && pix[kTopRight].isNull() && pix[kBottomLeft].isNull() && pix[kBottomRight].isNull()) {
		fillRect(rect);
		return;
	}

	const auto ratio = style::DevicePixelRatio();
	const auto fillCorner = [&](int left, int top, int index) { p.drawPixmap(left, top, pix[index]); };
	const auto cornerSize = [&](int index)
	{
		const auto &p = pix[index];
		return p.isNull() ? 0 : p.width() / ratio;
	};
	const auto verticalSkip = [&](int left, int right) { return std::max(cornerSize(left), cornerSize(right)); };
	...
	if (top) {
		...
		if (right) {
			fillCorner(rect.left() + rect.width() - right, rect.top(), kTopRight);
	...
	if (bottom) {
		...
		if (left) {
			fillCorner(rect.left(), rect.top() + rect.height() - left, kBottomLeft);
		...
		if (right) {
			fillCorner(rect.left() + rect.width() - right, rect.top() + rect.height() - right, kBottomRight);
...
void FillComplexEllipse(QPainter &p, not_null<const ChatStyle *> st, QRect rect) {
