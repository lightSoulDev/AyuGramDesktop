...
[[nodiscard]] std::optional<QColor> ParseColor(const QString &text) {
	...
	const auto hex = [&](int from) -> std::optional<int>
	{
		const auto parse = [](QChar ch) -> std::optional<int>
		{
			...
			return (code >= 'a' && code <= 'f') ? std::make_optional(10 + (code - 'a'))
				: (code >= 'A' && code <= 'F')	? std::make_optional(10 + (code - 'A'))
				: (code >= '0' && code <= '9')	? std::make_optional(code - '0')
												: std::nullopt;
		};
...
[[nodiscard]] QColor ResolveRipple(QColor background) {
	...
	return QColor::fromHsv(
		hue, saturation, lightness - (lightness > kLightnessThreshold ? kLightnessDelta : -kLightnessDelta), alpha);
}
...
{
	WebviewWithLifetime(QWidget *parent = nullptr, Webview::WindowConfig config = Webview::WindowConfig());
...
Panel::Button::Button(QWidget *parent, const style::RoundButton &st)
	: RippleButton(parent, st.ripple), _st(st), _bg(st::windowBgActive->c),
	...
	resize(_st.padding.left() + _text.maxWidth() + _st.padding.right(),
...
void Panel::Button::updateBg(not_null<const style::color *> paletteBg) {
	...
	_bgLifetime = style::PaletteChanged() | rpl::start_with_next([=] { updateBg((*paletteBg)->c); });
}
...
void Panel::Button::updateFg(not_null<const style::color *> paletteFg) {
	...
	_fgLifetime = style::PaletteChanged() | rpl::start_with_next([=] { updateFg((*paletteFg)->c); });
}
...
void Panel::Button::toggleProgress(bool shown) {
	if (!_progress) {
		...
		_progress = std::make_unique<Progress>(this, [=] { return _progress->widget.rect(); });
		_progress->widget.paintRequest() |
			rpl::start_with_next(
				[=](QRect clip)
				{
					auto p = QPainter(&_progress->widget);
					p.setOpacity(_progress->shownAnimation.value(_progress->shown ? 1. : 0.));
					auto thickness = st::paymentsLoading.thickness;
					const auto rect =
						_progress->widget.rect().marginsRemoved({thickness, thickness, thickness, thickness});
					InfiniteRadialAnimation::Draw(p,
	...
	_progress->shownAnimation.start(callback, shown ? 0. : 1., shown ? 1. : 0., kProgressDuration);
	if (shown) {
	...
	sizeValue() |
		rpl::start_with_next(
			[=](QSize outer)
			{
				...
				_progress->widget.setGeometry(QRect{QPoint(right - skip - size.width(), top + skip), size});
			},
			_progress->geometryLifetime);

	_progress->widget.show();
	_progress->widget.raise();
	if (_progress->shown && Ui::AppInFocus() && Ui::InFocusChain(_progress->widget.window())) {
...
QImage Panel::Button::prepareRippleMask() const { return RippleAnimation::RoundRectMask(size(), st::callRadius); }
...
Panel::WebviewWithLifetime::WebviewWithLifetime(QWidget *parent, Webview::WindowConfig config)
	: window(parent, std::move(config)) {}

Panel::Progress::Progress(QWidget *parent, Fn<QRect()> rect)
	: widget(parent), animation(
						  [=]
						  {
...
Panel::Panel(Args &&args)
	: _storageId(args.storageId), _delegate(args.delegate), _menuButtons(args.menuButtons),
	  _widget(std::make_unique<SeparatePanel>(Ui::SeparatePanelArgs{
		  .menuSt = &st::botWebViewMenu,
	  })),
	  _fullscreen(args.fullscreen), _allowClipboardRead(args.allowClipboardRead) {
	...
	rpl::duplicate(args.title) |
		rpl::start_with_next(
			[=](const QString &title)
			{
				const auto value = tr::lng_credits_box_history_entry_miniapp(tr::now) + u": "_q + title;
	...
	_widget->fullScreenValue() |
		rpl::start_with_next([=](bool fullscreen) { _fullscreen = fullscreen; }, _widget->lifetime());
	...
	_widget->closeEvents() | rpl::filter([=] { return !_hiddenForPayment; }) |
		rpl::start_with_next([=] { _delegate->botClose(); }, _widget->lifetime());

	_widget->backRequests() | rpl::start_with_next([=] { postEvent("back_button_pressed"); }, _widget->lifetime());

	rpl::merge(style::PaletteChanged(), _themeUpdateForced.events()) |
		rpl::filter([=] { return !_themeUpdateScheduled; }) |
		rpl::start_with_next(
			[=]
			{
...
void Panel::setupDownloadsProgress(not_null<RpWidget *> button,
	...
	button->sizeValue() |
		rpl::start_with_next([=](QSize size) { widget->setGeometry(QRect(QPoint(), size)); }, widget->lifetime());
		...
		State(QWidget *parent)
			: animation(
				  [=](crl::time now)
				  {
					  ...
					  const auto current = total ? (progress.ready / float64(total)) : 0.;
			...
			[=](DownloadsProgress progress)
			{
				const auto toggle = [&](bool shown)
				{
					...
					state->fade.start(
						...
						{
							widget->update();
							if (!state->shown && !state->fade.animating() &&
								(!state->progress.total || (state->progress.ready == state->progress.total))) {
				...
				if (!state->shown && progress.loading) {
					...
					toggle(true);
						   ...
						   (state->progress.ready < state->progress.total && progress.ready == progress.total)) {
	...
	widget->paintRequest() |
		rpl::start_with_next(
			[=]
			{
				const auto opacity = std::clamp(state->fade.value(state->shown ? 2. : 0.) - 1., 0., 1.);
				if (!opacity) {
					return;
				}
				...
				const auto color = fullscreen ? st::radialFg
					: palette				  ? palette->boxTitleCloseFg()
											  : st::paymentsLoading.color;
				const auto &st = fullscreen ? st::fullScreenPanelMenu : st::separatePanelMenu;
				...
				const auto inner = QRectF(rect).marginsRemoved(QMarginsF{shift, shift, shift, shift});
...
void Panel::toggleProgress(bool shown) {
	if (!_progress) {
		...
		_progress = std::make_unique<Progress>(_widget.get(), [=] { return progressRect(); });
		_progress->widget.paintRequest() |
			rpl::start_with_next(
				[=](QRect clip)
				{
...
bool Panel::progressWithBackground() const { return (_progress->widget.width() == _widget->innerGeometry().width()); }
...
QRect Panel::progressRect() const {
	...
	return QRect(rect.x() + (rect.width() - size) / 2, rect.y() + (rect.height() - size) / 2, size, size);
}
...
bool Panel::showWebview(Args &&args, const Webview::ThemeParams &params) {
	...
	rpl::duplicate(args.downloadsProgress) | rpl::start_with_next([=] { _downloadsUpdated.fire({}); }, lifetime());
	...
	_widget->setMenuAllowed(
		[=](const Ui::Menu::MenuCallback &callback)
		{
			...
			if (!list.empty()) {
				auto value = rpl::single(std::move(list)) |
					rpl::then(_downloadsUpdated.events() | rpl::map([=] { return _delegate->botDownloads(); }));
				const auto action = [=](uint32 id, DownloadsAction type) { _delegate->botDownloadsAction(id, type); };
				callback(Ui::Menu::MenuCallback::Args{
					.text = tr::lng_downloads_section(tr::now),
					.icon = &st::menuIconDownload,
					.fillSubmenu = FillAttachBotDownloadsSubmenu(std::move(value), action),
				});
				callback({
			...
			if (main || (_menuButtons & MenuButton::RemoveFromMenu)) {
				const auto handler = [=]
				{ _delegate->botHandleMenuButton(main ? MenuButton::RemoveFromMainMenu : MenuButton::RemoveFromMenu); };
				callback({
					.text = (main ? tr::lng_bot_remove_from_side_menu : tr::lng_bot_remove_from_menu)(tr::now),
					.handler = handler,
		...
		[=, progress = std::move(args.downloadsProgress)](not_null<RpWidget *> button, bool fullscreen)
		{ setupDownloadsProgress(button, rpl::duplicate(progress), fullscreen); });
...
void Panel::createWebviewBottom() {
	...
	const auto label = CreateChild<FlatLabel>(_webviewBottom.get(), _bottomText.value(), st::paymentsWebviewBottom);
	_webviewBottomLabel = label;

	const auto height = padding.top() + label->heightNoMargins() + padding.bottom();
	rpl::combine(_webviewBottom->widthValue(), label->widthValue()) |
		rpl::start_with_next([=](int outerWidth, int width) { label->move((outerWidth - width) / 2, padding.top()); },
							 label->lifetime());
	label->show();
	_webviewBottom->resize(_webviewBottom->width(), height);

	rpl::combine(_webviewParent->geometryValue() | rpl::map([=] { return _widget->innerGeometry(); }),
				 bottom->heightValue()) |
		rpl::start_with_next(
			[=](QRect inner, int height)
			{
...
bool Panel::createWebview(const Webview::ThemeParams &params) {
	...
	container->show();
	_webview = std::make_unique<WebviewWithLifetime>(container,
	...
	_widget->allowChildFullScreenControls(!raw->widget()->inherits("QWindowContainer"));
#endif // !Q_OS_WIN && !Q_OS_MAC

	QObject::connect(raw->widget(),
					 &QObject::destroyed,
					 [=]
					 {
						 const auto parent = _webviewParent.data();
						 if (!_webview || &_webview->window != raw || !parent || _widget->inner() != parent) {
							 // If we destroyed _webview ourselves,
							 // or if we changed _widget->inner ourselves,
							 // we don't show any message, nothing crashed.
							 return;
						 }
						 crl::on_main(this, [=] { showCriticalError({"Error: WebView has crashed."}); });
					 });

	rpl::combine(container->geometryValue(), _footerHeight.value()) |
		rpl::start_with_next(
			[=](QRect geometry, int footer)
			{
	...
	raw->setNavigationStartHandler(
		[=](const QString &uri, bool newWindow)
		{
	...
	raw->setNavigationDoneHandler([=](bool success) { hideWebviewProgress(); });
	...
	base::qt_signal_producer(qApp, &QGuiApplication::focusWindowChanged) |
		rpl::filter(
			[=](QWindow *focused)
			{
				...
				return widget && !widget->isHidden() && handle && (focused == handle);
			}) |
		rpl::start_with_next([=] { _webview->window.focus(); }, _webview->lifetime);
...
void Panel::sendFullScreen() {
	postEvent("fullscreen_changed", _fullscreen.current() ? "{ is_fullscreen: true }" : "{ is_fullscreen: false }");
}

void Panel::sendSafeArea() { postEvent("safe_area_changed", "{ top: 0, right: 0, bottom: 0, left: 0 }"); }

void Panel::sendContentSafeArea() {
	const auto shift = st::separatePanelClose.rippleAreaPosition.y();
	const auto top = _fullscreen.current() ? (shift + st::fullScreenPanelClose.height + (shift / 2)) : 0;
	...
	postEvent("content_safe_area_changed", u"{ top: %1, right: 0, bottom: 0, left: 0 }"_q.arg(report));
}

void Panel::setTitle(rpl::producer<QString> title) { _widget->setTitle(std::move(title)); }
...
void Panel::switchInlineQueryMessage(const QJsonObject &args) {
	...
	for (const auto &value : typeArray) {
		...
		if (valid.contains(type)) {
			...
			LOG(("BotWebView Error: "
				 "Bad chat type in switchInlineQueryMessage: %1.")
					.arg(type));
			types.clear();
			break;
		}
	...
	auto callback = crl::guard(this,
							   [=](QString error)
							   {
								   if (error.isEmpty()) {
									   ...
									   postEvent("prepared_message_failed", u"{ error: \"%1\" }"_q.arg(error));
								   }
							   });
...
void Panel::processEmojiStatusRequest(const QJsonObject &args) {
	...
	const auto duration = TimeId(base::SafeRound(args["duration"].toDouble()));
	if (!emojiId) {
		postEvent("emoji_status_failed", "{ error: \"SUGGESTED_EMOJI_INVALID\" }");
		return;
	} else if (duration < 0) {
		postEvent("emoji_status_failed", "{ error: \"DURATION_INVALID\" }");
		return;
	}
	auto callback = crl::guard(this,
							   [=](QString error)
							   {
								   if (error.isEmpty()) {
									   ...
									   postEvent("emoji_status_failed", u"{ error: \"%1\" }"_q.arg(error));
								   }
							   });
...
void Panel::processEmojiStatusAccessRequest() {
	auto callback = crl::guard(this,
							   [=](bool allowed)
							   {
								   postEvent("emoji_status_access_requested",
											 allowed ? "{ status: \"allowed\" }" : "{ status: \"cancelled\" }");
							   });
...
void Panel::openPopup(const QJsonObject &args) {
	...
	if (weak) {
		postEvent("popup_closed", result.id ? QJsonObject{{u"button_id"_q, *result.id}} : EventData());
	}
}
...
void Panel::replyRequestWriteAccess(bool allowed) {
	postEvent("write_access_requested", QJsonObject{{u"status"_q, allowed ? u"allowed"_q : u"cancelled"_q}});
}
...
void Panel::replyRequestPhone(bool shared) {
	postEvent("phone_requested", QJsonObject{{u"status"_q, shared ? u"sent"_q : u"cancelled"_q}});
}
...
void Panel::invokeCustomMethod(const QJsonObject &args) {
	...
	const auto finish = [=](QJsonObject response) { replyCustomMethod(requestId, std::move(response)); };
	auto callback = crl::guard(
		this,
		[=](CustomMethodResult result)
		{
			if (result) {
				auto error = QJsonParseError();
				const auto parsed = QJsonDocument::fromJson("{ \"result\": " + *result + '}', &error);
				if (error.error != QJsonParseError::NoError || !parsed.isObject() || parsed.object().size() != 1) {
	...
	const auto params = QJsonDocument(args["params"].toObject()).toJson(QJsonDocument::Compact);
...
void Panel::processButtonMessage(std::unique_ptr<Button> &button, const QJsonObject &args) {
	if (args.isEmpty()) {
		_delegate->botClose();
		return;
	}

	const auto shown = [&] { return button && !button->isHidden(); };
	const auto wasShown = shown();
	const auto guard = gsl::finally(
		[&]
		{
			if (shown() != wasShown) {
				crl::on_main(this, [=] { sendViewport(); });
			}
		});
...
void Panel::processBackButtonMessage(const QJsonObject &args) { _widget->setBackAllowed(args["is_visible"].toBool()); }

void Panel::processSettingsButtonMessage(const QJsonObject &args) { _hasSettingsButton = args["is_visible"].toBool(); }
...
void Panel::processHeaderColor(const QJsonObject &args) {
	...
	} else if (const auto color = LookupNamedColor(args["color_key"].toString())) {
		_widget->overrideTitleColor((*color)->c);
		_headerColorLifetime =
			style::PaletteChanged() | rpl::start_with_next([=] { _widget->overrideTitleColor((*color)->c); });
	} else {
...
void Panel::overrideBodyColor(std::optional<QColor> color) {
	...
	const auto luminance = 0.2126 * color->redF() + 0.7152 * color->greenF() + 0.0722 * color->blueF();
	const auto textColor = (luminance > 0.5) ? QColor(0, 0, 0) : QColor(255, 255, 255);
	const auto textLuminance = (luminance > 0.5) ? 0 : 1;
	const auto adaptiveOpacity = (luminance - textLuminance + contrast) / contrast;
...
void Panel::processBackgroundColor(const QJsonObject &args) {
	...
	if (const auto color = ParseColor(args["color"].toString())) {
		...
		_bodyColorLifetime = style::PaletteChanged() | rpl::start_with_next([=] { overrideBodyColor((*color)->c); });
	} else {
...
void Panel::processDownloadRequest(const QJsonObject &args) {
	...
	const auto done = crl::guard(this,
								 [=](bool started)
								 {
									 postEvent("file_download_requested",
											   started ? "{ status: \"downloading\" }" : "{ status: \"cancelled\" }");
								 });
...
void Panel::createButton(std::unique_ptr<Button> &button) {
		...
		raw->paintRequest() |
			rpl::start_with_next(
				[=]
				{
					...
					p.drawRoundedRect(
						raw->rect().marginsAdded({0, 2 * st::callRadius, 0, 0}), st::callRadius, st::callRadius);
				},
				raw->lifetime());
	}
	button = std::make_unique<Button>(_bottomButtonsBg.get(), st::botWebViewBottomButton);
	...
	rpl::combine(raw->shownValue(), raw->heightValue()) |
		rpl::start_with_next([=] { layoutButtons(); }, raw->lifetime());
}
...
void Panel::layoutButtons() {
	...
	const auto inner = _widget->innerGeometry();
	const auto shown = [](std::unique_ptr<Button> &button) { return button && !button->isHidden(); };
	const auto any = shown(_mainButton) || shown(_secondaryButton);
	_webviewBottom->setVisible(!any && !_fullscreen.current() && !_layerShown);
	if (any) {
		_bottomButtonsBg->setVisible(!_layerShown);

		const auto one = shown(_mainButton) ? _mainButton.get() : _secondaryButton.get();
		const auto both = shown(_mainButton) && shown(_secondaryButton);
		const auto vertical =
			both && ((_secondaryPosition == RectPart::Top) || (_secondaryPosition == RectPart::Bottom));
		const auto padding = st::botWebViewBottomPadding;
		const auto height = padding.top() +
			(vertical ? (_mainButton->height() + st::botWebViewBottomSkip.y() + _secondaryButton->height())
					  : one->height()) +
			padding.bottom();
		_bottomButtonsBg->setGeometry(inner.x(), inner.y() + inner.height() - height, inner.width(), height);
		...
		if (!both) {
			...
			_secondaryButton->move(left, bottom);
			_mainButton->resizeToWidth(available);
			_mainButton->move(inner.width() - padding.right() - available, bottom);
		} else {
			available = (available - st::botWebViewBottomSkip.x()) / 2;
			_mainButton->resizeToWidth(available);
			bottom -= _mainButton->height();
			_mainButton->move(left, bottom);
			_secondaryButton->resizeToWidth(available);
			_secondaryButton->move(inner.width() - padding.right() - available, bottom);
		}
	} else if (_bottomButtonsBg) {
		_bottomButtonsBg->hide();
	}
	const auto footer = _layerShown ? 0
		: any						? _bottomButtonsBg->height()
		: _fullscreen.current()		? 0
									: _webviewBottom->height();
	_widget->setBottomBarHeight((!_layerShown && any) ? footer : 0);
	_footerHeight = footer;
}

void Panel::showBox(object_ptr<BoxContent> box) { showBox(std::move(box), LayerOption::KeepOther, anim::type::normal); }

void Panel::showBox(object_ptr<BoxContent> box, LayerOptions options, anim::type animated) {
	if (const auto widget = _webview ? _webview->window.widget() : nullptr) {
		_layerShown = true;
		const auto hideNow = !widget->isHidden();
		const auto raw = box.data();
		_webview->boxes.push_back(raw);
		raw->boxClosing() | rpl::filter([=] { return _webview != nullptr; }) |
			rpl::start_with_next(
				[=]
				{
					auto &list = _webview->boxes;
					list.erase(ranges::remove_if(list, [&](QPointer<RpWidget> b) { return !b || (b == raw); }),
							   end(list));
					if (list.empty()) {
	...
	_widget->showBox(std::move(box), LayerOption::KeepOther, anim::type::normal);
}

void Panel::showToast(TextWithEntities &&text) { _widget->showToast(std::move(text)); }

not_null<QWidget *> Panel::toastParent() const { return _widget->uiShow()->toastParent(); }

void Panel::hideLayer(anim::type animated) { _widget->hideLayer(animated); }
...
void Panel::showCriticalError(const TextWithEntities &text) {
	...
	auto error = base::make_unique_q<PaddingWrap<FlatLabel>>(
		_widget.get(),
		object_ptr<FlatLabel>(_widget.get(), rpl::single(text), st::paymentsCriticalError),
		st::paymentsCriticalErrorPadding);
	error->entity()->setClickHandlerFilter(
		[=](const ClickHandlerPtr &handler, Qt::MouseButton)
		{
...
void Panel::updateThemeParams(const Webview::ThemeParams &params) {
	...
	_webview->window.updateTheme(
		params.bodyBg, params.scrollBg, params.scrollBgOver, params.scrollBarBg, params.scrollBarBgOver);
...
void Panel::postEvent(const QString &event) { postEvent(event, {}); }

void Panel::postEvent(const QString &event, EventData data) {
	if (!_webview) {
		LOG(("BotWebView Error: Post event \"%1\" on crashed webview.").arg(event));
		return;
	}
	auto written = v::is<QString>(data) ? v::get<QString>(data).toUtf8()
										: QJsonDocument(v::get<QJsonObject>(data)).toJson(QJsonDocument::Compact);
	_webview->window.eval(R"(
if (window.TelegramGameProxy) {
	window.TelegramGameProxy.receiveEvent(
		")" + event.toUtf8() +
						  '"' + (written.isEmpty() ? QByteArray() : ", " + written) + R"();
}
)");
}
	...
	switch (info.error) {
		case Error::NoWebview2:
			return tr::lng_payments_webview_install_edge(
				tr::now,
				lt_link,
				Text::Link("Microsoft Edge WebView2 Runtime", "https://go.microsoft.com/fwlink/p/?LinkId=2124703"),
				Ui::Text::WithEntities);
		case Error::NoWebKitGTK: return {tr::lng_payments_webview_install_webkit(tr::now)};
		case Error::NoOpenGL: return {tr::lng_payments_webview_enable_opengl(tr::now)};
		case Error::NonX11: return {tr::lng_payments_webview_switch_x11(tr::now)};
		case Error::OldWindows: return {tr::lng_payments_webview_update_windows(tr::now)};
		default: return {QString::fromStdString(info.details)};
	}
}

void Panel::showWebviewError(const QString &text, const Webview::Available &information) {
	showCriticalError(TextWithEntities{text}.append("\n\n").append(ErrorText(information)));
}

rpl::lifetime &Panel::lifetime() { return _widget->lifetime(); }

std::unique_ptr<Panel> Show(Args &&args) { return std::make_unique<Panel>(std::move(args)); }
