...
[[nodiscard]] crl::time RequestUpdatesEach(not_null<Session *> owner) {
	...
	return appConfig->get<int>(u"chatlist_update_period"_q, 3600) * crl::time(1000);
}
...
TextWithEntities ForceCustomEmojiStatic(TextWithEntities text) {
	for (auto &entity : text.entities) {
		if (entity.type() == EntityType::CustomEmoji) {
			entity = EntityInText(
				EntityType::CustomEmoji, entity.offset(), entity.length(), u"force-static:"_q + entity.data());
		}
	}
	return text;
}

ChatFilter::ChatFilter(FilterId id,
	...
	: _id(id), _title(std::move(title.text)), _iconEmoji(std::move(iconEmoji)), _colorIndex(colorIndex),
	  _always(std::move(always)), _pinned(std::move(pinned)), _never(std::move(never)),
	  _flags(title.isStatic ? (flags | Flag::StaticTitle) : (flags & ~Flag::StaticTitle)) {}

ChatFilter ChatFilter::FromTL(const MTPDialogFilter &data, not_null<Session *> owner) {
	return data.match(
		[&](const MTPDdialogFilter &data)
		{
			const auto flags = (data.is_contacts() ? Flag::Contacts : Flag(0)) |
				(data.is_non_contacts() ? Flag::NonContacts : Flag(0)) | (data.is_groups() ? Flag::Groups : Flag(0)) |
				(data.is_broadcasts() ? Flag::Channels : Flag(0)) | (data.is_bots() ? Flag::Bots : Flag(0)) |
			...
			auto &&to_histories = ranges::views::transform(
				...
				ranges::views::filter([](History *history) { return history != nullptr; }) |
				ranges::views::transform([](History *history) { return not_null<History *>(history); });
			auto &&always = ranges::views::concat(data.vinclude_peers().v) | to_histories;
			auto pinned = ranges::views::all(data.vpinned_peers().v) | to_histories | ranges::to_vector;
			auto &&never = ranges::views::all(data.vexclude_peers().v) | to_histories;
			auto &&all = ranges::views::concat(always, pinned);
			auto list = base::flat_set<not_null<History *>>{all.begin(), all.end()};
			return ChatFilter(data.vid().v,
							  {
								  Api::ParseTextWithEntities(&owner->session(), data.vtitle()),
								  data.is_title_noanimate(),
							  },
							  qs(data.vemoticon().value_or_empty()),
							  data.vcolor() ? std::make_optional(data.vcolor()->v) : std::nullopt,
							  flags,
							  std::move(list),
							  std::move(pinned),
							  {never.begin(), never.end()});
		},
		[](const MTPDdialogFilterDefault &) { return ChatFilter(); },
		[&](const MTPDdialogFilterChatlist &data)
		{
				...
				ranges::views::transform(
					[&](const MTPInputPeer &data)
					{
						const auto peer = data.match(
							...
							[&](const MTPDinputPeerChat &data) { return (PeerData *) owner->chat(data.vchat_id().v); },
							...
							[&](const MTPDinputPeerSelf &data) { return (PeerData *) owner->session().user(); },
							[&](const auto &data) { return (PeerData *) nullptr; });
						return peer ? owner->history(peer).get() : nullptr;
					}) |
				ranges::views::filter([](History *history) { return history != nullptr; }) |
				ranges::views::transform([](History *history) { return not_null<History *>(history); });
			...
			return ChatFilter(data.vid().v,
							  ...
							  qs(data.vemoticon().value_or_empty()),
							  data.vcolor() ? std::make_optional(data.vcolor()->v) : std::nullopt,
							  (Flag::Chatlist | (data.is_has_my_invites() ? Flag::HasMyLinks : Flag()) |
							   (data.is_title_noanimate() ? Flag::StaticTitle : Flag(0))),
							  std::move(list),
							  std::move(pinned),
							  {});
...
MTPDialogFilter ChatFilter::tl(FilterId replaceId) const {
	...
	auto title = MTP_textWithEntities(MTP_string(_title.text),
									  Api::EntitiesToMTP(nullptr, _title.entities, Api::ConvertOption::SkipLocal));
	if (_flags & Flag::Chatlist) {
		using TLFlag = MTPDdialogFilterChatlist::Flag;
		const auto flags = TLFlag::f_emoticon | (_colorIndex ? TLFlag::f_color : TLFlag(0)) |
			(staticTitle() ? TLFlag::f_title_noanimate : TLFlag(0));
		return MTP_dialogFilterChatlist(MTP_flags(flags),
...
FilterId ChatFilter::id() const { return _id; }

const TextWithEntities &ChatFilter::titleText() const { return _title; }

ChatFilterTitle ChatFilter::title() const { return {_title, !!(_flags & Flag::StaticTitle)}; }

QString ChatFilter::iconEmoji() const { return _iconEmoji; }

std::optional<uint8> ChatFilter::colorIndex() const { return _colorIndex; }

ChatFilter::Flags ChatFilter::flags() const { return _flags; }

bool ChatFilter::staticTitle() const { return _flags & Flag::StaticTitle; }

bool ChatFilter::chatlist() const { return _flags & Flag::Chatlist; }

bool ChatFilter::hasMyLinks() const { return _flags & Flag::HasMyLinks; }

const base::flat_set<not_null<History *>> &ChatFilter::always() const { return _always; }

const std::vector<not_null<History *>> &ChatFilter::pinned() const { return _pinned; }

const base::flat_set<not_null<History *>> &ChatFilter::never() const { return _never; }

bool ChatFilter::contains(not_null<History *> history, bool ignoreFakeUnread) const {
	const auto flag = [&]
	{
		const auto peer = history->peer;
		if (const auto user = peer->asUser()) {
			return user->isBot() ? Flag::Bots : user->isContact() ? Flag::Contacts : Flag::NonContacts;
	...
	const auto state =
		(_flags & (Flag::NoMuted | Flag::NoRead)) ? history->chatListBadgesState() : Dialogs::BadgesState();
	return false ||
		((_flags & flag) &&
		 (!(_flags & Flag::NoMuted) || !history->muted() ||
		  (state.mention && history->folderKnown() && !history->folder())) &&
		 (!(_flags & Flag::NoRead) || state.unread || state.mention ||
		  (!ignoreFakeUnread && history->fakeUnreadWhileOpened())) &&
		 (!(_flags & Flag::NoArchived) || (history->folderKnown() && !history->folder()))) ||
		_always.contains(history);
}

ChatFilters::ChatFilters(not_null<Session *> owner)
	: _owner(owner), _moreChatsTimer([=] { checkLoadMoreChatsLists(); }) {
	_list.emplace_back();
...
not_null<Dialogs::MainList *> ChatFilters::chatsList(FilterId filterId) {
	...
	if (!pointer) {
		auto limit = rpl::single(rpl::empty_value()) | rpl::then(_owner->session().appConfig().refreshed()) |
			rpl::map([=] { return _owner->pinnedChatsLimit(filterId); });
		pointer = std::make_unique<Dialogs::MainList>(
			&_owner->session(), filterId, _owner->maxPinnedChatsLimitValue(filterId));
	}
	return pointer.get();
}
...
void ChatFilters::setPreloaded(const QVector<MTPDialogFilter> &result, bool tagsEnabled) {
	_loadRequestId = -1;
...
void ChatFilters::load() { load(false); }
...
bool ChatFilters::tagsEnabled() const { return _tagsEnabled.current(); }

rpl::producer<bool> ChatFilters::tagsEnabledValue() const { return _tagsEnabled.value(); }

void ChatFilters::requestToggleTags(bool value, Fn<void()> fail) {
	if (_toggleTagsRequestId) {
		return;
	}
	_toggleTagsRequestId = _owner->session()
							   .api()
							   .request(MTPmessages_ToggleDialogFilterTags(MTP_bool(value)))
							   .done(
...
void ChatFilters::apply(const MTPUpdate &update) {
	...
	update.match(
		[&](const MTPDupdateDialogFilter &data)
		{
		...
		[&](const MTPDupdateDialogFilters &data) { load(true); },
		...
		[](auto &&) { Unexpected("Update in ChatFilters::apply."); });
}

ChatFilterLink ChatFilters::add(FilterId id, const MTPExportedChatlistInvite &update) {
	...
	auto chats = data.vpeers().v |
		ranges::views::transform([&](const MTPPeer &peer) { return _owner->history(peerFromMTP(peer)); }) |
		ranges::to_vector;
...
void ChatFilters::edit(FilterId id, const QString &url, const QString &title) {
	auto &links = _chatlistLinks[id];
	...
	if (i != end(links)) {
		...
		_owner->session()
			.api()
			.request(MTPchatlists_EditExportedInvite(MTP_flags(MTPchatlists_EditExportedInvite::Flag::f_title),
			...
			.done(
				[=](const MTPExportedChatlistInvite &result)
				{
...
void ChatFilters::destroy(FilterId id, const QString &url) {
	...
	if (i != end(links)) {
		...
		_linksRequestId =
			api->request(MTPchatlists_DeleteExportedInvite(MTP_inputChatlistDialogFilter(MTP_int(id)), MTP_string(url)))
				.send();
	}
}

rpl::producer<std::vector<ChatFilterLink>> ChatFilters::chatlistLinks(FilterId id) const {
	return _chatlistLinksUpdated.events_starting_with_copy(id) | rpl::filter(rpl::mappers::_1 == id) |
		rpl::map(
			   [=]
			   {
				   const auto i = _chatlistLinks.find(id);
				   return (i != end(_chatlistLinks)) ? i->second : std::vector<ChatFilterLink>();
			   });
}
...
void ChatFilters::reloadChatlistLinks(FilterId id) {
	const auto api = &_owner->session().api();
	api->request(_linksRequestId).cancel();
	_linksRequestId = api->request(MTPchatlists_GetExportedInvites(MTP_inputChatlistDialogFilter(MTP_int(id))))
						  .done(
							  [=](const MTPchatlists_ExportedInvites &result)
							  {
								  const auto &data = result.data();
...
void ChatFilters::applyInsert(ChatFilter filter, int position) {
	...
	_list.insert(begin(_list) + position, ChatFilter(filter.id(), {}, {}, {}, {}, {}, {}, {}));
...
bool ChatFilters::applyChange(ChatFilter &filter, ChatFilter &&updated) {
	...
	const auto rulesChanged = exceptionsChanged || ((filter.flags() & rulesMask) != (updated.flags() & rulesMask)) ||
		(filter.never() != updated.never());
	const auto pinnedChanged = (filter.pinned() != updated.pinned());
	const auto chatlistChanged =
		(filter.chatlist() != updated.chatlist()) || (filter.hasMyLinks() != updated.hasMyLinks());
	const auto listUpdated = rulesChanged || pinnedChanged || (filter.titleText() != updated.titleText()) ||
		(filter.staticTitle() != updated.staticTitle()) || (filter.iconEmoji() != updated.iconEmoji());
	const auto colorChanged = filter.colorIndex() != updated.colorIndex();
	const auto colorExistenceChanged = (!filter.colorIndex()) != (!updated.colorIndex());
	...
	if (rulesChanged) {
		...
		{ return (!areTagsEnabled || entryToRefreshHeight) ? false : row->entry()->hasChatsFilterTags(0); };
		...
		if (exceptionsChanged && !filter.always().empty()) {
			...
			Ui::PostponeCall(&_owner->session(), [=] { _owner->session().api().requestMoreDialogsIfNeeded(); });
		}
	}
	if (pinnedChanged) {
		const auto filterList = _owner->chatsFilters().chatsList(id);
...
bool ChatFilters::applyOrder(const QVector<MTPint> &order) {
	...
	auto indices = ranges::views::all(_list) | ranges::views::transform(&ChatFilter::id) | ranges::to_vector;
...
const ChatFilter &ChatFilters::applyUpdatedPinned(FilterId id, const std::vector<Dialogs::Key> &dialogs) {
	const auto i = ranges::find(_list, id, &ChatFilter::id);
	Assert(i != end(_list));
	...
	set(ChatFilter(
		id, i->title(), i->iconEmoji(), i->colorIndex(), i->flags(), std::move(always), std::move(pinned), i->never()));
	return *i;
}

void ChatFilters::saveOrder(const std::vector<FilterId> &order, mtpRequestId after) {
	if (after) {
	...
	_saveOrderRequestId =
		api->request(MTPmessages_UpdateDialogFiltersOrder(wrapped)).afterRequest(_saveOrderAfterId).send();
}
...
const std::vector<ChatFilter> &ChatFilters::list() const { return _list; }

FilterId ChatFilters::defaultId() const { return lookupId(0); }

FilterId ChatFilters::lookupId(int index) const {
	// Expects(index >= 0 && index < _list.size());
	if (!(index >= 0 && index < _list.size())) {
		return FilterId(); // ViGram: fix crash when using `hideAllChatsFolder`
	}
	...
	return !index ? FilterId() : (index <= int(i - begin(_list))) ? _list[index - 1].id() : _list[index].id();
}

bool ChatFilters::loaded() const { return _loaded; }

bool ChatFilters::has() const { return _list.size() > 1; }

rpl::producer<> ChatFilters::changed() const { return _listChanged.events(); }

rpl::producer<FilterId> ChatFilters::isChatlistChanged() const { return _isChatlistChanged.events(); }

rpl::producer<TagColorChanged> ChatFilters::tagColorChanged() const { return _tagColorChanged.events(); }

bool ChatFilters::loadNextExceptions(bool chatsListLoaded) {
	if (_exceptionsLoadRequestId) {
		return true;
	} else if (!chatsListLoaded && (_owner->chatsList()->fullSize().current() < kLoadExceptionsAfter)) {
		return false;
	}
		...
		if (i != end(_list)) {
			...
			for (const auto &history : i->always()) {
				if (!history->folderKnown()) {
					inputs.push_back(MTP_inputDialogPeer(history->peer->input));
				}
			}
		}
		return result;
	...
	const auto api = &_owner->session().api();
	_exceptionsLoadRequestId = api->request(MTPmessages_GetPeerDialogs(MTP_vector(inputs)))
								   .done(
...
void ChatFilters::requestSuggested() {
	...
	if (_suggestedLastReceived > 0 && crl::now() - _suggestedLastReceived < kRefreshSuggestedTimeout) {
		return;
	}
	const auto api = &_owner->session().api();
	_suggestedRequestId = api->request(MTPmessages_GetSuggestedDialogFilters())
							  .done(
								  [=](const MTPVector<MTPDialogFilterSuggested> &data)
								  {
									  _suggestedRequestId = 0;
									  _suggestedLastReceived = crl::now();

									  _suggested = ranges::views::all(data.v) |
										  ranges::views::transform(
													   [&](const MTPDialogFilterSuggested &f)
													   {
														   return f.match(
															   [&](const MTPDdialogFilterSuggested &data)
															   {
...
bool ChatFilters::suggestedLoaded() const { return (_suggestedLastReceived > 0); }

const std::vector<SuggestedFilter> &ChatFilters::suggestedFilters() const { return _suggested; }

rpl::producer<> ChatFilters::suggestedUpdated() const { return _suggestedUpdated.events(); }

rpl::producer<Ui::MoreChatsBarContent> ChatFilters::moreChatsContent(FilterId id) {
	if (!id) {
	...
	return [=](auto consumer)
	{
		...
		_moreChatsUpdated.events_starting_with_copy(id) |
			rpl::start_with_next(
				[=]
				{
...
const std::vector<not_null<PeerData *>> &ChatFilters::moreChats(FilterId id) const {
...
void ChatFilters::moreChatsHide(FilterId id, bool localOnly) {
	if (!localOnly) {
		const auto api = &_owner->session().api();
		api->request(MTPchatlists_HideChatlistUpdates(MTP_inputChatlistDialogFilter(MTP_int(id)))).send();
	}
...
void ChatFilters::loadMoreChatsList(FilterId id) {
	...
	auto &api = _owner->session().api();
	entry.requestId =
		api.request(MTPchatlists_GetChatlistUpdates(MTP_inputChatlistDialogFilter(MTP_int(id))))
			.done(
				[=](const MTPchatlists_ChatlistUpdates &result)
				{
					const auto &data = result.data();
					_owner->processUsers(data.vusers());
					_owner->processChats(data.vchats());
					auto list = ranges::views::all(data.vmissing_peers().v) |
						ranges::views::transform([&](const MTPPeer &peer) { return _owner->peer(peerFromMTP(peer)); }) |
						ranges::to_vector;
...
bool CanRemoveFromChatFilter(const ChatFilter &filter, not_null<History *> history) {
	using Flag = ChatFilter::Flag;
	const auto flagsWithoutNoReadNoArchivedNoMuted =
		filter.flags() & ~(Flag::NoRead | Flag::NoArchived | Flag::NoMuted);
	return (filter.always().size() > 1 || flagsWithoutNoReadNoArchivedNoMuted) && filter.contains(history);
}
