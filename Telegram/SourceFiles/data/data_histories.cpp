...
MTPInputReplyTo ReplyToForMTP(not_null<History *> history, FullReplyTo replyTo) {
	const auto owner = &history->owner();
	if (replyTo.storyId) {
		if (const auto peer = owner->peerLoaded(replyTo.storyId.peer)) {
			return MTP_inputReplyToStory(peer->input, MTP_int(replyTo.storyId.story));
		}
	} else if (replyTo.messageId || replyTo.topicRootId) {
		const auto to = LookupReplyTo(history, replyTo.messageId);
		const auto replyingToTopic = replyTo.topicRootId ? history->peer->forumTopicFor(replyTo.topicRootId) : nullptr;
		const auto replyingToTopicId = replyTo.topicRootId
			? (replyingToTopic ? replyingToTopic->rootId() : Data::ForumTopic::kGeneralId)
			: (to ? to->topicRootId() : Data::ForumTopic::kGeneralId);
		const auto replyToTopicId =
			(to && (to->history() != history || to->id != replyingToTopicId)) ? to->topicRootId() : replyingToTopicId;
		const auto external =
			replyTo.messageId && (replyTo.messageId.peer != history->peer->id || replyingToTopicId != replyToTopicId);
		const auto quoteEntities =
			Api::EntitiesToMTP(&history->session(), replyTo.quote.entities, Api::ConvertOption::SkipLocal);
		...
		return MTP_inputReplyToMessage(
			MTP_flags((replyTo.topicRootId ? Flag::f_top_msg_id : Flag()) |
					  ...
					  (quoteEntities.v.isEmpty() ? Flag() : Flag::f_quote_entities)),
			MTP_int(replyTo.messageId ? replyTo.messageId.msg : 0),
			MTP_int(replyTo.topicRootId),
			(external ? owner->peer(replyTo.messageId.peer)->input : MTPInputPeer()),
...
MTPInputMedia WebPageForMTP(const Data::WebPageDraft &draft, bool required) {
	using Flag = MTPDinputMediaWebPage::Flag;
	return MTP_inputMediaWebPage(MTP_flags(((false && required) ? Flag() : Flag::f_optional) |
...
Histories::Histories(not_null<Session *> owner) : _owner(owner), _readRequestsTimer([=] { sendReadRequests(); }) {}

Session &Histories::owner() const { return *_owner; }

Main::Session &Histories::session() const { return _owner->session(); }
...
not_null<History *> Histories::findOrCreate(PeerId peerId) {
	...
	const auto &[i, ok] = _map.emplace(peerId, std::make_unique<History>(&owner(), peerId));
	return i->second.get();
}
...
void Histories::clearAll() { _map.clear(); }
...
void Histories::readInbox(not_null<History *> history) {
	...
	if (history->lastServerMessageKnown()) {
		...
		DEBUG_LOG(("Reading: last known, reading till %1.").arg(last ? last->id.bare : 0));
		readInboxTill(history, last ? last->id : 0);
		return;
	} else if (history->loadedAtBottom()) {
		if (const auto lastId = history->maxMsgId()) {
			DEBUG_LOG(("Reading: loaded at bottom, maxMsgId %1.").arg(lastId.bare));
	...
	requestDialogEntry(history,
					   [=]
					   {
						   ...
						   const auto last = history->lastServerMessage();
						   DEBUG_LOG(("Reading: got entry, reading till %1.").arg(last ? last->id.bare : 0));
						   readInboxTill(history, last ? last->id : 0);
					   });
}
...
void Histories::readInboxTill(not_null<History *> history, MsgId tillId) { readInboxTill(history, tillId, false); }

void Histories::readInboxTill(not_null<History *> history, MsgId tillId, bool force) {
	Expects(IsServerMsgId(tillId) || (!tillId && !force));

	DEBUG_LOG(("Reading: readInboxTill %1, force %2.").arg(tillId.bare).arg(Logs::b(force)));

	const auto syncGuard = gsl::finally(
		[&]
		{
			DEBUG_LOG(("Reading: in guard, unread %1.").arg(history->unreadCount()));
			if (history->unreadCount() > 0) {
				if (const auto last = history->lastServerMessage()) {
					DEBUG_LOG(("Reading: checking last %1 and %2.").arg(last->id.bare).arg(tillId.bare));
					if (last->id == tillId) {
	...
	if (maybeState && maybeState->sentReadTill >= tillId) {
		DEBUG_LOG(("Reading: readInboxTill finish 3 with %1.").arg(maybeState->sentReadTill.bare));
		return;
		...
		return;
	} else if (!needsRequest && (!maybeState || !maybeState->willReadTill)) {
		return;
	}
	const auto stillUnread = history->countStillUnreadLocal(tillId);
	if (!force && stillUnread && history->unreadCountKnown() && *stillUnread == history->unreadCount()) {
		DEBUG_LOG(("Reading: count didn't change so just update till %1").arg(tillId.bare));
	...
	if (force || !stillUnread || !*stillUnread) {
		DEBUG_LOG(("Reading: will read till %1 with still unread %2").arg(tillId.bare).arg(stillUnread.value_or(-666)));
		...
		DEBUG_LOG(("Reading: will read till %1 with postponed").arg(tillId.bare));
		...
		DEBUG_LOG(("Reading: will read till %1 postponed already").arg(tillId.bare));
	}
	DEBUG_LOG(("Reading: marking now with till %1 and still %2").arg(tillId.bare).arg(*stillUnread));
	history->setInboxReadTill(tillId);
...
void Histories::requestDialogEntry(not_null<Data::Folder *> folder) {
	...
	auto peers = QVector<MTPInputDialogPeer>(1, MTP_inputDialogPeerFolder(MTP_int(folder->id())));
	session()
		.api()
		.request(MTPmessages_GetPeerDialogs(MTP_vector(std::move(peers))))
		.done(
		...
		.fail([=] { _dialogFolderRequests.remove(folder); })
		.send();
}

void Histories::requestDialogEntry(not_null<History *> history, Fn<void()> callback) {
...
void Histories::postponeRequestDialogEntries() {
	...
	Core::App().postponeCall(crl::guard(&session(), [=] { sendDialogRequests(); }));
}
...
void Histories::sendDialogRequests() {
	...
	const auto histories = ranges::views::all(_dialogRequestsPending) |
	...
	const auto dialogPeer = [](not_null<History *> history) { return MTP_inputDialogPeer(history->peer->input); };
	ranges::transform(histories, ranges::back_inserter(peers), dialogPeer);
...
void Histories::changeDialogUnreadMark(not_null<History *> history, bool unread) {
	history->setUnreadMark(unread);

	using Flag = MTPmessages_MarkDialogUnread::Flag;
	session()
		.api()
		.request(MTPmessages_MarkDialogUnread(MTP_flags(unread ? Flag::f_unread : Flag(0)),
											  MTP_inputDialogPeer(history->peer->input)))
		.send();
}

void Histories::requestFakeChatListMessage(not_null<History *> history) {
						...
						.fail(
							[=]
							{
								...
								history->setFakeChatListMessageFrom(MTP_messages_messages(
									MTP_vector<MTPMessage>(0), MTP_vector<MTPChat>(0), MTP_vector<MTPUser>(0)));
								finish();
							})
						.send();
				});
...
void Histories::requestGroupAround(not_null<HistoryItem *> item) {
					...
					[=](Fn<void()> finish)
					{
						return session()
							...
							.done(
								[=](const MTPmessages_Messages &result)
								{
									_owner->processExistingMessages(history->peer->asChannel(), result);
									_chatListGroupRequests.remove(key);
									history->migrateToOrMe()->applyChatListGroup(history->peer->id, result);
									finish();
								})
							.fail(
								[=]
	...
	_chatListGroupRequests.emplace(key, ChatListGroupRequest{.aroundId = id, .requestId = requestId});
}
...
void Histories::reportDelivery(not_null<HistoryItem *> item) {
	...
	crl::on_main(&session(), [=] { reportPendingDeliveries(); });
}
...
void Histories::reportPendingDeliveries() {
	...
	for (auto i = begin(pending); i != end(pending);) {
		...
		session()
			.api()
			.request(MTPmessages_ReportMessagesDelivery(MTP_flags(0), peer->input, MTP_vector(std::move(list))))
			.done(finish)
			.fail(finish)
			.send();
	}
...
void Histories::sendReadRequests() {
	...
	// ViGram sendReadMessages
	const auto settings = &AyuSettings::getInstance();
	if (!settings->sendReadMessages) {
		DEBUG_LOG(("[ViGram] Don't read messages"));
		_states.clear();
		return;
	}
	...
	for (auto &[history, state] : _states) {
		if (!state.willReadTill) {
			...
			DEBUG_LOG(("Reading: sending with till %1.").arg(state.willReadTill.bare));
...
void Histories::sendReadRequest(not_null<History *> history, State &state) {
	...
	DEBUG_LOG(("Reading: sending request now with till %1.").arg(tillId.bare));
	sendRequest(history,
				RequestType::ReadInbox,
				[=](Fn<void()> finish)
				{
					DEBUG_LOG(("Reading: sending request invoked with till %1.").arg(tillId.bare));
					const auto finished = [=]
					{
						auto state = lookup(history);
						if (state == nullptr) {
							// don’t care + didn’t ask + cry about it + who asked + stay mad + get real + L + bleed +
							// mald seethe cope harder + dilate + incorrect + hoes mad + pound sand + basic skill issue
							// + typo + ratio + ur dad left + you fell off + no u + the audacity + triggered + repelled
							// + ur a minor + k. + any askers + get a life + ok and? + cringe + copium + go outside +
							// touch grass + kick rocks + quote tweet + think again + not based + not funny didn’t laugh
							// + social credits -999, 999, 999, 999 + get good + reported + ad hominem + ok boomer +
							// small pp + ur allergic to sunlight + GG! + get rekt + trolled + your loss + muted +
							// banned + kicked + permaban + useless + i slept with ur mom + yo momma + yo momma so fat +
							// redpilled + no bitches allowed + i said it better + tiktok fan + get a life +
							// unsubscribed + plundered + go tell reddit + donowalled + simp + get sticked bug LOL +
							// talk nonsense + trump supporter + your’re a full time discord mod + you’re* + grammar
							// issue + nerd + get clapped + kys + lorem ipsum dolor sit amet + go outside + bleach + lol
							// + gay + retard + autistic + reported + ask deez + ez clap + straight cash + idgaf + ratio
							// again + stay mad + read FAQ + youre lost + you “re” + stay pressed + reverse double take
							// back + pedophile + cancelled + done for + don't give a damn + get a job + sus + baka +
							// sussy baka + get blocked + mad free + freer than air + furry + rip bozo + you're a
							// (insert stereotype) + slight_smile + aired + cringe again + Super Idol的笑容 + mad cuz
							// bad + my pronouns are xe, xem & xyr + irrelevant + deal with it + screencapped your bio +
							// karen/kyle + jealous + you're deaf + balls + i'll be right back + go ahead whine about it
							// + not straight + eat paper + you lose + count to three + your problem + no one cares +
							// log off + don't care even more + sex offender + sex defender + get religion + not okay +
							// glhf + NFT owner + you make bad memes + problematic + fall in line + dog water + you look
							// like a wall + you don’t know 2 + 2 with yo head ass + you are going to my cringe
							// compilation + you can’t count to five + try again + you failed kindergarten + rickrolled
							// + no lifer + guten freunden schickt man einen deutschen panzer + you have a anime profile
							// picture + an* + fatherless + motherless + sisterless + brotherless + orphan + you can't
							// catch this ratio + catch some bitches + I don't care about your opinion + genshin player
							// + you dress like garbage + 日本語がお上手ですね + get fucked + you can’t understand what
							// the word intelligence means with your dumb ass + you have hair + queued + put some
							// thought into what you're going to do with that + stfu + go to bed + yes, i'm taller than
							// you + i think your joke is funny + i rejected your mother's advances + marooned + you
							// can’t read + I win + final ratio
					...
					if (const auto channel = history->peer->asChannel()) {
						return session()
							.api()
							.request(MTPchannels_ReadHistory(channel->inputChannel, MTP_int(tillId)))
							.done(finished)
							.fail(finished)
							.send();
					} else {
						return session()
							.api()
							.request(MTPmessages_ReadHistory(history->peer->input, MTP_int(tillId)))
							.done(
...
void Histories::checkEmptyState(not_null<History *> history) {
	...
	{
		return state.postponed.empty() && !state.postponedRequestEntry && state.sent.empty() &&
			(state.willReadTill == 0) && (state.sentReadTill == 0);
	};
...
bool Histories::postponeHistoryRequest(const State &state) const {
	const auto proj = [](const auto &pair) { return pair.second.type; };
...
bool Histories::postponeEntryRequest(const State &state) const {
	return ranges::any_of(state.sent, [](const auto &pair) { return pair.second.type != RequestType::History; });
}

void Histories::deleteMessages(not_null<History *> history, const QVector<MTPint> &ids, bool revoke) {
	sendRequest(history,
				RequestType::Delete,
				[=](Fn<void()> finish)
				{
					...
					if (const auto channel = history->peer->asChannel()) {
						...
						return session()
							.api()
							.request(MTPmessages_DeleteMessages(MTP_flags(revoke ? Flag::f_revoke : Flag(0)),
...
void Histories::deleteAllMessages(not_null<History *> history, MsgId deleteTillId, bool justClear, bool revoke) {
	sendRequest(
		history,
		RequestType::Delete,
		[=](Fn<void()> finish)
		{
			...
			if (!justClear && revoke && channel && channel->canDelete()) {
				return session()
					.api()
					.request(MTPchannels_DeleteChannel(channel->inputChannel))
					.done(
				...
				return session()
					.api()
					.request(MTPchannels_DeleteHistory(MTP_flags(revoke ? Flag::f_for_everyone : Flag(0)),
				...
				return session()
					.api()
					.request(MTPmessages_DeleteChat(chat->inputChat))
					.done(finish)
					.fail(
						[=](const MTP::Error &error)
						{
							if (error.type() == "PEER_ID_INVALID") {
								...
								session()
									.api()
									.request(MTPmessages_AddChatUser(chat->inputChat, MTP_inputUserSelf(), MTP_int(0)))
									.done(
										[=](const MTPmessages_InvitedUsers &result)
										{
											const auto &data = result.data();
											session().api().applyUpdates(data.vupdates());
											deleteAllMessages(history, deleteTillId, justClear, revoke);
										})
									.send();
							}
							finish();
				...
				const auto flags =
					Flag(0) | (justClear ? Flag::f_just_clear : Flag(0)) | (revoke ? Flag::f_revoke : Flag(0));
				return session()
					.api()
					.request(MTPmessages_DeleteHistory(MTP_flags(flags),
													   peer->input,
													   MTP_int(0),
													   MTPint(), // min_date
													   MTPint() // max_date
					...
					.done(
						...
						{
							const auto offset = session().api().applyAffectedHistory(peer, result);
							if (offset > 0) {
								deleteAllMessages(history, deleteTillId, justClear, revoke);
							}
							finish();
...
void Histories::deleteMessagesByDates(not_null<History *> history,
									  QDate firstDayToDelete,
									  QDate lastDayToDelete,
									  bool revoke) {
	const auto firstSecondToDelete = base::unixtime::serialize({firstDayToDelete, QTime(0, 0)});
	const auto lastSecondToDelete = base::unixtime::serialize({lastDayToDelete, QTime(23, 59, 59)});
	deleteMessagesByDates(history, firstSecondToDelete - 1, lastSecondToDelete + 1, revoke);
}

void Histories::deleteMessagesByDates(not_null<History *> history, TimeId minDate, TimeId maxDate, bool revoke) {
	sendRequest(history,
				RequestType::Delete,
				[=](Fn<void()> finish)
				{
					...
					using Flag = MTPmessages_DeleteHistory::Flag;
					const auto flags =
						Flag::f_just_clear | Flag::f_min_date | Flag::f_max_date | (revoke ? Flag::f_revoke : Flag(0));
					return session()
						.api()
						.request(MTPmessages_DeleteHistory(
							MTP_flags(flags), peer->input, MTP_int(0), MTP_int(minDate), MTP_int(maxDate)))
						.done(
							[=](const MTPmessages_AffectedHistory &result)
							{
								const auto offset = session().api().applyAffectedHistory(peer, result);
...
void Histories::deleteMessages(const MessageIdsList &ids, bool revoke) {
	...
	for (const auto &itemId : ids) {
		if (const auto item = _owner->message(itemId)) {
			const auto history = item->history();
			if (item->isScheduled()) {
				const auto wasOnServer = !item->isSending() && !item->hasFailed();
				...
				const auto wasOnServer = !item->isSending() && !item->hasFailed();
				if (wasOnServer) {
					quickIdsByShortcut[item->shortcutId()].push_back(
						MTP_int(_owner->shortcutMessages().lookupId(item)));
				} else {
	...
	for (const auto &[peer, ids] : scheduledIdsByPeer) {
		peer->session()
			.api()
			.request(MTPmessages_DeleteScheduledMessages(peer->input, MTP_vector<MTPint>(ids)))
			.done([peer = peer](const MTPUpdates &result) { peer->session().api().applyUpdates(result); })
			.send();
	}
	for (const auto &[shortcutId, ids] : quickIdsByShortcut) {
		const auto api = &_owner->session().api();
		api->request(MTPmessages_DeleteQuickReplyMessages(MTP_int(shortcutId), MTP_vector<MTPint>(ids)))
			.done([=](const MTPUpdates &result) { api->applyUpdates(result); })
			.send();
	}
...
int Histories::sendRequest(not_null<History *> history,
						   RequestType type,
	...
	if (type == RequestType::History && postponeHistoryRequest(state)) {
		state.postponed.emplace(id, PostponedHistoryRequest{std::move(generator)});
		return id;
	}
	const auto requestId = generator([=] { checkPostponed(history, id); });
	state.sent.emplace(id, SentRequest{std::move(generator), requestId, type});
	if (!state.postponedRequestEntry && postponeEntryRequest(state) && _dialogRequests.contains(history)) {
		state.postponedRequestEntry = true;
	}
	if (postponeHistoryRequest(state)) {
		const auto resendHistoryRequest = [&](auto &pair)
		{
			...
			state.postponed.emplace(id, PostponedHistoryRequest{std::move(sent.generator)});
			session().api().request(sent.id).cancel();
			return true;
		};
		state.sent.erase(ranges::remove_if(state.sent, resendHistoryRequest), end(state.sent));
	}
	return id;
}

void Histories::sendCreateTopicRequest(not_null<History *> history, MsgId rootId) {
	...
	session().data().registerMessageRandomId(randomId, {history->peer->id, rootId});
	const auto api = &session().api();
	using Flag = MTPchannels_CreateForumTopic::Flag;
	api->request(MTPchannels_CreateForumTopic(
					 MTP_flags(Flag::f_icon_color | (topic->iconId() ? Flag::f_icon_emoji_id : Flag(0))),
		...
		.done([=](const MTPUpdates &result) { api->applyUpdates(result, randomId); })
		.fail([=](const MTP::Error &error) { api->sendMessageFail(error, history->peer, randomId); })
		.send();
}

bool Histories::isCreatingTopic(not_null<History *> history, MsgId rootId) const {
	const auto forum = history->asForum();
	return forum && forum->creating(rootId);
}

int Histories::sendPreparedMessage(not_null<History *> history,
								   FullReplyTo replyTo,
								   uint64 randomId,
	...
	if (isCreatingTopic(history, replyTo.topicRootId)) {
		const auto id = ++_requestAutoincrement;
		const auto creatingId = FullMsgId(history->peer->id, replyTo.topicRootId);
	...
	return v::match(message(history, realReplyTo),
					[&](const auto &request)
					{
						const auto type = RequestType::Send;
						return sendRequest(history,
										   type,
										   [=](Fn<void()> finish)
										   {
											   ...
											   history->sendRequestId =
												   api->request(base::duplicate(request))
													   .done(
...
void Histories::checkTopicCreated(FullMsgId rootId, MsgId realRoot) {
	...
	if (i != end(_creatingTopics)) {
		...
		for (const auto &item : history->clientSideMessages()) {
			const auto replace = [&](MsgId nowId) { return (nowId == rootId.msg) ? realRoot : nowId; };
			if (item->topicRootId() == rootId.msg) {
				item->setReplyFields(replace(item->replyToId()), realRoot, true);
			}
		}
	}
}

FullMsgId Histories::convertTopicReplyToId(not_null<History *> history, FullMsgId replyToId) const {
	const auto id =
		(history->peer->id == replyToId.peer) ? convertTopicReplyToId(history, replyToId.msg) : replyToId.msg;
	return {replyToId.peer, id};
}

MsgId Histories::convertTopicReplyToId(not_null<History *> history, MsgId replyToId) const {
	if (!replyToId) {
		return {};
	}
...
void Histories::cancelDelayedByTopicRequest(int id) {
	for (auto &[rootId, messages] : _creatingTopics) {
		messages.erase(ranges::remove(messages, id, &DelayedByTopicMessage::requestId), end(messages));
	}
	_creatingTopicRequests.remove(id);
}

void Histories::finishSentRequest(not_null<History *> history, not_null<State *> state, int id) {
	...
	if (!state->postponed.empty() && !postponeHistoryRequest(*state)) {
		for (auto &[id, postponed] : base::take(state->postponed)) {
			const auto requestId = postponed.generator([=, id = id] { checkPostponed(history, id); });
			state->sent.emplace(id, SentRequest{std::move(postponed.generator), requestId, RequestType::History});
		}
	}
	if (state->postponedRequestEntry && !postponeEntryRequest(*state)) {
		const auto i = _dialogRequests.find(history);
		Assert(i != end(_dialogRequests));
		const auto &[j, ok] = _dialogRequestsPending.emplace(history, std::move(i->second));
		Assert(ok);
