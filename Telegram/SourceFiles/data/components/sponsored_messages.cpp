...
SponsoredMessages::SponsoredMessages(not_null<Main::Session *> session)
	: _session(session), _clearTimer([=] { clearOldRequests(); }) {}
...
void SponsoredMessages::clearOldRequests() {
	...
	while (true) {
		const auto i =
			ranges::find_if(_requests,
							[&](const auto &value)
							{
								const auto &request = value.second;
								return !request.requestId && (request.lastReceived + kRequestTimeLimit <= now);
							});
		if (i == end(_requests)) {
			break;
		}
...
SponsoredMessages::AppendResult SponsoredMessages::append(not_null<History *> history) {
	...
	auto &list = it->second;
	if (list.showedAll || !TooEarlyForRequest(list.received) || list.postsBetween) {
		return SponsoredMessages::AppendResult::None;
	}

	const auto entryIt = ranges::find_if(list.entries, [](const Entry &e) { return e.item == nullptr; });
	...
	entryIt->item.reset(history->addSponsoredMessage(
		entryIt->itemFullId.msg, entryIt->sponsored.from, entryIt->sponsored.textWithEntities));

	return SponsoredMessages::AppendResult::Appended;
}

void SponsoredMessages::inject(not_null<History *> history,
	...
	while (true) {
		const auto entryIt = ranges::find_if(list.entries, [](const auto &e) { return e.item == nullptr; });
		if (entryIt == end(list.entries)) {
			list.showedAll = true;
			return;
		}
		const auto lastView = (entryIt != begin(list.entries)) ? (entryIt - 1)->item->mainView()
			: (injectAfterMsgId == ShowAtUnreadMsgId)		   ? history->firstUnreadMessage()
															   : [&]
		{
			const auto message = history->peer->owner().message(history->peer->id, injectAfterMsgId);
		...
		auto blockIt = ranges::find(history->blocks, lastView->block(), &BlockPtr::get);
		if (blockIt == end(history->blocks)) {
			return;
		}
		const auto messages = [&]() -> const std::vector<ViewPtr> & { return (*blockIt)->messages; };
		auto lastViewIt = ranges::find(messages(), lastView, &ViewPtr::get);
		auto appendAtLeastToEnd = false;
		while ((summaryBetween < list.postsBetween) || (summaryHeight < betweenHeight)) {
			lastViewIt++;
			...
			summaryHeight += viewHeight ? viewHeight : (*lastViewIt)->resizeGetHeight(fallbackWidth);
		}
		// SponsoredMessages::Details can be requested within
		// the constructor of HistoryItem, so itemFullId is used as a key.
		entryIt->itemFullId = FullMsgId(history->peer->id, _session->data().nextLocalMessageId());
		if (appendAtLeastToEnd) {
			entryIt->item.reset(history->addSponsoredMessage(
				entryIt->itemFullId.msg, entryIt->sponsored.from, entryIt->sponsored.textWithEntities));
		} else {
			const auto makedMessage = history->makeMessage(entryIt->itemFullId.msg,
														   entryIt->sponsored.from,
														   entryIt->sponsored.textWithEntities,
														   (*lastViewIt)->data());
			entryIt->item.reset(makedMessage.get());
			history->addNewInTheMiddle(makedMessage.get(),
...
void SponsoredMessages::request(not_null<History *> history, Fn<void()> done) {
	...
	}
	{
		...
		if (it != end(_data)) {
			...
			const auto proj = [](const Entry &e) { return e.item != nullptr; };
			if (ranges::any_of(list.entries, proj)) {
				return;
			}
		}
	}
	request.requestId = _session->api()
							.request(MTPmessages_GetSponsoredMessages(history->peer->input))
							.done(
							...
							.fail([=] { _requests.remove(history); })
							.send();
}

void SponsoredMessages::parse(not_null<History *> history, const MTPmessages_sponsoredMessages &list) {
	auto &request = _requests[history];
	...
	list.match(
		[&](const MTPDmessages_sponsoredMessages &data)
		{
			...
			if (const auto postsBetween = data.vposts_between()) {
				...
				list.state = history->peer->isChannel() ? State::AppendToEnd : State::AppendToTopBar;
			}
		},
		[](const MTPDmessages_sponsoredMessagesEmpty &) {});
}

FullMsgId SponsoredMessages::fillTopBar(not_null<History *> history, not_null<Ui::RpWidget *> widget) {
	const auto it = _data.find(history);
	if (it != end(_data)) {
		auto &list = it->second;
		if (!list.entries.empty()) {
			...
			Ui::FillSponsoredMessageBar(
				widget, _session, fullId, entry.sponsored.from, entry.sponsored.textWithEntities);
			return fullId;
		}
	}
	return {};
...
rpl::producer<> SponsoredMessages::itemRemoved(const FullMsgId &fullId) {
	...
	auto &list = it->second;
	const auto entryIt = ranges::find_if(list.entries, [&](const Entry &e) { return e.itemFullId == fullId; });
	if (entryIt == end(list.entries)) {
		return rpl::never<>();
	}
	if (!entryIt->optionalDestructionNotifier) {
		entryIt->optionalDestructionNotifier = std::make_unique<rpl::lifetime>();
		entryIt->optionalDestructionNotifier->add([this, fullId] { _itemRemoved.fire_copy(fullId); });
	}
	return _itemRemoved.events() | rpl::filter(rpl::mappers::_1 == fullId) | rpl::to_empty;
}

void SponsoredMessages::append(not_null<History *> history, List &list, const MTPSponsoredMessage &message) {
			...
			data.vmedia()->match(
				[&](const MTPDmessageMediaPhoto &media)
				{
					if (const auto tlPhoto = media.vphoto()) {
						tlPhoto->match([&](const MTPDphoto &data) { mediaPhoto = history->owner().processPhoto(data); },
				...
				[&](const MTPDmessageMediaDocument &media)
				{
					if (const auto tlDocument = media.vdocument()) {
						tlDocument->match(
							...
							{
								const auto d = history->owner().processDocument(data, media.valt_documents());
								if (d->isVideoFile() || d->isSilentVideo() || d->isAnimation() || d->isGifv()) {
									mediaDocument = d;
								}
							},
	...
	const auto from = SponsoredFrom{
		...
		.photoId = data.vphoto() ? history->session().data().processPhoto(*data.vphoto())->id : PhotoId(0),
		.mediaPhotoId = (mediaPhoto ? mediaPhoto->id : 0),
		.mediaDocumentId = (mediaDocument ? mediaDocument->id : 0),
		.backgroundEmojiId =
			data.vcolor().has_value() ? data.vcolor()->data().vbackground_emoji_id().value_or_empty() : uint64(0),
		.colorIndex = uint8(data.vcolor().has_value() ? data.vcolor()->data().vcolor().value_or_empty() : 0),
		.isLinkInternal = !UrlRequiresConfirmation(qs(data.vurl())),
		.isRecommended = data.is_recommended(),
		.canReport = data.is_can_report(),
	};
	auto sponsorInfo = data.vsponsor_info()
		? tr::lng_sponsored_info_submenu(tr::now, lt_text, {.text = qs(*data.vsponsor_info())}, Ui::Text::RichLangValue)
		: TextWithEntities();
	auto additionalInfo = TextWithEntities::Simple(data.vadditional_info() ? qs(*data.vadditional_info()) : QString());
		...
		.textWithEntities =
			{
				...
				.entities = Api::EntitiesFromMTP(_session, data.ventities().value_or_empty()),
			},
		.history = history,
		.link = from.link,
	...
	const auto itemId = entry.itemFullId = FullMsgId(history->peer->id, _session->data().nextLocalMessageId());
	...
	if (mediaPhoto) {
		preload = std::make_unique<PhotoPreload>(mediaPhoto, fileOrigin, preloaded);
	} else if (mediaDocument && VideoPreload::Can(mediaDocument)) {
		preload = std::make_unique<VideoPreload>(mediaDocument, fileOrigin, preloaded);
	}
...
const SponsoredMessages::Entry *SponsoredMessages::find(const FullMsgId &fullId) const {
...
void SponsoredMessages::view(const FullMsgId &fullId) {
	...
	request.requestId =
		_session->api()
			.request(MTPmessages_ViewSponsoredMessage(entryPtr->item ? entryPtr->item->history()->peer->input
			...
			.fail([=] { _viewRequests.remove(randomId); })
			.send();
}

SponsoredMessages::Details SponsoredMessages::lookupDetails(const FullMsgId &fullId) const {
	const auto entryPtr = find(fullId);
	if (!entryPtr) {
		return {};
	}
	...
	auto info = (!data.sponsorInfo.text.isEmpty() && !data.additionalInfo.text.isEmpty())
		? InfoList{data.sponsorInfo, data.additionalInfo}
		: !data.sponsorInfo.text.isEmpty()	  ? InfoList{data.sponsorInfo}
		: !data.additionalInfo.text.isEmpty() ? InfoList{data.additionalInfo}
											  : InfoList{};
	return {
		.info = std::move(info),
		.link = data.link,
...
void SponsoredMessages::clicked(const FullMsgId &fullId, bool isMedia, bool isFullscreen) {
	const auto entryPtr = find(fullId);
	if (!entryPtr) {
		return;
	}
	...
	_session->api()
		.request(MTPmessages_ClickSponsoredMessage(
			MTP_flags(Flag(0) | (isMedia ? Flag::f_media : Flag(0)) | (isFullscreen ? Flag::f_fullscreen : Flag(0))),
			entryPtr->item ? entryPtr->item->history()->peer->input : _session->data().peer(fullId.peer)->input,
			MTP_bytes(randomId)))
		.send();
}
...
auto SponsoredMessages::createReportCallback(const FullMsgId &fullId)
	...
	{
#ifdef _DEBUG
		~State() { qDebug() << "SponsoredMessages Report ~State()."; }
#endif
	...
	return [=](Result::Id optionId, Fn<void(Result)> done)
	{
		...
		const auto erase = [=]
		{
			...
			if (it != end(_data)) {
				...
				const auto proj = [&](const Entry &e) { return e.itemFullId == fullId; };
		...
		state->requestId =
			_session->api()
				.request(MTPmessages_ReportSponsoredMessage(
					history->peer->input, MTP_bytes(entry->sponsored.randomId), MTP_bytes(optionId)))
				.done(
					[=](const MTPchannels_SponsoredMessageReportResult &result, mtpRequestId requestId)
					{
						...
						state->requestId = 0;
							...
							[](const TLAdsHidden &data) -> Result { return {.result = Result::FinalStep::Hidden}; },
...
SponsoredMessages::State SponsoredMessages::state(not_null<History *> history) const {
	const auto it = _data.find(history);
