...
[[nodiscard]] std::vector<ReactionId> ListFromMTP(const MTPDmessages_reactions &data) {
...
[[nodiscard]] std::vector<MyTagInfo> ListFromMTP(const MTPDmessages_savedReactionTags &data) {
...
[[nodiscard]] int SentReactionsLimit(not_null<HistoryItem *> item) {
	...
	return session->premium() ? config->get<int>("reactions_user_max_premium", 3)
							  : config->get<int>("reactions_user_max_default", 1);
}

[[nodiscard]] bool IsMyRecent(const MTPDmessagePeerReaction &data,
							  const ReactionId &id,
							  not_null<PeerData *> peer,
							  const base::flat_map<ReactionId, std::vector<RecentReaction>> &recent,
							  bool min) {
	if (peer->isSelf()) {
		return true;
	} else if (!min) {
	...
	const auto k = ranges::find(j->second, peer, &RecentReaction::peer);
	return (k != end(j->second)) && k->my;
}

[[nodiscard]] bool
IsMyTop(const MTPDmessageReactor &data, PeerData *peer, const std::vector<MessageReactionsTopPaid> &top, bool min) {
	if (peer && peer->isSelf()) {
		return true;
	} else if (!min) {
		return data.is_my();
...
[[nodiscard]] std::optional<PeerId> MaybeShownPeer(uint32 privacySet, PeerId shownPeer) {
	return privacySet ? shownPeer : std::optional<PeerId>();
}

[[nodiscard]] MTPPaidReactionPrivacy PaidReactionShownPeerToTL(not_null<Main::Session *> session,
															   std::optional<PeerId> shownPeer) {
	return !shownPeer							? MTPPaidReactionPrivacy()
		: !*shownPeer							? MTP_paidReactionPrivacyAnonymous()
		: (*shownPeer == session->userPeerId()) ? MTP_paidReactionPrivacyDefault()
												: MTP_paidReactionPrivacyPeer(session->data().peer(*shownPeer)->input);
}

} // namespace

PossibleItemReactionsRef LookupPossibleReactions(not_null<HistoryItem *> item, bool paidInFront) {
	if (!item->canReact()) {
		return {};
	}
	...
	if (const auto channel = peer->asChannel()) {
		if ((!channel->amCreator()) && (channel->adminRights() & ChatAdminRight::Anonymous) &&
	...
	if (item->reactionsAreTags()) {
		...
		for (const auto &reaction : all) {
			if (premiumPossible || ranges::contains(tags, reaction.id, &Reaction::id)) {
				if (added.emplace(reaction.id).second) {
					result.recent.push_back(&reaction);
				}
			}
		...
		add([&](const Reaction &reaction) { return ranges::contains(all, reaction.id, &MessageReaction::id); });
		for (const auto &reaction : all) {
		...
		if (allowed.paidEnabled && !added.contains(Data::ReactionId::Paid())) {
			result.recent.push_back(reactions->lookupPaid());
		}
	} else {
		result.recent.reserve((allowed.paidEnabled ? 1 : 0) +
							  ((allowed.type == AllowedReactionsType::Some) ? allowed.some.size() : full.size()));
		...
		add(
			[&](const Reaction &reaction)
			{
				...
				if (id.custom() && !premiumPossible) {
					return false;
				} else if ((allowed.type == AllowedReactionsType::Some) && !ranges::contains(allowed.some, id)) {
					return false;
				} else if (id.custom() && allowed.type == AllowedReactionsType::Default) {
					return false;
				}
				return true;
			});
		...
		result.customAllowed = (allowed.type == AllowedReactionsType::All) && premiumPossible;

		const auto favoriteId = reactions->favoriteId();
		if (favoriteId.custom() && result.customAllowed &&
...
PossibleItemReactions::PossibleItemReactions(const PossibleItemReactionsRef &other)
	: recent(other.recent | ranges::views::transform([](const auto &value) { return *value; }) | ranges::to_vector),
	  stickers(other.stickers | ranges::views::transform([](const auto &value) { return *value; }) | ranges::to_vector),
	  customAllowed(other.customAllowed), tags(other.tags) {}

Reactions::Reactions(not_null<Session *> owner)
	: _owner(owner), _topRefreshTimer([=] { refreshTop(); }), _repaintTimer([=] { repaintCollected(); }),
	  _sendPaidTimer([=] { sendPaid(); }) {
	refreshDefault();

	_myTags.emplace(nullptr);

	base::timer_each(kRefreshFullListEach) |
		rpl::start_with_next(
			[=]
			{
				refreshDefault();
	...
	_owner->session().changes().messageUpdates(MessageUpdate::Flag::Destroyed) |
		rpl::start_with_next(
			[=](const MessageUpdate &update)
			{
				...
				if (const auto i = _sendingPaid.find(item); i != end(_sendingPaid)) {
					_sendingPaid.erase(i);
					_owner->session().credits().invalidate();
					crl::on_main(&_owner->session(), [=] { sendPaid(); });
				}
			},
			_lifetime);
	...
	crl::on_main(&owner->session(),
				 [=]
				 {
					 ...
					 rpl::single(rpl::empty) | rpl::then(_owner->session().mtp().config().updates()) |
						 rpl::map(
							 [=]
							 {
						 ...
						 rpl::filter([=](const ReactionId &id) { return !_saveFaveRequestId; }) |
						 rpl::start_with_next([=](ReactionId &&id) { applyFavorite(id); }, _lifetime);
				 });
}

Reactions::~Reactions() = default;

Main::Session &Reactions::session() const { return _owner->session(); }

void Reactions::refreshTop() { requestTop(); }

void Reactions::refreshRecent() { requestRecent(); }
...
void Reactions::refreshRecentDelayed() {
	...
	base::call_delayed(kRecentRequestTimeout,
					   &_owner->session(),
					   [=]
					   {
...
void Reactions::refreshDefault() { requestDefault(); }

void Reactions::refreshMyTags(SavedSublist *sublist) { requestMyTags(sublist); }
...
void Reactions::refreshMyTagsDelayed() {
	...
	base::call_delayed(kMyTagsRequestTimeout,
					   &_owner->session(),
					   [=]
					   {
...
void Reactions::refreshTags() { requestTags(); }
...
const std::vector<Reaction> &Reactions::list(Type type) const {
	switch (type) {
		...
		case Type::MyTags: return _myTags.find((SavedSublist *) nullptr)->second.tags;
...
ReactionId Reactions::favoriteId() const { return _favoriteId; }

const Reaction *Reactions::favorite() const { return _favorite ? &*_favorite : nullptr; }
...
void Reactions::setFavorite(const ReactionId &id) {
	...
	_saveFaveRequestId = api->request(MTPmessages_SetDefaultReaction(ReactionToMTP(id)))
							 .done([=] { _saveFaveRequestId = 0; })
							 .fail([=] { _saveFaveRequestId = 0; })
							 .send();
...
void Reactions::renameTag(const ReactionId &id, const QString &name) {
	...
	_owner->session()
		.api()
		.request(MTPmessages_UpdateSavedReactionTag(
			MTP_flags(name.isEmpty() ? Flag(0) : Flag::f_title), ReactionToMTP(id), MTP_string(name)))
		.send();
}
...
DocumentData *Reactions::chooseGenericAnimation(not_null<DocumentData *> custom) const {
	const auto sticker = custom->sticker();
	const auto i =
		sticker ? ranges::find(_available, ::Data::ReactionId{{sticker->alt}}, &::Data::Reaction::id) : end(_available);
...
void Reactions::fillPaidReactionAnimations() const {
	...
	for (auto i = 0; i != kCount; ++i) {
		...
		_paidReactionCache.emplace(document, document->createMediaView());
	}
...
DocumentData *Reactions::randomLoadedFrom(std::vector<not_null<DocumentData *>> list) const {
	if (list.empty()) {
		return nullptr;
	}
	ranges::shuffle(list);
	...
	const auto k =
		ranges::find_if(list, [&](not_null<DocumentData *> value) { return value->createMediaView()->loaded(); });
...
rpl::producer<> Reactions::topUpdates() const { return _topUpdated.events(); }

rpl::producer<> Reactions::recentUpdates() const { return _recentUpdated.events(); }

rpl::producer<> Reactions::defaultUpdates() const { return _defaultUpdated.events(); }

rpl::producer<> Reactions::favoriteUpdates() const { return _favoriteUpdated.events(); }

rpl::producer<> Reactions::myTagsUpdates() const {
	return _myTagsUpdated.events() | rpl::filter(!rpl::mappers::_1) | rpl::to_empty;
}

rpl::producer<> Reactions::tagsUpdates() const { return _tagsUpdated.events(); }

rpl::producer<ReactionId> Reactions::myTagRenamed() const { return _myTagRenamed.events(); }

rpl::producer<> Reactions::effectsUpdates() const { return _effectsUpdated.events(); }
...
void Reactions::preloadImageFor(const ReactionId &id) {
	...
	const auto document = (i == end(list)) ? nullptr : i->centerIcon ? i->centerIcon : i->selectAnimation.get();
...
void Reactions::preloadAnimationsFor(const ReactionId &id) {
	...
	{
		const auto view = document ? document->activeMediaView() : nullptr;
		if (view) {
			view->checkStickerLarge();
		}
	};
	...
	const auto findId = custom ? ReactionId{{customSticker ? customSticker->alt : QString()}} : id;
...
QImage Reactions::resolveEffectImageFor(EffectId id) {
	return (id == kFakeEffectId) ? QImage() : resolveImageFor({DocumentId(id)});
}
...
QImage Reactions::resolveImageFor(const ReactionId &id) {
	...
	const auto resolve = [&](QImage &image, int size)
	{
		...
		const auto frameSize = set.fromSelectAnimation ? (size / 2) : size;
		// Must not be colored to text.
		image = set.icon->frame(QColor()).scaled(
			frameSize * factor, frameSize * factor, Qt::IgnoreAspectRatio, Qt::SmoothTransformation);
		if (set.fromSelectAnimation) {
			auto result = QImage(size * factor, size * factor, QImage::Format_ARGB32_Premultiplied);
			result.fill(Qt::transparent);

			auto p = QPainter(&result);
			p.drawImage((size - frameSize) * factor / 2, (size - frameSize) * factor / 2, image);
			p.end();
	...
	if (set.image.isNull() && set.icon) {
		resolve(set.image, set.effect ? st::effectInfoImage : st::reactionInlineImage);
...
void Reactions::resolveReactionImages() {
	for (auto &[id, set] : _images) {
		...
		const auto document = (i == end(_available)) ? nullptr
			: i->centerIcon							 ? i->centerIcon
													 : i->selectAnimation.get();
		if (document) {
			loadImage(set, document, !i->centerIcon);
		} else {
			LOG(("API Error: Reaction '%1' not found!").arg(ReactionIdToLog(id)));
		}
	}
}
...
void Reactions::resolveEffectImages() {
	for (auto &[id, set] : _images) {
		...
		const auto document = (i == end(_effects)) ? nullptr : i->centerIcon ? i->centerIcon : nullptr;
		if (document) {
			loadImage(set, document, false);
		} else if (i != end(_effects)) {
			generateImage(set, i->title);
		} else {
			LOG(("API Error: Effect '%1' not found!").arg(ReactionIdToLog(id)));
		}
		if (i != end(_effects)) {
			preloadEffect(*i);
		}
...
void Reactions::loadImage(ImageSet &set, not_null<DocumentData *> document, bool fromSelectAnimation) {
	if (!set.image.isNull() || set.icon) {
		return;
	} else if (!set.media) {
		if (!set.effect) {
	...
	if (set.media->loaded()) {
		...
		document->session().downloaderTaskFinished() |
			rpl::start_with_next([=] { downloadTaskFinished(); }, _imagesLoadLifetime);
	}
}
...
void Reactions::requestTop() {
	...
	_topRequestId = api.request(MTPmessages_GetTopReactions(MTP_int(kTopReactionsLimit), MTP_long(_topHash)))
						.done(
							[=](const MTPmessages_Reactions &result)
							{
								_topRequestId = 0;
								result.match([&](const MTPDmessages_reactions &data) { updateTop(data); },
...
void Reactions::requestRecent() {
	...
	_recentRequestId =
		api.request(MTPmessages_GetRecentReactions(MTP_int(kRecentReactionsLimit), MTP_long(_recentHash)))
			.done(
				[=](const MTPmessages_Reactions &result)
				{
					_recentRequestId = 0;
					result.match([&](const MTPDmessages_reactions &data) { updateRecent(data); },
								 [](const MTPDmessages_reactionsNotModified &) {});
				})
			.fail(
				[=]
...
void Reactions::requestDefault() {
	...
	auto &api = _owner->session().api();
	_defaultRequestId =
		api.request(MTPmessages_GetAvailableReactions(MTP_int(_defaultHash)))
			.done(
				[=](const MTPmessages_AvailableReactions &result)
				{
					_defaultRequestId = 0;
					result.match([&](const MTPDmessages_availableReactions &data) { updateDefault(data); },
...
void Reactions::requestGeneric() {
	...
	auto &api = _owner->session().api();
	_genericRequestId = api.request(MTPmessages_GetStickerSet(MTP_inputStickerSetEmojiGenericAnimations(),
															  MTP_int(0) // hash
															  ))
							.done(
								...
								{
									_genericRequestId = 0;
									result.match([&](const MTPDmessages_stickerSet &data) { updateGeneric(data); },
												 [](const MTPDmessages_stickerSetNotModified &)
												 { LOG(("API Error: Unexpected messages.stickerSetNotModified.")); });
								})
							.fail([=] { _genericRequestId = 0; })
							.send();
}
...
void Reactions::requestMyTags(SavedSublist *sublist) {
	...
	my.requestId =
		api.request(MTPmessages_GetSavedReactionTags(MTP_flags(sublist ? Flag::f_peer : Flag()),
													 (sublist ? sublist->peer()->input : MTP_inputPeerEmpty()),
													 MTP_long(my.hash)))
			.done(
				[=](const MTPmessages_SavedReactionTags &result)
				{
					auto &my = _myTags[sublist];
					my.requestId = 0;
					result.match([&](const MTPDmessages_savedReactionTags &data) { updateMyTags(sublist, data); },
...
void Reactions::requestTags() {
	...
	auto &api = _owner->session().api();
	_tagsRequestId = api.request(MTPmessages_GetDefaultTagReactions(MTP_long(_tagsHash)))
						 .done(
							 [=](const MTPmessages_Reactions &result)
							 {
								 _tagsRequestId = 0;
								 result.match([&](const MTPDmessages_reactions &data) { updateTags(data); },
											  [](const MTPDmessages_reactionsNotModified &) {});
							 })
						 .fail(
							 [=]
...
void Reactions::requestEffects() {
	...
	auto &api = _owner->session().api();
	_effectsRequestId =
		api.request(MTPmessages_GetAvailableEffects(MTP_int(_effectsHash)))
			.done(
				[=](const MTPmessages_AvailableEffects &result)
				{
					_effectsRequestId = 0;
					result.match([&](const MTPDmessages_availableEffects &data) { updateEffects(data); },
...
void Reactions::updateMyTags(SavedSublist *sublist, const MTPDmessages_savedReactionTags &data) {
	auto &my = _myTags[sublist];
...
void Reactions::effectsUpdated() { _effectsUpdated.fire({}); }
...
void Reactions::customEmojiResolveDone(not_null<DocumentData *> document) {
	...
	const auto myTagSublists = (k != end(_unresolvedMyTags)) ? base::take(k->second) : base::flat_set<SavedSublist *>();
...
std::vector<Reaction> Reactions::resolveByIds(const std::vector<ReactionId> &ids,
...
std::optional<Reaction> Reactions::resolveByInfo(const MyTagInfo &info, SavedSublist *sublist) {
...
std::vector<Reaction> Reactions::resolveByInfos(const std::vector<MyTagInfo> &infos,
												base::flat_map<ReactionId, base::flat_set<SavedSublist *>> &unresolved,
	...
	for (const auto &tag : infos) {
		if (auto resolved = resolveByInfo(tag, sublist)) {
			result.push_back(*resolved);
		} else if (const auto i = unresolved.find(tag.id); i != end(unresolved)) {
...
void Reactions::resolve(const ReactionId &id) {
	if (const auto emoji = id.emoji(); !emoji.isEmpty()) {
		...
		_owner->customEmojiManager().resolve(customId, resolveListener());
	}
}
...
std::optional<Reaction> Reactions::parse(const MTPAvailableReaction &entry) {
	...
	return std::make_optional(Reaction{
		...
		.appearAnimation = _owner->processDocument(data.vappear_animation()),
		.selectAnimation = _owner->processDocument(data.vselect_animation()),
		...
		.centerIcon = (data.vcenter_icon() ? _owner->processDocument(*data.vcenter_icon()).get() : nullptr),
		.aroundAnimation =
			(data.varound_animation() ? _owner->processDocument(*data.varound_animation()).get() : nullptr),
...
std::optional<Reaction> Reactions::parse(const MTPAvailableEffect &entry) {
	...
	const auto around = aroundId ? _owner->document(aroundId).get() : nullptr;
...
void Reactions::send(not_null<HistoryItem *> item, bool addToRecent) {
	...
	const auto flags = (chosen.empty() ? Flag(0) : Flag::f_reaction) | (addToRecent ? Flag::f_add_to_recent : Flag(0));
	i->second = api.request(MTPmessages_SendReaction(
								MTP_flags(flags),
								item->history()->peer->input,
								MTP_int(id.msg),
								MTP_vector<MTPReaction>(
									chosen | ranges::views::filter([](const ReactionId &id) { return !id.paid(); }) |
									ranges::views::transform(ReactionToMTP) | ranges::to<QVector<MTPReaction>>())))
					.done(
					...
					.fail([=](const MTP::Error &error) { _sentRequests.remove(id); })
					.send();
}
...
void Reactions::poll(not_null<HistoryItem *> item, crl::time now) {
	...
	if (!grouped || item->history()->peer->isUser()) {
		...
		if (!_repaintItems.contains(item)) {
			...
			if (!_repaintTimer.isActive() || _repaintTimer.remainingTime() > left) {
				_repaintTimer.callOnce(left);
			}
		}
	} else if (!_pollingItems.contains(item)) {
		if (_pollItems.empty() && !_pollRequestId) {
			crl::on_main(&_owner->session(), [=] { pollCollected(); });
		}
...
void Reactions::clearTemporary() { _temporary.clear(); }
...
Reaction *Reactions::lookupTemporary(const ReactionId &id) {
	if (id.paid()) {
		...
		const auto document = _owner->document(customId);
		if (document->sticker()) {
			return &_temporary.emplace(customId, CustomReaction(document)).first->second;
		}
		_owner->customEmojiManager().resolve(customId, resolveListener());
		return nullptr;
	}
...
not_null<DocumentData *> Reactions::paidToastAnimation() {
	if (!_paidToastAnimation) {
		_paidToastAnimation = ChatHelpers::GenerateLocalTgsSticker(&_owner->session(), u"star_reaction_toast"_q);
	}
	return _paidToastAnimation;
}

rpl::producer<std::vector<Reaction>> Reactions::myTagsValue(SavedSublist *sublist) {
	refreshMyTags(sublist);
	const auto list = [=] { return _myTags[sublist].tags; };
	return rpl::single(list()) |
		rpl::then(_myTagsUpdated.events() | rpl::filter(rpl::mappers::_1 == sublist) | rpl::map(list));
}
...
crl::time Reactions::sendingScheduledPaidAt(not_null<HistoryItem *> item) const {
	const auto i = _sendPaidItems.find(item);
	return (i != end(_sendPaidItems)) ? i->second : crl::time();
}

crl::time Reactions::ScheduledPaidDelay() { return kPaidAccumulatePeriod; }
...
void Reactions::pollCollected() {
	...
	for (const auto &[peer, ids] : toRequest) {
		...
		_pollRequestId = api.request(MTPmessages_GetMessagesReactions(peer->input, MTP_vector<MTPint>(ids)))
							 .done(
								 [=](const MTPUpdates &result)
								 {
									 _owner->session().api().applyUpdates(result);
...
bool Reactions::sending(not_null<HistoryItem *> item) const {
	return _sentRequests.contains(item->fullId()) || _sendingPaid.contains(item);
}

bool Reactions::HasUnread(const MTPMessageReactions &data) {
	return data.match(
		[&](const MTPDmessageReactions &data)
		{
			if (const auto &recent = data.vrecent_reactions()) {
				for (const auto &one : recent->v) {
					if (one.match([&](const MTPDmessagePeerReaction &data) { return data.is_unread(); })) {
						return true;
					}
				}
			}
...
void Reactions::CheckUnknownForUnread(not_null<Session *> owner, const MTPMessage &message) {
	message.match(
...
void Reactions::sendPaidPrivacyRequest(not_null<HistoryItem *> item, PaidReactionSend send) {
	...
	const auto id = item->fullId();
	auto &api = _owner->session().api();
	const auto requestId = api.request(MTPmessages_TogglePaidReactionPrivacy(
										   item->history()->peer->input,
										   MTP_int(id.msg),
...
void Reactions::sendPaidRequest(not_null<HistoryItem *> item, PaidReactionSend send) {
	Expects(!_sendingPaid.contains(item));
	...
	const auto requestId =
		api.request(MTPmessages_SendPaidReaction(
						...
						MTP_int(send.count),
						MTP_long(randomId),
						(!send.shownPeer ? MTPPaidReactionPrivacy()
										 : PaidReactionShownPeerToTL(&_owner->session(), *send.shownPeer))))
			.done(
				[=](const MTPUpdates &result)
				{
...
void Reactions::sendPaidFinish(not_null<HistoryItem *> item, PaidReactionSend send, bool success) {
	item->finishPaidReactionSending(send, success);
	sendPaid();
}

MessageReactions::MessageReactions(not_null<HistoryItem *> item) : _item(item) {}
			...
			finishPaidSending(
				{
					...
					.valid = true,
					.shownPeer = MaybeShownPeer(paid->sendingPrivacySet, paid->sendingShownPeer),
				},
				false);
		}
	}
...
void MessageReactions::add(const ReactionId &id, bool addToRecent) {
	...
	_list.erase(ranges::remove_if(_list,
								  [&](MessageReaction &one)
								  {
									  if (one.id.paid()) {
										  return false;
									  ...
									  if (j != end(_recent)) {
										  if (removed) {
											  ...
											  j->second.erase(ranges::remove(j->second, true, &RecentReaction::my),
															  end(j->second));
											  if (j->second.empty()) {
												  _recent.erase(j);
											  }
...
void MessageReactions::remove(const ReactionId &id) {
	...
	if (i == end(_list)) {
		...
		Assert(j == end(_recent) || !ranges::contains(j->second, self, &RecentReaction::peer));
		return;
	}
	i->my = false;
	const auto tags = _item->reactionsAreTags();
	...
	if (j != end(_recent)) {
		if (removed) {
			...
			j->second.erase(ranges::remove(j->second, true, &RecentReaction::my), end(j->second));
			if (j->second.empty()) {
				_recent.erase(j);
			}
		}
...
bool MessageReactions::checkIfChanged(const QVector<MTPReactionCount> &list,
	...
	for (const auto &count : list) {
		const auto changed = count.match(
			[&](const MTPDreactionCount &data)
			{
				const auto id = ReactionFromMTP(data.vreaction());
...
bool MessageReactions::change(const QVector<MTPReactionCount> &list,
							  const QVector<MTPMessagePeerReaction> &recent,
	...
	for (const auto &count : list) {
		count.match(
			[&](const MTPDreactionCount &data)
			{
				...
				if (i == end(_list)) {
					changed = true;
					_list.push_back({.id = id, .count = nowCount, .my = (!min && chosen)});
				} else {
	...
	if (!min && !order.empty()) {
		...
		const auto proj = [&](const MessageReaction &reaction) { return reaction.my ? order[reaction.id] : minimal; };
	...
	for (const auto &reactor : top) {
		...
		const auto peerId = (data.is_anonymous() || !data.vpeer_id()) ? PeerId() : peerFromMTP(*data.vpeer_id());
	...
	if (paidTop.empty()) {
		...
		if (min && _paid) {
			const auto mine = [](const TopPaid &entry) { return entry.my != 0; };
			if (!ranges::contains(paidTop, true, mine)) {
				const auto nonTopMine = [](const TopPaid &entry) { return entry.my && !entry.top; };
		...
		ranges::sort(paidTop, std::greater(), [](const TopPaid &entry) { return entry.count; });
		if (!_paid) {
...
const std::vector<MessageReaction> &MessageReactions::list() const { return _list; }

auto MessageReactions::recent() const -> const base::flat_map<ReactionId, std::vector<RecentReaction>> & {
	return _recent;
}
...
bool MessageReactions::empty() const { return _list.empty(); }
...
void MessageReactions::scheduleSendPaid(int count, std::optional<PeerId> shownPeer) {
	Expects(count >= 0);
...
int MessageReactions::scheduledPaid() const { return _paid ? _paid->scheduled : 0; }

void MessageReactions::cancelScheduledPaid() {
	if (_paid) {
		if (_paid->scheduledFlag) {
			if (const auto amount = int(_paid->scheduled)) {
				_item->history()->session().credits().unlock(StarsAmount(amount));
			}
			_paid->scheduled = 0;
...
PaidReactionSend MessageReactions::startPaidSending() {
	...
	return {
		...
		.valid = true,
		.shownPeer = MaybeShownPeer(_paid->sendingPrivacySet, _paid->sendingShownPeer),
	};
}

void MessageReactions::finishPaidSending(PaidReactionSend send, bool success) {
	Expects(_paid != nullptr);
	Expects(send.count == _paid->sending);
	Expects(send.valid == (_paid->sendingFlag == 1));
	Expects(send.shownPeer == MaybeShownPeer(_paid->sendingPrivacySet, _paid->sendingShownPeer));
	...
	if (!_paid->scheduledFlag && _paid->top.empty()) {
		...
		const auto i = ranges::find_if(_paid->top, [](const TopPaid &top) { return top.my; });
		if (i != end(_paid->top)) {
			i->peer = send.shownPeer ? _item->history()->owner().peer(*send.shownPeer).get() : nullptr;
		}
	}
...
bool MessageReactions::localPaidData() const { return _paid && (_paid->scheduledFlag || _paid->sendingFlag); }

int MessageReactions::localPaidCount() const { return _paid ? (_paid->scheduled + _paid->sending) : 0; }
...
PeerId MessageReactions::localPaidShownPeer() const {
	...
	return !_paid											   ? PeerId()
		: (_paid->scheduledFlag && _paid->scheduledPrivacySet) ? _paid->scheduledShownPeer
		: (_paid->sendingFlag && _paid->sendingPrivacySet)	   ? _paid->sendingShownPeer
															   : minePaidShownPeer();
}
...
std::vector<ReactionId> MessageReactions::chosen() const {
	return _list | ranges::views::filter(&MessageReaction::my) | ranges::views::transform(&MessageReaction::id) |
		ranges::to_vector;
}
