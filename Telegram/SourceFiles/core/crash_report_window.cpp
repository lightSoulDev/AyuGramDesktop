...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"ViGram"_q : title);
...
NotStartedWindow::NotStartedWindow() : _label(this), _log(this), _close(this) {
	_label.setText(u"Could not start ViGram Desktop!\nYou can see complete log below:"_q);
...
LastCrashedWindow::LastCrashedWindow(const QByteArray &crashdump, Fn<void()> launch)
	: _dumpraw(crashdump), _label(this), _pleaseSendReport(this), _yourReportName(this), _minidump(this), _report(this),
	  _send(this), _sendSkip(this, false), _networkSettings(this), _continue(this), _showReport(this),
	  _saveReport(this), _getApp(this), _includeUsername(this), _reportText(QString::fromUtf8(crashdump)),
	  _reportShown(false), _reportSaved(false),
	  _sendingState(crashdump.isEmpty() ? SendingNoReport : SendingUpdateCheck), _updating(this),
	...
	if (_sendingState == SendingNoReport) {
		_label.setText(u"Last time ViGram Desktop was not closed properly."_q);
	} else {
		_label.setText(u"Last time ViGram Desktop crashed :("_q);
	}
	...
	connect(&_getApp,
			&QPushButton::clicked,
			[=] { QDesktopServices::openUrl(u"https://github.com/ViGram/AyuGramDesktop"_q); });
...
void LastCrashedWindow::checkingFinished() {
...

	{
		QString version = getReportField(qstr("version"), qstr("Version:"));
		if (!version.isEmpty()) {
			const auto sentryVersion = QString("vigram-desktop@%1").arg(version);
...
void LastCrashedWindow::updateControls() {
	...
	QSize s(2 * padding +
				QFontMetrics(_label.font()).horizontalAdvance(u"Last time ViGram Desktop was not closed properly."_q) +
