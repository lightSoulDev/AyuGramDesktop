...
const style::InfoTopBar &TopBarStyle(Wrap wrap) { return (wrap == Wrap::Layer) ? st::infoLayerTopBar : st::infoTopBar; }
...
[[nodiscard]] bool HasCustomTopBar(not_null<const Controller *> controller) {
	...
	return (section.type() == Section::Type::BotStarRef) ||
		((section.type() == Section::Type::Settings) && section.settingsType()->hasCustomTopBar());
}

[[nodiscard]] Fn<Ui::StringWithNumbers(int)> SelectedTitleForMedia(Section::MediaType type) {
...
SelectedItems::SelectedItems(Section::MediaType mediaType) : title(SelectedTitleForMedia(mediaType)) {}

WrapWidget::WrapWidget(QWidget *parent,
					   not_null<Window::SessionController *> window,
					   Wrap wrap,
					   not_null<Memento *> memento)
	: SectionWidget(parent, window, rpl::producer<PeerData *>()),
	  _isSeparatedWindow(window->windowId().type == Window::SeparateType::SharedMedia), _wrap(wrap),
	  _controller(createController(window, memento->content())), _topShadow(this), _bottomShadow(this) {
	_topShadow->toggleOn(topShadowToggledValue() | rpl::filter([](bool shown) { return true; }));

	_bottomShadow->toggleOn(_desiredBottomShadowVisibilities.events() | rpl::flatten_latest() |
				...
				[=]
				{
					if (_wrap.current() == Wrap::Layer) {
						...
						_controller->parentController()->showBackFromStack(
							Window::SectionShow(anim::type::normal, anim::activation::background));
					} else {
		...
		rpl::filter(
			[=]
			{
				return requireTopBarSearch() &&
					(Core::App().activeWindow() == &_controller->parentController()->window());
			}) |
		rpl::start_with_next(
...
void WrapWidget::restoreHistoryStack(std::vector<std::shared_ptr<ContentMemento>> stack) {
	Expects(!stack.empty());
...
void WrapWidget::startInjectingActivePeerProfiles() {
	...
	rpl::combine(_wrap.value(), _controller->parentController()->activeChatValue()) |
		rpl::filter((_1 == Wrap::Side) && _2) | rpl::map(_2) |
		rpl::start_with_next([this](Dialogs::Key key) { injectActiveProfile(key); }, lifetime());
}
...
void WrapWidget::injectActivePeerProfile(not_null<PeerData *> peer) {
	const auto firstPeer = hasStackHistory() ? _historyStack.front().section->peer() : _controller->peer();
	const auto firstSectionType =
		hasStackHistory() ? _historyStack.front().section->section().type() : _controller->section().type();
	const auto firstSectionMediaType = [&]
	{
		if (firstSectionType == Section::Type::Profile || firstSectionType == Section::Type::SavedSublists ||
			firstSectionType == Section::Type::Downloads) {
			return Section::MediaType::kCount;
		}
		return hasStackHistory() ? _historyStack.front().section->section().mediaType()
	...
	const auto expectedType = savedSublistsInfo ? Section::Type::SavedSublists
		: sharedMediaInfo						? Section::Type::Media
												: Section::Type::Profile;
	const auto expectedMediaType = savedSublistsInfo ? Section::MediaType::kCount
		: sharedMediaInfo							 ? Section::MediaType::Photo
													 : Section::MediaType::kCount;
	if (firstSectionType != expectedType || firstSectionMediaType != expectedMediaType || firstPeer != peer) {
		auto section = savedSublistsInfo ? Section(Section::Type::SavedSublists)
			: sharedMediaInfo			 ? Section(Section::MediaType::Photo)
										 : Section(Section::Type::Profile);
		injectActiveProfileMemento(std::move(Memento(peer, section).takeStack().front()));
	}
}

void WrapWidget::injectActiveProfileMemento(std::shared_ptr<ContentMemento> memento) {
	auto injected = StackItem();
	injected.section = std::move(memento);
	_historyStack.insert(_historyStack.begin(), std::move(injected));
	if (_content) {
		setupTop();
		finishShowContent();
	}
...
std::unique_ptr<Controller> WrapWidget::createController(not_null<Window::SessionController *> window,
														 not_null<ContentMemento *> memento) {
	auto result = std::make_unique<Controller>(this, window, memento);
	return result;
}

Key WrapWidget::key() const { return _controller->key(); }
...
Dialogs::RowDescriptor WrapWidget::activeChat() const {
	if (const auto peer = key().peer()) {
		...
		return (key().storiesTab() == Stories::Tab::Saved)
			? Dialogs::RowDescriptor(storiesPeer->owner().history(storiesPeer), FullMsgId())
			: Dialogs::RowDescriptor();
	} else if (key().settingsSelf() || key().isDownloads() || key().reactionsContextId() || key().poll() ||
			   key().starrefPeer() || key().statisticsTag().peer) {
...
void WrapWidget::createTopBar() {
	...
	auto selectedItems = _topBar ? _topBar->takeSelectedItems() : SelectedItems(Section::MediaType::kCount);
	_topBar.create(this, _controller.get(), TopBarStyle(wrapValue), std::move(selectedItems));
	_topBar->selectionActionRequests() |
		rpl::start_with_next([=](SelectionAction action) { _content->selectionAction(action); }, _topBar->lifetime());
	...
	if (hasBackButton()) {
		...
		auto close = _topBar->addButton(base::make_unique_q<Ui::IconButton>(_topBar, st::infoTopBarClose));
		close->addClickHandler([this] { _controller->parentController()->closeThirdSection(); });
	}
	_topBar->storyClicks() |
		rpl::start_with_next(
			[=]
	...
	if (wrapValue == Wrap::Layer) {
		auto close = _topBar->addButton(base::make_unique_q<Ui::IconButton>(_topBar, st::infoLayerTopBarClose));
		close->addClickHandler([this]
							   { checkBeforeClose([=] { _controller->parentController()->hideSpecialLayer(); }); });
	} else if (requireTopBarSearch()) {
		auto search = _controller->searchFieldController();
		Assert(search != nullptr);
		setupShortcuts();
		_topBar->createSearchView(
			search, _controller->searchEnabledByContent(), _controller->takeSearchStartsFocused());
	}
	_topBar->lower();
...
void WrapWidget::setupTopBarMenuToggle() {
	...
	if (section.type() == Section::Type::Profile && (wrap() != Wrap::Side || hasStackHistory())) {
		addTopBarMenuButton();
		...
		if (section.settingsType() == ::Settings::Information::Id() ||
			...
			if (!self->username().isEmpty()) {
				...
				const auto &st = (wrap() == Wrap::Layer) ? st::infoLayerTopBarQr : st::infoTopBarQr;
				const auto button = _topBar->addButton(base::make_unique_q<Ui::IconButton>(_topBar, st));
				button->addClickHandler([show, self]
										{ show->show(Box(Ui::FillPeerQrBox, self, std::nullopt, nullptr)); });
			}
		}
	} else if (key.storiesPeer() && key.storiesPeer()->isSelf() && key.storiesTab() == Stories::Tab::Saved) {
		const auto &st = (wrap() == Wrap::Layer) ? st::infoLayerTopBarEdit : st::infoTopBarEdit;
		const auto button = _topBar->addButton(base::make_unique_q<Ui::IconButton>(_topBar, st));
		button->addClickHandler([=] { _controller->showSettings(::Settings::Information::Id()); });
	} else if (section.type() == Section::Type::Downloads) {
		auto &manager = Core::App().downloadManager();
		rpl::merge(rpl::single(false),
				   manager.loadingListChanges() | rpl::map_to(false),
				   manager.loadedAdded() | rpl::map_to(true),
				   manager.loadedRemoved() | rpl::map_to(false)) |
			rpl::start_with_next(
				[=, &manager](bool definitelyHas)
				{
					const auto has = [&]
					{
...
void WrapWidget::checkBeforeCloseByEscape(Fn<void()> close) {
	if (_topBar) {
		_topBar->checkBeforeCloseByEscape(
			[&]
			{ _content->checkBeforeCloseByEscape(crl::guard(this, [=] { WrapWidget::checkBeforeClose(close); })); });
	} else {
		_content->checkBeforeCloseByEscape(crl::guard(this, [=] { WrapWidget::checkBeforeClose(close); }));
	}
}
...
void WrapWidget::addTopBarMenuButton() {
	...
	_topBarMenuToggle.reset(_topBar->addButton(base::make_unique_q<Ui::IconButton>(
		_topBar, (wrap() == Wrap::Layer ? st::infoLayerTopBarMenu : st::infoTopBarMenu))));
	_topBarMenuToggle->addClickHandler([this] { showTopBarMenu(false); });
...
bool WrapWidget::closeByOutsideClick() const { return _content->closeByOutsideClick(); }
...
void WrapWidget::addProfileCallsButton() {
	...
	user->session().changes().peerFlagsValue(user, Data::PeerUpdate::Flag::HasCalls) |
		rpl::filter([=] { return user->hasCalls(); }) | rpl::take(1) |
		rpl::start_with_next(
			[=]
			{
				_topBar
					->addButton(base::make_unique_q<Ui::IconButton>(
						_topBar, (wrap() == Wrap::Layer ? st::infoLayerTopBarCall : st::infoTopBarCall)))
					->addClickHandler([=] { Core::App().calls().startOutgoingCall(user, false); });
			},
			_topBar->lifetime());
...
void WrapWidget::showTopBarMenu(bool check) {
	...
	_topBarMenu = base::make_unique_q<Ui::PopupMenu>(QWidget::window(), st::popupMenuExpandedSeparator);

	_topBarMenu->setDestroyedCallback(
		[this]
		{
	...
	_topBarMenu->popup(_topBarMenuToggle->mapToGlobal(st::infoLayerTopBarMenuPosition));
}

bool WrapWidget::requireTopBarSearch() const {
	if (!_topBar || !_controller->searchFieldController() || (_controller->wrap() == Wrap::Layer) ||
		(_controller->section().type() == Section::Type::Profile) || key().isDownloads()) {
		return false;
	} else if (hasStackHistory() || _controller->section().type() == Section::Type::RequestsList) {
		return true;
	}
	return false;
}

bool WrapWidget::showBackFromStackInternal(const Window::SectionShow &params) {
	if (hasStackHistory()) {
		auto last = std::move(_historyStack.back());
		_historyStack.pop_back();
		showNewContent(last.section.get(), params.withWay(Window::SectionShow::Way::Backward));
		return true;
	}
...
not_null<Ui::RpWidget *> WrapWidget::topWidget() const { return _topBar; }
...
void WrapWidget::finishShowContent() {
	...
	if (_topBar) {
		...
		_topBar->setStoriesArchive(_controller->key().storiesTab() == Stories::Tab::Archive);
	}
	_desiredHeights.fire(desiredHeightForContent());
	_desiredShadowVisibilities.fire(_content->desiredShadowVisibility());
	_desiredBottomShadowVisibilities.fire(_content->desiredBottomShadowVisibility());
	if (auto selection = _content->selectedListValue()) {
		_selectedLists.fire(std::move(selection));
	} else {
		_selectedLists.fire(rpl::single(SelectedItems(Storage::SharedMediaType::Photo)));
	}
	...
	_content->scrollBottomSkipValue() | rpl::start_with_next([=] { updateContentGeometry(); }, _content->lifetime());
}

rpl::producer<bool> WrapWidget::topShadowToggledValue() const {
	return _desiredShadowVisibilities.events() | rpl::flatten_latest() |
...
rpl::producer<int> WrapWidget::desiredHeightForContent() const {
	...
	return rpl::single(0) |
		rpl::then(rpl::combine(
			_content->desiredHeightValue(), (_topBar ? _topBar->heightValue() : rpl::single(0)), _1 + _2));
}
...
object_ptr<ContentWidget> WrapWidget::createContent(not_null<ContentMemento *> memento,
													not_null<Controller *> controller) {
	return memento->createWidget(this, controller, contentGeometry());
}

rpl::producer<Wrap> WrapWidget::wrapValue() const { return _wrap.value(); }

void WrapWidget::setWrap(Wrap wrap) { _wrap = wrap; }

rpl::producer<> WrapWidget::contentChanged() const { return _contentChanges.events(); }

bool WrapWidget::hasTopBarShadow() const { return _topShadow->toggled(); }

QPixmap WrapWidget::grabForShowAnimation(const Window::SectionSlideParams &params) {
...
void WrapWidget::showAnimatedHook(const Window::SectionSlideParams &params) {
	if (params.withTopBarShadow) {
		_topShadow->setVisible(true);
	}
...
bool WrapWidget::showInternal(not_null<Window::SectionMemento *> memento, const Window::SectionShow &params) {
	if (auto infoMemento = dynamic_cast<Memento *>(memento.get())) {
		...
		auto skipInternal = hasStackHistory() && (params.way == Window::SectionShow::Way::ClearStack);
...
rpl::producer<int> WrapWidget::desiredHeightValue() const {
	return _desiredHeights.events_starting_with(desiredHeightForContent()) | rpl::flatten_latest();
}
...
bool WrapWidget::returnToFirstStackFrame(not_null<ContentMemento *> memento, const Window::SectionShow &params) {
	if (!hasStackHistory()) {
		return false;
	}
	auto firstPeer = _historyStack.front().section->peer();
	auto firstSection = _historyStack.front().section->section();
	if (firstPeer == memento->peer() && firstSection.type() == memento->section().type() &&
...
void WrapWidget::showNewContent(not_null<ContentMemento *> memento, const Window::SectionShow &params) {
	const auto saveToStack = (_content != nullptr) && (params.way == Window::SectionShow::Way::Forward);
	const auto needAnimation = (_content != nullptr) && (params.animated != anim::type::instant);
	auto animationParams = SectionSlideParams();
	auto newController = createController(_controller->parentController(), memento);
	...
	if (needAnimation) {
		animationParams.withTopBarShadow = hasTopBarShadow() && newContent->hasTopBarShadow();
		animationParams.oldContentCache = grabForShowAnimation(animationParams);
		const auto layer = (wrap() == Wrap::Layer);
		animationParams.withFade = layer;
		animationParams.topSkip = layer ? st::boxRadius : 0;

		if (HasCustomTopBar(_controller.get()) || HasCustomTopBar(newController.get())) {
			const auto s = QSize(newContent->width(), animationParams.topSkip);
			auto image = Ui::RippleAnimation::MaskByDrawer(s,
														   false,
														   [&](QPainter &p)
														   {
...

	{
		...
		const auto oldController = std::exchange(_controller, std::move(newController));
		if (newContent) {
			setupTop();
	...
	if (animationParams) {
		...
		showAnimated(saveToStack ? SlideDirection::FromRight : SlideDirection::FromLeft, animationParams);
	}
}
...
void WrapWidget::keyPressEvent(QKeyEvent *e) {
	if (e->key() == Qt::Key_Escape || e->key() == Qt::Key_Back) {
		checkBeforeCloseByEscape((hasStackHistory() || wrap() != Wrap::Layer)
									 ...
									 : Fn<void()>([=] { _controller->parentController()->hideSpecialLayer(); }));
		return;
	}
...
void WrapWidget::updateContentGeometry() {
	if (_content) {
		...
		_bottomShadow->moveToLeft(0, _content->y() + _content->height() - _content->scrollBottomSkip());
	}
}

bool WrapWidget::floatPlayerHandleWheelEvent(QEvent *e) { return _content->floatPlayerHandleWheelEvent(e); }

QRect WrapWidget::floatPlayerAvailableRect() { return _content->floatPlayerAvailableRect(); }

object_ptr<Ui::RpWidget> WrapWidget::createTopBarSurrogate(QWidget *parent) {
...
void WrapWidget::updateGeometry(QRect newGeometry, bool expanding, int additionalScroll, int maxVisibleHeight) {
...
int WrapWidget::scrollTillBottom(int forHeight) const {
	return _content->scrollTillBottom(forHeight - (_topBar ? _topBar->height() : 0));
}

int WrapWidget::scrollBottomSkip() const { return _content->scrollBottomSkip(); }

rpl::producer<int> WrapWidget::scrollTillBottomChanges() const {
	return _scrollTillBottomChanges.events_starting_with(_content->scrollTillBottomChanges()) | rpl::flatten_latest();
}

rpl::producer<bool> WrapWidget::grabbingForExpanding() const { return _grabbingForExpanding.value(); }

const Ui::RoundRect *WrapWidget::bottomSkipRounding() const { return _content->bottomSkipRounding(); }

bool WrapWidget::hasBackButton() const { return !_isSeparatedWindow && (wrap() == Wrap::Narrow || hasStackHistory()); }

bool WrapWidget::willHaveBackButton(const Window::SectionShow &params) const {
	using Way = Window::SectionShow::Way;
	const auto willSaveToStack = (_content != nullptr) && (params.way == Way::Forward);
	const auto willClearStack = (params.way == Way::ClearStack);
	const auto willHaveStack = !willClearStack && (hasStackHistory() || willSaveToStack);
