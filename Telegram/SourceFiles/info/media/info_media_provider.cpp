...
Provider::Provider(not_null<AbstractController *> controller)
	: _controller(controller), _peer(_controller->key().peer()),
	  _topicRootId(_controller->key().topic() ? _controller->key().topic()->rootId() : 0),
	  _migrated(_controller->migrated()), _type(_controller->section().mediaType()),
	  _slice(sliceKey(_universalAroundId)) {
	_controller->session().data().itemRemoved() |
		rpl::start_with_next([this](auto item) { itemRemoved(item); }, _lifetime);
...
Type Provider::type() { return _type; }
...
rpl::producer<bool> Provider::hasSelectRestrictionChanges() {
	...
	auto noForwards = chat ? Data::PeerFlagValue(chat, ChatDataFlag::NoForwards)
						   : Data::PeerFlagValue(channel, ChannelDataFlag::NoForwards) | rpl::type_erased();

	auto rights = chat ? chat->adminRightsValue() : channel->adminRightsValue();
	auto canDelete =
		std::move(rights) | rpl::map([=] { return chat ? chat->canDeleteMessages() : channel->canDeleteMessages(); });
	return rpl::combine(std::move(noForwards), std::move(canDelete)) |
		rpl::map([=] { return hasSelectRestriction(); }) | rpl::distinct_until_changed() | rpl::skip(1);
}
...
bool Provider::sectionHasFloatingHeader() {
	switch (_type) {
		...
		case Type::MusicFile: return false;
		case Type::File:
...
QString Provider::sectionTitle(not_null<const BaseLayout *> item) {
	switch (_type) {
		...
		case Type::RoundFile:
		case Type::RoundVoiceFile:
		case Type::File: return langMonthFull(item->dateTime().date());

		case Type::Link: return langDayOfMonthFull(item->dateTime().date());

		case Type::MusicFile: return QString();
	}
	Unexpected("Type in ListSection::setHeader()");
}

bool Provider::sectionItemBelongsHere(not_null<const BaseLayout *> item, not_null<const BaseLayout *> previous) {
	...
	switch (_type) {
		...
		case Type::RoundFile:
		case Type::RoundVoiceFile:
		case Type::File: return date.year() == sectionDate.year() && date.month() == sectionDate.month();

		case Type::Link: return date == sectionDate;

		case Type::MusicFile: return true;
	}
...
std::optional<int> Provider::fullCount() { return _slice.fullCount(); }
...
void Provider::checkPreload(QSize viewport,
	...
	const auto preloadIdsLimit = preloadIdsLimitMin + (visibleHeight / minItemHeight);
	...
	const auto minScreenDelta = kPreloadedScreensCount - kPreloadIfLessThanScreens;
	const auto minUniversalIdDelta = (minScreenDelta * visibleHeight) / minItemHeight;
	const auto preloadAroundItem = [&](not_null<BaseLayout *> layout)
	{
		...
		if (!preloadRequired) {
			auto delta = _slice.distance(sliceKey(_universalAroundId), sliceKey(universalId));
...
void Provider::refreshViewer() {
	...
	_controller->mediaSource(idForViewer, _idsLimit, _idsLimit) |
		rpl::start_with_next(
...
rpl::producer<> Provider::refreshed() { return _refreshed.events(); }

std::vector<ListSection> Provider::fillSections(not_null<Overview::Layout::Delegate *> delegate) {
	markLayoutsStale();
...
rpl::producer<not_null<BaseLayout *>> Provider::layoutRemoved() { return _layoutRemoved.events(); }
...
bool Provider::isAfter(not_null<const HistoryItem *> a, not_null<const HistoryItem *> b) {
	return (GetUniversalId(a) < GetUniversalId(b));
}

void Provider::setSearchQuery(QString query) { Unexpected("Media::Provider::setSearchQuery."); }

SparseIdsMergedSlice::Key Provider::sliceKey(UniversalMsgId universalId) const {
...
FullMsgId Provider::computeFullId(UniversalMsgId universalId) const {
	Expects(universalId != 0);

	return (universalId > 0) ? FullMsgId(_peer->id, universalId)
							 : FullMsgId((_migrated ? _migrated : _peer.get())->id, ServerMaxMsgId + universalId);
}

BaseLayout *Provider::getLayout(UniversalMsgId universalId, not_null<Overview::Layout::Delegate *> delegate) {
	auto it = _layouts.find(universalId);
	if (it == _layouts.end()) {
		if (auto layout = createLayout(universalId, delegate, _type)) {
			layout->initDimensions();
			it = _layouts.emplace(universalId, std::move(layout)).first;
		} else {
			return nullptr;
		}
	}
...
std::unique_ptr<BaseLayout>
Provider::createLayout(UniversalMsgId universalId, not_null<Overview::Layout::Delegate *> delegate, Type type) {
	const auto item = _controller->session().data().message(computeFullId(universalId));
	if (!item) {
		return nullptr;
	}
	...
	switch (type) {
		...
		case Type::File:
			if (const auto file = getFile()) {
				return std::make_unique<Document>(delegate, item, DocumentFields{.document = file}, songSt);
			}
			return nullptr;
		case Type::MusicFile:
			if (const auto file = getFile()) {
		...
		case Type::RoundVoiceFile:
			...
			return nullptr;
		case Type::Link: return std::make_unique<Link>(delegate, item, item->media());
		case Type::RoundFile: return nullptr;
	}
	Unexpected("Type in ListWidget::createLayout()");
}

ListItemSelectionData Provider::computeSelectionData(not_null<const HistoryItem *> item, TextSelection selection) {
...
bool Provider::allowSaveFileAs(not_null<const HistoryItem *> item, not_null<DocumentData *> document) {
	return item->allowsForward();
}

QString Provider::showInFolderPath(not_null<const HistoryItem *> item, not_null<DocumentData *> document) {
	return document->filepath(true);
}

void Provider::applyDragSelection(ListSelectedMap &selected,
	...
	for (auto &layoutItem : _layouts) {
		...
		if (universalId <= fromId && universalId > tillId) {
			...
			ChangeItemSelection(selected, item, computeSelectionData(item, FullSelection));
		}
	}
}

int64 Provider::scrollTopStatePosition(not_null<HistoryItem *> item) { return GetUniversalId(item).bare; }
...
void Provider::saveState(not_null<Memento *> memento, ListScrollTopState scrollState) {
...
void Provider::restoreState(not_null<Memento *> memento, Fn<void(ListScrollTopState)> restoreScrollState) {
