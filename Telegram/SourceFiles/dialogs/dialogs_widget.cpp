...
[[nodiscard]] QImage UpdateIcon() {
	...
	auto result = QImage(Size(iconSize) * style::DevicePixelRatio(), QImage::Format_ARGB32_Premultiplied);
...
Widget::BottomButton::BottomButton(QWidget *parent,
	...
	if (_hasTextIcon) {
		rpl::single(rpl::empty_value()) | rpl::then(style::PaletteChanged()) |
			rpl::start_with_next([this] { _textIcon = UpdateIcon(); }, lifetime());
	}
}
...
Widget::Widget(QWidget *parent, not_null<Window::SessionController *> controller, Layout layout)
	: Window::AbstractSectionWidget(parent, controller, nullptr), _api(&controller->session().mtp()),
	  ...
	  _stories((_layout != Layout::Child)
				   ...
				   : nullptr),
	  _searchTimer([=] { search(); }), _singleMessageSearch(&controller->session()) {
	...
	_inner = _scroll->setOwnedWidget(object_ptr<InnerWidget>(
		this, controller, rpl::combine(_childListPeerId.value(), _childListShown.value(), makeChildListShown)));
	_scrollToTop->raise();
	_lockUnlock->toggle(false, anim::type::instant);

	_inner->updated() | rpl::start_with_next([=] { listScrollUpdated(); }, lifetime());

	rpl::combine(session().api().dialogsLoadMayBlockByDate(), session().api().dialogsLoadBlockedByDate()) |
		rpl::start_with_next([=](bool mayBlock, bool isBlocked) { refreshLoadMoreButton(mayBlock, isBlocked); },
							 lifetime());
	...
	session().changes().historyUpdates(Data::HistoryUpdate::Flag::MessageSent) |
		...
		rpl::start_with_next([=](const Data::HistoryUpdate &update) { jumpToTop(true); }, lifetime());
	...
	_inner->searchRequests() |
		rpl::start_with_next([=](SearchRequestDelay delay) { searchRequested(delay); }, lifetime());
	_inner->completeHashtagRequests() |
		rpl::start_with_next([=](const QString &tag) { completeHashtag(tag); }, lifetime());
	_inner->refreshHashtagsRequests() | rpl::start_with_next([=] { searchCursorMoved(); }, lifetime());
	...
	_inner->cancelSearchFromRequests() |
		rpl::start_with_next(
			...
			lifetime());
	_inner->changeSearchFromRequests() | rpl::start_with_next([=] { showSearchFrom(); }, lifetime());
	_inner->chosenRow() | rpl::start_with_next([=](const ChosenRow &row) { chosenRow(row); }, lifetime());
	...
	_scroll->geometryChanged() |
		rpl::start_with_next(crl::guard(_inner, [=] { _inner->parentGeometryChanged(); }), lifetime());
	_scroll->scrolls() | rpl::start_with_next([=] { listScrollUpdated(); }, lifetime());

	session().data().chatsListChanges() |
		rpl::filter([=](Data::Folder *folder) { return (folder == _inner->shownFolder()); }) |
		rpl::start_with_next([=] { Ui::PostponeCall(this, [=] { listScrollUpdated(); }); }, lifetime());

	setAttribute(Qt::WA_InputMethodEnabled);
	controller->widget()->imeCompositionStarts() | rpl::filter([=] { return redirectImeToSearch(); }) |
		rpl::start_with_next([=] { _search->setFocusFast(); }, lifetime());

	_search->changes() |
		rpl::start_with_next([=] { crl::on_main(this, [=] { applySearchUpdate(); }); }, _search->lifetime());

	_search->submits() | rpl::start_with_next([=] { submit(); }, _search->lifetime());
	...
	if (!Core::UpdaterDisabled()) {
		...
		rpl::merge(rpl::single(rpl::empty), checker.isLatest(), checker.failed(), checker.ready()) |
			rpl::start_with_next([=] { checkUpdateStatus(); }, lifetime());
	}
	...
	rpl::single(rpl::empty) | rpl::then(session().domain().local().localPasscodeChanged()) |
		rpl::start_with_next([=] { updateLockUnlockVisibility(); }, lifetime());
	...
	_inner->listBottomReached() | rpl::start_with_next([=] { loadMoreBlockedByDate(); }, lifetime());
	...
	style::PaletteChanged() | rpl::start_with_next([=] { _scroll->setOverscrollBg(overscrollBg()); }, lifetime());
	...
	if (_layout != Layout::Child) {
		...
		controller->openedFolder().changes() |
			rpl::start_with_next([=](Data::Folder *folder) { changeOpenedFolder(folder, anim::type::normal); },
								 lifetime());
...
void Widget::chosenRow(const ChosenRow &row) {
	...
	const auto topicJump = history ? history->peer->forumTopicFor(row.message.fullId.msg) : nullptr;
	if (topicJump) {
	...
	} else if (history && row.userpicClick && (row.message.fullId.msg == ShowAtUnreadMsgId) &&
		...
		return;
			   ...
			   (!controller()->adaptive().isOneColumn() || !history->peer->forum()->channel()->viewForumAsMessages())) {
		const auto forum = history->peer->forum();
		if (controller()->shownForum().current() == forum) {
			controller()->closeForum();
		} else if (row.newWindow) {
			controller()->showInNewWindow(Window::SeparateId(Window::SeparateType::Forum, history));
		} else {
			controller()->showForum(forum, Window::SectionShow().withChildColumn());
		...
		const auto showAtMsgId =
			controller()->uniqueChatsInSearchResults() ? ShowAtUnreadMsgId : row.message.fullId.msg;
		auto params = Window::SectionShow(Window::SectionShow::Way::ClearStack);
		params.highlightPart.text = _searchState.query;
		if (!params.highlightPart.empty()) {
			params.highlightPartOffsetHint = kSearchQueryOffsetHint;
		...
		rpl::start_with_next(
			[=](FilterId id)
			{
				...
				_moreChatsBar->closeClicks() |
					rpl::start_with_next([=] { Api::ProcessFilterUpdate(controller(), id, {}); },
										 _moreChatsBar->lifetime());
				...
				_moreChatsBar->heightValue() |
					rpl::start_with_next([=] { updateControlsGeometry(); }, _moreChatsBar->lifetime());
			},
			lifetime());
}
			...
			[=](Ui::DownloadBarContent &&content)
			{
				...
				if (create) {
					_downloadBar->heightValue() |
						rpl::start_with_next([=] { updateControlsGeometry(); }, _downloadBar->lifetime());
...
void Widget::setupStories() {
	...
	_stories->verticalScrollEvents() |
		rpl::start_with_next([=](not_null<QWheelEvent *> e) { _scroll->viewportEvent(e); }, _stories->lifetime());
	...
	_stories->collapsedGeometryChanged() | rpl::start_with_next([=] { updateLockUnlockPosition(); }, lifetime());

	_stories->clicks() |
		rpl::start_with_next([=](uint64 id) { controller()->openPeerStories(PeerId(int64(id)), currentSource()); },
							 lifetime());

	_stories->showMenuRequests() |
		rpl::start_with_next([=](const Stories::ShowMenuRequest &request) { FillSourceMenu(controller(), request); },
							 lifetime());

	_stories->loadMoreRequests() |
		rpl::start_with_next([=] { session().data().stories().loadMore(currentSource()); }, lifetime());
	...
	_stories->emptyValue() | rpl::skip(1) | rpl::start_with_next([=] { updateStoriesVisibility(); }, lifetime());

	_stories->widthValue() | rpl::start_with_next([=] { updateLockUnlockPosition(); }, lifetime());
}
...
void Widget::updateSuggestions(anim::type animated) {
	...
	if (!suggest && _suggestions) {
		...
		_suggestions->clearSearchQueryRequests() |
			rpl::start_with_next([=] { setSearchQuery(QString()); }, _suggestions->lifetime());
		...
		_suggestions->popularAppChosen() |
			rpl::start_with_next([=](not_null<PeerData *> peer) { controller()->showPeerInfo(peer); },
								 _suggestions->lifetime());
...
void Widget::refreshTopBars() {
	if (_openedFolder || _openedForum) {
		if (!_subsectionTopBar) {
			...
			_subsectionTopBar->searchCancelled() |
				rpl::start_with_next([=] { escape(); }, _subsectionTopBar->lifetime());
			_subsectionTopBar->searchSubmitted() |
				rpl::start_with_next([=] { submit(); }, _subsectionTopBar->lifetime());
			_subsectionTopBar->searchQuery() |
				rpl::start_with_next([=](QString query) { applySearchUpdate(); }, _subsectionTopBar->lifetime());
			_subsectionTopBar->jumpToDateRequest() |
				rpl::start_with_next([=] { showCalendar(); }, _subsectionTopBar->lifetime());
			_subsectionTopBar->chooseFromUserRequest() |
				rpl::start_with_next([=] { showSearchFrom(); }, _subsectionTopBar->lifetime());
			updateControlsGeometry();
		}
	...
	if (_openedForum) {
		...
		_forumReportBar = std::make_unique<HistoryView::ContactStatus>(controller(), this, channel, true);
		_forumRequestsBar = std::make_unique<Ui::RequestsBar>(
			this, HistoryView::RequestsBarContentByPeer(channel, st::historyRequestsUserpics.size, true));
		_forumGroupCallBar = std::make_unique<Ui::GroupCallBar>(
			this,
			HistoryView::GroupCallBarContentByPeer(channel, st::historyGroupCallUserpics.size, true),
			Core::App().appDeactivatedValue());
		_forumTopShadow = std::make_unique<Ui::PlainShadow>(this);

		_forumRequestsBar->barClicks() |
			rpl::start_with_next([=] { RequestsBoxController::Start(controller(), channel); },
		...
		rpl::combine(
			...
			rpl::start_with_next([=] { updateControlsGeometry(); }, _forumRequestsBar->lifetime());
	} else {
...
void Widget::raiseWithTooltip() {
	raise();
	if (_stories) {
		Ui::PostponeCall(this, [=] { _stories->raiseTooltip(); });
	}
}
...
void Widget::updateStoriesVisibility() {
	...
	const auto hidden = (_showAnimation != nullptr) || _openedForum || !_widthAnimationCache.isNull() || _childList ||
		_searchHasFocus || _searchSuggestionsLocked || !_searchState.query.isEmpty() || _searchState.inChat ||
		_stories->empty();
...
bool Widget::search(bool inCache, SearchRequestDelay delay) {
	...
	if (trimmed.isEmpty() && !fromPeer && inTags.empty()) {
		...
		if (inPeer) {
			...
			const auto savedPeer = sublist ? sublist->peer().get() : nullptr;
	...
	if (searchForPeersRequired(peerQuery)) {
		if (inCache) {
			...
			_peerSearchRequest =
				_api.request(MTPcontacts_Search(MTP_string(_peerSearchQuery), MTP_int(SearchPeopleLimit)))
					.done([=](const MTPcontacts_Found &result, mtpRequestId requestId)
						  { peerSearchReceived(result, requestId); })
					.fail([=](const MTP::Error &error, mtpRequestId requestId) { peerSearchFailed(error, requestId); })
					.send();
...
void Widget::searchTopics() {
	...
	_api.request(base::take(_topicSearchRequest)).cancel();
	_topicSearchRequest =
		_api.request(MTPchannels_GetForumTopics(MTP_flags(MTPchannels_GetForumTopics::Flag::f_q),
		...
		_historiesRequest = histories.sendRequest(
			...
			[=](Fn<void()> finish)
			{
				...
				_migratedProcess.requestId =
					session()
						.api()
						.request(MTPmessages_Search(flags,
...
void Widget::requestPublicPosts(bool fromStart) {
	...
	_postsProcess.requestId =
		session()
			.api()
			.request(MTPchannels_SearchPosts(MTP_string(_searchState.query.trimmed().mid(1)),
...
void Widget::peerSearchReceived(const MTPcontacts_Found &result, mtpRequestId requestId) {
	const auto state = _inner->state();
...
void Widget::searchApplyEmpty(SearchRequestType type, not_null<SearchProcessState *> process) {
	process->full = true;
	searchReceived(
		type, MTP_messages_messages(MTP_vector<MTPMessage>(), MTP_vector<MTPChat>(), MTP_vector<MTPUser>()), process);
}
...
void Widget::peerSearchApplyEmpty(mtpRequestId id) {
	...
	peerSearchReceived(
		MTP_contacts_found(
			MTP_vector<MTPPeer>(0), MTP_vector<MTPPeer>(0), MTP_vector<MTPChat>(0), MTP_vector<MTPUser>(0)),
		id);
}
...
void Widget::updateForceDisplayWide() {
	...
	controller()->setChatsForceDisplayWide(
		...
		!_searchState.query.isEmpty() || _searchState.inChat);
}
...
bool Widget::applySearchState(SearchState state) {
	...
	const auto migrateFrom = (peer && !topic) ? peer->migrateFrom() : nullptr;
...
void Widget::updateLockUnlockVisibility(anim::type animated) {
	...
	const auto hidden = !session().domain().local().hasLocalPasscode() || _showAnimation || _openedForum ||
		!_widthAnimationCache.isNull() || _childList || _searchHasFocus || _searchSuggestionsLocked ||
		_searchState.inChat || !_searchState.query.isEmpty();
	if (_lockUnlock->toggled() == hidden) {
		const auto stories = _stories && !_stories->empty();
		_lockUnlock->toggle(!hidden, stories ? anim::type::instant : animated);
		if (!hidden) {
			updateLockUnlockPosition();
		}
		updateControlsGeometry();
...
void Widget::updateControlsGeometry() {
	...
	_updateScrollGeometryCached = [=]
	{
		const auto moreChatsBarTop = expandedStoriesTop + ((!_stories || _stories->isHidden()) ? 0 : _aboveScrollAdded);
		if (_moreChatsBar) {
		...
		const auto scrollTop = chatFiltersTop +
			((_chatFilters && _searchState.query.isEmpty()) ? (_chatFilters->height() * (1. - narrowRatio)) : 0);
...
PeerData *Widget::searchInPeer() const {
	return (_searchState.tab == ChatSearchTab::MyMessages || _searchState.tab == ChatSearchTab::PublicPosts) ? nullptr
		...
		: _searchState.inChat.sublist() ? session().user().get()
