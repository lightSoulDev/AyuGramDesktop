...
[[nodiscard]] InnerWidget::ChatsFilterTagsKey SerializeFilterTagsKey(FilterId filterId, uint8 more, bool active) {
	return (filterId & 0xFFFFFFFF) | (static_cast<int64_t>(more) << 32) | (static_cast<int64_t>(active) << 40);
}
...
[[nodiscard]] int PinnedDialogsCount(FilterId filterId, not_null<Dialogs::IndexedList *> list) {
	auto result = 0;
	for (const auto &row : *list) {
		if (row->entry()->fixedOnTopIndex()) {
			continue;
...
MakeSearchEmpty(QWidget *parent, SearchState state, Fn<void()> resetChatTypeFilter) {
	...
	const auto fromPeer =
		(state.tab == ChatSearchTab::MyMessages || state.tab == ChatSearchTab::PublicPosts || !state.inChat.peer() ||
		 !(state.inChat.peer()->isChat() || state.inChat.peer()->isMegagroup()))
		? nullptr
		: state.fromPeer;
	const auto waiting = trimmed.isEmpty() && state.tags.empty() && !fromPeer;
	const auto suggestAllChats =
		!waiting && state.tab == ChatSearchTab::MyMessages && state.filter != ChatTypeFilter::All;
	const auto icon = waiting ? SearchEmptyIcon::Search : SearchEmptyIcon::NoResults;
	...
	if (waiting) {
		...
		text.append(tr::lng_search_tab_no_results(tr::now, Ui::Text::Bold));
		if (!trimmed.isEmpty()) {
			const auto preview = (trimmed.size() > kQueryPreviewLimit + 3)
				? (trimmed.mid(0, kQueryPreviewLimit) + Ui::kQEllipsis)
				: trimmed;
			text.append("\n").append(
				tr::lng_search_tab_no_results_text(tr::now, lt_query, trimmed.mid(0, kQueryPreviewLimit)));
			if (suggestAllChats) {
				text.append("\n\n").append(Ui::Text::Link(tr::lng_search_tab_try_in_all(tr::now)));
			} else if (hashtag) {
				text.append("\n").append(tr::lng_search_tab_no_results_retry(tr::now));
			}
		}
	}
	auto result = object_ptr<SearchEmpty>(parent, icon, rpl::single(std::move(text)));
	if (suggestAllChats) {
		result->handlerActivated() | rpl::start_with_next(resetChatTypeFilter, result->lifetime());
	}
	result->show();
...
[[nodiscard]] QString ChatTypeFilterLabel(ChatTypeFilter filter) {
	switch (filter) {
		case ChatTypeFilter::All: return tr::lng_search_filter_all(tr::now);
		case ChatTypeFilter::Private: return tr::lng_search_filter_private(tr::now);
		case ChatTypeFilter::Groups: return tr::lng_search_filter_group(tr::now);
		case ChatTypeFilter::Channels: return tr::lng_search_filter_channel(tr::now);
	}
...
InnerWidget::InnerWidget(QWidget *parent,
	...
	: RpWidget(parent), _controller(controller), _shownList(controller->session().data().chatsList()->indexed()),
	  _st(&st::defaultDialogRow),
	  _pinnedShiftAnimation([=](crl::time now) { return pinnedShiftAnimationCallback(now); }),
	...
	session().data().dialogsRowReplacements() |
		rpl::start_with_next([=](Data::Session::DialogsRowReplacement r) { dialogRowReplaced(r.old, r.now); },
							 lifetime());
		...
		rpl::start_with_next(
			[=](const Data::SendActionManager::AnimationUpdate &update)
			{
				const auto updateRect = Ui::RowPainter::SendActionAnimationRect(
					_st, update.left, update.width, update.height, width(), update.textUpdated);
				updateDialogRow(RowDescriptor(update.thread, FullMsgId()),
								updateRect,
	...
	session().data().sendActionManager().speakingAnimationUpdated() |
		rpl::start_with_next([=](not_null<History *> history) { repaintDialogRowCornerStatus(history); }, lifetime());

	setupOnlineStatusCheck();

	rpl::merge(session().data().chatsListChanges(), session().data().chatsListLoadedEvents()) |
		rpl::filter([=](Data::Folder *folder)
					{ return !_savedSublists && !_openedForum && (folder == _openedFolder); }) |
		rpl::start_with_next([=] { refresh(); }, lifetime());

	rpl::merge(session().settings().archiveCollapsedChanges() | rpl::map_to(false),
	...
	session().data().chatsFilters().tagsEnabledValue() | rpl::distinct_until_changed() |
		rpl::start_with_next(
			[=](bool tags)
			{
	...
	session().settings().archiveInMainMenuChanges() | rpl::start_with_next([=] { refresh(); }, lifetime());

	session().changes().historyUpdates(Data::HistoryUpdate::Flag::IsPinned | Data::HistoryUpdate::Flag::ChatOccupied) |
		rpl::start_with_next(
	...
	session().changes().peerUpdates(UpdateFlag::Name | UpdateFlag::Photo | UpdateFlag::IsContact |
									UpdateFlag::FullInfo | UpdateFlag::EmojiStatus) |
		rpl::start_with_next(
			[=](const Data::PeerUpdate &update)
			{
				if (update.flags &
					(UpdateFlag::Name | UpdateFlag::Photo | UpdateFlag::FullInfo | UpdateFlag::EmojiStatus)) {
	...
	session().changes().messageUpdates(Data::MessageUpdate::Flag::DialogRowRefresh) |
		rpl::start_with_next([=](const Data::MessageUpdate &update)
							 { refreshDialogRow({update.item->history(), update.item->fullId()}); },
							 lifetime());

	session().changes().entryUpdates(Data::EntryUpdate::Flag::Repaint | Data::EntryUpdate::Flag::Height) |
		rpl::start_with_next(
			[=](const Data::EntryUpdate &update)
			{
				...
				const auto repaintId = (_state == WidgetState::Default) ? _filterId : 0;
				if (const auto links = entry->chatListLinks(repaintId)) {
					repaintDialogRow(repaintId, links->main);
				}
				if (session().supportMode() && !session().settings().supportAllSearchResults()) {
	...
	_controller->activeChatsFilter() |
		rpl::start_with_next([=](FilterId filterId) { switchToFilter(filterId); }, lifetime());

	_controller->window().widget()->globalForceClicks() |
		rpl::start_with_next([=](QPoint globalPosition) { processGlobalForceClick(globalPosition); }, lifetime());
...
void InnerWidget::refreshWithCollapsedRows(bool toTop) {
	...
	const auto archive = !_shownList->empty() ? _shownList->begin()->get()->folder() : nullptr;
...
int InnerWidget::skipTopHeight() const {
	return (_skipTopDialog && !_shownList->empty()) ? _shownList->begin()->get()->height() : 0;
}

int InnerWidget::collapsedRowsOffset() const { return 0; }

int InnerWidget::dialogsOffset() const {
	return collapsedRowsOffset() + (_collapsedRows.size() * st::dialogsImportantBarHeight) - skipTopHeight();
}
...
int InnerWidget::shownHeight(int till) const {
	return !till								  ? 0
		: (till > 0 && till < _shownList->size()) ? (_shownList->begin() + till)->get()->top()
												  : _shownList->height();
}

int InnerWidget::pinnedOffset() const { return dialogsOffset() + shownHeight(fixedOnTopCount()); }

int InnerWidget::hashtagsOffset() const { return searchInChatOffset() + searchInChatSkip(); }

int InnerWidget::filteredOffset() const { return hashtagsOffset() + (_hashtagResults.size() * st::mentionHeight); }

int InnerWidget::filteredIndex(int y) const {
	return ranges::lower_bound(_filterResults, y, ranges::less(), &FilterResult::bottom) - begin(_filterResults);
}
...
int InnerWidget::peerSearchOffset() const { return filteredOffset() + filteredHeight() + st::searchedBarHeight; }

int InnerWidget::searchInChatOffset() const { return (_searchTags ? _searchTags->height() : 0); }

int InnerWidget::searchInChatSkip() const { return _searchIn ? _searchIn->height() : 0; }

int InnerWidget::previewOffset() const {
	auto result = peerSearchOffset();
	if (!_peerSearchResults.empty()) {
		result += (_peerSearchResults.size() * st::dialogsRowHeight) + st::searchedBarHeight;
	}
	return result;
}
...
int InnerWidget::searchedOffset() const {
	...
	if (!_previewResults.empty()) {
		result += (_previewResults.size() * st::dialogsRowHeight) + st::searchedBarHeight;
	}
	return result;
}
...
void InnerWidget::changeOpenedForum(Data::Forum *forum) {
	...
	if (forum) {
		rpl::merge(forum->chatsListChanges(), forum->chatsListLoadedEvents()) |
			rpl::start_with_next([=] { refresh(); }, _openedForumLifetime);
	}
...
void InnerWidget::paintEvent(QPaintEvent *e) {
	...
	p.setInactive(_controller->isGifPausedAtLeastFor(Window::GifPauseReason::Any));
	if (!_savedSublists && _controller->contentOverlapped(this, e)) {
		return;
	}
	const auto activeEntry = _controller->activeChatEntryCurrent();
	const auto videoPaused = _controller->isGifPausedAtLeastFor(Window::GifPauseReason::Any);
	...
	const auto paintRow = [&](not_null<Row *> row, bool selected, bool mayBeActive)
	{
		...
		const auto expanding = forum && (key.history()->peer->id == childListShown.peerId);
		...
		if (context.narrow) {
			...
			const auto a = active;
			context.st = forum ? &st::taggedForumDialogRow : &st::taggedDialogRow;
			auto availableWidth =
				context.width - context.st->padding.right() - st::dialogsUnreadPadding - context.st->nameLeft;
			auto more = uint8(0);
			const auto &list = session().data().chatsFilters().list();
			for (const auto &filter : list) {
				if (!row->entry()->inChatList(filter.id()) || (filter.id() == context.filter)) {
					continue;
				}
		...
		context.selected = _menuRow.key ? (row->key() == _menuRow.key)
			: _chatPreviewRow.key		? (row->key() == _chatPreviewRow.key)
										: selected;
		context.topicJumpSelected = selected && _selectedTopicJump && (!_pressed || _pressedTopicJump);
	...
	if (_state == WidgetState::Default) {
		...
		const auto selected =
			isPressed() ? (_pressed ? _pressed->key() : Key()) : (_selected ? _selected->key() : Key());
		if (shownBottom) {
			...
			const auto reorderingIndex = reorderingPinned ? (promoted + _aboveIndex) : -1;
			const auto reorderingRow = (reorderingIndex >= 0 && reorderingIndex < list.size())
			...
			const auto paintDialog = [&](not_null<Row *> row)
			{
				...
				const auto xadd = 0;
				const auto yadd = base::in_range(pinned, 0, count) ? qRound(_pinnedRows[pinned].yadd.current()) : 0;
				if (xadd || yadd) {
		...
		if (!_hashtagResults.empty()) {
			...
			if (from < _hashtagResults.size()) {
				...
				auto htagwidth = fullWidth - htagleft - st::defaultDialogRow.padding.right();
		...
		if (!_filterResults.empty()) {
			...
			auto to = std::min(filteredIndex(r.y() + r.height() - skip) + 1, int(_filterResults.size()));
			const auto height = filteredHeight(from);
			p.translate(0, height);
			for (; from < to; ++from) {
				const auto selected = isPressed() ? (from == _filteredPressed) : (from == _filteredSelected);
		...
		if (!_peerSearchResults.empty()) {
			...
			p.drawTextLeft(st::searchedBarPosition.x(),
						   st::searchedBarPosition.y(),
						   width(),
						   tr::lng_search_global_results(tr::now));
			...
			if (from < _peerSearchResults.size()) {
				...
				for (; from < to; ++from) {
					...
					const auto active = !activeEntry.fullId && activePeer &&
						((peer == activePeer) || (peer->migrateTo() == activePeer));
					const auto selected = (from == (isPressed() ? _peerSearchPressed : _peerSearchSelected));
					paintPeerSearchResult(p,
										  result.get(),
										  {
		...
		if (!_previewResults.empty()) {
			...
			p.drawTextLeft(st::searchedBarPosition.x(), st::searchedBarPosition.y(), width(), text);
			const auto moreFont =
				(_selectedMorePosts || _pressedMorePosts) ? st::searchedBarFont->underline() : st::searchedBarFont;
			{
			...
			if (from < _previewResults.size()) {
				for (; from < to; ++from) {
					...
					const auto selected = _menuRow.key ? isSearchResultActive(result.get(), _menuRow)
						: _chatPreviewRow.key		   ? isSearchResultActive(result.get(), _chatPreviewRow)
													   : (from == (isPressed() ? _previewPressed : _previewSelected));
					Ui::RowPainter::Paint(p,
										  result.get(),
										  {
											  .st = _st,
											  .folder = _openedFolder,
		...
		if (!_searchResults.empty()) {
			const auto text = showUnreadInSearchResults
				? u"Search results"_q
				: tr::lng_search_found_results(tr::now, lt_count, _searchedMigratedCount + _searchedCount);
			p.fillRect(0, 0, fullWidth, st::searchedBarHeight, st::searchedBarBg);
			p.setFont(st::searchedBarFont);
			p.setPen(st::searchedBarFg);
			p.drawTextLeft(st::searchedBarPosition.x(), st::searchedBarPosition.y(), width(), text);
			const auto filterOver = _selectedChatTypeFilter || _pressedChatTypeFilter;
			const auto filterFont = filterOver ? st::searchedBarFont->underline() : st::searchedBarFont;
			if (hasChatTypeFilter()) {
				...
				p.drawTextLeft((width() - st::searchedBarPosition.x() - _chatTypeFilterWidth),
							   st::searchedBarPosition.y(),
							   width(),
							   text);
			}
			...
			if (from < _searchResults.size()) {
				for (; from < to; ++from) {
					...
					const auto selected = _menuRow.key ? isSearchResultActive(result.get(), _menuRow)
						: _chatPreviewRow.key		   ? isSearchResultActive(result.get(), _chatPreviewRow)
													   : (from == (isPressed() ? _searchedPressed : _searchedSelected));
					Ui::RowPainter::Paint(p,
										  result.get(),
										  {
											  .st = _st,
...
[[nodiscard]] RightButton *InnerWidget::maybeCacheRightButton(Row *row) {
	if (const auto user = MaybeBotWithApp(row)) {
		...
		if (it == _rightButtons.end()) {
			...
			const auto size =
				QSize(rightButton.text.maxWidth() + rightButton.text.minHeight(), st::dialogRowOpenBotHeight);
			const auto generateBg = [&](const style::color &c)
			{
				auto bg = QImage(style::DevicePixelRatio() * size, QImage::Format_ARGB32_Premultiplied);
			...
			return &(_rightButtons.emplace(user->id, std::move(rightButton)).first->second);
		} else {
			return &(it->second);
		}
	}
...
Ui::VideoUserpic *InnerWidget::validateVideoUserpic(not_null<History *> history) {
	...
	return _videoUserpics.emplace(peer, std::make_unique<Ui::VideoUserpic>(peer, repaint)).first->second.get();
}
...
void InnerWidget::paintCollapsedRows(Painter &p, QRect clip) const {
	...
	for (const auto &row : _collapsedRows) {
		...
		const auto selected = (index == _collapsedSelected) || (index == _collapsedPressed);
		paintCollapsedRow(p, row.get(), selected);
	}
}

void InnerWidget::paintCollapsedRow(Painter &p, not_null<const CollapsedRow *> row, bool selected) const {
...
bool InnerWidget::isRowActive(not_null<Row *> row, const RowDescriptor &entry) const {
	const auto key = row->key();
	return (entry.key == key) || (entry.key.sublist() && key.peer() && key.peer()->isSelf());
}

bool InnerWidget::isSearchResultActive(not_null<FakeRow *> result, const RowDescriptor &entry) const {
	const auto item = result->item();
	const auto peer = item->history()->peer;
	return (item->fullId() == entry.fullId) ||
		(peer->migrateTo() && (peer->migrateTo()->id == entry.fullId.peer) && (item->id == -entry.fullId.msg)) ||
		(uniqueSearchResults() && peer == entry.key.peer());
}

void InnerWidget::paintPeerSearchResult(Painter &p,
										not_null<const PeerSearchResult *> result,
										const Ui::PaintContext &context) {
	QRect fullRect(0, 0, context.width, st::dialogsRowHeight);
	p.fillRect(fullRect, (context.active ? st::dialogsBgActive : context.selected ? st::dialogsBgOver : currentBg()));
	if (!context.active) {
	...
	userpicPeer->paintUserpicLeft(p,
	...
	if (result->name.isEmpty()) {
		result->name.setText(st::semiboldTextStyle, peer->name(), Ui::NameTextOptions());
	}

	if (const auto info = peer->botVerifyDetails()) {
		if (!result->badge.ready(info)) {
			result->badge.set(info, peer->owner().customEmojiManager().factory(), [=] { updateSearchResult(peer); });
		}
		...
		rectForName.setLeft(rectForName.left() + chatTypeIcon->width() + st::dialogsChatTypeSkip);
	}
	const auto badgeWidth = result->badge.drawGetWidth(
		p,
		{
			...
			.exteraOfficial = &ThreeStateIcon(st::dialogsExteraOfficialIcon, context.active, context.selected),
			.exteraSupporter = &ThreeStateIcon(st::dialogsExteraSupporterIcon, context.active, context.selected),
			.premium = &ThreeStateIcon(st::dialogsPremiumIcon, context.active, context.selected),
	...
	if (!context.active && username.startsWith(_peerSearchQuery, Qt::CaseInsensitive)) {
		...
		if (w >= tr.width()) {
			...
			p.drawText(
				tr.left(), tr.top() + st::dialogsTextFont->ascent, st::dialogsTextFont->elided(first, tr.width()));
		} else {
			p.setPen(st::dialogsTextFgService);
			p.drawText(tr.left(), tr.top() + st::dialogsTextFont->ascent, first);
			p.setPen(st::dialogsTextFg);
			p.drawText(tr.left() + w,
		...
		p.drawText(
			tr.left(), tr.top() + st::dialogsTextFont->ascent, st::dialogsTextFont->elided('@' + username, tr.width()));
	}
...
QBrush InnerWidget::currentBg() const {
	return anim::brush(st::dialogsBg, st::dialogsBgOver, _childListShown.current().shown);
}

void InnerWidget::paintSearchTags(Painter &p, const Ui::PaintContext &context) const {
...
void InnerWidget::showPeerMenu() {
	...
	const auto pos = QPoint(width() - padding.right(), _selected->top() + _selected->height() + padding.bottom());
	auto event = QContextMenuEvent(QContextMenuEvent::Keyboard, pos, mapToGlobal(pos));
...
bool InnerWidget::lookupIsInBotAppButton(Row *row, QPoint localPosition) {
	if (const auto user = MaybeBotWithApp(row)) {
		...
		if (it != _rightButtons.end()) {
			...
			const auto r =
				QRect(width() - s.width() - st::dialogRowOpenBotRight, st::dialogRowOpenBotTop, s.width(), s.height());
...
void InnerWidget::selectByMouse(QPoint globalPosition) {
	...
	const auto inTags =
		_searchTags && QRect(tagBase, QSize(width() - 2 * _searchTagsLeft, _searchTags->height())).contains(local);
	...
	if (_state == WidgetState::Default) {
		...
		const auto collapsedSelected = (mouseY >= 0 && mouseY < _collapsedRows.size() * st::dialogsImportantBarHeight)
			? (mouseY / st::dialogsImportantBarHeight)
			: -1;
		const auto selected = (collapsedSelected >= 0) ? nullptr
			: (mouseY >= offset)					   ? _shownList->rowAtY(mouseY - offset)
													   : nullptr;
		const auto mappedY = selected ? mouseY - offset - selected->top() : 0;
		const auto selectedTopicJump = selected && selected->lookupIsInTopicJump(local.x(), mappedY);
		const auto selectedBotApp = selected && lookupIsInBotAppButton(selected, QPoint(local.x(), mappedY));
		if (_collapsedSelected != collapsedSelected || _selected != selected ||
			_selectedTopicJump != selectedTopicJump || _selectedBotApp != selectedBotApp) {
			updateSelectedRow();
			_selected = selected;
			...
			updateSelectedRow();
			setCursor((_selected || _collapsedSelected >= 0) ? style::cur_pointer : style::cur_default);
		}
	} else if (_state == WidgetState::Filtered) {
		...
		if (!_filterResults.empty()) {
			auto skip = filteredOffset();
			auto filteredSelected = (mouseY >= skip) ? filteredIndex(mouseY - skip) : -1;
			if (filteredSelected < 0 || filteredSelected >= _filterResults.size()) {
				filteredSelected = -1;
			}
			const auto mappedY = (filteredSelected >= 0) ? mouseY - skip - _filterResults[filteredSelected].top : 0;
			const auto selectedTopicJump = (filteredSelected >= 0) &&
				_filterResults[filteredSelected].row->lookupIsInTopicJump(local.x(), mappedY);
			const auto selectedBotApp = (filteredSelected >= 0) &&
				lookupIsInBotAppButton(_filterResults[filteredSelected].row, QPoint(local.x(), mappedY));
			if (_filteredSelected != filteredSelected || _selectedTopicJump != selectedTopicJump ||
				_selectedBotApp != selectedBotApp) {
				updateSelectedRow();
		...
		if (!_previewResults.empty()) {
			...
			if (mouseY <= skip && mouseY >= from) {
				const auto left = width() - _morePostsWidth - 2 * st::searchedBarPosition.x();
		...
		if (!_searchResults.empty()) {
			...
			if (hasChatTypeFilter() && mouseY <= skip && mouseY >= from) {
				const auto left = width() - _chatTypeFilterWidth - 2 * st::searchedBarPosition.x();
...
RowDescriptor InnerWidget::computeChatPreviewRow() const {
	...
	if (const auto peer = result.key.peer()) {
		const auto topicId = _pressedTopicJump ? _pressedTopicJumpRootId : 0;
...
void InnerWidget::processGlobalForceClick(QPoint globalPosition) {
	...
	if (_pressButton == Qt::LeftButton && parent->rect().contains(parent->mapFromGlobal(globalPosition))) {
		showChatPreview();
	}
}
...
void InnerWidget::mousePressEvent(QMouseEvent *e) {
	...
	if (base::in_range(_collapsedSelected, 0, _collapsedRows.size())) {
		...
		row->addRipple(e->pos(),
					   QSize(width(), st::dialogsImportantBarHeight),
		...
		if (addBotAppRipple(origin, updateCallback)) {
			...
			row->addRipple(origin, QSize(width(), _pressed->height()), updateCallback);
		}
		_dragStart = e->pos();
		...
		row->addRipple(origin,
					   QSize(width(), st::mentionHeight),
		...
		row->addRipple(e->pos() - QPoint(0, searchedOffset() + _searchedPressed * _st->height),
					   QSize(width(), _st->height),
					   row->repaint());
	}
	ClickHandler::pressed();
	if (anim::Disabled() && !_chatPreviewScheduled && (!_pressed || !_pressed->entry()->isPinnedDialog(_filterId))) {
...
bool InnerWidget::addBotAppRipple(QPoint origin, Fn<void()> updateCallback) {
	...
	const auto size = _pressedBotAppData->bg.size() / style::DevicePixelRatio();
	if (!_pressedBotAppData->ripple) {
		_pressedBotAppData->ripple = std::make_unique<Ui::RippleAnimation>(
			st::defaultRippleAnimation, Ui::RippleAnimation::RoundRectMask(size, size.height() / 2), updateCallback);
	}
	const auto shift = QPoint(width() - size.width() - st::dialogRowOpenBotRight, st::dialogRowOpenBotTop);
...
const std::vector<Key> &InnerWidget::pinnedChatsOrder() const {
	...
	return _savedSublists ? owner->pinnedChatsOrder(&owner->savedMessages())
...
void InnerWidget::checkReorderPinnedStart(QPoint localPosition) {
	if (!_pressed || _dragging || (_state != WidgetState::Default) || _pressedBotApp) {
		return;
	} else if (qAbs(localPosition.y() - _dragStart.y()) < style::ConvertScale(kStartReorderThreshold)) {
		return;
	}
	_dragging = _pressed;
...
bool InnerWidget::updateReorderPinned(QPoint localPosition) {
	...
	if (_dragStart.y() > localPosition.y() && _draggingIndex > 0) {
		shift =
			-floorclamp(_dragStart.y() - localPosition.y() + (draggingHeight / 2), draggingHeight, 0, _draggingIndex);
		...
		shift = floorclamp(localPosition.y() - _dragStart.y() + (draggingHeight / 2),
						   draggingHeight,
						   0,
	...
	_pinnedRows[_draggingIndex].yadd = anim::value(yaddWas - shiftHeight, localPosition.y() - _dragStart.y());
...
void InnerWidget::mouseReleaseEvent(QMouseEvent *e) { mousePressReleased(e->globalPos(), e->button(), e->modifiers()); }

void InnerWidget::mousePressReleased(QPoint globalPosition, Qt::MouseButton button, Qt::KeyboardModifiers modifiers) {
	...
	if (!wasDragging && button == Qt::LeftButton) {
		if ((collapsedPressed >= 0 && collapsedPressed == _collapsedSelected) ||
			(pressed && pressed == _selected && pressedTopicJump == _selectedTopicJump &&
			...
			(pressedChatTypeFilter && pressedChatTypeFilter == _selectedChatTypeFilter)) {
			if (pressedBotApp && (pressed || filteredPressed >= 0)) {
				const auto &row = pressed ? pressed : _filterResults[filteredPressed].row.get();
				if (const auto user = MaybeBotWithApp(row)) {
...
void InnerWidget::setPressed(Row *pressed, bool pressedTopicJump, bool pressedBotApp) {
...
void InnerWidget::setFilteredPressed(int pressed, bool pressedTopicJump, bool pressedBotApp) {
	if (_filteredPressed != pressed || (pressed >= 0 && _pressedTopicJump != pressedTopicJump) ||
		...
		if (pressed >= 0 || !pressedTopicJump || !pressedBotApp) {
			...
			const auto history = pressedTopicJump ? _filterResults[pressed].row->history() : nullptr;
			const auto item = history ? history->chatListMessage() : nullptr;
			_pressedTopicJumpRootId = item ? item->topicRootId() : MsgId();
		}
	}
...
void InnerWidget::moveSearchIn() {
	...
	const auto searchInWidth = std::max(width(), st::columnMinimalWidthLeft - _narrowWidth);
	...
	session().data().chatListEntryRefreshes() |
		...
		rpl::start_with_next(
			[=](const Event &event)
			{
				...
				if (!_dragging && (from != to) && (_state == WidgetState::Default) &&
					(key.topic()		 ? (key.topic()->forum() == _openedForum)
						 : key.sublist() ? _savedSublists
				...
				if (event.existenceChanged) {
					if (!entry->inChatList()) {
						...
						const auto i = ranges::find(_filterResults, key, &FilterResult::key);
						if (i != _filterResults.end()) {
							if (_filteredSelected == (i - _filterResults.begin()) && (i + 1) == _filterResults.end()) {
								_filteredSelected = -1;
							}
					...
					update(0, std::min(from, to), width(), std::abs(from - to) + event.moved.height);
				}
			},
			lifetime());
}
...
void InnerWidget::updateSearchResult(not_null<PeerData *> peer) {
	if (_state == WidgetState::Filtered) {
		const auto i = ranges::find(_peerSearchResults, peer, &PeerSearchResult::peer);
		if (i != end(_peerSearchResults)) {
			const auto top = peerSearchOffset();
			const auto index = (i - begin(_peerSearchResults));
			rtlupdate(0, top + index * st::dialogsRowHeight, width(), st::dialogsRowHeight);
		}
	}
}

void InnerWidget::updateDialogRow(RowDescriptor row, QRect updateRect, UpdateRowSections sections) {
...
void InnerWidget::refreshShownList() {
	const auto list = _savedSublists ? session().data().savedMessages().chatsList()->indexed()
		: _openedForum				 ? _openedForum->topicsList()->indexed()
		: _filterId					 ? session().data().chatsFilters().chatsList(_filterId)->indexed()
...
void InnerWidget::clearSelection() {
	...
	if (isSelected()) {
		...
		_selected = nullptr;
		_filteredSelected = _searchedSelected = _previewSelected = _peerSearchSelected = _hashtagSelected = -1;
...
void InnerWidget::fillArchiveSearchMenu(not_null<Ui::PopupMenu *> menu) {
	...
	if (!folder || !folder->chatsList()->fullSize().current() || _searchState.inChat) {
		return;
	}
	const auto skip = session().settings().skipArchiveInSearch();
	const auto text =
		skip ? tr::lng_dialogs_show_archive_in_search(tr::now) : tr::lng_dialogs_skip_archive_in_search(tr::now);
	menu->addAction(text,
					[=]
					{
...
bool InnerWidget::showChatPreview() {
	...
	const auto callback = crl::guard(this, [=](bool shown) { chatPreviewShown(shown, row); });
...
bool InnerWidget::scheduleChatPreview(QPoint positionOverride) {
	const auto row = computeChatPreviewRow();
	const auto callback = crl::guard(this, [=](bool shown) { chatPreviewShown(shown, row); });
	_chatPreviewScheduled = _controller->scheduleChatPreview(row, callback, nullptr, positionOverride);
...
void InnerWidget::contextMenuEvent(QContextMenuEvent *e) {
	...
	_menu =
		base::make_unique_q<Ui::PopupMenu>(this, row.fullId ? st::defaultPopupMenu : st::popupMenuExpandedSeparator);
	if (row.fullId) {
		...
		Window::FillDialogsEntryMenu(_controller,
									 Dialogs::EntryState{
										 .key = row.key,
...
bool InnerWidget::processTouchEvent(not_null<QTouchEvent *> e) {
	const auto point =
		e->touchPoints().empty() ? std::optional<QPoint>() : e->touchPoints().front().screenPos().toPoint();
	switch (e->type()) {
...
bool InnerWidget::hasChatTypeFilter() const {
	return !_searchResults.empty() && (_searchState.tab == ChatSearchTab::MyMessages);
}
...
void InnerWidget::applySearchState(SearchState state) {
	...
	const auto ignoreInChat = (state.tab == ChatSearchTab::MyMessages) || (state.tab == ChatSearchTab::PublicPosts);
	...
	if (peer && peer->isSelf()) {
		...
		_searchTags = std::make_unique<SearchTags>(&peer->owner(), reactions->myTagsValue(sublist), state.tags);

		_searchTags->repaintRequests() |
			rpl::start_with_next(
				[=]
				{
					const auto height = _searchTags->height();
			...
			rpl::start_with_next(
				[=](Data::ReactionId id)
				{
					HistoryView::ShowTagInListMenu(
						&_menu, _lastMousePosition.value_or(QCursor::pos()), this, id, _controller);
				},
				_searchTags->lifetime());
	...
	_searchFromShown = ignoreInChat ? nullptr : sublist ? sublist->peer().get() : state.fromPeer;
	if (state.inChat) {
	...
	const auto words = mentionsSearch ? QStringList(newFilter) : TextUtilities::PrepareSearchWords(newFilter);
	newFilter = words.isEmpty() ? QString() : words.join(' ');
	if (newFilter != _filter || otherChanged) {
		_filter = newFilter;
		if (_filter.isEmpty() && !_searchState.fromPeer && _searchState.tags.empty()) {
			clearFilter();
		} else {
			...
			const auto append = [&](not_null<IndexedList *> list)
			{
				...
				auto i = _filterResults.insert(end(_filterResults), begin(results), end(results));
	...
	if (_state != WidgetState::Default) {
		_searchWaiting = true;
		_searchRequests.fire(otherChanged ? SearchRequestDelay::Instant : SearchRequestDelay::Delayed);
		if (_searchWaiting) {
			refresh(true);
		}
	}
...
void InnerWidget::trackResultsHistory(not_null<History *> history) {
	...
	channel->flagsValue() | rpl::skip(1) |
		rpl::filter([=](const ChannelData::Flags::Change &change) { return (change.diff & ChannelDataFlag::Forum); }) |
		rpl::start_with_next(
			[=]
			{
				...
				auto removed = false;
				for (auto i = begin(_filterResultsGlobal); i != end(_filterResultsGlobal);) {
					if (const auto topic = i->first.topic()) {
						if (topic->channel() == channel) {
							removed = true;
							_filterResults.erase(ranges::remove(_filterResults, i->first, &FilterResult::key),
												 end(_filterResults));
	...
	if (const auto forum = channel->forum()) {
		forum->topicDestroyed() |
			rpl::start_with_next(
				...
				{
					auto removed = false;
					const auto sfrom = ranges::remove(_searchResults, topic.get(), &FakeRow::topic);
					if (sfrom != end(_searchResults)) {
						_searchResults.erase(sfrom, end(_searchResults));
						removed = true;
					}
					const auto ffrom = ranges::remove(_filterResults, Key(topic), &FilterResult::key);
...
Data::Thread *InnerWidget::updateFromParentDrag(QPoint globalPosition) {
	...
	if (_state == WidgetState::Default) {
		...
		if (base::in_range(_filteredSelected, 0, _filterResults.size())) {
			...
			return session().data().history(_peerSearchResults[_peerSearchSelected]->peer);
		} else if (base::in_range(_previewSelected, 0, _previewResults.size())) {
...
void InnerWidget::setLoadMoreCallback(Fn<void()> callback) { _loadMoreCallback = std::move(callback); }

void InnerWidget::setLoadMoreFilteredCallback(Fn<void()> callback) { _loadMoreFilteredCallback = std::move(callback); }

auto InnerWidget::chosenRow() const -> rpl::producer<ChosenRow> { return _chosenRow.events(); }

rpl::producer<> InnerWidget::updated() const { return _updated.events(); }

rpl::producer<int> InnerWidget::scrollByDeltaRequests() const { return _draggingScroll.scrolls(); }

rpl::producer<> InnerWidget::listBottomReached() const { return _listBottomReached.events(); }

rpl::producer<ChatSearchTab> InnerWidget::changeSearchTabRequests() const { return _changeSearchTabRequests.events(); }

auto InnerWidget::changeSearchFilterRequests() const -> rpl::producer<ChatTypeFilter> {
	return _changeSearchFilterRequests.events();
}

rpl::producer<> InnerWidget::cancelSearchRequests() const { return _cancelSearchRequests.events(); }

rpl::producer<> InnerWidget::cancelSearchFromRequests() const { return _cancelSearchFromRequests.events(); }

rpl::producer<> InnerWidget::changeSearchFromRequests() const { return _changeSearchFromRequests.events(); }

rpl::producer<Ui::ScrollToRequest> InnerWidget::mustScrollTo() const { return _mustScrollTo.events(); }

rpl::producer<Ui::ScrollToRequest> InnerWidget::dialogMoved() const { return _dialogMoved.events(); }

rpl::producer<SearchRequestDelay> InnerWidget::searchRequests() const { return _searchRequests.events(); }

rpl::producer<QString> InnerWidget::completeHashtagRequests() const { return _completeHashtagRequests.events(); }

rpl::producer<> InnerWidget::refreshHashtagsRequests() const { return _refreshHashtagsRequests.events(); }

void InnerWidget::visibleTopBottomUpdated(int visibleTop, int visibleBottom) {
	_visibleTop = visibleTop;
	_visibleBottom = visibleBottom;
	preloadRowsData();
	const auto loadTill = _visibleTop + PreloadHeightsCount * (_visibleBottom - _visibleTop);
...
bool InnerWidget::uniqueSearchResults() const { return _controller->uniqueChatsInSearchResults(); }
...
bool InnerWidget::hasHistoryInResults(not_null<History *> history) const {
	...
	const auto inSearchResults =
		ranges::find(_searchResults, history, [](const Result &result) { return result->item()->history(); }) !=
		end(_searchResults);
	if (inSearchResults) {
		return true;
	}
	const auto inFilteredResults =
		ranges::find(_filterResults, Key(history), &FilterResult::key) != end(_filterResults);
	if (inFilteredResults) {
		return true;
	}
	const auto inPeerSearchResults =
		ranges::find(_peerSearchResults, history->peer, [](const auto &result) { return result->peer; }) !=
...
void InnerWidget::searchReceived(std::vector<not_null<HistoryItem *>> messages,
	...
	const auto key =
		(!_openedForum || _searchState.inChat.topic()) ? _searchState.inChat : Key(_openedForum->history());
	if (inject && (!_searchState.inChat || inject->history() == _searchState.inChat.history())) {
		Assert(_searchResults.empty());
		Assert(!toPreview);
		const auto index = int(_searchResults.size());
		_searchResults.push_back(std::make_unique<FakeRow>(key, inject, [=] { repaintSearchResult(index); }));
	...
	for (const auto &item : messages) {
		...
		if (toPreview || !uniquePeers || !hasHistoryInResults(history)) {
			...
			const auto repaint =
				toPreview ? Fn<void()>([=] { repaintSearchResult(index); }) : [=] { repaintPreviewResult(index); };
...
void InnerWidget::peerSearchReceived(const QString &query, const QVector<MTPPeer> &my, const QVector<MTPPeer> &result) {
	if (_state != WidgetState::Filtered) {
		return;
	}
...
bool InnerWidget::needCollapsedRowsRefresh() const {
	const auto archive = !_shownList->empty() ? _shownList->begin()->get()->folder() : nullptr;
	const auto collapsedHasArchive = !_collapsedRows.empty() && (_collapsedRows.back()->folder != nullptr);
	const auto archiveIsCollapsed = (archive != nullptr) && session().settings().archiveCollapsed();
	const auto archiveIsInMainMenu = (archive != nullptr) && session().settings().archiveInMainMenu();
...
void InnerWidget::refreshEmpty() {
	if (_state == WidgetState::Filtered) {
		const auto empty =
			_filterResults.empty() && _searchResults.empty() && _peerSearchResults.empty() && _hashtagResults.empty();
		if (_searchLoading || _searchWaiting || !empty) {
			...
			_searchEmpty =
				MakeSearchEmpty(this, _searchState, [=] { _changeSearchFilterRequests.fire(ChatTypeFilter::All); });
	...
	const auto state = !_shownList->empty() ? EmptyState::None
		: _savedSublists
		? (data->savedMessages().chatsList()->loaded() ? EmptyState::EmptySavedSublists : EmptyState::Loading)
		: _openedForum ? (_openedForum->topicsList()->loaded() ? EmptyState::EmptyForum : EmptyState::Loading)
		: (!_filterId && data->contactsLoaded().current()) ? EmptyState::NoContacts
	...
	_empty->overrideLinkClickHandler(
		[=]
		{
			if (_emptyState == EmptyState::NoContacts) {
				...
				_controller->show(Box(NewForumTopicBox, _controller, _openedForum->history()));
			}
		});
	_empty->setVisible(_state == WidgetState::Default);
}
...
void InnerWidget::clearMouseSelection(bool clearSelection) {
	...
	if (clearSelection) {
		if (_state == WidgetState::Default) {
			...
			_filteredSelected = _peerSearchSelected = _previewSelected = _searchedSelected = _hashtagSelected = -1;
		}
		setCursor(style::cur_default);
	}
}
...
bool InnerWidget::hasFilteredResults() const { return !_filterResults.empty() && _hashtagResults.empty(); }

auto InnerWidget::searchTagsChanges() const -> rpl::producer<std::vector<Data::ReactionId>> {
	return _searchTags ? _searchTags->selectedChanges() : rpl::never<std::vector<Data::ReactionId>>();
}
...
void InnerWidget::updateSearchIn() {
	if (!_searchState.inChat && _searchHashOrCashtag == HashOrCashtag::None) {
		...
		_searchIn->show();
		_searchIn->changeFromRequests() | rpl::start_to_stream(_changeSearchFromRequests, _searchIn->lifetime());
		_searchIn->cancelFromRequests() | rpl::start_to_stream(_cancelSearchFromRequests, _searchIn->lifetime());
		_searchIn->cancelInRequests() | rpl::start_to_stream(_cancelSearchRequests, _searchIn->lifetime());
		_searchIn->tabChanges() | rpl::start_to_stream(_changeSearchTabRequests, _searchIn->lifetime());
	}

	const auto sublist = _searchState.inChat.sublist();
	const auto topic = _searchState.inChat.topic();
	const auto peer = _searchState.inChat.owningHistory() ? _searchState.inChat.owningHistory()->peer.get()
		: _openedForum									  ? _openedForum->channel().get()
														  : nullptr;
	const auto topicIcon = !topic ? nullptr
		: topic->iconId() ? Ui::MakeEmojiThumbnail(&topic->owner(), Data::SerializeCustomEmojiId(topic->iconId()))
						  : Ui::MakeEmojiThumbnail(
								&topic->owner(),
								Data::TopicIconEmojiEntity({
									.title = (topic->isGeneral() ? Data::ForumGeneralIconTitle() : topic->title()),
									.colorId = (topic->isGeneral() ? Data::ForumGeneralIconColor(st::windowSubTextFg->c)
																   : topic->colorId()),
								}));
	const auto peerIcon = peer ? Ui::MakeUserpicThumbnail(peer)
		: sublist			   ? Ui::MakeUserpicThumbnail(sublist->peer())
							   : nullptr;
	const auto myIcon = Ui::MakeIconThumbnail(st::menuIconChats);
	const auto publicIcon =
		(_searchHashOrCashtag != HashOrCashtag::None) ? Ui::MakeIconThumbnail(st::menuIconChannel) : nullptr;
	const auto peerTabType = (peer && peer->isBroadcast())	? ChatSearchPeerTabType::Channel
		: (peer && (peer->isChat() || peer->isMegagroup())) ? ChatSearchPeerTabType::Group
															: ChatSearchPeerTabType::Chat;
	const auto fromImage = _searchFromShown ? Ui::MakeUserpicThumbnail(_searchFromShown) : nullptr;
	const auto fromName = _searchFromShown ? _searchFromShown->shortName() : QString();
	_searchIn->apply(
		{
	...
	if (_state == WidgetState::Default) {
		...
		if (_collapsedSelected < 0 && !_selected) {
			...
			auto cur = (_collapsedSelected >= 0)
				? _collapsedSelected
				: int(_collapsedRows.size() + (_shownList->cfind(_selected) - _shownList->cbegin() - skip));
			cur =
				std::clamp(cur + direction, 0, static_cast<int>(_collapsedRows.size() + _shownList->size() - skip - 1));
		...
		if ((_hashtagSelected < 0 || _hashtagSelected >= _hashtagResults.size()) &&
			...
			int32 cur = base::in_range(_hashtagSelected, 0, _hashtagResults.size()) ? _hashtagSelected
				...
				: (_searchedSelected + _previewResults.size() + _peerSearchResults.size() + _filterResults.size() +
				   _hashtagResults.size());
			cur =
				std::clamp(cur + direction,
						   0,
		...
		if (base::in_range(_hashtagSelected, 0, _hashtagResults.size())) {
			...
			const auto from = peerSearchOffset() + _peerSearchSelected * st::dialogsRowHeight +
			...
			const auto from =
				previewOffset() + _previewSelected * _st->height + (_previewSelected ? 0 : -st::searchedBarHeight);
			const auto height = _st->height + (_previewSelected ? 0 : st::searchedBarHeight);
			scrollToItem(from, height);
		} else {
			const auto from =
				searchedOffset() + _searchedSelected * _st->height + (_searchedSelected ? 0 : -st::searchedBarHeight);
...
void InnerWidget::selectSkipPage(int32 pixels, int32 direction) {
	...
	if (direction > 0) {
		for (auto i = _shownList->cfind(_selected), end = _shownList->cend(); i != end && (toSkip--); ++i) {
			_selected = *i;
		}
	} else {
...
void InnerWidget::scrollToItem(int top, int height) { _mustScrollTo.fire({top, top + height}); }
...
void InnerWidget::preloadRowsData() {
	...
	if (_state == WidgetState::Default) {
		...
		if (yFrom < otherStart) {
			for (auto i = _shownList->findByY(yFrom), end = _shownList->cend(); i != end; ++i) {
		...
		if (from < _filterResults.size()) {
			const auto to = std::min(((yTo - filteredOffset()) / _st->height) + 1, int(_filterResults.size()));
			for (; from < to; ++from) {
		...
		if (from < _peerSearchResults.size()) {
			const auto to =
				std::min(((yTo - peerSearchOffset()) / st::dialogsRowHeight) + 1, int(_peerSearchResults.size()));
			for (; from < to; ++from) {
		...
		if (from < 0) from = 0;
		if (from < _previewResults.size()) {
			const auto to = std::min(((yTo - previewOffset()) / _st->height) + 1, int(_previewResults.size()));
			for (; from < to; ++from) {
		...
		if (from < 0) from = 0;
		if (from < _searchResults.size()) {
			const auto to = std::min(((yTo - searchedOffset()) / _st->height) + 1, int(_searchResults.size()));
			for (; from < to; ++from) {
...
bool InnerWidget::chooseCollapsedRow(Qt::KeyboardModifiers modifiers) {
	if (_state != WidgetState::Default) {
		return false;
	} else if ((_collapsedSelected < 0) || (_collapsedSelected >= _collapsedRows.size())) {
		return false;
	}
...
void InnerWidget::switchToFilter(FilterId filterId) {
	...
	const auto &list = session().data().chatsFilters().list();
	const auto filterIt = filterId ? ranges::find(list, filterId, &Data::ChatFilter::id) : end(list);
...
void InnerWidget::saveChatsFilterScrollState(FilterId filterId) { _chatsFilterScrollStates[filterId] = -y(); }
...
QImage *InnerWidget::cacheChatsFilterTag(const Data::ChatFilter &filter, uint8 more, bool active) {
	if (!filter.id() && !more) {
		return nullptr;
	}
...
bool InnerWidget::chooseHashtag() {
	if (_state != WidgetState::Filtered) {
		return false;
	} else if ((_hashtagSelected < 0) || (_hashtagSelected >= _hashtagResults.size())) {
		return false;
	}
...
ChosenRow InnerWidget::computeChosenRow() const {
	if (_state == WidgetState::Default) {
		if ((_collapsedSelected >= 0) && (_collapsedSelected < _collapsedRows.size())) {
			const auto &row = _collapsedRows[_collapsedSelected];
			Assert(row->folder != nullptr);
			return {
				.key = row->folder,
		...
		if (base::in_range(_filteredSelected, 0, _filterResults.size())) {
			...
			return {.key = session().data().history(peer), .message = Data::UnreadMessagePosition};
		} else if (base::in_range(_previewSelected, 0, _previewResults.size())) {
			const auto result = _previewResults[_previewSelected].get();
			const auto topic = result->topic();
			const auto item = result->item();
			return {.key = (topic ? (Entry *) topic : (Entry *) item->history()), .message = item->position()};
		} else if (base::in_range(_searchedSelected, 0, _searchResults.size())) {
...
bool InnerWidget::isUserpicPress() const {
	return (_lastRowLocalMouseX >= 0) && (_lastRowLocalMouseX < _st->nameLeft) &&
		(_collapsedSelected < 0 || _collapsedSelected >= _collapsedRows.size());
}

bool InnerWidget::isUserpicPressOnWide() const { return isUserpicPress() && (width() > _narrowWidth); }

bool InnerWidget::chooseRow(Qt::KeyboardModifiers modifiers, MsgId pressedTopicRootId) {
	if (chooseHashtag()) {
		return true;
	...
	const auto modifyChosenRow = [&](ChosenRow row, Qt::KeyboardModifiers modifiers)
	{
...
RowDescriptor InnerWidget::chatListEntryBefore(const RowDescriptor &which) const {
	if (!which.key) {
		return RowDescriptor();
	}
	if (_state == WidgetState::Default) {
		if (const auto row = _shownList->getRow(which.key)) {
			...
			if (i != _shownList->cbegin()) {
				return RowDescriptor((*(i - 1))->key(), FullMsgId(PeerId(), ShowAtUnreadMsgId));
			}
		}
		return RowDescriptor();
	}
	...
	if (!_searchResults.empty()) {
		...
		if (isSearchResultActive(_searchResults[0].get(), which)) {
			if (_peerSearchResults.empty()) {
				...
				return RowDescriptor(_filterResults.back().key(), FullMsgId(PeerId(), ShowAtUnreadMsgId));
			}
			return RowDescriptor(session().data().history(_peerSearchResults.back()->peer),
								 FullMsgId(PeerId(), ShowAtUnreadMsgId));
		}
	}
	if (const auto history = whichThread->asHistory()) {
		if (!_peerSearchResults.empty() && _peerSearchResults[0]->peer == history->peer) {
			if (_filterResults.empty()) {
				return RowDescriptor();
			}
			return RowDescriptor(_filterResults.back().key(), FullMsgId(PeerId(), ShowAtUnreadMsgId));
...
RowDescriptor InnerWidget::chatListEntryAfter(const RowDescriptor &which) const {
	if (!which.key) {
		return RowDescriptor();
	}
	if (_state == WidgetState::Default) {
	...
	if (const auto history = whichThread->asHistory()) {
		for (auto i = _peerSearchResults.cbegin(), e = _peerSearchResults.cend(); i != e; ++i) {
			if ((*i)->peer == history->peer) {
				++i;
				if (i != e) {
					return RowDescriptor(session().data().history((*i)->peer), FullMsgId(PeerId(), ShowAtUnreadMsgId));
	...
	for (auto i = _filterResults.cbegin(), e = _filterResults.cend(); i != e; ++i) {
		if ((*i).key() == which.key) {
			...
			if (i != e) {
				...
				return RowDescriptor(session().data().history(_peerSearchResults.front()->peer),
									 FullMsgId(PeerId(), ShowAtUnreadMsgId));
			} else if (!_searchResults.empty()) {
				return RowDescriptor(_searchResults.front()->item()->history(),
									 _searchResults.front()->item()->fullId());
...
RowDescriptor InnerWidget::chatListEntryFirst() const {
	if (_state == WidgetState::Default) {
		...
		return RowDescriptor(_filterResults.front().key(), FullMsgId(PeerId(), ShowAtUnreadMsgId));
	} else if (!_peerSearchResults.empty()) {
		return RowDescriptor(session().data().history(_peerSearchResults.front()->peer),
							 FullMsgId(PeerId(), ShowAtUnreadMsgId));
	} else if (!_searchResults.empty()) {
		return RowDescriptor(_searchResults.front()->item()->history(), _searchResults.front()->item()->fullId());
	}
	return RowDescriptor();
}
...
RowDescriptor InnerWidget::chatListEntryLast() const {
	if (_state == WidgetState::Default) {
		...
		return RowDescriptor(_searchResults.back()->item()->history(), _searchResults.back()->item()->fullId());
	} else if (!_peerSearchResults.empty()) {
		return RowDescriptor(session().data().history(_peerSearchResults.back()->peer),
							 FullMsgId(PeerId(), ShowAtUnreadMsgId));
	} else if (!_filterResults.empty()) {
...
void InnerWidget::setupOnlineStatusCheck() {
	session().changes().peerUpdates(Data::PeerUpdate::Flag::OnlineStatus | Data::PeerUpdate::Flag::GroupCall |
...
void InnerWidget::repaintDialogRowCornerStatus(not_null<History *> history) {
	...
	const auto size = user ? st::dialogsOnlineBadgeSize : st::dialogsCallBadgeSize;
	const auto stroke = st::dialogsOnlineBadgeStroke;
	const auto skip = user ? st::dialogsOnlineBadgeSkip : st::dialogsCallBadgeSkip;
	const auto updateRect = QRect(_st->photoSize - skip.x() - size, _st->photoSize - skip.y() - size, size, size)
								.marginsAdded({stroke, stroke, stroke, stroke})
								.translated(st::defaultDialogRow.padding.left(), st::defaultDialogRow.padding.top());
...
void InnerWidget::updateRowCornerStatusShown(not_null<History *> history) {
	...
	const auto findRow = [&](not_null<History *> history) -> std::pair<Row *, int>
	{
		if (state() == WidgetState::Default) {
			const auto row = _shownList->getRow({history});
			return {row, row ? defaultRowTop(row) : 0};
		}
		const auto i = ranges::find(_filterResults, Key(history), &FilterResult::key);
	...
	if (const auto &[row, top] = findRow(history); row != nullptr) {
		const auto visible = (top < _visibleBottom) && (top + _st->height > _visibleTop);
		row->updateCornerBadgeShown(history->peer, visible ? Fn<void()>(crl::guard(this, repaint)) : nullptr);
	}
}
...
RowDescriptor InnerWidget::resolveChatNext(RowDescriptor from) const {
	...
	return row.key ? computeJump(chatListEntryAfter(row), JumpSkip::NextOrEnd) : row;
}

RowDescriptor InnerWidget::resolveChatPrevious(RowDescriptor from) const {
	const auto row = from.key ? from : _controller->activeChatEntryCurrent();
	return row.key ? computeJump(chatListEntryBefore(row), JumpSkip::PreviousOrBegin) : row;
}
...
void InnerWidget::setupShortcuts() {
	Shortcuts::Requests() |
		rpl::filter(
			[=]
			{
				return isActiveWindow() && !_controller->isLayerShown() && !_controller->window().locked() &&
		...
		rpl::start_with_next(
			[=](not_null<Shortcuts::Request *> request)
			{
				...
				const auto previous = row.key ? computeJump(chatListEntryBefore(row), JumpSkip::PreviousOrBegin) : row;
				const auto next = row.key ? computeJump(chatListEntryAfter(row), JumpSkip::NextOrEnd) : row;
				const auto first = [&]
				{
				...
				if (row.key) {
					request->check(Command::ChatPrevious) && request->handle([=] { return jumpToDialogRow(previous); });
					request->check(Command::ChatNext) && request->handle([=] { return jumpToDialogRow(next); });
				} else if (_state == WidgetState::Default ? !_shownList->empty() : !_filterResults.empty()) {
					request->check(Command::ChatNext) && request->handle([=] { return jumpToDialogRow(first); });
				}
				request->check(Command::ChatFirst) && request->handle([=] { return jumpToDialogRow(first); });
				request->check(Command::ChatLast) && request->handle([=] { return jumpToDialogRow(last); });
				request->check(Command::ChatSelf) &&
					request->handle(
						[=]
						{
							_controller->showThread(session().data().history(session().user()),
													ShowAtUnreadMsgId,
													Window::SectionShow::Way::ClearStack);
							return true;
						});
				request->check(Command::ShowArchive) &&
					request->handle(
						[=]
						{
							const auto folder = session().data().folderLoaded(Data::Folder::kId);
				...
				if (session().data().chatsFilters().has()) {
					...
					auto &&folders =
						ranges::views::zip(Shortcuts::kShowFolder, ranges::views::ints(0, ranges::unreachable));
							...
							request->handle(
								[=]
								{
									if (select <= filtersCount) {
										_controller->setActiveChatsFilter(filters->lookupId(select));
									}
									return true;
								});
					}
				...
				auto &&pinned = ranges::views::zip(kPinned, ranges::views::ints(0, ranges::unreachable));
				for (const auto &[command, index] : pinned) {
					request->check(command) &&
						request->handle(
							[=, index = index]
							{
								const auto list = (_filterId ? session().data().chatsFilters().chatsList(_filterId)
				...
				request->check(Command::FolderNext) &&
					request->handle(
						[=]
						{
				...
				request->check(Command::FolderPrevious) &&
					request->handle(
						[=]
						{
							using namespace Window;
				...
				request->check(Command::ReadChat) &&
					request->handle(
						[=]
						{
				...
				(!_openedForum) && request->check(Command::ArchiveChat) &&
					request->handle(
						[=]
						{
							...
							Window::ToggleHistoryArchived(_controller->uiShow(), history, !isArchived);
							return true;
						});

				request->check(Command::ShowContacts) &&
					request->handle(
						[=]
						{
...
RowDescriptor InnerWidget::computeJump(const RowDescriptor &to, JumpSkip skip) const {
	auto result = to;
	if (result.key) {
		const auto down = (skip == JumpSkip::NextOrEnd) || (skip == JumpSkip::NextOrOriginal);
		const auto needSkip = [&]
		{
			return (result.key.folder() != nullptr) ||
				(session().supportMode() && !result.key.entry()->chatListBadgesState().unread);
		};
		while (needSkip()) {
			const auto next = down ? chatListEntryAfter(result) : chatListEntryBefore(result);
			if (next.key) {
				result = next;
			} else {
				if (skip == JumpSkip::PreviousOrOriginal || skip == JumpSkip::NextOrOriginal) {
					result = to;
				}
				break;
			}
...
rpl::producer<UserId> InnerWidget::openBotMainAppRequests() const { return _openBotMainAppRequests.events(); }
