...
class ItemZoom final : public Ui::Menu::Action, public Ui::AbstractTooltipShower
{
public:
	ItemZoom(not_null<RpWidget *> parent, const not_null<Delegate *> delegate, const style::Menu &st)
		: Ui::Menu::Action(parent, st, Ui::CreateChild<QAction>(parent), nullptr, nullptr), _delegate(delegate),
		  _st(st) {
		init();
	}
	...
	void init() {
		...
		const auto reset = Ui::CreateChild<Ui::RoundButton>(this, rpl::single<QString>(QString()), st::ivResetZoom);
		processTooltip(reset);
		const auto resetLabel =
			Ui::CreateChild<Ui::FlatLabel>(reset, tr::lng_background_reset_default(), st::ivResetZoomLabel);
		resetLabel->setAttribute(Qt::WA_TransparentForMouseEvents);
		reset->setTextTransform(Ui::RoundButton::TextTransform::NoTransform);
		reset->setClickedCallback([this] { _delegate->ivSetZoom(kDefaultZoom); });
		reset->show();
		const auto plus = Ui::CreateSimpleCircleButton(this, st::defaultRippleAnimationBgOver);
		...
		const auto step = []
		{
			return base::IsAltPressed() ? kZoomTinyStep : base::IsCtrlPressed() ? kZoomSmallStep : kZoomStep;
		};
		plus->setClickedCallback([this, step] { _delegate->ivSetZoom(_delegate->ivZoom() + step()); });
		plus->show();
		const auto minus = Ui::CreateSimpleCircleButton(this, st::defaultRippleAnimationBgOver);
		...
		minus->paintRequest() |
			rpl::start_with_next(
				[=, fg = _st.itemFg]
				{
					...
					p.setPen(fg);
					p.setFont(st::normalFont);
					p.drawText(QRectF(r).translated(0, style::ConvertFloatScale(-1)), QChar(0x2013), style::al_center);
				},
				minus->lifetime());
		processTooltip(minus);
		minus->setClickedCallback([this, step] { _delegate->ivSetZoom(_delegate->ivZoom() - step()); });
		minus->show();

		{
			const auto maxWidthText = u"000%"_q;
			_text.setText(_st.itemStyle, maxWidthText);
			Ui::Menu::ItemBase::setMinWidth(_text.maxWidth() + st::ivResetZoomInnerPadding + resetLabel->width() +
											plus->width() + minus->width() + _st.itemPadding.right() * 2);
		}
		...
		rpl::combine(sizeValue(), reset->sizeValue()) |
			rpl::start_with_next(
				[=](const QSize &size, const QSize &)
				{
					reset->setFullWidth(0 + resetLabel->width() + st::ivResetZoomInnerPadding);
					resetLabel->moveToLeft((reset->width() - resetLabel->width()) / 2,
										   (reset->height() - resetLabel->height()) / 2);
					reset->moveToRight(_st.itemPadding.right(), (size.height() - reset->height()) / 2);
					plus->moveToRight(_st.itemPadding.right() + reset->width(), (size.height() - plus->height()) / 2);
					minus->moveToRight(_st.itemPadding.right() + plus->width() + reset->width(),
	...
	void paintEvent(QPaintEvent *event) override {
		...
		_text.draw(p,
				   {
					   .position = QPoint(_st.itemIconPosition.x(), (height() - _text.minHeight()) / 2),
					   .outerWidth = width(),
	...
	bool tooltipWindowActive() const override { return true; }
...
[[nodiscard]] QByteArray ComputeStyles(int zoom) {
	...
	return Ui::ComputeStyles(map, phrases, zoom) + ';' +
		Ui::ComputeSemiTransparentOverStyle("light-button-bg-over", st::lightButtonBgOver, st::windowBg);
}
...
[[nodiscard]] QByteArray WrapPage(const Prepared &page, int zoom) {
	...
	const auto js = QByteArray() + (page.hasCode ? "IV.initPreBlocks();" : "") +
		(page.hasEmbeds ? "IV.initEmbedBlocks();" : "") + "IV.init();" + page.script;

	return R"(<!DOCTYPE html>
<html)"_q +
		classAttribute + R"( style=")" + Ui::EscapeForAttribute(ComputeStyles(zoom)) + R"(">
	<head>
		<meta charset="utf-8">
	...
	<body>
		...
		<div class="page-scroll" tabindex="-1">)"_q +
		page.content.trimmed() + R"(</div>
		<script>)"_q +
		js + R"(</script>
	</body>
</html>
...
[[nodiscard]] QString HttpsToTonsite(QString value) {
	...
	return "tonsite://" + host + ((split < 0) ? u"/"_q : part.mid(split));
}

} // namespace

Controller::Controller(not_null<Delegate *> delegate, Fn<ShareBoxResult(ShareBoxDescriptor)> showShareBox)
	: _delegate(delegate),
	  _updateStyles(
		  [=]
		  {
			  if (_webview) {
				  ...
				  const auto styleZoom = webviewZoomController ? kDefaultZoom : _delegate->ivZoom();
...
void Controller::updateTitleGeometry(int newWidth) const {
	...
	_subtitleWrap->paintRequest() |
		rpl::start_with_next([=](QRect clip) { QPainter(_subtitleWrap.get()).fillRect(clip, st::windowBg); },
							 _subtitleWrap->lifetime());

	const auto progressBack = _subtitleBackShift.value(_back->toggled() ? 1. : 0.);
	const auto progressForward = _subtitleForwardShift.value(_forward->toggled() ? 1. : 0.);
	const auto backAdded = _back->width() + st::ivSubtitleSkip - st::ivSubtitleLeft;
	const auto forwardAdded = _forward->width();
	const auto left = st::ivSubtitleLeft + anim::interpolate(0, backAdded, progressBack) +
...
void Controller::initControls() {
	...
	_subtitleText =
		_index.value() | rpl::filter(rpl::mappers::_1 >= 0) | rpl::map([=](int index) { return _pages[index].name; });
	_subtitle = std::make_unique<Ui::FlatLabel>(_subtitleWrap.get(), _subtitleText.value(), st::ivSubtitle);
	...
	_windowTitleText.value() |
		rpl::start_with_next([=](const QString &title) { _window->setWindowTitle(title); }, _subtitle->lifetime());

	_menuToggle.create(_subtitleWrap.get(), st::ivMenuToggle);
	_menuToggle->setClickedCallback([=] { showMenu(); });

	_back.create(_subtitleWrap.get(), object_ptr<Ui::IconButton>(_subtitleWrap.get(), st::ivBack));
	...
	_forward.create(_subtitleWrap.get(), object_ptr<Ui::IconButton>(_subtitleWrap.get(), st::ivForward));
	...
	_back->toggledValue() |
		rpl::start_with_next(
			[=](bool toggled)
			{
				_subtitleBackShift.start([=] { updateTitleGeometry(_window->body()->width()); },
										 toggled ? 0. : 1.,
										 toggled ? 1. : 0.,
										 st::fadeWrapDuration);
			},
	...
	_forward->toggledValue() |
		rpl::start_with_next(
			[=](bool toggled)
			{
				_subtitleForwardShift.start([=] { updateTitleGeometry(_window->body()->width()); },
											toggled ? 0. : 1.,
											toggled ? 1. : 0.,
											st::fadeWrapDuration);
			},
...
void Controller::show(const Webview::StorageId &storageId,
					  Prepared page,
					  base::flat_map<QByteArray, rpl::producer<bool>> inChannelValues) {
	page.script = fillInChannelValuesScript(std::move(inChannelValues));
	InvokeQueued(_container, [=, page = std::move(page)]() mutable { showInWindow(storageId, std::move(page)); });
}
...
bool Controller::IsGoodTonSiteUrl(const QString &uri) { return !TonsiteToHttps(uri).isEmpty(); }

void Controller::showTonSite(const Webview::StorageId &storageId, QString uri) {
	...
	_subtitleText = _url.value() |
		rpl::filter([=](const QString &url) { return !url.isEmpty() && url != u"about:blank"_q; }) |
		rpl::map([=](QString value) { return HttpsToTonsite(value); });
	_windowTitleText = _subtitleText.value();
	_menuToggle->hide();
}

QByteArray Controller::fillInChannelValuesScript(base::flat_map<QByteArray, rpl::producer<bool>> inChannelValues) {
...
QByteArray Controller::toggleInChannelScript(const QByteArray &id, bool in) const {
...
void Controller::createWindow() {
	...
	base::qt_signal_producer(qApp, &QGuiApplication::focusWindowChanged) |
		...
		rpl::start_with_next([=] { setInnerFocus(); }, window->lifetime());

	initControls();

	window->body()->widthValue() |
		rpl::start_with_next([=](int width) { updateTitleGeometry(width); }, _subtitle->lifetime());

	window->setGeometry(_delegate->ivGeometry());
	window->setMinimumSize({st::windowMinWidth, st::windowMinHeight});

	window->geometryValue() | rpl::distinct_until_changed() | rpl::skip(1) |
		rpl::start_with_next([=] { _delegate->ivSaveGeometry(window); }, window->lifetime());

	_container = Ui::CreateChild<Ui::RpWidget>(window->body().get());
	rpl::combine(window->body()->sizeValue(), _subtitleWrap->heightValue()) |
		rpl::start_with_next([=](QSize size, int title)
							 { _container->setGeometry(QRect(QPoint(), size).marginsRemoved({0, title, 0, 0})); },
							 _container->lifetime());

	_container->paintRequest() |
		rpl::start_with_next([=](QRect clip) { QPainter(_container).fillRect(clip, st::windowBg); },
							 _container->lifetime());
...
void Controller::createWebview(const Webview::StorageId &storageId) {
	...
	_webview = std::make_unique<Webview::Window>(_container,
	...
	if (const auto webviewZoomController = raw->zoomController()) {
		webviewZoomController->zoomValue() |
			rpl::start_with_next(
				...
				lifetime());
		_delegate->ivZoomValue() |
			rpl::start_with_next([=](int value) { webviewZoomController->setZoom(value); }, lifetime());
	}
	...
	QObject::connect(widget,
					 ...
					 [=]
					 {
						 ...
						 crl::on_main(window, [=] { showWebviewError({"Error: WebView has crashed."}); });
						 base::take(_webview);
					 });
						 ...
						 [=]
						 {
							 ...
							 if (event == u"keydown"_q) {
								 ...
								 if (_navigateToIndexWhenReady >= 0) {
									 script += navigateScript(std::exchange(_navigateToIndexWhenReady, -1),
	...
	raw->setDataRequestHandler(
		[=](Webview::DataRequest request)
		{
			...
			{
				request.done({
					.stream = std::make_unique<Webview::DataStreamFromMemory>(std::move(data), std::move(mime)),
				});
			...
			if (id.starts_with("page") && id.ends_with(".html")) {
				if (!_subscribedToColors) {
					...
					rpl::merge(Lang::Updated(), style::PaletteChanged(), _delegate->ivZoomValue() | rpl::to_empty) |
						rpl::start_with_next([=] { _updateStyles.call(); }, _webview->lifetime());
				}
				auto index = 0;
				const auto result = std::from_chars(id.data() + 4, id.data() + id.size() - 5, index);
				if (result.ec != std::errc() || index < 0 || index >= _pages.size()) {
					return Webview::DataResult::Failed;
				}
				const auto webviewZoomController = _webview->zoomController();
				const auto styleZoom = webviewZoomController ? kDefaultZoom : _delegate->ivZoom();
				return finishWith(WrapPage(_pages[index], styleZoom), "text/html; charset=utf-8");
			...
			if (QRegularExpression(pattern).match(qstring).hasMatch()) {
				...
				if (!bytes.isEmpty()) {
					...
					const auto full = (qstring == u"page.js"_q) ? (ReadResource("morphdom.js") + bytes) : bytes;
	...
	raw->navigationHistoryState() |
		rpl::start_with_next(
			[=](Webview::NavigationHistoryState state)
			{
				_back->toggle(state.canGoBack || state.canGoForward, anim::type::normal);
				_forward->toggle(state.canGoForward, anim::type::normal);
				_back->entity()->setDisabled(!state.canGoBack);
				_back->entity()->setIconOverride(state.canGoBack ? nullptr : &st::ivBackIconDisabled,
												 state.canGoBack ? nullptr : &st::ivBackIconDisabled);
				_back->setAttribute(Qt::WA_TransparentForMouseEvents, !state.canGoBack);
...
void Controller::showWebviewError(TextWithEntities text) {
	auto error = Ui::CreateChild<Ui::PaddingWrap<Ui::FlatLabel>>(
		_container,
		object_ptr<Ui::FlatLabel>(_container, rpl::single(text), st::paymentsCriticalError),
	...
	error->show();
	_container->sizeValue() |
		rpl::start_with_next([=](QSize size) { error->setGeometry(0, 0, size.width(), size.height() * 2 / 3); },
							 error->lifetime());
}

void Controller::showInWindow(const Webview::StorageId &storageId, Prepared page) {
...
QByteArray Controller::navigateScript(int index, const QString &hash) {
	return "IV.navigateTo(" + QByteArray::number(index) + ", '" +
		Ui::EscapeForScriptString(qthelp::url_decode(hash).toUtf8()) + "');";
}

QByteArray Controller::reloadScript(int index) { return "IV.reloadPage(" + QByteArray::number(index) + ");"; }
...
bool Controller::active() const { return _window && _window->isActiveWindow(); }

void Controller::showJoinedTooltip() {
	if (_webview && _ready) {
		_webview->eval("IV.showTooltip('" + Ui::EscapeForScriptString(tr::lng_action_you_joined(tr::now).toUtf8()) +
					   "');");
	}
}
...
QString Controller::composeCurrentUrl() const {
	...
	return _pages[index].url + (_hash.isEmpty() ? u""_q : ('#' + _hash));
}
...
void Controller::showMenu() {
	...
	_menu = base::make_unique_q<Ui::PopupMenu>(_window.get(), st::popupMenuWithIcons);
	if (_webview && _ready) {
		_webview->eval("IV.menuShown(true);");
	}
	_menu->setDestroyedCallback(crl::guard(_window.get(),
										   [this, weakButton = Ui::MakeWeak(_menuToggle.data()), menu = _menu.get()]
										   {
	...
	const auto openInBrowser =
		crl::guard(_window.get(), [=] { _events.fire({.type = Event::Type::OpenLinkExternal, .url = url}); });
	_menu->addAction(tr::lng_iv_open_in_browser(tr::now), openInBrowser, &st::menuIconIpAddress);

	_menu->addAction(tr::lng_iv_share(tr::now), [=] { showShareMenu(); }, &st::menuIconShare);

	_menu->addSeparator();
	_menu->addAction(base::make_unique_q<ItemZoom>(_menu, _delegate, _menu->menu()->st()));

	_menu->setForcedOrigin(Ui::PanelAnimation::Origin::TopRight);
	_menu->popup(_window->body()->mapToGlobal(QPoint(_window->body()->width(), 0) + st::ivMenuPosition));
}
...
void Controller::close() { _events.fire({Event::Type::Close}); }

void Controller::quit() { _events.fire({Event::Type::Quit}); }

rpl::lifetime &Controller::lifetime() { return _lifetime; }
...
void Controller::showShareMenu() {
	...
	_shareWrap = std::make_unique<Ui::RpWidget>(_shareHidesContent ? _window->body().get() : nullptr);
	if (!_shareHidesContent) {
		...
		_shareContainer.reset(QWidget::createWindowContainer(
			_shareWrap->windowHandle(), _window->body().get(), Qt::FramelessWindowHint | Qt::WindowStaysOnTopHint));
	}
	_window->body()->sizeValue() |
		rpl::start_with_next(
			[=](QSize size)
			{
				const auto widget = _shareHidesContent ? _shareWrap.get() : _shareContainer.get();
				widget->setGeometry(QRect(QPoint(), size));
			},
			_shareWrap->lifetime());
	...
	std::move(result.destroyRequests) | rpl::start_with_next([=] { destroyShareMenu(); }, _shareWrap->lifetime());
